 ...
 *
 * Vai trò: Admin · Kế toán · Nhân viên.
 *   - Admin, Kế toán  → vai app 'ketoan' (toàn quyền tính toán, duyệt; Admin thêm quản lý người dùng)
 *   - Nhân viên       → vai app 'nhap'   (chỉ thêm/sửa/xoá khoản của mình khi còn chờ duyệt)
 *
...
class KHBC_Auth {
	...
	const VAI = array( 'Admin', 'Kế toán', 'Nhân viên' );

	/** Người đang gọi trong request này (đặt bởi KHBC_API::handle). */
	...
	public static function login( $pin ) {
		...
		if ( self::is_locked() ) { return KHBC_Util::err( 'Nhập sai quá nhiều lần — thử lại sau 10 phút' ); }
		$u = self::user_by_pin( $pin );
		if ( ! $u ) {
			self::bump_fails();
			return KHBC_Util::err( 'PIN không đúng hoặc chưa được cấp' );
		}
		self::clear_fails();
		$tok = self::issue_token( $u );
		KHBC_Store::log_as( $u['ten'], $u['vai'], 'login', 'Đăng nhập bằng PIN' );
		return KHBC_Util::ok( array( 'user' => self::out_user( $u, $tok ) ) );
	}

	public static function ai_dang_dang_nhap( $token = '' ) {
		global $wpdb;
		$u = self::user_by_token( $token );
		if ( ! $u ) { return KHBC_Util::err( 'Phiên đã hết hạn', array( 'code' => 'no_session' ) ); }
		$t   = KHBC_DB::t( 'phien' );
		...
		return KHBC_Util::ok( array( 'user' => self::out_user( $u ) ) );
	}
	...
	public static function logout( $token = '' ) {
		...
		if ( $tok !== '' ) { $wpdb->delete( KHBC_DB::t( 'phien' ), array( 'token' => $tok ) ); }
		return KHBC_Util::ok();
	}
	...
	public static function change_pin( $old, $new ) {
		global $wpdb;
		$u = self::$nguoi;
		if ( ! $u || empty( $u['id'] ) ) { return KHBC_Util::err( 'Chưa đăng nhập' ); }
		$old = trim( (string) $old );
		$new = trim( (string) $new );
		if ( ! preg_match( '/^\d{4,8}$/', $new ) ) { return KHBC_Util::err( 'PIN mới phải gồm 4–8 chữ số' ); }
		$row = self::user_row( $u['id'] );
		if ( ! $row || ! password_verify( $old, $row['pin_hash'] ) ) { return KHBC_Util::err( 'PIN hiện tại không đúng' ); }
		if ( self::pin_trung( $new, $row['id'] ) ) { return KHBC_Util::err( 'PIN này đã có người dùng khác — chọn PIN khác' ); }
		$wpdb->update( KHBC_DB::t( 'nguoi_dung' ), array( 'pin_hash' => password_hash( $new, PASSWORD_DEFAULT ), 'sua_luc' => KHBC_Util::now_sql() ), array( 'id' => (int) $row['id'] ) );
		KHBC_Store::log( 'changePin', 'Đổi PIN của chính mình' );
		return KHBC_Util::ok();
	}
	...
	public static function issue_token( $u ) {
		...
		$wpdb->insert( KHBC_DB::t( 'phien' ), array(
			'token'   => $tok,
			'user_id' => (int) $u['id'],
			'ten'     => (string) $u['ten'],
	...
	public static function sso_secret() {
		$s = trim( (string) get_option( 'khbc_sso_secret', '' ) );
	 ...
	 * với vai suy từ vai trang tổng (ADMIN→Admin, KE_TOAN→Kế toán, còn lại→Nhân viên) và không PIN
	...
	public static function resolve_sso_user( $ident ) {
		...
		$vai   = $hub === 'ADMIN' ? 'Admin' : ( $hub === 'KE_TOAN' ? 'Kế toán' : 'Nhân viên' );
		$u = null;
		...
		if ( ! $u ) {
			...
			$wpdb->insert( $t, array( 'ten' => $ten, 'pin_hash' => '', 'vai' => $vai, 'email' => $email, 'hoat_dong' => 1, 'tao_luc' => KHBC_Util::now_sql(), 'sua_luc' => KHBC_Util::now_sql() ) );
			$u = self::user_row( $wpdb->insert_id );
			KHBC_Store::log_as( $ten, $vai, 'sso_tao', 'Tạo người dùng từ SSO trang tổng' );
		}
		return $u;
	}
}
