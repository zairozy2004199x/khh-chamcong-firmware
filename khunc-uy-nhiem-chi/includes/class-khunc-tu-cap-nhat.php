 ...
 *      Đọc số phiên bản từ raw.githubusercontent.com/<repo>/<nhánh>/khunc-uy-nhiem-chi/khunc-uy-nhiem-chi.php,
 *      gói tải là zip cả nhánh (codeload.github.com); sau khi giải nén, lọc ra đúng thư mục
 *      `khunc-uy-nhiem-chi/` rồi mới đưa vào wp-content/plugins (xem chon_thu_muc()).
 *      Cùng cách với plugin Ghế Massage (vhcp-ghe) đang chạy: đẩy code lên nhánh là WordPress thấy bản mới.
 *   2. GitHub Releases với tag `khunc-uy-nhiem-chi-vX.Y.Z` (workflow plugin-release.yml). Repo riêng tư
...
class KHUNC_TuCapNhat {

	const REPO      = 'zairozy2004199x/khh-chamcong-firmware';
	const THU_MUC   = 'khunc-uy-nhiem-chi';
	const TIEN_TO   = 'khunc-uy-nhiem-chi-v';
	const NHANH_MAC_DINH = 'main';
	const O_KHOA    = 'vhcp_gh_token';
	const O_NHANH   = 'khunc_gh_nhanh';
	const O_NHO     = 'khunc_gh_ban_moi';
	 ...
	 *
	 * Anh Thắng 16/09/2026: *"kèm plugin tự cập nhật vào nhé"* — Ủy Nhiệm Chi đã có sẵn bộ tự
	...
	private static function ban_hien() {
		return defined( 'KHUNC_VERSION' ) ? (string) KHUNC_VERSION : '0';
	}
	...
	public static function khai_ds( $ds ) {
		...
		$ds[] = array(
			'ma'    => 'khunc-uy-nhiem-chi',
			'ten'   => 'Ủy Nhiệm Chi (K&H)',
			'duong' => self::duong(),
			'hien'  => self::ban_hien(),
			'lop'   => __CLASS__,
		);
	...
	private static function duong() { return plugin_basename( KHUNC_DIR . 'khunc-uy-nhiem-chi.php' ); }

	private static function dau() {
		$dau  = array( 'headers' => array( 'Accept' => 'application/vnd.github+json', 'User-Agent' => 'khunc-tu-cap-nhat' ), 'timeout' => 15 );
	...
	public static function ban_moi( $bo_qua_nho = false ) {
		...
		update_option( 'khunc_gh_kiem_luc', time() );
	...
	private static function tu_nhanh() {
		$nhanh = self::nhanh();
		$url = 'https://raw.githubusercontent.com/' . self::REPO . '/' . rawurlencode( $nhanh ) . '/' . self::THU_MUC . '/khunc-uy-nhiem-chi.php';
		...
		$ver = $m[1];
		if ( version_compare( $ver, KHUNC_VERSION, '<=' ) ) { return array( 'khong' => 1, 'ver_xa' => $ver, 'nguon' => 'nhanh', 'nhanh' => $nhanh ); }
		return array(
			'ver'     => $ver,
	...
	private static function tu_release() {
		...
		foreach ( $ds as $rel ) {
			...
			if ( version_compare( $ver, KHUNC_VERSION, '<=' ) ) { continue; }
	...
	public static function chi_tiet( $ket_qua, $viec, $args ) {
		...
		$o = new stdClass();
		$o->name = 'Ủy Nhiệm Chi (K&H)'; $o->slug = $args->slug; $o->version = $moi['ver'];
	 ...
	 * lấy đúng `khunc-uy-nhiem-chi/` bên trong đưa lên làm nguồn cài. Zip của Release thì đã đúng
	...
	public static function chon_thu_muc( $nguon, $nguon_xa, $nang_cap, $dau = array() ) {
		...
		if ( $wp_filesystem->exists( trailingslashit( $con ) . 'khunc-uy-nhiem-chi.php' ) ) {
			...
			return new WP_Error( 'khunc_move', 'Không tách được thư mục plugin khỏi gói tải về.' );
		}
		if ( trailingslashit( $nguon ) === trailingslashit( $can ) ) { return $nguon; }
		if ( $wp_filesystem->exists( trailingslashit( $nguon ) . 'khunc-uy-nhiem-chi.php' ) && $wp_filesystem->move( $nguon, $can, true ) ) {
