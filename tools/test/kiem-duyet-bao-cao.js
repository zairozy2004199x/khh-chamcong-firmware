/* ══════════════════════════════════════════════════════════════════════════════════════════════
 * KIỂM MÀN DUYỆT BÁO CÁO — THỨ TỰ HIỆN VÀ BỘ LỌC NGÀY
 *
 * 🔴 BÀI KIỂM ĐẦU TIÊN CHO PHẦN WEB CỦA KHO NÀY. Trước nay ở đây chỉ có CI biên dịch firmware,
 *    không có gì canh JavaScript của trang — mọi thay đổi giao diện đều phải bấm tay mới biết.
 *
 * ⚠️ NÓ KHÔNG DÒ CHUỖI. Hai biểu thức thật (lọc và sắp xếp) được BỐC THẲNG từ mã nguồn ra rồi
 *    CHẠY. Ai đổi luật mà quên sửa bài kiểm thì đỏ ngay; còn dò chuỗi thì chỉ canh được đúng
 *    dòng chữ hôm nay, đổi cách viết là mù.
 *
...
/* ══════════════════════════════════════════════════════════════════════════════════════════════
 * MÁY CHỦ PHẢI GỬI MỐC NỘP XUỐNG
 *
 * 🔴 Thiếu nó thì client rơi về ngày báo cáo, và báo cáo nộp muộn lại nằm lọt giữa danh sách —
 *    đúng cái lỗi vừa sửa, mà mọi phép thử phía trên vẫn xanh vì chúng tự dựng dữ liệu có sẵn
 *    `taoLuc`.
 *
 * ⚠️ ĐÂY LÀ PHÉP DÒ MÃ, KHÔNG PHẢI CHẠY THẬT — kho này chưa có khung chạy PHP. Nó canh được
...
t('🔴 truy vấn danh sách báo cáo có lấy cột tao_luc', /h\.tao_luc/.test(srcKt));
t("🔴 và gửi xuống client dưới khoá 'taoLuc'", /'taoLuc'\s*=>\s*\(string\)\s*\(\s*isset\(\s*\$r\['tao_luc'\]/.test(srcKt));
/* ⛔ MÃ TƯƠNG ĐƯƠNG CÓ CHỦ Ý: `usort` bên máy chủ cũng xếp theo `tao_luc`, nhưng client SẮP LẠI
   toàn bộ sau khi lọc — nên gỡ nhánh ấy đi thì màn hình không đổi, và phá thử "máy chủ bỏ xếp
   theo lúc nộp" luôn sống. Giữ nó vì bản plugin cũ hơn (client chưa vá) vẫn dùng đúng thứ tự
   máy chủ trả về. Đừng xoá nó chỉ vì thấy phép thử không đỏ. */
t('máy chủ cũng xếp theo lúc nộp (thứ tự cho client bản cũ)', /\$ta < \$tb \? 1 : -1/.test(srcKt));
