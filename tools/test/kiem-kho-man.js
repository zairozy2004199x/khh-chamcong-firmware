...
t('🔴 ô gõ chiếm ~nửa dòng (không còn cả dòng), để lệch và số máy đứng cùng dòng', /\.o-go\{flex:0 0 5\d%/.test(dt));
t('🔴 SL hàng bán · Lệch khai · Máy bán tổng xếp liền nhau bằng order', /"SL hàng bán"\]\{order:20\}/.test(dt) && /"Lệch khai"\]\{order:21\}/.test(dt) && /"Máy bán tổng"\]\{order:22\}/.test(dt));
t('🔴 Hàng tồn còn · Lệch kho · Tồn tính xếp liền nhau', /"Hàng tồn còn"\]\{order:30\}/.test(dt) && /"Lệch kho"\]\{order:31\}/.test(dt) && /"Tồn tính"\]\{order:32\}/.test(dt));
t('Nhập · Tồn đầu; ghi chú cuối', /"Nhập"\]\{order:10\}/.test(dt) && /"Tồn đầu"\]\{order:12\}/.test(dt) && /\.o-ghi\{order:40\}/.test(dt));
t('lệch canh giữa dòng (margin auto hai bên)', /\.o-lech\{margin-left:auto;margin-right:auto/.test(dt));
