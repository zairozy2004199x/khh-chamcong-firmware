 ...
 *
 * ⚠️ Tệp này được PHP phục vụ ở địa chỉ `/<slug>/sw.js` (xem `VHCP_Pwa::ra_sw`), và `__BAN__` bị
 ...
 * =================================================================================================
 * 1. KHÔNG BAO GIỜ NHỚ MỘT LƯỢT GỌI DỮ LIỆU. Mọi đường hỏi/ghi của app đều mang `vhcp_api`,
...
var BAN    = '__BAN__';
var KHO_VO = 'vhcp-vo-' + BAN;      // vỏ app: trang /<slug>/
var KHO_TT = 'vhcp-tt-' + BAN;      // tệp tĩnh: css, js, ảnh
var TT_TOI_DA = 120;
...
self.addEventListener('install', function (e) {
  e.waitUntil(
    caches.open(KHO_VO).then(function (kho) {
      /* Nạp sẵn đúng TRANG APP. Trang này giống hệt nhau với mọi người — `VHCP_App::render()`
  ...
  e.waitUntil(
    caches.keys().then(function (ten) {
      return Promise.all(ten.map(function (t) {
        if (t.indexOf('vhcp-') !== 0) { return null; }       // kho của bộ khác, không đụng
 ...
 *
 * 🔴 DÒ BA ĐƯỜNG, KHÔNG PHẢI MỘT. App có ba cổng gọi máy chủ (xem `VHCP_App::head_block`):
 *    `/wp-json/`, `admin-ajax.php`, và chính URL trang kèm `vhcp_api` — cổng cuối sinh ra vì
 ...
 */
function laDuLieu(u) {
  return u.searchParams.has('vhcp_api')
