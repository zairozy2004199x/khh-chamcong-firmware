...
class VHCP_Cfg {
		...
		return array(
			...
			/* 🔴 AI BẤM ĐƯỢC NÚT "＋ TẠO ĐƠN MỚI" — anh Thắng 26/09/2026, đơn chi phí Vận Hành:
			   *"giờ check kế toán sao chưa có nút tạo đơn"*. Trước bản này nút gác cứng theo
			   ...
			   vai nào ở `createDon` (`VHCP_Api::required_roles()`), nên đây chỉ là màn hình —
	...
	public static function cfg_static() {
		...
		foreach ( self::rows_of( $all, self::COSO ) as $r ) {
			...
			$out['coso'][] = array( 'ten' => $r[0], 'maDonVi' => VHCP_Util::ma_so( $r[1] ), 'phanLoaiLon' => $r[2], 'tenMisa' => $r[3], 'dongCua' => isset( $r[4] ) ? (string) $r[4] : '',
				...
				'boPhan' => VHCP_DonVi::khoi_cua( isset( $r[7] ) ? $r[7] : '' ),
				/* TÊN BÊN DOANH THU (25/09/2026): tên cửa hàng y như trên web Doanh thu (POS/FABi) để bảng
				   "Doanh thu vs Chi phí" khớp chắc, khỏi đoán theo tên. Rỗng = khớp lỏng. Xem `VHCP_DoanhThu::anh_xa`. */
		...
		foreach ( $out['coso'] as $x ) {
			...
			if ( $k !== '' ) {
				...
				if ( '' === $bp ) {
					$kh = VHCP_DonVi::khoi_cua( isset( $x['donVi'] ) ? $x['donVi'] : '' );
	...
	public static function save_config( $cfg ) {
		...
		if ( isset( $cfg['coso'] ) && is_array( $cfg['coso'] ) ) {
			...
			foreach ( $cfg['coso'] as $x ) {
				...
				$rows[] = array( $tn, VHCP_Util::ma_so( $g( $x, 'maDonVi' ) ), $g( $x, 'phanLoaiLon' ), $g( $x, 'tenMisa' ), $dc, $dv, $tinh, $bp, $tdt );
			}
		...
		if ( isset( $cfg['doanhThu'] ) && is_array( $cfg['doanhThu'] ) ) {
			/* 🔴 CHỈ ADMIN — địa chỉ + khoá chia sẻ sang web Doanh thu. Khoá rỗng = giữ nguyên (xem `VHCP_DoanhThu::luu`). */
			if ( 'Admin' !== VHCP_Auth::vai_tro() ) {
				return VHCP_Util::err( 'Chỉ Admin mới đổi kết nối web Doanh thu được.' );
			}
			$kq_dt = VHCP_DoanhThu::luu( $cfg['doanhThu'] );
			if ( empty( $kq_dt['success'] ) ) { return $kq_dt; }
		}
		if ( isset( $cfg['vending'] ) && is_array( $cfg['vending'] ) ) {
			/* 🔴 CHỈ ADMIN — địa chỉ + khoá chia sẻ sang web Vending HCMC. Khoá rỗng = giữ (xem `VHCP_Vending::luu`). */
			if ( 'Admin' !== VHCP_Auth::vai_tro() ) { return VHCP_Util::err( 'Chỉ Admin mới đổi kết nối web Vending được.' ); }
			$kq_vd = VHCP_Vending::luu( $cfg['vending'] );
		...
		if ( isset( $cfg['tinhNang'] ) && is_array( $cfg['tinhNang'] ) ) {
			...
			if ( 'Admin' !== VHCP_Auth::vai_tro() ) {
				return VHCP_Util::err( 'Chỉ Admin mới bật/tắt tính năng được.' );
			}
			$luu = self::tinh_nang_luu();
			foreach ( $cfg['tinhNang'] as $ma => $tt ) {
				$ma = (string) $ma; $tt = mb_strtolower( trim( (string) $tt ) );
				if ( ! isset( self::TINH_NANG[ $ma ] ) ) { return VHCP_Util::err( 'Không có tính năng mã "' . $ma . '" trong bản này.' ); }
				if ( ! in_array( $tt, self::TN_TRANG_THAI, true ) ) { return VHCP_Util::err( 'Trạng thái "' . $tt . '" không hợp lệ — chỉ tat · admin · bat.' ); }
				$luu[ $ma ] = $tt;
			}
			$rows = array();
			foreach ( $luu as $ma => $tt ) { $rows[] = array( $ma, $tt ); }
			self::write( self::TN, $rows );
			VHCP_Log::log_action( array( 'actor' => VHCP_Auth::nguoi(), 'role' => VHCP_Auth::vai_tro(), 'action' => 'Đổi cờ tính năng',
	...
	public static function tinh_nang_bat( $ma, $vai = null ) {
		...
		$vai = null === $vai ? VHCP_Auth::vai_tro() : (string) $vai;
		return 'Admin' === $vai;
	}
	 ...
	 *
	 * 🔴 ĐẦU MỤC TRƯỚC NAY KHÔNG KHAI ĐƯỢC. `dau_muc_ds()` đọc `get_option('vhcp_dau_muc_ds')`
	...
	public static function khoi_coso_chuan( $v ) {
		...
		if ( '' === $v || '*' === $v ) { return $v; }
		return in_array( $v, array_keys( VHCP_DonVi::KHOI_THEO_DON_VI ), true ) ? $v : '*';
	}
	...
	public static function xuat_goi_cau_hinh( $bangs = null ) {
		...
		return array(
			'success'  => true,
			/* 🔴 DẤU NHẬN DẠNG KHÔNG ĐƯỢC BẮT ĐẦU BẰNG `vhcp`. `tach-ban-vung.sh` đổi mọi `vhcp` sang
			   tiền tố riêng của từng bản vùng (thêm mã hn / mtd / vp vào sau); bản đầu đặt
			   'vhcp-goi-cau-hinh' và bản Hà Nội hoá thành một chuỗi khác — gói tải từ Khu vui chơi
			...
			'phienBan' => defined( 'VHCP_VERSION' ) ? (string) VHCP_VERSION : '',
			'khoi'     => VHCP_DB::KHOI,
	...
	public static function nhap_goi_cau_hinh( $goi, $bangs ) {
		if ( 'Admin' !== VHCP_Auth::vai_tro() ) {
			return VHCP_Util::err( 'Chỉ Admin nhập được gói cấu hình — nhập là ghi đè cả bảng.' );
		}
		$goi = is_object( $goi ) ? (array) $goi : (array) $goi;
		if ( ! isset( $goi['loai'] ) || 'goi-cau-hinh-van-hanh-chi-phi' !== (string) $goi['loai'] ) {
			return VHCP_Util::err( 'Tệp không phải gói cấu hình của Vận Hành Chi Phí.' );
		}
		$bang = isset( $goi['bang'] ) ? ( is_object( $goi['bang'] ) ? (array) $goi['bang'] : $goi['bang'] ) : null;
		if ( ! is_array( $bang ) || ! $bang ) { return VHCP_Util::err( 'Gói rỗng — không có bảng nào.' ); }
		$ds   = self::goi_bang_ds();
		...
		if ( ! $chon ) { return VHCP_Util::err( 'Chưa tích bảng nào có trong gói (hoặc bảng ấy không được phép nhân bản).' ); }
		$kq = array();
		foreach ( $chon as $b ) {
			$rows = array();
		...
		return VHCP_Util::ok( array( 'bang' => $kq ) );
	}
