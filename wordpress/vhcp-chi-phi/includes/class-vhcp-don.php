...
class VHCP_Don {
	...
	public static function luong_mac_dinh( $nguoi_lap ) {
		return VHCP_Cfg::luong_cua_bo_phan( VHCP_Cfg::bo_phan_hang_nguoi( $nguoi_lap ) );
	}
	...
	public static function get_bootstrap() {
		...
		return array(
			 ...
			 * ══════════════════════════════════════════════════════════════════════════════ */
			'luongBo'    => self::luong_mac_dinh( VHCP_Auth::nguoi() ),
			...
			/* Khối nào được bày nút trên thanh KHỐI — `null` = đủ ba. Đi thẳng từ `xem_duoc()`
			   nên không đẻ thêm trục quyền nào; xem chốt dài ở `VHCP_DonVi::khoi_xem_duoc()`. */
			'khoiXem'    => VHCP_DonVi::khoi_xem_duoc(),
			...
			'vaiTuyBien' => VHCP_Cfg::vai_tuy_bien(),
	...
	public static function get_don( $ma_don, $with_products = true ) {
		...
		foreach ( $cp as $x ) {
			$dong = array(
				...
				'taoLuc'     => VHCP_Util::fmt( $x['tao_luc'] ),
			);
			/* TRỤC PHÂN TÍCH — đọc qua khung, và ĐỌC chứ không đưa nguyên chuỗi trong sổ xuống:
			   ...
			   thì bên kia lặng lẽ nói khác. Xem `VHCP_Truc::doc()`. */
			$lines[] = array_merge( $dong, VHCP_Truc::doc_dong( $x ) );
		}
	...
	public static function set_tam_ung( $ma_don, $coso, $so ) {
		...
		if ( $d && self::la_truc_tiep( $d ) && VHCP_Util::num( $so ) > 0 ) {
			return VHCP_Util::err( 'Đơn này đi luồng TRỰC TIẾP — tiền đã chi, không có bước tạm ứng nên không nhập số xin. Cần tạm ứng thì nhờ kế toán bấm "🔁 Đổi luồng" sang Qua tạm ứng.' );
		}
		 ...
		 * được (`VHCP_DonVi::don_nhieu_coso()`). Nhưng lưu một dòng tạm ứng vào là đơn LẬP TỨC
		 ...
		 *    họ quyết.
		 * ══════════════════════════════════════════════════════════════════════════════════════ */
		if ( VHCP_DonVi::MO_KHI_KHONG_TAM_UNG ) {
			$khac = array();
	...
	private static function line_data( $id, $ma_don, $rec ) {
		...
		/* ══════════════════════════════════════════════════════════════════════════════════
		 * TRỤC PHÂN TÍCH — mỗi trục một cột, khai một chỗ (`VHCP_Truc::ds()`).
		 * ══════════════════════════════════════════════════════════════════════════════════
		...
		foreach ( VHCP_Truc::ghi_dong( $get ) as $cot => $gt ) {
			$row[ $cot ] = $gt;
		}
		return $row;
	}
	...
	public static function giai_doan_doc( $v ) {
		/* Từ 22/09/2026 luật nằm ở khung TRỤC PHÂN TÍCH (`VHCP_Truc`) — hai hàm này ở lại làm
		   lối gọi quen cho mã cũ, KHÔNG giữ bản luật thứ hai. */
		return VHCP_Truc::doc( 'giaiDoan', $v );
	}

	public static function giai_doan_chuan( $v ) {
		return VHCP_Truc::chuan( 'giaiDoan', $v );
	}
	...
	public static function tk_of_line( $nhom, $phan_loai_tt, $coso = '' ) {
		...
		/* Mã danh mục mà là 141/331 thì không đem gắn lên dòng — xem `VHCP_Cfg::bo_ma_ben_tra_`. */
		if ( $tk_no === '' && ! VHCP_Cfg::la_tk_ben_tra( $cat['tkNo'] ) ) { $tk_no = $cat['tkNo']; }
		$tk_co = $cat['tkCo'];
		if ( $tk_co === '' ) {
	 ...
	 * ═════════════════════════════════════════════════════════════════════════════════════════
	 * Loại chi phí ở đơn tuần chính là NHÓM MẶT HÀNG: nó suy ra TK Nợ (`VHCP_Cfg::tkno_loai`).
	...
	public static function set_line_nhom( $id, $nhom ) {
		...
		if ( ! in_array( $vai, array( 'Admin', 'Kế toán cá nhân', 'Kế toán NCC' ), true ) ) {
			return VHCP_Util::err( 'Chỉ kế toán gắn lại loại chi phí được — '
				. 'đây là mã hạch toán, không phải nội dung dòng.' );
		}
		$nhom = trim( (string) $nhom );
		if ( '' === $nhom ) { return VHCP_Util::err( 'Chọn loại chi phí' ); }

		$tk = self::tk_of_line( $nhom, (string) $cur['phan_loai_tt'], (string) $cur['coso'] );
		$wpdb->update( VHCP_DB::t( 'chiphi' ), array(
			'nhom'  => $nhom,
			'tk_no' => $tk['tk_no'],
			'tk_co' => $tk['tk_co'],
		), array( 'id' => (string) $id ) );
		...
		return VHCP_Util::ok( array( 'nhom' => $nhom, 'tkNo' => $tk['tk_no'], 'tkCo' => $tk['tk_co'] ) );
	}
	...
	public static function set_line_tk_no( $id, $tk, $them = 0 ) {
		...
		if ( '' === $tk ) {
			...
			if ( $them && ! preg_match( '/^\d{3,10}$/', $tk ) ) {
				return VHCP_Util::err( 'Mã TK Nợ tự thêm phải là 3–10 chữ số, nhận được "' . $tk . '".' );
			}
			if ( ! $them && ! in_array( $tk, VHCP_Cfg::tkno_da_khai(), true ) ) {
	...
	public static function gui_duyet_tam_ung( $ma_don ) {
		...
		if ( self::la_truc_tiep( $d ) ) {
			return VHCP_Util::err( 'Đơn này đi luồng TRỰC TIẾP — không qua tạm ứng. Bấm "Gửi quyết toán" để gửi thẳng cho kế toán.' );
		}
		$t = VHCP_DB::t( 'chiphi' );
	...
	public static function gui_quyet_toan( $ma_don ) {
		...
		if ( (string) $d['trang_thai'] !== $tu ) {
			return VHCP_Util::err( 'Chỉ gửi khi đơn "' . self::ten_tt( $tu, isset( $d['khoi'] ) ? $d['khoi'] : '', self::luong_don( $d ) ) . '"' );
		}
		...
		if ( self::la_truc_tiep( $d ) ) {
			...
			if ( null !== $xin && $xin > 0 ) {
				return VHCP_Util::err( 'Đơn TRỰC TIẾP không có bước tạm ứng, mà đơn này đang xin tạm ứng ' . VHCP_Util::tien( $xin )
	...
	public static function danh_dau_thanh_toan( $ma_don, $nguoi ) {
		...
		if ( ! self::tt_trong_luong( 'Đã thanh toán', $khoi, $lg ) ) {
			return VHCP_Util::err( 'Luồng của đơn này không có bước "Đã thanh toán" riêng — tiền đã đưa từ lúc cấp tạm ứng.' );
		}
	...
	public static function tong_de_duyet( $ma_don ) {
		...
		if ( ! VHCP_DonVi::MO_KHI_KHONG_TAM_UNG ) { return $t; }
		/* Có dòng tạm ứng (kể cả 0đ — nó vẫn là lời chốt gian) thì con số của đơn là số xin. */
		if ( VHCP_DonVi::don_co_tam_ung( $ma_don ) ) { return $t; }
		return self::tong_thanh_tien( $ma_don ) + VHCP_Util::num( self::du_phong_cua( $ma_don ) );
	}
	...
	public static function doi_luong_don( $ma_don, $luong_moi, $ly_do = '' ) {
		...
		if ( '' === $moi ) {
			return VHCP_Util::err( 'Mã luồng "' . trim( (string) $luong_moi ) . '" không có — chỉ nhận: ' . implode( ' · ', array_keys( self::LUONG_MA ) ) . '.' );
		}
		$cu = self::luong_don( $d );
		$khoi = isset( $d['khoi'] ) ? $d['khoi'] : '';
		...
		if ( self::luong_cua( $khoi, $cu ) === self::luong_cua( $khoi, $moi ) ) {
			return VHCP_Util::err( 'Đơn đang đi đúng luồng "' . self::LUONG_MA[ $moi ] . '" rồi — không có gì để đổi.' );
		}
		$st = (string) $d['trang_thai'];
		if ( '' === $st ) { $st = 'Nháp'; }
		if ( self::da_chot( $st ) || 'Đã xuất MISA' === $st ) {
			return VHCP_Util::err( 'Đơn đã "' . self::ten_tt( $st, $khoi, $cu ) . '" — sổ đã chốt, không đổi luồng được nữa.' );
		}
		...
		if ( $tien_da_ra ) {
			return VHCP_Util::err( 'Đơn đang "' . self::ten_tt( $st, $khoi, $cu ) . '" — tiền đã cấp, không đổi luồng được. Muốn đổi thì Trả lại đơn rồi đi lại từ đầu.' );
		}
		...
		self::ghi_vet( $ma_don, 'Đổi luồng đơn',
			...
			. ( $duyet_cu > 0 ? ' · gỡ số đã duyệt ' . VHCP_Util::tien( $duyet_cu ) : '' )
		...
		return VHCP_Util::ok( array( 'luongCu' => $cu, 'luongMoi' => $moi, 'trangThaiCu' => $st, 'trangThai' => 'Nháp', 'goDuyet' => $duyet_cu > 0 ) );
	}
