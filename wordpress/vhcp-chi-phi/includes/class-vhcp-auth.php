...
class VHCP_Auth {
	...
	public static function xem_duoc_loai( $ten_loai ) {
		...
		/* 🔴 VÙNG TẮT LỌC THEO VAI thì MỌI VAI thấy đủ loại — xem `VHCP_Cfg::loc_loai_theo_vai()`.
		...
		if ( ! VHCP_Cfg::loc_loai_theo_vai() ) { return true; }
		return VHCP_Cfg::loai_thuoc_vai( $ten_loai, $vai );
	}
	...
	public static function coso_cua_nguoi( $ten ) {
		...
		if ( '' === $k ) { return array(); }
		/* ⚠️ Gác CÙNG HÀM với lời gọi — luật `tools/test/kiem-goi-cheo.php`. */
		if ( ! class_exists( 'VHCP_Cfg' ) || ! method_exists( 'VHCP_Cfg', 'get_users' ) ) { return array(); }
		foreach ( (array) call_user_func( array( 'VHCP_Cfg', 'get_users' ) ) as $u ) {
			$u = (array) $u;
