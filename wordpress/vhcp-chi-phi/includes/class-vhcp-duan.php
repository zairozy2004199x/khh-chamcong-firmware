...
class VHCP_DuAn {
	...
	public static function dat_hm( $ma_da, $row, $tt, $them = array() ) {
		...
		if ( 'xong' === $tt ) {
			...
			if ( ! $co_ct && ! $khai_ko ) {
				/* Câu chối gọi tên ĐÚNG mấy ô của bảng đang mở: bảng NCC có hai ô (uỷ nhiệm chi +
				   hoá đơn), bảng tạm ứng chỉ có một. Nói "ít nhất một chứng từ" với người chỉ thấy
				   một ô là bắt họ đi tìm cái ô thứ hai không tồn tại. */
				return VHCP_Util::err( $la_ncc
					? 'Chưa có chứng từ nào: uỷ nhiệm chi và hoá đơn đều trống, cột ẢNH / HỒ SƠ của '
						. 'dòng cũng trống. Đính ít nhất một chứng từ (hoặc ảnh bill ngay trên dòng) — '
						. 'hoặc tích "Không có chứng từ nào" rồi khoá.'
					: 'Hạng mục này chưa có chứng từ nào: ô hoá đơn trống, mà cột ẢNH / HỒ SƠ của dòng '
						. 'cũng trống. Đính hoá đơn (hoặc ảnh bill ngay trên dòng) — hoặc tích '
						. '"Không có hoá đơn" rồi chốt, nếu khoản này thật sự không có.' );
			}
