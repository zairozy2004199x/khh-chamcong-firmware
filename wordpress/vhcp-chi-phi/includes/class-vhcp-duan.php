...
class VHCP_DuAn {
	...
	public static function list_du_an() {
		...
		if ( class_exists( 'VHCP_DoanhThu' ) ) { $out = VHCP_DoanhThu::doi_gian( $out ); }
	...
	public static function get_du_an( $ma_da ) {
		...
		foreach ( self::lines_of( $ma_da ) as $r ) {
			...
			$lines[] = array(
				...
				'taoLuc'    => VHCP_Util::fmt( isset( $r['tao_luc'] ) ? $r['tao_luc'] : '' ),
	...
	public static function xoa_dot_tra( $ma_da, $dot, $loai = 'tu' ) {
		if ( 'Admin' !== VHCP_Auth::vai_tro() ) { return VHCP_Util::err( 'Chỉ Admin dọn được lệnh bị trả.' ); }
		if ( ! self::find( $ma_da ) ) { return VHCP_Util::err( 'Không tìm thấy dự án' ); }
		$d = self::dot_cua( $ma_da, $dot, $loai );
		if ( ! $d ) { return VHCP_Util::err( 'Không tìm thấy lệnh ' . (int) $dot ); }
		if ( 'tra' !== $d['tt'] ) {
			return VHCP_Util::err( 'Chỉ dọn được lệnh ĐÃ BỊ TRẢ LẠI. Lệnh này đang ở "' . $d['tt']
		...
		foreach ( self::lines_of( $ma_da ) as $l ) {
			...
			if ( $h && (int) $h['dot'] === (int) $dot ) {
				return VHCP_Util::err( 'Hạng mục "' . trim( (string) $l['noi_dung'] ) . '" vẫn trỏ vào lệnh này — chưa dọn được.' );
			}
		}
		VHCP_Log::log_action( array(
			'actor'  => VHCP_Auth::nguoi(), 'role' => VHCP_Auth::vai_tro(),
		...
		VHCP_Meta::set_json( self::dot_meta_( $loai ) . $ma_da, $o );
		return VHCP_Util::ok( array( 'daXoa' => (int) $dot, 'conLai' => count( $o ) ) );
	}
	...
	public static function cap_tien_phan( $ma_da, $dot, $them = array() ) {
		...
		if ( $cho_lan > 0 ) {
			...
			if ( $so > $con_lan ) {
				...
				foreach ( (array) ( isset( $d['lich'] ) ? $d['lich'] : array() ) as $y ) {
					...
					$c = VHCP_Util::num( isset( $y['soTien'] ) ? $y['soTien'] : 0 ) - ( isset( $da_lan[ $l ] ) ? $da_lan[ $l ] : 0 );
					if ( $c > 0 ) { $con_khac += $c; }
				}
				if ( $con_khac > 0 ) {
		...
		if ( $het ) {
			...
			/* `$nay = 0` chỉ xảy ra khi lệnh mất hàng (sổ cũ) — `update_line` không cho gõ cả dự
			   ...
			   số đã đưa. */
			$du_nay = self::da_cap_tong( $moi ) - ( $nay > 0 ? $nay : VHCP_Util::num( $moi['soTien'] ) );
	...
	public static function them_dong_muc_con_cu( $ma_da, $rec ) {
		...
		$data['tao_luc'] = VHCP_Util::now_sql();   // xem 🔴 ở `add_line()`
		$ok_dl = $wpdb->insert( VHCP_DB::t( 'da_line' ), $data );
		/* 🔴 SOI KẾT QUẢ — xem `kiem-ghi-so-phai-soi-ket-qua.php`. */
		if ( ! $ok_dl ) { $ct = trim( (string) $wpdb->last_error );
			return VHCP_Util::err( 'Không ghi được dòng dự án' . ( '' !== $ct ? ' — ' . $ct : '.' ) ); }
	...
	public static function dat_o_line( $ma_da, $row, $cot, $gt ) {
		...
		if ( ! isset( $map[ $cot ] ) ) {
			return VHCP_Util::err( 'Ô "' . $cot . '" không sửa thẳng trong bảng được — bấm ✏️ để mở form.' );
		}
		...
		$sua = array();
		if ( 'note' === $cot ) {
			$sua['note'] = VHCP_Util::st( $gt );
		} else {
			$so = VHCP_Util::num( $gt );
			/* Mục con không có dự toán riêng — tiền của nó nằm ở cột thực tế / thành tiền. */
			if ( 'duToan' === $cot && '' !== trim( (string) $cur['cap_cha'] ) ) {
				return VHCP_Util::err( 'Mục con không có ô dự toán riêng — dự toán nằm ở hạng mục lớn.' );
			}
			$sua[ $map[ $cot ] ] = $so;
			if ( 'soLuong' === $cot || 'donGia' === $cot ) {
				$sl = ( 'soLuong' === $cot ) ? $so : VHCP_Util::num( $cur['so_luong'] );
				$dg = ( 'donGia' === $cot )  ? $so : VHCP_Util::num( $cur['don_gia'] );
	 ...
	 * ═════════════════════════════════════════════════════════════════════════════════════════
	 * Loại chi phí quyết định TK Nợ / TK Có / mã đối tượng (`VHCP_Cfg::resolve_tk`). Chọn sai là
	 * dòng ấy vào sai tài khoản, và cái sai chỉ lộ ra lúc xuất MISA — khi đã quá muộn để hỏi lại
	 * người nhập họ mua cái gì.
	 *
	 * ⚠️ CHỈ KẾ TOÁN — cùng luật với `VHCP_Don::set_line_nhom()` bên đơn tuần. Gắn mã hạch toán
	...
	public static function dat_loai_cp_line( $ma_da, $row, $loai_cp ) {
		...
		if ( '' === $loai_cp ) { return VHCP_Util::err( 'Chọn loại chi phí' ); }

		$tk = VHCP_Cfg::resolve_tk( $loai_cp, (string) $cur['hinh_thuc'],
		...
		return VHCP_Util::ok( array( 'loaiCp' => $loai_cp, 'tkNo' => $tk['tk_no'], 'tkCo' => $tk['tk_co'] ) );
	}
