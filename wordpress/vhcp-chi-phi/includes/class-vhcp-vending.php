 ...
 *
 * 🔴 KHÔNG NHẬP HAI LẦN. Bản đồ id bên Vending → mã đơn giữ ở `VHCP_Meta` (`vending_map`). Kéo lại
...
class VHCP_Vending {
	...
	public static function dong_bo( $a = array() ) {
		...
		foreach ( $kq['chiPhi'] as $x ) {
			...
			$wpdb->update( $t_don, array( 'trang_thai' => $tt_moi, 'ngay_qt' => $ngay, 'nguoi_qt' => $x['approver'], 'ngay_gui_qt' => VHCP_Util::now_sql(),
		...
		VHCP_Meta::set( self::META_LAN, VHCP_Util::now_sql() . ' · ' . $thang . ' · mới ' . $moi . ' · cập nhật ' . $cap );
		VHCP_Log::log_action( array( 'actor' => VHCP_Auth::nguoi(), 'role' => VHCP_Auth::vai_tro(), 'action' => 'Kéo chi phí Vending về',
		...
		return VHCP_Util::ok( array( 'thang' => $thang, 'tu' => $tu, 'den' => $den, 'web' => $kq['web'], 'tong' => count( $kq['chiPhi'] ),
