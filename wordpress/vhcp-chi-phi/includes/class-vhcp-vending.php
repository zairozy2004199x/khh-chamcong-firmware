...
class VHCP_Vending {
	...
	public static function cau_hinh() {
		...
		return array( 'san' => $co, 'url' => $admin ? $url : '', 'khoaCo' => $co,
			...
			'diaChiNhanAjax' => function_exists( 'admin_url' ) ? admin_url( 'admin-ajax.php?action=vhcp_vending_nhan' ) : 'admin-ajax.php?action=vhcp_vending_nhan',
	...
	public static function rest_nhan( $req ) {
		...
		if ( is_array( $body ) && ! empty( $body['ping'] ) ) {
			return VHCP_Util::ok( array( 'ping' => true, 'web' => function_exists( 'get_bloginfo' ) ? (string) get_bloginfo( 'name' ) : '', 'banChiPhi' => defined( 'VHCP_VERSION' ) ? VHCP_VERSION : '' ) );
		}
