 ...
 * ⚠️ Chi phí = THỰC CHI theo `VHCP_Don::thuc_chi()` (chưa cấp tiền → 0), tính theo NGÀY của dòng
 *    hạng mục rơi trong tháng — cùng tháng với doanh thu, không theo kỳ của đơn.
 */
class VHCP_DoanhThu {

	const O_URL  = 'vhcp_dt_url';
	const O_KHOA = 'vhcp_dt_khoa';
	...
	public static function luu( $d ) {
		...
		if ( null !== $url ) {
			if ( '' !== $url && ! preg_match( '#^https?://[^\s/]+#i', $url ) ) {
				return VHCP_Util::err( 'Địa chỉ web Doanh thu phải bắt đầu bằng http:// hoặc https://' );
			}
		...
		self::quen();
		VHCP_Log::log_action( array( 'actor' => VHCP_Auth::nguoi(), 'role' => VHCP_Auth::vai_tro(), 'action' => 'Sửa kết nối web Doanh thu',
	...
	public static function quen() {
		update_option( 'vhcp_dt_doi', (string) microtime( true ), false );
	}

	private static function khoa_nho( $tu, $den ) {
		return 'vhcp_dt_' . md5( self::url() . '|' . (string) get_option( 'vhcp_dt_doi', '' ) . '|' . $tu . '|' . $den );
	}
	...
	public static function kiem_tra() {
		...
		return VHCP_Util::ok( array( 'soCuaHang' => count( $kq['cuaHang'] ), 'tu' => $tu, 'den' => $den,
	...
	public static function khoang_thang( $thang ) {
		$t = trim( (string) $thang );
		if ( ! preg_match( '/^(\d{4})-(\d{2})$/', $t, $m ) ) { $t = VHCP_Util::now()->format( 'Y-m' ); preg_match( '/^(\d{4})-(\d{2})$/', $t, $m ); }
	 ...
	 * `$khoi` (mb/mn/…) rỗng = cả hệ; đơn chưa đóng dấu khối vẫn tính (cùng luật `VHCP_Report::finance`).
	 */
	public static function chi_phi_coso( $tu, $den, $khoi = '' ) {
		...
		foreach ( VHCP_Don::cp_rows() as $r ) {
			...
			$ngay = VHCP_Util::parse_date( isset( $r['ngay'] ) ? $r['ngay'] : '' );
			if ( null === $ngay || $ngay < $tu || $ngay > $den ) { continue; }
			$eff = VHCP_Don::thuc_chi( $r['thanh_tien'], $r['thuc_mua'], $tt_don[ $m ] );
			if ( ! $eff ) { continue; }
	...
	public static function so_sanh( $a = array() ) {
		...
		$coso = isset( $cfg['coso'] ) ? VHCP_Cfg::coso_theo_don_vi( $cfg['coso'] ) : array();
	 ...
	 *
	 * Vào: `$items` = mảng của `VHCP_DuAn::list_du_an()` (mỗi phần tử có `ten`, `loai`). Ra: CÙNG
	...
	public static function doi_gian( array $items ) {
		try {
			...
			$coso_ds = class_exists( 'VHCP_Cfg' ) ? (array) VHCP_Cfg::cfg_static()['coso'] : array();
			...
			$can = array(); $hom_nay = VHCP_Util::now()->format( 'Y-m-d' );
			foreach ( $items as $i => $x ) {
				...
				$dong_ngay = VHCP_Util::parse_date( isset( $c['dongCua'] ) ? $c['dongCua'] : '' );
