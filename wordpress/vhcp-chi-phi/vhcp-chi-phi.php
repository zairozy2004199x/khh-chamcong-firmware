...
require_once VHCP_DIR . 'includes/class-vhcp-truc.php';
...
require_once VHCP_DIR . 'includes/class-vhcp-doanh-thu.php';
/* Chi phí Vending HCMC kéo về thành đơn thật — dùng khoang_thang của lớp Doanh thu nên nạp sau nó. */
require_once VHCP_DIR . 'includes/class-vhcp-vending.php';
...
require_once VHCP_DIR . 'includes/class-vhcp-gop.php';
...
/* Lớp vỏ app điện thoại (PWA) — gắn vào chính đường của trang, không đẻ đường thứ hai.
   Nạp SAU `class-vhcp-app.php`: nó dựng địa chỉ từ `VHCP_App::cac_slug()`. */
require_once VHCP_DIR . 'includes/class-vhcp-pwa.php';
...
// Điểm nhận chi phí web Vending HCMC đẩy sang (máy chủ khác, gác bằng khoá chung) — xem VHCP_Vending::routes()
add_action( 'rest_api_init', array( 'VHCP_Vending', 'routes' ) );
// …và cửa dự phòng admin-ajax.php cho chính điểm nhận ấy (hosting chặn /wp-json/), không cần đăng nhập — khoá là cửa
add_action( 'wp_ajax_nopriv_vhcp_vending_nhan', array( 'VHCP_Vending', 'ajax_nhan' ) );
add_action( 'wp_ajax_vhcp_vending_nhan', array( 'VHCP_Vending', 'ajax_nhan' ) );
...
/* ⚠️ CÙNG ƯU TIÊN 5, VÀ ĐẶT NGAY SAU. Hai bên cùng khai luật đường dẫn, mà lượt nạp lại bảng
   luật (`vhcp_flush_rewrite`, ưu tiên 99) phải thấy ĐỦ cả hai — khai muộn hơn 99 là luật của
   manifest/sw không vào bảng, và app báo "manifest không đọc được" mà không nói vì sao. */
add_action( 'init', array( 'VHCP_Pwa', 'init' ), 5 );
