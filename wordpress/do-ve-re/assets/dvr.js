...
async function run(){
  ...
  state.last = q;
  state.offers = buildOffers(q);
  state.live = false;
  const con = new Set(state.offers.map(o => o.al.code));
...
function renderResults(q){
  ...
  state.hienThi = list.slice(0, 14);
  $("#rows").innerHTML = state.hienThi.map((o, i) => {
    ...
    return '<article class="card' + (nhat ? " best" : "") + '">'
      ...
      + '<div class="c-act">'
        + '<button class="choose" type="button" data-chon="' + i + '">Chọn</button>'
...
async function lookupTax(){
  ...
  st.textContent = "bản xem thử không tra cứu được — bản cài trên web của anh tự điền tên và địa chỉ công ty";
  return;
...
/* ==================================================================
   Kho đơn — dùng kho dữ liệu của trang khi có, không thì lưu tạm trong máy.
   ================================================================== */
const CFG = window.DVR_CFG || {};
const PHI = { pct: Number(CFG.feePct != null ? CFG.feePct : 3), flat: 0,
              min: Number(CFG.feeMin != null ? CFG.feeMin : 50000) };
const GIU = Number(CFG.holdMinutes || 30);
const NGANHANG = { label: CFG.bankName || "", account: CFG.bankAccount || "", owner: CFG.bankOwner || "" };
const TRANGTHAI = {
  cho_thanh_toan: "Chờ khách chuyển khoản", het_han: "Quá hạn giữ giá",
  da_nhan_tien: "Đã nhận tiền, chờ mua vé", da_xuat_ve: "Đã xuất vé",
  hoan_tien: "Đã hoàn tiền", huy: "Đã huỷ"
};
const tinhPhi = v => Math.max(Math.round(v * PHI.pct / 100) + PHI.flat, PHI.min);

const Kho = {
  db: null, KEY: "dovere.demo.don.v1", nghe: null,
  get rest(){ return CFG.rest || ""; },
  async goi(path, opts){
    const o = Object.assign({ credentials: "same-origin", headers: {} }, opts || {});
    o.headers = Object.assign({ "X-WP-Nonce": CFG.nonce || "" }, o.headers);
    if(o.body){ o.headers["Content-Type"] = "application/json"; o.body = JSON.stringify(o.body); }
    const r = await fetch(this.rest + path, o);
    if(!r.ok) throw new Error("máy chủ trả mã " + r.status);
    return r.json();
  },
  async moKho(){
    this.db = !!this.rest;
    if(!$("#khoTrangThai")){ this.veDon(); return this.db; }
    $("#khoTrangThai").textContent = this.db
      ? (CFG.admin ? "Đơn lưu trên máy chủ của web này — khách đặt ở máy nào cũng hiện ở đây."
                   : "Đơn lưu trên máy chủ. Chỉ quản trị viên xem được danh sách đầy đủ.")
      : "Chưa nối được máy chủ — đơn đang lưu tạm trong trình duyệt này.";
  
    this.veDon();
    if(this.db) setInterval(() => { if(!$("#tab-don").hidden) this.veDon(); }, 15000);
    return this.db;
  },
  cuc(){ try { return JSON.parse(localStorage.getItem(this.KEY) || "{}"); } catch(e){ return {}; } },
  ghiCuc(o){ try { localStorage.setItem(this.KEY, JSON.stringify(o)); } catch(e){} },
  maDon(){
    const d = new Date();
    return "DVR" + String(d.getFullYear()).slice(2) + pad(d.getMonth() + 1) + String(Math.floor(1000 + Math.random() * 9000));
  },
  async tao(don){
    if(this.db){
      const r = await this.goi("orders", { method: "POST", body: don });
      this.veDon();
      return r.order;
    }
    don.code = this.maDon();
    don.createdAt = new Date().toISOString();
    don.expiresAt = new Date(Date.now() + GIU * 60000).toISOString();
    don.status = "cho_thanh_toan";
    don.log = [{ at: don.createdAt, what: "Khách tạo đơn" }];
    const c = this.cuc(); c[don.code] = don; this.ghiCuc(c);
    this.veDon();
    return don;
  },
  async lay(code){
    if(this.db){
      try { return (await this.goi("orders/" + encodeURIComponent(code))).order; }
      catch(e){ return null; }
    }
    return this.cuc()[code] || null;
  },
  async danhSach(){
    let ds;
    if(this.db){
      try { ds = (await this.goi("orders")).orders || []; }
      catch(e){ ds = []; }
    } else {
      ds = Object.values(this.cuc());
    }
    return ds.sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt)));
  },
  async sua(code, vá, ghiChu){
    if(this.db){
      try {
        const r = await this.goi("orders/" + encodeURIComponent(code), { method: "POST", body: { patch: vá, note: ghiChu } });
        this.veDon();
        return r.order;
      } catch(e){ toast("Không lưu được thay đổi: " + e.message); return null; }
    }
    const don = await this.lay(code);
    if(!don) return null;
    Object.assign(don, vá);
    don.log = (don.log || []).concat([{ at: new Date().toISOString(), what: ghiChu }]);
    const c = this.cuc(); c[code] = don; this.ghiCuc(c);
    this.veDon();
    return don;
  },
  hienTrang(don){
    return don.status === "cho_thanh_toan" && Date.parse(don.expiresAt) < Date.now() ? "het_han" : don.status;
  }
};

/* ---------------- chuyển tab ---------------- */
function moTab(ten){
  ["gia", "dat", "don"].forEach(t => {
    const pane = $("#tab-" + t), nut = document.querySelector('[data-tab="' + t + '"]');
    if(pane) pane.hidden = t !== ten;                     /* khách không có tab Đơn hàng */
    if(nut) nut.setAttribute("aria-pressed", t === ten ? "true" : "false");
  });
...
document.querySelector(".tabs").addEventListener("click", e => {
  ...
  moTab(b.dataset.tab);
  if(b.dataset.tab === "don") Kho.veDon();
});

/* ---------------- bấm Chọn ở bảng giá ---------------- */
let CHUYEN = null;
$("#rows").addEventListener("click", e => {
  const b = e.target.closest("[data-chon]"); if(!b) return;
  const o = state.hienThi[+b.dataset.chon], q = state.last;
  CHUYEN = {
    route: q.from.code + "-" + q.to.code, date: q.dep, airline: o.al.code, hang: o.al.name,
    number: o.code, dep: o.dep, arr: o.arr, stops: o.stops, bag: o.bag,
    cabin: o.cabin || q.cabin, adt: q.adt, chd: q.chd, inf: q.inf,
    fare: Math.round(o.total || paxPrice(o.price, q)), moiKhach: o.price
  };
  veDat();
  moTab("dat");
});
...
let soKhach = 1;
function veKhach(){
  $("#datPax").innerHTML = Array.from({ length: soKhach }, (_, i) =>
    '<div class="pax"><span class="lbl">Khách ' + (i + 1) + '</span>'
    + '<div class="field"><label for="k' + i + 'n">Họ và tên (như trên giấy tờ)</label>'
    + '<input id="k' + i + 'n" data-k="full" placeholder="NGUYEN VAN A"></div>'
    + '<div class="two"><div class="field"><label for="k' + i + 'd">Ngày sinh</label><input type="date" id="k' + i + 'd" data-k="dob"></div>'
    + '<div class="field"><label for="k' + i + 'i">Số CCCD / hộ chiếu</label><input id="k' + i + 'i" data-k="idNo" class="num-in"></div></div></div>'
  ).join("");
}
function veDat(){
  if(!CHUYEN) return;
  const [a, b] = CHUYEN.route.split("-");
  const [y, m, d] = CHUYEN.date.split("-");
  $("#datTitle").textContent = a + " → " + b;
  $("#datChuyen").hidden = false;
  $("#datForm").hidden = false;
  $("#datHang").textContent = CHUYEN.airline + " " + CHUYEN.number + " · " + CHUYEN.dep + " — " + CHUYEN.arr;
  $("#datGio").textContent = d + "/" + m + "/" + y + " · " + (CHUYEN.stops ? CHUYEN.stops + " điểm dừng" : "bay thẳng")
    + " · " + (CHUYEN.bag ? "có ký gửi" : "chỉ xách tay") + " · " + CHUYEN.hang;
  soKhach = Math.max(1, CHUYEN.adt + CHUYEN.chd);
  veKhach();
  const phi = tinhPhi(CHUYEN.fare);
  $("#dPaxNote").textContent = "(" + soKhach + " khách" + (CHUYEN.inf ? " + " + CHUYEN.inf + " em bé" : "") + ")";
  $("#dFare").textContent = vnd(CHUYEN.fare) + "đ";
  $("#dFee").textContent = vnd(phi) + "đ";
  $("#dTotal").textContent = vnd(CHUYEN.fare + phi) + "đ";
  $("#datTra").hidden = true;
  $("#datForm").hidden = false;
  $("#dSubmit").disabled = false;
}
$("#datThem").addEventListener("click", () => { soKhach++; veKhach(); });

let demNguoc = null, theoDoi = null;
$("#datForm").addEventListener("submit", async e => {
  e.preventDefault();
  $("#dErr").textContent = "";
  const pax = [...document.querySelectorAll("#datPax .pax")].map(c => {
    const o = {};
    c.querySelectorAll("[data-k]").forEach(el => o[el.dataset.k] = el.value.trim());
    return o;
  }).filter(p => p.full);
  const phone = $("#dPhone").value.replace(/\D/g, ""), email = $("#dEmail").value.trim();
  const loi = [];
  if(!pax.length) loi.push("chưa có hành khách nào");
  if(pax.some(p => p.full.trim().split(/\s+/).length < 2)) loi.push("họ tên phải đủ họ và tên");
  if(!/^0\d{9}$/.test(phone)) loi.push("số điện thoại phải là 10 số bắt đầu bằng 0");
  if(!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) loi.push("email chưa đúng");
  if(loi.length){ $("#dErr").textContent = "Chưa tạo được đơn: " + loi.join("; "); return; }

  $("#dSubmit").disabled = true;
  const phi = tinhPhi(CHUYEN.fare);
  const don = await Kho.tao({
    flight: CHUYEN, pax,
    contact: { name: $("#dName").value.trim(), phone: $("#dPhone").value.trim(), email },
    invoice: { tax: $("#dTax").value.trim(), company: $("#dCompany").value.trim() },
    money: { fare: CHUYEN.fare, fee: phi, total: CHUYEN.fare + phi, paid: 0, cost: 0 },
    pnr: ""
  });
  veTra(don);
  toast("Đã tạo đơn " + don.code + " — xem ở tab Đơn hàng.");
});

function veTra(don){
  $("#datForm").hidden = true;
  $("#datTra").hidden = false;
  $("#dCode").textContent = don.code;
  $("#dBank").textContent = NGANHANG.label;
  $("#dAcc").textContent = NGANHANG.account;
  $("#dOwner").textContent = NGANHANG.owner;
  $("#dAmount").textContent = vnd(don.money.total) + "đ";
  $("#dNote").textContent = don.code;
  clearInterval(demNguoc);
  const den = Date.parse(don.expiresAt);
  const dem = () => {
    const con = den - Date.now();
    if(con <= 0){ $("#dCount").textContent = "Đã quá hạn giữ giá — đặt lại đơn theo giá mới."; clearInterval(demNguoc); return; }
    $("#dCount").textContent = "Giá giữ thêm " + Math.floor(con / 60000) + " phút " + pad(Math.floor(con % 60000 / 1000)) + " giây.";
  };
  dem(); demNguoc = setInterval(dem, 1000);
  clearInterval(theoDoi);
  theoDoi = setInterval(async () => {
    const m = await Kho.lay(don.code);
    if(!m) return;
    const st = Kho.hienTrang(m);
    $("#dStatus").textContent = TRANGTHAI[st];
    $("#dStatus").className = "tag" + (st === "da_xuat_ve" ? " ok" : "");
    if(st !== "cho_thanh_toan"){ clearInterval(demNguoc); $("#dCount").textContent = ""; }
    if(m.pnr){ $("#dPnr").hidden = false; $("#dPnrVal").textContent = m.pnr; clearInterval(theoDoi); }
  }, 2000);
}

/* ---------------- màn đơn hàng ---------------- */
let locDon = "";
Kho.veDon = async function(){
  if(!$("#donRows")) return;                              /* khách: không có màn đơn hàng */
  const ds = await this.danhSach();
  if($("#cntDon")) $("#cntDon").textContent = ds.length;
  const thu = ds.filter(o => ["da_nhan_tien", "da_xuat_ve"].includes(o.status)).reduce((n, o) => n + (o.money.paid || 0), 0);
  const mua = ds.filter(o => o.status === "da_xuat_ve").reduce((n, o) => n + (o.money.cost || 0), 0);
  $("#donTiles").innerHTML =
    '<div class="tile"><span class="lbl">Đơn</span><b>' + ds.length + '</b></div>'
    + '<div class="tile"><span class="lbl">Đã thu của khách</span><b>' + vnd(thu) + 'đ</b></div>'
    + '<div class="tile"><span class="lbl">Đã trả cho hãng</span><b>' + vnd(mua) + 'đ</b></div>'
    + '<div class="tile"><span class="lbl">Chênh lệch</span><b style="color:var(--cheap)">' + vnd(thu - mua) + 'đ</b></div>';

  const loc = locDon ? ds.filter(o => this.hienTrang(o) === locDon) : ds;
  $("#donRows").innerHTML = loc.length ? loc.map(o => {
    const st = this.hienTrang(o);
    const traTien = st === "cho_thanh_toan" || st === "het_han";
    const muaVe = st === "da_nhan_tien";
    return '<tr><td><b class="num">' + o.code + '</b><div class="hint">'
      ...
      + '<td>' + o.flight.route + '<div class="hint">' + o.flight.airline + " " + o.flight.number + " · " + o.flight.date + " · " + o.flight.dep + '</div>'
      + (o.pnr ? '<div class="num" style="color:var(--cheap)">' + o.pnr + '</div>' : "") + '</td>'
      + '<td>' + o.pax.map(p => p.full).join("<br>") + '<div class="hint">' + (o.contact.phone || "") + '</div></td>'
      + '<td class="num">' + vnd(o.money.total) + 'đ'
      + (o.money.paid ? '<div class="hint">đã nhận ' + vnd(o.money.paid) + 'đ</div>' : "")
      + (o.money.cost ? '<div class="hint">mua ' + vnd(o.money.cost) + 'đ · chênh ' + vnd(o.money.paid - o.money.cost) + 'đ</div>' : "") + '</td>'
      + '<td><span class="tag' + (st === "da_xuat_ve" ? " ok" : "") + '">' + TRANGTHAI[st] + '</span>'
      + '<div class="hint" style="margin-top:5px">' + (o.log || []).slice(-1).map(l => l.what).join("") + '</div></td>'
      + '<td><div class="acts">'
      + (traTien ? '<button data-viec="paid" data-code="' + o.code + '">Đã nhận tiền</button>' : "")
      + (muaVe ? '<button data-viec="booking" data-code="' + o.code + '">Mã đặt chỗ</button>' : "")
      + (st !== "da_xuat_ve" && st !== "hoan_tien" && st !== "huy"
          ? '<button data-viec="refund" data-code="' + o.code + '">Hoàn tiền</button>' : "")
      + '</div></td></tr>';
  }).join("") : '<tr><td colspan="6" class="hint" style="padding:20px">Không có đơn nào ở nhóm này.</td></tr>';
};

if($("#donLoc")) $("#donLoc").addEventListener("click", e => {
  const b = e.target.closest("[data-st]"); if(!b) return;
  locDon = b.dataset.st;
  [...b.parentElement.children].forEach(c => c.setAttribute("aria-pressed", c === b ? "true" : "false"));
  Kho.veDon();
});
if($("#donRows")) $("#donRows").addEventListener("click", async e => {
  const b = e.target.closest("[data-viec]"); if(!b) return;
  const { viec, code } = b.dataset;
  const don = await Kho.lay(code);
  if(!don) return;
  if(viec === "paid"){
    await Kho.sua(code, { status: "da_nhan_tien", money: { ...don.money, paid: don.money.total } },
      "Đánh dấu đã nhận tiền · " + vnd(don.money.total) + "đ");
    toast("Đã nhận tiền đơn " + code + ". Bản trên web của anh gửi email báo khách ngay lúc này.");
  } else if(viec === "booking"){
    const pnr = prompt("Mã đặt chỗ (PNR) của đơn " + code + ":");
    if(!pnr) return;
    const gia = prompt("Giá mua vào (đ) — để tính chênh lệch, bỏ trống cũng được:") || 0;
    await Kho.sua(code, { status: "da_xuat_ve", pnr: String(pnr).toUpperCase(),
      money: { ...don.money, cost: +String(gia).replace(/\D/g, "") || 0 } },
      "Đã mua vé · " + String(pnr).toUpperCase());
    toast("Đơn " + code + " đã xuất vé. Bản trên web gửi mã đặt chỗ vào email khách.");
  } else if(viec === "refund"){
    await Kho.sua(code, { status: "hoan_tien" }, "Hoàn tiền cho khách");
    toast("Đã đánh dấu hoàn tiền đơn " + code + ".");
  }
});
