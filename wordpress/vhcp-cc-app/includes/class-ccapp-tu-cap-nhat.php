 ...
 *
 *   Lớp này chỉ lọc tag theo TIỀN TỐ của chính plugin mình (`vhcp-cc-app-v…`), nên chín plugin
...
class CCAPP_TuCapNhat {
	...
	private static function ban_hien() {
		return defined( 'CCAPP_VERSION' ) ? (string) CCAPP_VERSION : '0';
	}
	...
	private static function duong() {
		return plugin_basename( CCAPP_DIR . 'vhcp-cc-app.php' );
	}
	...
	public static function ban_moi( $bo_qua_nho = false ) {
		...
		$dau = array( 'headers' => array(
			...
			'User-Agent' => 'vhcp-cc-app-tu-cap-nhat',
		), 'timeout' => 15 );
		...
		$hien  = defined( 'CCAPP_VERSION' ) ? CCAPP_VERSION : '0';
		$tot   = null;
		foreach ( $ds as $rel ) {
