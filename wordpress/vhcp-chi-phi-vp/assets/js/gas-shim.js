 ...
 * được dịch thành 1 request POST tới /wp-json/vhcpvp/v1/call {fn:'getDon', args:[ma]}.
 *
...
(function () {
	...
	var CFG        = window.VHCPVP_CFG || {};
	var TOKEN_KEY  = 'vhcpvp_token';
	var USER_KEY   = 'vhcpvp_user';
	var reloading  = false;
	 ...
	 */
	var DUONG_KEY = 'vhcpvp_duong';
	var duong = 'rest';
	try {
		var d = localStorage.getItem( DUONG_KEY );
		if ( d === 'ajax' || d === 'trang' ) { duong = d; }
		else if ( localStorage.getItem( 'vhcpvp_ajax' ) === '1' ) { duong = 'ajax'; }
	} catch ( e ) {}
	function nhoDuong( d ) {
		duong = d;
	...
	function guiRest( fn, args, tok ) {
		return fetch( CFG.endpoint, {
			...
			headers: { 'Content-Type': 'application/json', 'X-VHCPVP-Token': tok },
	...
	/**
	 * Đường CUỐI: gửi vào chính URL của app (…/chi-phi/?vhcpvp_api=1).
	 *
	...
	function call( fn, args, onOk, onErr ) {
		...
		function thu( d ) {
			...
			return boGui( d )( fn, args, tok ).then( docKetQua ).then( function ( r ) {
				...
				try { localStorage.removeItem( DUONG_KEY ); localStorage.removeItem( 'vhcpvp_ajax' ); } catch ( e ) {}
				return r;
			} );
		}
	...
	window.addEventListener( 'load', function () {
		if ( typeof window.logout === 'function' && ! window.logout.__vhcpvp ) {
			var orig = window.logout;
			window.logout = function () {
				var tok = getToken();
				if ( tok ) { call( 'vhcpvpLogout', [ tok ], null, null ); }
				clearSession();
				return orig.apply( this, arguments );
			};
			window.logout.__vhcpvp = true;
		}
	} );
})();
