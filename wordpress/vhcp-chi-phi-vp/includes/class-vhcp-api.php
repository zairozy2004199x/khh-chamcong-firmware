<?php
/**
 * REST API — 1 cửa duy nhất: POST /wp-json/vhcpvp/v1/call  {fn, args:[...], token}
 *
...
class VHCPVP_API {
	...
	private static function required_roles( $fn ) {
		...
		// VHCPVP_Cfg::save_config() theo vai trò người đang gọi.
		if ( in_array( $fn, $cau_hinh, true ) )   { return array( 'Admin', 'Quản lý', 'Kế toán cá nhân', 'Kế toán NCC' ); }
		return array();
	}

	public static function register_routes() {
		register_rest_route( 'vhcpvp/v1', '/call', array(
			'methods'             => 'POST',
	...
	public static function ajax() {
		...
		$req = new WP_REST_Request( 'POST', '/vhcpvp/v1/call' );
		$req->set_param( 'fn', $fn );
	...
	/**
	 * Nhận lệnh ngay trên URL của app (…/chi-phi/?vhcpvp_api=1).
	 *
	...
	public static function trang() {
		...
		if ( $tok === '' && isset( $_SERVER['HTTP_X_VHCPVP_TOKEN'] ) ) {
			$tok = sanitize_text_field( wp_unslash( $_SERVER['HTTP_X_VHCPVP_TOKEN'] ) );
		}

		// GET không kèm lệnh: dùng để giao diện thử xem đường này có đi được không.
		if ( $fn === '' ) {
			status_header( 200 );
			echo wp_json_encode( array( 'ok' => true, 'data' => array( 'song' => true, 'ver' => VHCPVP_VERSION ) ) );
			exit;
		}
	...
	public static function map() {
		return array(
			...
			'aiDangDangNhap'        => array( 'VHCPVP_Auth', 'ai_dang_dang_nhap' ),
			'changePin'             => array( 'VHCPVP_Auth', 'change_pin' ),
			'vhcpvpLogout'            => array( 'VHCPVP_Auth', 'logout' ),
			'logAction'             => array( 'VHCPVP_Log', 'log_action' ),
			'getLog'                => array( 'VHCPVP_Log', 'get_log' ),
			'getDonLog'             => array( 'VHCPVP_Don', 'nhat_ky_don' ),
			'getDuAnLog'            => array( 'VHCPVP_DuAn', 'nhat_ky_du_an' ),
			'timDon'                => array( 'VHCPVP_Don', 'tim_don' ),
			'dsLoaiChiPhi'          => array( 'VHCPVP_Don', 'ds_loai_chi_phi' ),
			...
			'soatGop'               => array( 'VHCPVP_Gop', 'soat' ),
			'cosoLa'                => array( 'VHCPVP_Cfg', 'coso_la' ),
			'doiTenCoSo'            => array( 'VHCPVP_Cfg', 'doi_ten_coso' ),
			'soatNhanSu'            => array( 'VHCPVP_Cfg', 'soat_nhan_su' ),
			'doiTenNguoi'           => array( 'VHCPVP_Cfg', 'doi_ten_nguoi' ),
			'getQuyen'              => array( 'VHCPVP_Cfg', 'get_quyen' ),
			'getQuyenConfig'        => array( 'VHCPVP_Cfg', 'get_quyen_config' ),
			'setQuyen'              => array( 'VHCPVP_Cfg', 'set_quyen' ),
			'resetQuyen'            => array( 'VHCPVP_Cfg', 'reset_quyen' ),

			'dongCuaCoSo'           => array( 'VHCPVP_Cfg', 'dong_cua_coso' ),
			'hutCoSoGhe'            => array( 'VHCPVP_Cfg', 'hut_coso_ghe_api' ),

			// đơn vận hành
			'listDons'              => array( 'VHCPVP_Don', 'list_dons' ),
			'createDon'             => array( 'VHCPVP_Don', 'tao_don_moi' ),
			'getDon'                => array( 'VHCPVP_Don', 'get_don' ),
			'setTamUng'             => array( 'VHCPVP_Don', 'set_tam_ung' ),
			'setDuPhong'            => array( 'VHCPVP_Don', 'set_du_phong' ),
			'setTuExtra'            => array( 'VHCPVP_Don', 'set_tu_extra' ),
			'addLine'               => array( 'VHCPVP_Don', 'add_line' ),
			'updateLine'            => array( 'VHCPVP_Don', 'update_line' ),
			'setLineThucMua'        => array( 'VHCPVP_Don', 'set_line_thuc_mua' ),
			...
			'setLineTkNo'           => array( 'VHCPVP_Don', 'set_line_tk_no' ),
			...
			'suaKyHong'             => array( 'VHCPVP_Don', 'sua_ky_hong' ),
			'deleteLine'            => array( 'VHCPVP_Don', 'delete_line' ),
			'duplicateLine'         => array( 'VHCPVP_Don', 'duplicate_line' ),
			'guiDuyetTamUng'        => array( 'VHCPVP_Don', 'gui_duyet_tam_ung' ),
			'guiQuyetToan'          => array( 'VHCPVP_Don', 'gui_quyet_toan' ),
			'saveQuyetToan'         => array( 'VHCPVP_Don', 'save_quyet_toan' ),
			'setHoaDonQT'           => array( 'VHCPVP_Don', 'set_hoa_don_qt' ),
			...
			'duyetTamUng'           => array( 'VHCPVP_Don', 'duyet_tam_ung_ghi_lenh' ),
			'capTamUng'             => array( 'VHCPVP_Don', 'cap_tam_ung' ),
			'xacNhanQuyetToanCN'    => array( 'VHCPVP_Don', 'xac_nhan_quyet_toan_cn' ),
			'xacNhanQuyetToanNCC'   => array( 'VHCPVP_Don', 'xac_nhan_quyet_toan_ncc' ),
			'traLaiDon'             => array( 'VHCPVP_Don', 'tra_lai_don' ),
			'deleteDon'             => array( 'VHCPVP_Don', 'delete_don' ),
			'deleteDonAdmin'        => array( 'VHCPVP_Don', 'delete_don_admin' ),
			'setTatToanTuan'        => array( 'VHCPVP_Don', 'set_tat_toan_tuan' ),
			/* Đẩy đơn / dòng chi sang đơn vị khác — kế toán POSH gửi cho kế toán cá nhân. */
			'chuyenDonVi'           => array( 'VHCPVP_Don', 'chuyen_don_vi' ),
			/* Nhảy đơn sang tuần khác khi không quyết toán kịp trong tuần của nó. */
			'chuyenKy'              => array( 'VHCPVP_Don', 'chuyen_ky' ),
			'doiCoSoDon'            => array( 'VHCPVP_Don', 'doi_coso_don' ),
			'datKhoangKyDon'        => array( 'VHCPVP_Don', 'dat_khoang_ky' ),
			'dsKyQuanh'             => array( 'VHCPVP_Don', 'ds_ky_quanh_api' ),
			/* Tổng xin đổi sau khi duyệt (nhân viên sửa hạng mục, hoặc luật tính đổi) — cho
			   quản lý chốt lại số, miễn là chưa cấp tiền. */
			'duyetLaiTamUng'        => array( 'VHCPVP_Don', 'duyet_lai_tam_ung' ),
			'doiLuongDon'           => array( 'VHCPVP_Don', 'doi_luong_don' ),
			'donBuTruCu'            => array( 'VHCPVP_Don', 'don_bu_tru_cu' ),
			...
			'haTamUngVe0'           => array( 'VHCPVP_Don', 'ha_tam_ung_ve_0' ),
			/* Admin sửa lại số tạm ứng của đơn đã cấp về đúng con số (duyệt/cấp nhầm số). */
			'suaTamUngDaCap'        => array( 'VHCPVP_Don', 'sua_tam_ung_da_cap' ),
			/* Admin trả ngược "Đã cấp" → "Chờ cấp tạm ứng" để làm lại đơn sai. */
			'goCapTamUng'           => array( 'VHCPVP_Don', 'go_cap_tam_ung' ),
			/* Thùng rác: xoá nhầm đơn hoặc dòng chi thì hoàn lại được. Cố ý KHÔNG khai vào
			   ...
			   Chốt "chỉ hoàn thao tác của chính mình" nằm trong `VHCPVP_Don::hoan_tac()`, cùng
			   chỗ với chốt đơn vị — gác ở lõi thì mọi đường vào đều đi qua. */
			'dsThungRac'            => array( 'VHCPVP_Don', 'ds_thung_rac' ),
			'hoanTac'               => array( 'VHCPVP_Don', 'hoan_tac' ),
			'dsDonVi'               => array( 'VHCPVP_DonVi', 'ds' ),
			'duyetTamUngNhieu'      => array( 'VHCPVP_Don', 'duyet_tam_ung_nhieu' ),
			'dsLenhTU'              => array( 'VHCPVP_Don', 'ds_lenh_tu' ),
			'chanDoanLenhTU'        => array( 'VHCPVP_Don', 'chan_doan_lenh_tu' ),
			'dungLenhBu'            => array( 'VHCPVP_Don', 'dung_lenh_bu' ),
			'capTamUngNhieu'        => array( 'VHCPVP_Don', 'cap_tam_ung_nhieu' ),
			'traLaiDonNhieu'        => array( 'VHCPVP_Don', 'tra_lai_don_nhieu' ),
			'khongDungTamUng'       => array( 'VHCPVP_Don', 'khong_dung_tam_ung' ),
			'xacNhanQTCNNhieu'      => array( 'VHCPVP_Don', 'xac_nhan_qt_cn_nhieu' ),
			'getSoDuDauKy'          => array( 'VHCPVP_Don', 'get_so_du_dau_ky' ),
			'setSoDuDauKy'          => array( 'VHCPVP_Don', 'set_so_du_dau_ky' ),

			// sổ chi phí (nhập phẳng: chọn loại chi phí -> nhập)
			'listSoChi'             => array( 'VHCPVP_SoChi', 'list_chi' ),
			'addSoChi'              => array( 'VHCPVP_SoChi', 'add' ),
			'updateSoChi'           => array( 'VHCPVP_SoChi', 'update' ),
			'deleteSoChi'           => array( 'VHCPVP_SoChi', 'delete' ),
			'exportMisaSoChi'       => array( 'VHCPVP_SoChi', 'export_misa' ),
			'markExportedSoChi'     => array( 'VHCPVP_SoChi', 'mark_exported' ),
			'unmarkExportedSoChi'   => array( 'VHCPVP_SoChi', 'unmark_exported' ),
			'ganMaTaiKhoanSoChi'    => array( 'VHCPVP_SoChi', 'gan_ma_tai_khoan' ),
			'ganMaTaiKhoanDon'      => array( 'VHCPVP_Don', 'gan_ma_tai_khoan' ),

			// chi phí kỹ thuật (dự án)
			'createDuAn'            => array( 'VHCPVP_DuAn', 'create_du_an' ),
			'ensureCoSoChung'       => array( 'VHCPVP_DuAn', 'ensure_co_so_chung' ),
			'listDuAn'              => array( 'VHCPVP_DuAn', 'list_du_an' ),
			'renameDuAn'            => array( 'VHCPVP_DuAn', 'rename_du_an' ),
			'getDuAn'               => array( 'VHCPVP_DuAn', 'get_du_an' ),
			'addDuAnLine'           => array( 'VHCPVP_DuAn', 'add_line' ),
			'datDuToanDuAn'         => array( 'VHCPVP_DuAn', 'set_du_toan_da' ),
			'datTrangThaiHangMuc'   => array( 'VHCPVP_DuAn', 'dat_hm' ),
			'listDonHangMuc'        => array( 'VHCPVP_DuAn', 'list_don_hm' ),
			'xinTamUngDuAn'         => array( 'VHCPVP_DuAn', 'xin_tam_ung_dot' ),
			'datTrangThaiLenhDuAn'  => array( 'VHCPVP_DuAn', 'dat_tt_dot' ),
			'capTienPhanDuAn'       => array( 'VHCPVP_DuAn', 'cap_tien_phan' ),
			/* Admin dọn lệnh đã bị trả — xem `VHCPVP_DuAn::xoa_dot_tra()`, chốt Admin nằm trong hàm. */
			'xoaLenhTraDuAn'        => array( 'VHCPVP_DuAn', 'xoa_dot_tra' ),
			'listLenhDuAn'          => array( 'VHCPVP_DuAn', 'list_lenh_da' ),
			...
			'updateDuAnLine'        => array( 'VHCPVP_DuAn', 'update_line' ),
			'deleteDuAnLine'        => array( 'VHCPVP_DuAn', 'delete_line' ),
			'submitDuAn'            => array( 'VHCPVP_DuAn', 'submit' ),
			'approveDuAn'           => array( 'VHCPVP_DuAn', 'approve' ),
			'returnDuAn'            => array( 'VHCPVP_DuAn', 'ret' ),
			'closeDuAn'             => array( 'VHCPVP_DuAn', 'close' ),
			'reopenDuAn'            => array( 'VHCPVP_DuAn', 'reopen' ),
			'deleteDuAn'            => array( 'VHCPVP_DuAn', 'delete' ),
			'confirmDuAnPay'        => array( 'VHCPVP_DuAn', 'confirm_pay' ),
			'unconfirmDuAnPay'      => array( 'VHCPVP_DuAn', 'unconfirm_pay' ),

			// marketing
			'createMkDon'           => array( 'VHCPVP_MK', 'create_don' ),
			'listMkDon'             => array( 'VHCPVP_MK', 'list_don' ),
			'getMkDon'              => array( 'VHCPVP_MK', 'get_don' ),
			'addMkDonLine'          => array( 'VHCPVP_MK', 'add_line' ),
			'updateMkDonLine'       => array( 'VHCPVP_MK', 'update_line' ),
			'deleteMkDonLine'       => array( 'VHCPVP_MK', 'delete_line' ),
			'editMkDon'             => array( 'VHCPVP_MK', 'edit_don' ),
			'closeMkDon'            => array( 'VHCPVP_MK', 'close_don' ),
			'reopenMkDon'           => array( 'VHCPVP_MK', 'reopen_don' ),
			'deleteMkDon'           => array( 'VHCPVP_MK', 'delete_don' ),

			// công tác / setup
			'createBP'              => array( 'VHCPVP_BP', 'create' ),
			'listBP'                => array( 'VHCPVP_BP', 'list_bp' ),
			'getBP'                 => array( 'VHCPVP_BP', 'get' ),
			'addBPLine'             => array( 'VHCPVP_BP', 'add_line' ),
			'updateBPLine'          => array( 'VHCPVP_BP', 'update_line' ),
			'deleteBPLine'          => array( 'VHCPVP_BP', 'delete_line' ),
			'renameBP'              => array( 'VHCPVP_BP', 'rename' ),
			'closeBP'               => array( 'VHCPVP_BP', 'close' ),
			'reopenBP'              => array( 'VHCPVP_BP', 'reopen' ),
			'deleteBP'              => array( 'VHCPVP_BP', 'delete' ),

			// tra theo mã tài khoản (gom mọi mảng theo mã, thay cho việc gom số)
			'traTheoMa'             => array( 'VHCPVP_TraMa', 'search' ),
			'ganMaTaiKhoanTatCa'    => array( 'VHCPVP_TraMa', 'gan_ma_tat_ca' ),
			'dongBoTkLoai'          => array( 'VHCPVP_Cfg', 'dong_bo_tk_loai' ),
			'getTaiKhoan'           => array( 'VHCPVP_Cfg', 'get_tai_khoan' ),
			'ghepHeThongTk'         => array( 'VHCPVP_Cfg', 'ghep_he_thong_tk' ),
			'doMangTuTaiKhoan'      => array( 'VHCPVP_Cfg', 'do_mang_tu_tk' ),
			'xoaLoaiTuTao'          => array( 'VHCPVP_Cfg', 'xoa_loai_tu_tao' ),
			'khaiChiPhiChoCoSo'     => array( 'VHCPVP_Cfg', 'khai_cho_coso' ),
			'loaiCuaCoSo'           => array( 'VHCPVP_Cfg', 'loai_cua_coso' ),
			'datLoaiChoCoSo'        => array( 'VHCPVP_Cfg', 'dat_loai_cho_coso' ),
			...
			'dongBoVending'         => array( 'VHCPVP_Vending', 'dong_bo' ),
			'kiemTraVending'        => array( 'VHCPVP_Vending', 'kiem_tra' ),
			...
			// xuất MISA
			'exportMisa'            => array( 'VHCPVP_Misa', 'export_misa' ),
			'exportMisaKyThuat'     => array( 'VHCPVP_Misa', 'export_ky_thuat' ),
			'exportMisaMarketing'   => array( 'VHCPVP_Misa', 'export_marketing' ),
			'exportMisaBP'          => array( 'VHCPVP_Misa', 'export_bp' ),
			...
			'uploadDuAnDoc'         => array( 'VHCPVP_Upload', 'upload_doc' ),
			'migrateOldImages'      => array( 'VHCPVP_Upload', 'migrate_old_images' ),
		);
	}
	...
	public static function handle( WP_REST_Request $req ) {
		...
		VHCPVP_Auth::dat_vai_tro( '', '' );

		if ( ! in_array( $fn, self::$public_fns, true ) ) {
			$token = (string) $req->get_param( 'token' );
			if ( $token === '' ) { $token = (string) $req->get_header( 'x_vhcpvp_token' ); }
			$user     = VHCPVP_Auth::user_by_token( $token );
			...
			VHCPVP_Auth::dat_vai_tro( $role_ht, $user ? (string) $user['name'] : '',
			...
			if ( $need ) {
				...
				if ( ! in_array( $role, $need, true ) ) {
					return new WP_REST_Response( array(
						...
						'error' => 'Vai trò "' . ( VHCPVP_Auth::vai_hien() !== '' ? VHCPVP_Auth::vai_hien() : 'không rõ' )
							. ( ( $role !== '' && $role !== VHCPVP_Auth::vai_hien() ) ? ' (kế thừa ' . $role . ')' : '' )
		...
		/* ═══════════════════════════════════════════════════════════════════════════════════
		   ...
		   ═══════════════════════════════════════════════════════════════════════════════════ */
		if ( in_array( $fn, self::$misa_fns, true ) && ! VHCPVP_Cfg::xuat_misa_duoc() ) {
			return new WP_REST_Response( array(
				'ok'    => false,
				'error' => 'Vai trò "' . ( VHCPVP_Auth::vai_hien() !== '' ? VHCPVP_Auth::vai_hien() : 'không rõ' )
		...
		/* 🔴 CHỐT ĐƠN VỊ (K&H · POSH) — MỘT LƯỢT CHO MỌI HÀM CÓ MÃ ĐƠN.
		   ...
		   gác — xem `VHCPVP_DonVi::chan_theo_ham()` cho lý do không rải chốt vào từng hàm.
		...
		$loi_dv = VHCPVP_DonVi::chan_theo_ham( $map[ $fn ], $args );
		if ( '' !== $loi_dv ) {
