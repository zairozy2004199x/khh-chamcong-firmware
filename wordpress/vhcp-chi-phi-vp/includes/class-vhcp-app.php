 ...
 *
 *   https://<tên miền>/chi-phi-vp/            (đường dẫn tĩnh, đổi được trong Cài đặt)
 *   https://<tên miền>/?vhcpvp=app           (dùng khi permalink đang để dạng ?p=)
 *   https://<tên miền>/chi-phi-vp/?sso=<token> (đăng nhập một lần từ trang tổng)
 */
...
class VHCPVP_App {
	...
	const SLUG_MAC_DINH = 'chi-phi-vp';

	/** Đường dẫn đời đầu — giữ sống để link cũ không chết. */
	const SLUG_CU = 'chi-phi-vp';

	public static function slug() {
		$s = get_option( 'vhcpvp_slug' );
	...
	public static function app_url() {
		...
		return add_query_arg( 'vhcpvp', 'app', home_url( '/' ) );
	}
	 ...
	 *
	 *      ô Cài đặt trên host đang lưu sẵn 'chi-phi-vp' (anh Thắng dùng link ấy từ đầu)
	 *        -> slug() trả 'chi-phi-vp'
	...
	public static function init() {
		foreach ( self::cac_slug() as $s ) {
			add_rewrite_rule( '^' . $s . '/?$', 'index.php?vhcpvp_app=1', 'top' );
		}
		add_filter( 'query_vars', array( __CLASS__, 'query_vars' ) );
		add_action( 'template_redirect', array( __CLASS__, 'maybe_render' ) );
		// Nạp lại đường dẫn: xem vhcpvp_flush_rewrite() ở file chính — phải chạy SAU khi cả
	...
	public static function maybe_render() {
		$is_app = ( (int) get_query_var( 'vhcpvp_app' ) === 1 );
		if ( ! $is_app && isset( $_GET['vhcpvp'] ) && $_GET['vhcpvp'] === 'app' ) { $is_app = true; }
		if ( ! $is_app ) { return; }
	...
	public static function head_block( $tieu_de = null, $trang = '', $fns = null ) {
		...
		$cfg = array(
			'endpoint' => esc_url_raw( rest_url( 'vhcpvp/v1/call' ) ),
		...
		$out .= '<link rel="stylesheet" href="' . esc_url( VHCPVP_URL . 'assets/css/vhcp.css' ) . '?ver=' . rawurlencode( VHCPVP_VERSION ) . '">' . "\n";
		$out .= '<script>window.VHCPVP_CFG=' . wp_json_encode( $cfg ) . ';';
		if ( $sso && ! empty( $sso['token'] ) ) {
			// Nạp sẵn token cho phiên SSO (ghi đè token cũ của máy này).
			$out .= 'try{localStorage.setItem("vhcpvp_token",' . wp_json_encode( $sso['token'] ) . ');}catch(e){}';
		}
		$out .= '</script>' . "\n";
		$out .= '<script src="' . esc_url( VHCPVP_URL . 'assets/js/gas-shim.js' ) . '?ver=' . rawurlencode( VHCPVP_VERSION ) . '"></script>' . "\n";
		 ...
		 *
		 * ⚠️ DÒ CẢ LỚP LẪN HÀM. Shortcode `[vhcpvp_app]` gọi `head_block()` từ ngữ cảnh khác, và
		 ...
		 * ══════════════════════════════════════════════════════════════════════════════════ */
		if ( class_exists( 'VHCPVP_Pwa' ) && method_exists( 'VHCPVP_Pwa', 'khoi_head' ) ) {
			$out .= VHCPVP_Pwa::khoi_head();
		}
		return $out;
	}

	public static function render() {
		$file = VHCPVP_DIR . 'templates/app.html';
		if ( ! is_readable( $file ) ) {
			status_header( 500 );
		...
		$html = str_replace( '<!--VHCPVP_HEAD-->', self::head_block(), $html );
		$html = str_replace( '<!--VHCPVP_CHAN-->', self::chan_block(), $html );
	...
	public static function chan_block() {
		/* ĐƯỜNG VỀ TRẠM CHẤM CÔNG — chỉ hiện khi trang này được mở từ lưới Ứng dụng bên ấy
		   (`?ve=tram`). Lý do đầy đủ ở `vhcpvp-cham-cong/includes/class-vhcc-ve-tram.php`: tóm
	...
	/** [vhcpvp_app height="900"] — nhúng app vào 1 trang WordPress bằng iframe. */
	public static function shortcode( $atts ) {
		$a = shortcode_atts( array( 'height' => '900' ), $atts, 'vhcpvp_app' );
