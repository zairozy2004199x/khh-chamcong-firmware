...
class VHCPVP_Sheet {
	 ...
	 */
	public static function tep_tam( $dau = 'vhcpvp' ) {
	...
	private static function liet_ke_bang( $id, $cach ) {
		...
		foreach ( $out as $x ) {
			$k = $x['gid'] !== '' ? 'g' . $x['gid'] : 't' . VHCPVP_Nap::kh( $x['ten'] );
	...
	public static function doan_tu_ten( $ten ) {
		...
		foreach ( array_keys( VHCPVP_Import::types() ) as $loai ) {
			if ( VHCPVP_Nap::kh( $loai ) === $k ) { return $loai; }
		}
	...
	public static function khop_ten_tab( $ten, $ds ) {
		$k = VHCPVP_Nap::kh( $ten );
		if ( $k === '' ) { return ''; }
		$bo_da = function ( $x ) { return VHCPVP_Nap::kh( preg_replace( '/^\s*DA\s+/iu', '', (string) $x ) ); };

		// 1) khớp nguyên tên (đã bỏ dấu, bỏ hoa/thường)
		foreach ( (array) $ds as $t ) { if ( VHCPVP_Nap::kh( $t ) === $k ) { return (string) $t; } }
	...
	public static function nap_ca_file( $url, $opts = array() ) {
		$id = self::doc_id( $url );
		if ( $id === '' ) { return VHCPVP_Util::err( 'Link không phải Google Sheet — dán link có dạng docs.google.com/spreadsheets/d/…' ); }
		...
		if ( count( $ten_tay ) ) {
			...
			foreach ( (array) ( isset( $lk['tabs'] ) ? $lk['tabs'] : array() ) as $x ) {
				$gid_theo_ten[ VHCPVP_Nap::kh( $x['ten'] ) ] = array( 'gid' => $x['gid'], 'ten' => $x['ten'] );
			}
			$ten_that  = array();
			...
			if ( count( $khong_thay ) && ! count( $tabs ) ) {
				return VHCPVP_Util::err( 'Bảng tính không có tab nào tên "' . implode( '", "', $khong_thay ) . '".'
			...
			if ( empty( $lk['tabs'] ) ) { return VHCPVP_Util::err( isset( $lk['loi'] ) ? $lk['loi'] : 'Không đọc được danh sách tab' ); }
			$tabs = $lk['tabs'];
		...
		foreach ( $tabs as $tab ) {
			...
			if ( isset( $wbk[ $tab['ten'] ] ) ) {
				...
				$csv_tab = $r['body'];
				$rows    = VHCPVP_Import::parse( $csv_tab );
			}
			...
			$diem = '';
			if ( $loai === '' ) {
				$doan = VHCPVP_Nap::doan_bang( $rows );
				if ( $doan['bang'] === '' ) {
					$viec[] = array(
						'tab'    => $tab['ten'],
		...
		usort( $viec, function ( $a, $b ) {
			$x = isset( $a['loai'] ) ? VHCPVP_Sheet::uu_tien( $a['loai'] ) : 99;
			$y = isset( $b['loai'] ) ? VHCPVP_Sheet::uu_tien( $b['loai'] ) : 99;
			return $x <=> $y;
		} );
		...
		foreach ( $viec as $v ) {
			...
			if ( $thu ) {
				...
				if ( strpos( $loai, 'TD_' ) === 0 ) {
					...
					$k = VHCPVP_Nap::khop( $bang_td, $v['rows'] );
					if ( empty( $k['loi'] ) ) {
						...
						if ( $bang_td === 'sochi' ) {
							...
							foreach ( $k['rows'] as $rr ) {
								$h = VHCPVP_Nap::o( $rr, $k['hd'], 'hang_muc' );
								if ( $h !== '' ) { $hm[ mb_strtolower( $h ) ] = 1; }
							}
							$tt = 0; $k0 = 0; $dt_cha = 0; $dt_con = 0;
							foreach ( $k['rows'] as $rr ) {
								$nd0  = VHCPVP_Nap::o( $rr, $k['hd'], 'noi_dung' );
								$la_c = isset( $hm[ mb_strtolower( $nd0 ) ] );
								$dt   = VHCPVP_Util::num( VHCPVP_Util::doc_so( VHCPVP_Nap::o_so( $rr, $k, 'du_toan' ) ) );
								if ( $la_c ) { $dt_cha += $dt; continue; }   // dòng tổng hợp: bỏ phần thực chi
								$dt_con += $dt;
								$so = VHCPVP_Util::num( VHCPVP_Util::doc_so( VHCPVP_Nap::o_so( $rr, $k, 'so_tien' ) ) );
								$tt += $so;
								if ( ! $so ) { $k0++; }
							}
			...
			$res = VHCPVP_Import::run( $loai, $v['csv'], array(
				'ma'      => $ma_chon,
				'coso'    => $cs_tab['coso'],
				'replace' => $xoa_lan_nay,
				'header'  => true,
	...
	public static function coso_cua_tab( $ten_tab ) {
		...
		$hit = array();
		foreach ( VHCPVP_Cfg::cfg_static()['coso'] as $x ) {
			$c = VHCPVP_Nap::kh( $x['ten'] );
			if ( $c === '' ) { continue; }
	...
	private static function tim_hoac_tao_du_an( $ten, $cho_tao, &$tao ) {
		$ten = trim( (string) $ten );
		if ( $ten === '' ) { return ''; }
		foreach ( VHCPVP_DuAn::all_with_lines() as $d ) {
		...
		$r = VHCPVP_DuAn::create_du_an( 'Setup lắp đặt', $ten, 'Nạp từ Google Sheet' );
	...
	private static function tao_dot_con_thieu( $rows, &$tao ) {
		$k = VHCPVP_Nap::khop( 'bp_line', $rows );
		if ( ! empty( $k['loi'] ) || ! isset( $k['hd']['ma'] ) ) { return; }
		$co = array();
		foreach ( VHCPVP_BP::all_with_lines() as $b ) { $co[ (string) $b['ma'] ] = 1; }
		$can = array();
		foreach ( $k['rows'] as $r ) {
			$m = VHCPVP_Nap::o( $r, $k['hd'], 'ma' );
			if ( $m !== '' && ! isset( $co[ $m ] ) ) { $can[ $m ] = 1; }
		}
		foreach ( array_keys( $can ) as $m ) {
			VHCPVP_BP::create_voi_ma( $m, 'Công tác', '(nạp từ Sheet) ' . $m, '', '', '', 'Nạp từ Google Sheet' );
