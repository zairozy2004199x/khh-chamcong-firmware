 ...
 *
 * Mỗi "sheet cấu hình" là các dòng trong bảng vhcpvp_cfg (cột `bang` = tên sheet cũ,
...
class VHCPVP_Cfg {
	...
	public static function headers( $bang ) {
		$h = array(
			...
			/* Hai cột cuối là ĐƠN VỊ (K&H · POSH) — xem `VHCPVP_DonVi`. "Đơn vị" là NHÀ (đơn
	...
	public static function dong_cua_coso( $ten, $dong = true, $nguoi = '' ) {
		$ten = trim( (string) $ten );
		if ( $ten === '' ) { return VHCPVP_Util::err( 'Chọn cơ sở' ); }
		$k = mb_strtolower( $ten );

		$rows = array(); $thay = false;
		$ngay = $dong ? VHCPVP_Util::now()->format( 'd/m/Y' ) : '';
		...
		if ( ! $thay ) { return VHCPVP_Util::err( 'Không thấy cơ sở "' . $ten . '" trong bảng cơ sở' ); }
		...
		$n = 0;
		if ( $dong ) {
			foreach ( VHCPVP_Don::list_dons() as $d ) {
				if ( mb_strtolower( trim( (string) $d['coso'] ) ) !== $k ) { continue; }
				if ( ! empty( $d['tatToan'] ) ) { continue; }
				VHCPVP_Don::set_tat_toan_tuan( $d['maDon'], true, ( $nguoi !== '' ? $nguoi : 'Đóng gian hàng' ) );
				$n++;
			}
		}
		...
		return VHCPVP_Util::ok( array( 'coso' => $ten, 'ngay' => $ngay, 'soDonTatToan' => $n ) );
	}
	 ...
	 *    Mã lạ cũng về rỗng: `VHCPVP_Don::luong_don()` là nơi duy nhất quyết định mã nào có thật,
	...
	public static function luong_cua_bo_phan( $bp ) {
		...
		foreach ( self::read( self::BP ) as $r ) {
			...
			return VHCPVP_Don::luong_don( array( 'luong' => isset( $r[1] ) ? $r[1] : '' ) );
		}
		return '';
	}
	 ...
	 *    đó là bộ phận sẽ đi duyệt đơn này. `VHCPVP_Auth::bo_phan_bo()` trả bộ phận của phiên
	...
	public static function vai_tuy_bien() {
		...
		foreach ( self::read( self::VAI ) as $r ) {
			 ...
			 *    (`VHCPVP_Auth::dat_vai_tro()` vốn đã đọc đúng chỗ ấy — cột của vai chưa từng
	...
	/* ⚠️ `bo_phan_cua_nguoi( $ten_vai )` ĐÃ BỎ — anh Thắng 21/09/2026: *"bỏ vai trò đi, cho
	   ...
	   (`VHCPVP_Auth::dat_vai_tro()`), một nơi duy nhất. */
	...
	public static function read_all() {
		$t    = VHCPVP_DB::t( 'cfg' );
		$out  = array();
		foreach ( VHCPVP_DB::rows( "SELECT bang, cols FROM $t ORDER BY bang ASC, stt ASC, id ASC" ) as $r ) {
			$bang = (string) $r['bang'];
			$a    = json_decode( $r['cols'], true );
			if ( ! is_array( $a ) ) { $a = array(); }
			$n = count( self::headers( $bang ) );
	...
	public static function write( $bang, $rows, $snapshot = true ) {
		...
		$t = VHCPVP_DB::t( 'cfg' );
		if ( $snapshot ) {
			VHCPVP_Meta::set_json( 'cfg_undo', array( 'name' => $bang, 'data' => self::read( $bang ) ) );
		}
	...
	public static function nhan_coso_ngoai( $ten, $don_vi ) {
		...
		self::append( self::COSO, array( $ten, '', '', '', '', VHCPVP_DonVi::chuan( $don_vi ) ) );
		return true;
	}
	 ...
	 *
	 *    Xóa KHÔNG ĐƯỢC là vì bật hằng này thì `vhcpvp_maybe_upgrade()` hút lại đủ 67 gian
	 ...
	 *
	 * ⚠️ TẮT cũng tắt luôn đầu PHÁT `vhcpvp_coso_posh_da_luu` (xem `bao_coso_posh_`). Hiện
	 ...
	 *    đời site (xem cờ `vhcpvp_hut_coso_ghe` ở `vhcp-chi-phi-vp.php`). Xóa là ở yên; muốn hút lại thì
	...
	public static function bao_coso_ghe() {
		$v = get_option( 'vhcpvp_bao_coso_ghe', null );
	...
	public static function lay_coso_ghe() {
		$v = get_option( 'vhcpvp_lay_coso_ghe', null );
	...
	public static function clear_cache() {
		self::$bp_memo = null;
		self::$memo = null;
		wp_cache_delete( 'vhcpvp_cfgstatic', 'vhcpvp' );
		wp_cache_delete( 'vhcpvp_quyen', 'vhcpvp' );
		wp_cache_delete( 'vhcpvp_ssomap', 'vhcpvp' );
		delete_transient( 'vhcpvp_cfgstatic' );
		delete_transient( 'vhcpvp_quyen' );
		delete_transient( 'vhcpvp_ssomap' );
	}
	...
	private static function gieo_mot_lan( $all, $bang, $dau, $lam ) {
		if ( VHCPVP_Meta::get( $dau ) ) { return false; }          // đã gieo (hoặc đã đóng dấu) rồi
		VHCPVP_Meta::set( $dau, '1' );                             // đóng dấu TRƯỚC, cả hai lối đi
	...
	private static function seed_from( $all ) {
		...
		if ( self::gieo_mot_lan( $all, self::COSO, 'seeded_coso_v1', function () {
			foreach ( VHCPVP_Cfg::default_coso() as $c ) { VHCPVP_Cfg::append( VHCPVP_Cfg::COSO, array( $c, '', '', '', '', '' ) ); }
		} ) ) { $did = true; }
		if ( self::gieo_mot_lan( $all, self::NHOM, 'seeded_nhom_v1', function () {
			foreach ( VHCPVP_Cfg::default_nhom() as $n ) { VHCPVP_Cfg::append( VHCPVP_Cfg::NHOM, array( $n[0], $n[1], '', '' ) ); }
		} ) ) { $did = true; }
		...
		if ( ! VHCPVP_Meta::get( 'seeded_thaodo_setup_v2' ) ) {
			$did  = true;
			...
			VHCPVP_Meta::set( 'seeded_thaodo_setup_v2', '1' );
		}
		...
		/* LOẠI "Chi phí cơ sở" MỞ THÊM CHO BỘ PHẬN KỸ THUẬT — anh Thắng 11/09/2026:
		   *"bổ sung loại chi phí ( chi phí cơ sở )"*, sau khi ô Loại chi phí của đơn Kỹ thuật
		   ...
		   ⚠️ LOẠI CHƯA KHAI BỘ PHẬN NÀO THÌ ĐỂ YÊN: bỏ trống nghĩa là DÙNG CHUNG cho mọi bộ
		      ...
		      ngược điều đang cần. */
		if ( ! VHCPVP_Meta::get( 'seeded_coso_kythuat_v1' ) ) {
			$did  = true;
		 ...
		 *
		 * Khối mặc định là khối của CHÍNH BẢN ĐANG CHẠY (`VHCPVP_DB::khoi()`): dữ liệu đang có ở
		...
		if ( self::gieo_mot_lan( $all, self::LOAI, 'seeded_loai_v1', function () {
			...
			foreach ( VHCPVP_Cfg::read( VHCPVP_Cfg::NHOM ) as $r ) {
				$r = array_values( (array) $r );
				if ( trim( (string) $r[0] ) === '' ) { continue; }
				VHCPVP_Cfg::append( VHCPVP_Cfg::LOAI, array( $r[0], isset( $r[2] ) ? $r[2] : '', '', '', isset( $r[3] ) ? $r[3] : '', '' ) );
			}
		} ) ) { $did = true; }
		return $did;
	}
	...
	public static function cfg_static() {
		...
		$hit = get_transient( 'vhcpvp_cfgstatic' );
		...
		foreach ( self::rows_of( $all, self::COSO ) as $r ) {
			...
			$out['coso'][] = array( 'ten' => $r[0], 'maDonVi' => VHCPVP_Util::ma_so( $r[1] ), 'phanLoaiLon' => $r[2], 'tenMisa' => $r[3], 'dongCua' => isset( $r[4] ) ? (string) $r[4] : '',
				'donVi' => VHCPVP_DonVi::chuan( isset( $r[5] ) ? $r[5] : '' ),
				...
				'boPhan' => VHCPVP_DonVi::khoi_cua( isset( $r[7] ) ? $r[7] : '' ),
				/* TÊN BÊN DOANH THU (25/09/2026): tên cửa hàng y như trên web Doanh thu (POS/FABi) để bảng
				   "Doanh thu vs Chi phí" khớp chắc, khỏi đoán theo tên. Rỗng = khớp lỏng. Xem `VHCPVP_DoanhThu::anh_xa`. */
				'tenDoanhThu' => trim( (string) ( isset( $r[8] ) ? $r[8] : '' ) ) );
		}
		foreach ( self::rows_of( $all, self::NHOM ) as $r ) {
			if ( trim( (string) $r[0] ) === '' ) { continue; }
			$out['nhom'][] = array( 'ten' => $r[0], 'loai' => ( $r[1] !== '' ? $r[1] : 'canhan' ), 'tkNo' => VHCPVP_Util::ma_so( $r[2] ), 'boPhan' => $r[3] );
		}
		foreach ( self::rows_of( $all, self::LOAI ) as $r ) {
			...
			$out['loaiChiPhi'][] = array( 'ten' => $r[0], 'tkNo' => VHCPVP_Util::ma_so( $r[1] ), 'tkCo' => VHCPVP_Util::ma_so( $r[2] ), 'maDt' => VHCPVP_Util::ma_so( $r[3] ), 'boPhan' => $r[4], 'note' => $r[5], 'tenMisa' => isset( $r[6] ) ? $r[6] : '', 'loaiTt' => isset( $r[7] ) ? $r[7] : '', 'donVi' => isset( $r[8] ) ? $r[8] : '', 'khoi' => isset( $r[9] ) ? $r[9] : '', 'vaiTro' => isset( $r[10] ) ? $r[10] : '', 'dauMuc' => isset( $r[11] ) ? $r[11] : '', 'cha' => isset( $r[12] ) ? $r[12] : '' );
		}
		foreach ( self::rows_of( $all, self::TKNO ) as $r ) {
			if ( trim( (string) $r[0] ) === '' ) { continue; }
			$out['tkNoMatrix'][] = array( 'nhom' => $r[0], 'pll' => $r[1], 'tkNo' => VHCPVP_Util::ma_so( $r[2] ) );
		}
		...
		foreach ( self::rows_of( $all, self::BP ) as $r ) {
			$t = trim( (string) ( isset( $r[0] ) ? $r[0] : '' ) );
			if ( '' === $t ) { continue; }
			$lg = VHCPVP_Don::luong_don( array( 'luong' => isset( $r[1] ) ? $r[1] : '' ) );
		...
		foreach ( self::rows_of( $all, self::USER ) as $r ) {
			...
			$out['users'][] = array( 'ten' => $r[0], 'pin' => VHCPVP_Util::pin_sach( $r[1] ), 'vaiTro' => ( $r[2] !== '' ? $r[2] : 'Nhân viên' ), 'coso' => $r[3], 'tkCo' => VHCPVP_Util::ma_so( $r[4] ), 'maDt' => VHCPVP_Util::ma_so( $r[5] ), 'boPhan' => $r[6],
				'donVi' => isset( $r[7] ) ? trim( (string) $r[7] ) : '',
				'xemDonVi' => isset( $r[8] ) ? trim( (string) $r[8] ) : '',
				/* MÃ NV — khoá thứ hai, xem khối dài ở `VHCPVP_Auth::login()`. Ô cuối cùng nên
				   dòng cũ chín ô không có nó; `isset` lo phần ấy. */
				'maNv' => isset( $r[9] ) ? VHCPVP_Util::ma_so( $r[9] ) : '',
				/* KHỐI — anh Thắng 21/09/2026: *"chỗ đơn vị thay bằng khối, tích nếu 1 người làm 2
				   ...
				   "không thuộc khối nào" — xem `VHCPVP_DonVi::khoi_xem_duoc()`. */
		...
		/* Cơ sở -> ĐƠN VỊ. Tra nhanh, khoá đã hạ chữ thường — mọi màn hỏi "dòng chi này của
		   bên nào" đều đi qua bảng này, xem `VHCPVP_DonVi::cua_coso()`. */
		$out['cosoDonVi'] = array();
		...
		foreach ( $out['coso'] as $x ) {
			...
			if ( $k !== '' ) {
				...
				$out['cosoDonVi'][ $k ] = VHCPVP_DonVi::chuan( isset( $x['donVi'] ) ? $x['donVi'] : '' );
				$bp = isset( $x['boPhan'] ) ? (string) $x['boPhan'] : '';
				if ( '' === $bp ) {
					$kh = VHCPVP_DonVi::khoi_cua( isset( $x['donVi'] ) ? $x['donVi'] : '' );
		...
		set_transient( 'vhcpvp_cfgstatic', $out, 300 );
		self::$memo = $out;
		return $out;
	}
	...
	public static function get_config( $cp_data = null ) {
		...
		if ( $cp_data === null ) { $cp_data = VHCPVP_Don::cp_rows(); }
	...
	public static function coso_theo_don_vi( $ds ) {
		if ( ! class_exists( 'VHCPVP_DonVi' ) || null === VHCPVP_DonVi::xem_duoc() ) { return $ds; }
		$ra = array();
		foreach ( (array) $ds as $x ) {
			if ( VHCPVP_DonVi::duoc_xem( isset( $x['donVi'] ) ? $x['donVi'] : '' ) ) { $ra[] = $x; }
		}
		return $ra;
	}
	...
	public static function save_config( $cfg ) {
		...
		if ( isset( $cfg['users'] ) && is_array( $cfg['users'] ) ) {
			...
			foreach ( $cfg['users'] as $x0 ) {
				...
				if ( '' !== $t0 ) {
					if ( isset( $da_ten[ $t0 ] ) ) {
						return VHCPVP_Util::err( 'Không lưu: có HAI dòng cùng tên "'
				...
				if ( '' !== $m0 ) {
					$mk = mb_strtolower( $m0 );
					if ( isset( $da_mnv[ $mk ] ) ) {
						return VHCPVP_Util::err( 'Không lưu: mã NV "' . $m0 . '" đang khai cho HAI '
		...
		if ( isset( $cfg['coso'] ) && is_array( $cfg['coso'] ) ) {
			...
			foreach ( $cfg['coso'] as $x ) {
				...
				$rows[] = array( $tn, VHCPVP_Util::ma_so( $g( $x, 'maDonVi' ) ), $g( $x, 'phanLoaiLon' ), $g( $x, 'tenMisa' ), $dc, $dv, $tinh, $bp, $tdt );
			}
			 ...
			 * ══════════════════════════════════════════════════════════════════════════════ */
			if ( class_exists( 'VHCPVP_DonVi' ) && null !== VHCPVP_DonVi::xem_duoc() ) {
				$giu = array();
				foreach ( self::read( self::COSO ) as $r0 ) {
					$r0 = array_values( (array) $r0 );
					$t0 = trim( (string) ( isset( $r0[0] ) ? $r0[0] : '' ) );
					if ( '' === $t0 ) { continue; }
					if ( ! VHCPVP_DonVi::duoc_xem( isset( $r0[5] ) ? $r0[5] : '' ) ) { $giu[] = $r0; }
				}
		...
		if ( isset( $cfg['loaiChiPhi'] ) && is_array( $cfg['loaiChiPhi'] ) ) {
			...
			foreach ( $cfg['loaiChiPhi'] as $x ) {
				...
				$rows[] = array( $tn, VHCPVP_Util::ma_so( $g( $x, 'tkNo' ) ), VHCPVP_Util::ma_so( $g( $x, 'tkCo' ) ), VHCPVP_Util::ma_so( $g( $x, 'maDt' ) ), $g( $x, 'boPhan' ), $nt, $g( $x, 'tenMisa' ), $g( $x, 'loaiTt' ), $g( $x, 'donVi' ), $kh, $g( $x, 'vaiTro' ), $g( $x, 'dauMuc' ), $g( $x, 'cha' ) );
			}
		...
		if ( isset( $cfg['tkNoMatrix'] ) && is_array( $cfg['tkNoMatrix'] ) ) {
			$rows = array();
			foreach ( $cfg['tkNoMatrix'] as $x ) { $x = (array) $x; $rows[] = array( $g( $x, 'nhom' ), $g( $x, 'pll' ), VHCPVP_Util::ma_so( $g( $x, 'tkNo' ) ) ); }
		...
		if ( isset( $cfg['users'] ) && is_array( $cfg['users'] ) ) {
			...
			if ( VHCPVP_Auth::vai_tro() !== '' && VHCPVP_Auth::vai_tro() !== 'Admin' ) {
				$admin_cu = array();
			...
			foreach ( $cfg['users'] as $x ) {
				...
				$rows[] = array(
					$g( $x, 'ten' ),
					VHCPVP_Util::pin_sach( $g( $x, 'pin' ) ),
					( $g( $x, 'vaiTro' ) !== '' ? $g( $x, 'vaiTro' ) : 'Nhân viên' ),
					$g( $x, 'coso' ),
					VHCPVP_Util::ma_so( $g( $x, 'tkCo' ) ),
					VHCPVP_Util::ma_so( $g( $x, 'maDt' ) ),
					$g( $x, 'boPhan' ),
					$g( $x, 'donVi' ),
					$g( $x, 'xemDonVi' ),
					VHCPVP_Util::ma_so( $g( $x, 'maNv' ) ),
					$g( $x, 'khoi' )
				);
			}
		...
		if ( isset( $cfg['vaiTro'] ) && is_array( $cfg['vaiTro'] ) ) {
			...
			if ( 'Admin' !== VHCPVP_Auth::vai_tro() ) {
				return VHCPVP_Util::err( 'Chỉ Admin mới thêm/sửa vai trò được.' );
			}
			$rows = array();
		...
		if ( isset( $cfg['boPhanDs'] ) && is_array( $cfg['boPhanDs'] ) ) {
			...
			if ( 'Admin' !== VHCPVP_Auth::vai_tro() ) {
				return VHCPVP_Util::err( 'Chỉ Admin mới thêm/sửa bộ phận được.' );
			}
			$rows = array(); $da = array();
			foreach ( $cfg['boPhanDs'] as $x ) {
				...
				/* 🔴 CỘT LUỒNG PHẢI ĐI THEO, và phải chịu được CẢ HAI dạng gửi lên. Màn cũ (và
				   ...
				   ⚠️ Chuẩn hoá qua `VHCPVP_Don::luong_don()` — một nơi duy nhất biết mã nào có
				      thật, và mã lạ về rỗng = "theo khối như cũ". */
				$lg = is_array( $x ) && isset( $x['luong'] )
					? VHCPVP_Don::luong_don( array( 'luong' => $x['luong'] ) )
			...
			if ( ! $rows ) {
				return VHCPVP_Util::err( 'Phải còn ít nhất một bộ phận. Xoá hết thì hệ tự dùng lại danh sách mặc định, không phải "không có bộ phận nào".' );
			}
			self::write( self::BP, $rows );
			self::$bp_memo = null;
		}
		if ( isset( $cfg['doanhThu'] ) && is_array( $cfg['doanhThu'] ) ) {
			/* 🔴 CHỈ ADMIN — địa chỉ + khoá chia sẻ sang web Doanh thu. Khoá rỗng = giữ nguyên (xem `VHCPVP_DoanhThu::luu`). */
			if ( 'Admin' !== VHCPVP_Auth::vai_tro() ) {
				return VHCPVP_Util::err( 'Chỉ Admin mới đổi kết nối web Doanh thu được.' );
			}
		...
		if ( isset( $cfg['tinhNang'] ) && is_array( $cfg['tinhNang'] ) ) {
			...
			if ( 'Admin' !== VHCPVP_Auth::vai_tro() ) {
				return VHCPVP_Util::err( 'Chỉ Admin mới bật/tắt tính năng được.' );
			}
			$luu = self::tinh_nang_luu();
			foreach ( $cfg['tinhNang'] as $ma => $tt ) {
				$ma = (string) $ma; $tt = mb_strtolower( trim( (string) $tt ) );
				if ( ! isset( self::TINH_NANG[ $ma ] ) ) { return VHCPVP_Util::err( 'Không có tính năng mã "' . $ma . '" trong bản này.' ); }
				if ( ! in_array( $tt, self::TN_TRANG_THAI, true ) ) { return VHCPVP_Util::err( 'Trạng thái "' . $tt . '" không hợp lệ — chỉ tat · admin · bat.' ); }
				$luu[ $ma ] = $tt;
			}
			$rows = array();
			foreach ( $luu as $ma => $tt ) { $rows[] = array( $ma, $tt ); }
			self::write( self::TN, $rows );
			VHCPVP_Log::log_action( array( 'actor' => VHCPVP_Auth::nguoi(), 'role' => VHCPVP_Auth::vai_tro(), 'action' => 'Đổi cờ tính năng',
		...
		if ( isset( $cfg['dauMucDs'] ) && is_array( $cfg['dauMucDs'] ) ) {
			...
			if ( ! $rows ) {
				return VHCPVP_Util::err( 'Phải còn ít nhất một đầu mục. Xoá hết thì hệ tự dùng lại danh sách mặc định.' );
			}
	...
	public static function undo_config() {
		$snap = VHCPVP_Meta::get_json( 'cfg_undo', null );
		if ( ! $snap || empty( $snap['name'] ) ) { return VHCPVP_Util::err( 'Chưa có lần lưu nào để hồi lại' ); }
		self::write( $snap['name'], isset( $snap['data'] ) ? $snap['data'] : array(), false );
		VHCPVP_Meta::del( 'cfg_undo' );
		self::clear_cache();
		return VHCPVP_Util::ok( array( 'name' => $snap['name'] ) );
	}
	...
	private static function quyen_cot_dang_luu() {
		$m = VHCPVP_Meta::get_json( self::QUYEN_COT_O, array() );
	...
	public static function va_cot_quyen_them_vai() {
		...
		if ( ! $rows ) {
			VHCPVP_Meta::set_json( self::QUYEN_COT_O, $roles );
			return 0;
		}
	...
	public static function get_quyen() {
		$hit = get_transient( 'vhcpvp_quyen' );
		...
		foreach ( self::read( self::QUYEN ) as $r ) {
			...
			foreach ( $roles as $i => $role ) {
				...
				$o[ $role ] = $co ? VHCPVP_Util::quyen_truthy( $r[ 2 + $i ] ) : null;   // null = chưa khai
			}
			$saved[ $key ] = $o;
		}
		$out = array();
		...
		set_transient( 'vhcpvp_quyen', $out, 300 );
		return $out;
	}
	 ...
	 */
	const CO_VA_QT = 'vhcpvp_va_qt_ke_toan';
	...
	public static function reset_quyen() {
		...
		self::write( self::QUYEN, $rows, false );
		VHCPVP_Meta::set_json( self::QUYEN_COT_O, $roles );   // xem `va_cot_quyen_them_vai()`
		self::clear_cache();
		return VHCPVP_Util::ok( array( 'soHanhDong' => count( $rows ) ) );
	}
	...
	public static function tinh_nang_bat( $ma, $vai = null ) {
		...
		$vai = null === $vai ? VHCPVP_Auth::vai_tro() : (string) $vai;
		return 'Admin' === $vai;
	}
	 ...
	 *
	 * 🔴 ĐẦU MỤC TRƯỚC NAY KHÔNG KHAI ĐƯỢC. `dau_muc_ds()` đọc `get_option('vhcpvp_dau_muc_ds')`
	...
	public static function khoi_coso_chuan( $v ) {
		...
		if ( '' === $v || '*' === $v ) { return $v; }
		return in_array( $v, array_keys( VHCPVP_DonVi::KHOI_THEO_DON_VI ), true ) ? $v : '*';
	}
	...
	public static function xuat_misa_duoc() {
		if ( 'Admin' === VHCPVP_Auth::vai_tro() ) { return true; }
		$k = self::khoi_cua_vai( VHCPVP_Auth::vai_hien() );
	...
	public static function loai_row( $ten, $khoi = '' ) {
		...
		/* ═══════════════════════════════════════════════════════════════════════════════════
		   ...
		   🔴 VÌ Ô KHỐI CỦA BẢNG NÀY KHÔNG BAO GIỜ RỖNG. `VHCPVP_Cfg::write()` lấp ô trống thành
	...
	public static function loai_tk( $ten, $khoi = '' ) {
		...
		return array(
			...
			/* Tên loại CHA — dòng này là chi phí con của nó. Rỗng = dòng ở tầng trên cùng.
			   Xem chốt ở `VHCPVP_Cfg::DAU_MUC_MAC_DINH` về ba tầng của cây. */
	 ...
	 *
	 * Dùng để gác ô "kế toán chỉnh TK Nợ của một dòng" (`VHCPVP_Don::set_line_tk_no`): kế toán
	...
	public static function loai_thuoc_don_vi( $ten_loai, $don_vi ) {
		...
		foreach ( self::cfg_static_raw_loai() as $x ) {
			...
			foreach ( explode( ',', $dv ) as $d ) {
				if ( '' !== trim( $d ) && VHCPVP_DonVi::bang( $d, $don_vi ) ) { return true; }
			}
			return false;
		}
	 ...
	 *    đúng kiểu lỗi không ai truy ra. `VHCPVP_Util::ma_so()` là chỗ dọn duy nhất.
	 *
	...
	public static function tk_thuoc_cay( $tk, $loc ) {
		...
		$a = VHCPVP_Util::ma_so( $tk );
		$b = VHCPVP_Util::ma_so( $loc );
		if ( '' === $a || '' === $b ) { return false; }
		if ( $a === $b ) { return true; }
		return 0 === strpos( $a, $b );
	}
	...
	public static function tkco_xuat( $loai, $khoi, $tk_dong, $tk_phan_loai = '' ) {
		...
		/* ⚠️ ĐỘT BIẾN TƯƠNG ĐƯƠNG, ghi lại để lần sau khỏi đuổi theo: bỏ `trim()` ở dòng dưới
		   KHÔNG đổi kết quả — `VHCPVP_Cfg::write()` đã cho ô này qua `VHCPVP_Util::ma_so()`, nên giá
	...
	public static function loai_cua_coso( $coso ) {
		...
		if ( '' === $coso ) { return VHCPVP_Util::err( 'Chưa chọn cơ sở' ); }
	...
	public static function khai_cho_coso( $rec ) {
		...
		$ten  = $g( 'ten' );
		$tkno = $g( 'tkNo' );
		if ( $ten === '' ) { return VHCPVP_Util::err( 'Nhập tên gọi chi phí' ); }
		if ( $tkno === '' ) { return VHCPVP_Util::err( 'Nhập số tài khoản (TK Nợ)' ); }
		...
		if ( ! count( $cosos ) && ! count( $mangs ) ) { return VHCPVP_Util::err( 'Tích ít nhất 1 cơ sở (hoặc 1 mảng) để áp mã' ); }
	...
	public static function get_tai_khoan() {
		return VHCPVP_Util::ok( array( 'taiKhoan' => self::tai_khoan(), 'mangTk' => self::mang_tk() ) );
	}
	...
	public static function do_mang_tu_tk( $goc = '641' ) {
		...
		if ( ! count( $chart ) ) { return VHCPVP_Util::err( 'Chưa nạp hệ thống tài khoản (wp-admin → Vận Hành Chi Phí → Nhập dữ liệu → CH_TaiKhoan)' ); }
		$goc = trim( (string) $goc );
		...
		if ( ! count( $cha ) ) { return VHCPVP_Util::err( 'Không thấy nhóm tài khoản nào dưới ' . $goc . ' có tài khoản con' ); }
	...
	public static function ghep_he_thong_tk( $opts = array() ) {
		...
		$chart = self::tai_khoan();
		$mang  = self::mang_tk();
		if ( ! count( $chart ) ) { return VHCPVP_Util::err( 'Chưa nạp hệ thống tài khoản (⚙️ Cấu hình → nạp CSV → CH_TaiKhoan)' ); }
		if ( ! count( $mang ) ) { return VHCPVP_Util::err( 'Chưa khai bảng "Mảng kinh doanh → nhóm TK" nên chưa biết tài khoản nào thuộc mảng nào' ); }
	...
	public static function dong_bo_tk_loai() {
		...
		if ( ! count( $loai ) ) { return VHCPVP_Util::ok( array( 'updated' => 0, 'thieuMa' => 0, 'tong' => 0 ) ); }
		...
		return VHCPVP_Util::ok( array( 'updated' => $upd, 'thieuMa' => $thieu, 'tong' => count( $rows ) ) );
	}
	...
	public static function doi_ten_coso( $cu, $moi ) {
		...
		$cu  = trim( (string) $cu );
		$moi = trim( (string) $moi );
		if ( '' === $cu || '' === $moi ) { return VHCPVP_Util::err( 'Thiếu tên cũ hoặc tên mới' ); }
		...
		self::clear_cache();
		return VHCPVP_Util::ok( array( 'dong' => $dem, 'nguoi' => $u_doi, 'cauHinh' => $c_doi ) );
	}
	...
	private static function sao_luu_users() {
		...
		$ds = VHCPVP_Meta::get_json( 'users_bak', array() );
		...
		array_unshift( $ds, array(
			...
			'boi'   => (string) VHCPVP_Auth::nguoi(),
			'so'    => count( $rows ),
			'rows'  => $rows,
		) );
		VHCPVP_Meta::set_json( 'users_bak', array_slice( $ds, 0, self::BAK_MAX ) );
	}
	...
	public static function khoi_phuc_users( $i = 0 ) {
		$ds = VHCPVP_Meta::get_json( 'users_bak', array() );
		$i  = (int) $i;
		if ( ! isset( $ds[ $i ]['rows'] ) ) { return VHCPVP_Util::err( 'Không có bản lưu số ' . $i ); }
		...
		if ( ! $them ) { return VHCPVP_Util::ok( array( 'them' => 0, 'tong' => count( $hien ) ) ); }
		self::write( self::USER, $hien );
		self::clear_cache();
		return VHCPVP_Util::ok( array( 'them' => $them, 'tong' => count( $hien ) ) );
	}
	...
	public static function xuat_goi_cau_hinh( $bangs = null ) {
		...
		return array(
			'success'  => true,
			/* 🔴 DẤU NHẬN DẠNG KHÔNG ĐƯỢC BẮT ĐẦU BẰNG `vhcpvp`. `tach-ban-vung.sh` đổi mọi `vhcpvp` sang
			   tiền tố riêng của từng bản vùng (thêm mã hn / mtd / vp vào sau); bản đầu đặt
			   'vhcpvp-goi-cau-hinh' và bản Hà Nội hoá thành một chuỗi khác — gói tải từ Khu vui chơi
			...
			'phienBan' => defined( 'VHCPVP_VERSION' ) ? (string) VHCPVP_VERSION : '',
			'khoi'     => VHCPVP_DB::KHOI,
			'luc'      => VHCPVP_Util::now_sql(),
			'nhan'     => $ds,
			'bang'     => $out,
		);
	}
	...
	public static function nhap_goi_cau_hinh( $goi, $bangs ) {
		if ( 'Admin' !== VHCPVP_Auth::vai_tro() ) {
			return VHCPVP_Util::err( 'Chỉ Admin nhập được gói cấu hình — nhập là ghi đè cả bảng.' );
		}
		$goi = is_object( $goi ) ? (array) $goi : (array) $goi;
		if ( ! isset( $goi['loai'] ) || 'goi-cau-hinh-van-hanh-chi-phi' !== (string) $goi['loai'] ) {
			return VHCPVP_Util::err( 'Tệp không phải gói cấu hình của Vận Hành Chi Phí.' );
		}
		$bang = isset( $goi['bang'] ) ? ( is_object( $goi['bang'] ) ? (array) $goi['bang'] : $goi['bang'] ) : null;
		if ( ! is_array( $bang ) || ! $bang ) { return VHCPVP_Util::err( 'Gói rỗng — không có bảng nào.' ); }
		$ds   = self::goi_bang_ds();
		...
		if ( ! $chon ) { return VHCPVP_Util::err( 'Chưa tích bảng nào có trong gói (hoặc bảng ấy không được phép nhân bản).' ); }
		$kq = array();
		foreach ( $chon as $b ) {
			$rows = array();
		...
		self::clear_cache();
		VHCPVP_Log::log_action( array(
			'actor'  => (string) VHCPVP_Auth::nguoi(),
		...
		return VHCPVP_Util::ok( array( 'bang' => $kq ) );
	}
	...
	public static function soat_nhan_su() {
		...
		if ( null === $hs ) {
			return VHCPVP_Util::ok( array( 'coNhanSu' => false, 'nhom' => array(), 'tong' => array() ) );
		}
	...
	public static function doi_ten_nguoi( $cu, $moi, $gop = false ) {
		...
		$moi = trim( (string) $moi );
		if ( '' === $cu || '' === $moi ) { return VHCPVP_Util::err( 'Thiếu tên cũ hoặc tên mới' ); }
		if ( $cu === $moi ) { return VHCPVP_Util::err( 'Hai tên giống hệt nhau — không có gì để đổi' ); }
		...
		if ( $co_dich && $k_cu !== $k_moi && ! $gop ) {
			$dem = self::dem_don_theo_ten();
			return VHCPVP_Util::err( 'Tên "' . $moi . '" đã có tài khoản. Đổi "' . $cu . '" thành tên đó '
		...
		$doi_cot( VHCPVP_DB::t( 'session' ), 'ten' );
		...
		self::clear_cache();
		return VHCPVP_Util::ok( array( 'dong' => $dem, 'doiDong' => array_sum( $dem ),
