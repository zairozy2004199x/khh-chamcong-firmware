...
class VHCPVP_Cfg {
	...
	private static function seed_from( $all ) {
		$did = false;
		if ( ! count( self::rows_of( $all, self::COSO ) ) && ! VHCPVP_Meta::get( 'seeded_coso_v1' ) ) {
			foreach ( self::default_coso() as $c ) { self::append( self::COSO, array( $c, '', '', '', '', '' ) ); }
			VHCPVP_Meta::set( 'seeded_coso_v1', '1' );
			$did = true;
		}
		if ( ! count( self::rows_of( $all, self::NHOM ) ) && ! VHCPVP_Meta::get( 'seeded_nhom_v1' ) ) {
			foreach ( self::default_nhom() as $n ) { self::append( self::NHOM, array( $n[0], $n[1], '', '' ) ); }
			VHCPVP_Meta::set( 'seeded_nhom_v1', '1' );
			$did = true;
		}
