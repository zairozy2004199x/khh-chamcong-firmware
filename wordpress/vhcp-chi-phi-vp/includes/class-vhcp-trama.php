...
class VHCPVP_TraMa {
	...
	public static function all_lines() {
		...
		// 1) Sổ chi phí
		foreach ( VHCPVP_SoChi::all_rows() as $r ) {
			$tien = VHCPVP_Util::num( $r['so_tien'] );
			if ( ! $tien ) { continue; }
		...
		/* 2) Đơn vận hành — thực chi đi qua `VHCPVP_Don::thuc_chi()`: đã cấp tiền thì lấy Thực mua
		...
		foreach ( VHCPVP_Don::don_rows() as $d ) {
			$don_ky[ (string) $d['ma_don'] ] = VHCPVP_Util::fmt( $d['ky'] );
			$don_tt[ (string) $d['ma_don'] ] = ( $d['trang_thai'] !== '' ? (string) $d['trang_thai'] : 'Nháp' );
		}
		foreach ( VHCPVP_Don::cp_rows() as $r ) {
			$ma_don = (string) $r['ma_don'];
			$tien = VHCPVP_Don::thuc_chi( $r['thanh_tien'], $r['thuc_mua'],
		...
		foreach ( VHCPVP_DuAn::all_with_lines() as $p ) {
			$ma_da  = (string) $p['ma_da'];
			$ngay   = isset( $app_map[ $ma_da ] ) ? $app_map[ $ma_da ] : VHCPVP_Util::fmt( $p['ngay_tao'] );
			$ten_da = (string) $p['ten'];
			...
			foreach ( $p['lines'] as $x ) {
				...
				$cap  = trim( (string) $x['cap_cha'] );
				$tien = VHCPVP_Util::num( $x['thuc_te'] );
				...
				$tkm   = VHCPVP_Misa::tk_mang( $x['loai_cp'], $is_tt, $x['tk_no'], '', $x['ma_dt'], $gian );
				$out[] = array(
					'mang' => 'kt',
					'ngay' => $ngay,
					'ky'   => self::ky_of( '', $ngay ),
		...
		// 4) Marketing
		$mk_don = array();
		foreach ( VHCPVP_MK::all_dons() as $d ) { $mk_don[ (string) $d['ma'] ] = $d; }
		foreach ( VHCPVP_MK::all_lines() as $r ) {
			$tien = VHCPVP_Util::num( $r['thuc_te'] );
			if ( ! $tien ) { continue; }
			$d     = isset( $mk_don[ (string) $r['ma_don'] ] ) ? $mk_don[ (string) $r['ma_don'] ] : array( 'coso' => '', 'ten' => '', 'ky' => '', 'ngay_tao' => '' );
			$is_tt = ( trim( (string) $r['hinh_thuc'] ) === 'Trực tiếp' );
			$tkm   = VHCPVP_Misa::tk_mang( $r['loai_cp'], $is_tt, $r['tk_no'], $r['tk_co'], $r['ma_dt'], (string) $d['coso'] );
			$ngay  = VHCPVP_Util::fmt( $r['ngay'] );
			if ( $ngay === '' ) { $ngay = VHCPVP_Util::fmt( $d['ngay_tao'] ); }
			$out[] = array(
				'mang' => 'mkt',
				'ngay' => $ngay,
		...
		// 5) Công tác / Setup
		foreach ( VHCPVP_BP::all_with_lines() as $b ) {
			$lo    = (string) $b['loai'];
			$mang  = ( $lo === 'Setup' ) ? 'setup' : 'ct';
			$ngay0 = VHCPVP_Util::fmt( $b['ngay_tao'] );
			foreach ( $b['lines'] as $x ) {
				$tien = VHCPVP_Util::num( $x['thuc_te'] );
				if ( ! $tien ) { continue; }
				$is_tt = ( trim( (string) $x['hinh_thuc'] ) === 'Trực tiếp' );
				$tkm   = VHCPVP_Misa::tk_mang( $x['loai_cp'], $is_tt, $x['tk_no'], $x['tk_co'], $x['ma_dt'], (string) $b['dia_diem'] );
	...
	public static function search( $opts = array() ) {
		...
		/* 🔴 TRA THEO MÃ CŨNG LỌC THEO ĐƠN VỊ. Màn này gom dòng tiền của MỌI mảng lại một chỗ
		   ...
		   Neo theo CƠ SỞ của từng dòng, đúng chốt dùng chung `VHCPVP_DonVi::cua_coso()` — mọi
		   dòng ở đây đều có cơ sở, kể cả dòng gom từ đơn vận hành. */
		$dv_xem = VHCPVP_DonVi::xem_duoc();
		if ( null !== $dv_xem ) {
			$loc = array();
			foreach ( $all as $r0 ) {
				if ( VHCPVP_DonVi::xem_duoc_coso( isset( $r0['coso'] ) ? $r0['coso'] : '' ) ) { $loc[] = $r0; }
			}
			$all = $loc;
		}
		...
		usort( $items, function ( $a, $b ) {
			$ka = VHCPVP_TraMa::sort_key_pub( $a['ngay'] );
			$kb = VHCPVP_TraMa::sort_key_pub( $b['ngay'] );
	...
	public static function gan_ma_tat_ca( $all = false ) {
		$res = array(
			'sochi' => VHCPVP_SoChi::gan_ma_tai_khoan( $all ),
			'don'   => VHCPVP_Don::gan_ma_tai_khoan( $all ),
			'kt'    => VHCPVP_DuAn::gan_ma_tai_khoan( $all ),
			'mkt'   => VHCPVP_MK::gan_ma_tai_khoan( $all ),
			'bp'    => VHCPVP_BP::gan_ma_tai_khoan( $all ),
		);
		...
		return VHCPVP_Util::ok( array( 'updated' => $tong, 'theoMang' => $per, 'thieuMa' => array_keys( $thieu ), 'chuaChonLoai' => $chua ) );
	}
