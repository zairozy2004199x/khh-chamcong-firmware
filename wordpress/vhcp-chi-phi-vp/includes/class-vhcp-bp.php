 ...
 * nay là các dòng trong vhcpvp_bp_line khóa theo (ma, row_no), row_no bắt đầu từ 5.
 */
...
class VHCPVP_BP {
	...
	public static function lines_of( $ma ) {
		...
		return VHCPVP_DB::rows( $wpdb->prepare( "SELECT * FROM $t WHERE ma=%s ORDER BY row_no ASC", (string) $ma ) );
	}
	 ...
	 * lập thì luôn có đúng một nhà — xem chốt dài ở `VHCPVP_DuAn::don_vi_cua()`.
	 */
	...
	public static function all_with_lines() {
		$ti = VHCPVP_DB::t( 'bp_index' );
		$tl = VHCPVP_DB::t( 'bp_line' );
		$rows = VHCPVP_DB::rows( "SELECT * FROM $ti ORDER BY stt ASC" );
		$by   = array();
		foreach ( VHCPVP_DB::rows( "SELECT * FROM $tl ORDER BY ma ASC, row_no ASC" ) as $l ) {
			$by[ (string) $l['ma'] ][] = $l;
		}
		foreach ( $rows as $i => $r ) {
			$k = (string) $r['ma'];
	...
	public static function create_voi_ma( $ma, $loai, $ten, $nguoi, $dia_diem, $ky, $creator ) {
		global $wpdb;
		$ma = trim( (string) $ma );
		if ( $ma === '' ) { return VHCPVP_Util::err( 'Thiếu mã đợt' ); }
		if ( self::find( $ma ) ) { return VHCPVP_Util::ok( array( 'ma' => $ma, 'daCo' => 1 ) ); }
	...
	public static function create( $loai, $ten, $nguoi, $dia_diem, $ky, $creator ) {
		...
		if ( ! in_array( $loai, array( 'Công tác', 'Setup' ), true ) ) { return VHCPVP_Util::err( 'Loại không hợp lệ' ); }
		$ten = VHCPVP_Util::san( $ten );
		if ( $ten === '' ) { return VHCPVP_Util::err( 'Tên đợt trống' ); }
		$ky = trim( (string) $ky );
		if ( $ky === '' ) { $ky = VHCPVP_Util::now()->format( 'm/Y' ); }
	...
	public static function get( $ma ) {
		$f = self::find( $ma );
		if ( ! $f ) { return VHCPVP_Util::err( 'Không tìm thấy đợt chi phí' ); }
		$lines = array();
	...
	public static function add_line( $ma, $rec ) {
		global $wpdb;
		$dot = self::find( $ma );
		if ( ! $dot ) { return VHCPVP_Util::err( 'Không tìm thấy' ); }
		$rec = (array) $rec;
		...
		$wpdb->insert( VHCPVP_DB::t( 'bp_line' ), $data );
	...
	public static function update_line( $ma, $row, $rec ) {
		...
		$t = VHCPVP_DB::t( 'bp_line' );
		if ( ! VHCPVP_DB::row( $wpdb->prepare( "SELECT id FROM $t WHERE ma=%s AND row_no=%d", (string) $ma, $row ) ) ) { return VHCPVP_Util::err( 'Dòng không hợp lệ' ); }
	...
	public static function delete_line( $ma, $row ) {
		global $wpdb;
		if ( ! self::find( $ma ) ) { return VHCPVP_Util::err( 'Không tìm thấy' ); }
		$row = (int) $row;
		if ( $row < self::DATA_ROW ) { return VHCPVP_Util::err( 'Dòng không hợp lệ' ); }
		$wpdb->delete( VHCPVP_DB::t( 'bp_line' ), array( 'ma' => (string) $ma, 'row_no' => $row ) );
		return VHCPVP_Util::ok();
	}
	...
	public static function rename( $ma, $ten ) {
		...
		if ( ! self::find( $ma ) ) { return VHCPVP_Util::err( 'Không tìm thấy' ); }
		$wpdb->update( VHCPVP_DB::t( 'bp_index' ), array( 'ten' => $ten ), array( 'ma' => (string) $ma ) );
	...
	public static function close( $ma ) {
		global $wpdb;
		if ( ! self::find( $ma ) ) { return VHCPVP_Util::err( 'Không tìm thấy' ); }
		$wpdb->update( VHCPVP_DB::t( 'bp_index' ), array( 'trang_thai' => 'Đã đóng' ), array( 'ma' => (string) $ma ) );
		return VHCPVP_Util::ok();
	}
	...
	public static function reopen( $ma ) {
		global $wpdb;
		if ( ! self::find( $ma ) ) { return VHCPVP_Util::err( 'Không tìm thấy' ); }
		$wpdb->update( VHCPVP_DB::t( 'bp_index' ), array( 'trang_thai' => 'Đang xử lý' ), array( 'ma' => (string) $ma ) );
		return VHCPVP_Util::ok();
	}
	...
	public static function gan_ma_tai_khoan( $all = false ) {
		...
		foreach ( self::all_with_lines() as $b ) {
			foreach ( $b['lines'] as $r ) {
				...
				$giu = VHCPVP_Cfg::ma_con_hop_le( $loai, (string) $b['dia_diem'], $r['tk_no'] );
				$tk = VHCPVP_Cfg::resolve_tk( $loai, trim( (string) $r['hinh_thuc'] ), array( 'tkNo' => $giu ), (string) $b['dia_diem'] );
	...
	public static function delete( $ma ) {
		global $wpdb;
		if ( ! self::find( $ma ) ) { return VHCPVP_Util::err( 'Không tìm thấy' ); }
		$wpdb->delete( VHCPVP_DB::t( 'bp_line' ), array( 'ma' => (string) $ma ) );
		$wpdb->delete( VHCPVP_DB::t( 'bp_index' ), array( 'ma' => (string) $ma ) );
		return VHCPVP_Util::ok();
	}
}
