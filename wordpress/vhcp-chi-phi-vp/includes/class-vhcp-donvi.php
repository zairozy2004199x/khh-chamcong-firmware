...
class VHCPVP_DonVi {
	 ...
	 *
	 * Đổi tên nhà mẹ (hay tắt hẳn luật này) bằng khoá `vhcpvp_dv_me`; để trống khoá = tắt.
	 * ══════════════════════════════════════════════════════════════════════════════════════ */
	...
	public static function don_vi_me() {
		$x = get_option( 'vhcpvp_dv_me', null );
	...
	private static function ds_trong_don() {
		global $wpdb;
		$t = VHCPVP_DB::t( 'don' );
		$c = VHCPVP_DB::rows( "SELECT DISTINCT don_vi FROM $t" );
		$out = array();
	 ...
	 *
	 * ⚠️ KHAI THÊM ĐƠN VỊ thì đặt khoá `vhcpvp_dv_nhieu_coso` (danh sách ngăn bằng dấu phẩy).
	...
	public static function nhieu_coso( $don_vi ) {
		$ds = get_option( 'vhcpvp_dv_nhieu_coso', null );
	...
	public static function cua_toi() {
		return self::cua_nguoi( VHCPVP_Auth::nguoi() );
	}
	...
	const KHOI_THEO_DON_VI = array(
		...
		/* 'vending' (25/09/2026): chi phí máy bán hàng kéo từ web Vending HCMC — đơn vị VENDING, xem `VHCPVP_Vending`. */
	...
	/**
	 * CHỐT MỘT LƯỢT CHO MỌI HÀM CÓ MÃ ĐƠN — gọi từ `VHCPVP_Api::handle()`.
	 *
	 * 🔴 VÌ SAO KHÔNG RẢI CHỐT VÀO TỪNG HÀM. Riêng `VHCPVP_Don` đã có 15 hàm đụng vào một đơn mà
	...
	public static function chan_theo_ham( $callable, $args ) {
		...
		/* ⚠️ HAI NHÁNH DƯỚI CHỈ DÀNH CHO `VHCPVP_Don`. `VHCPVP_Mk::add_line()` cũng nhận `$ma_don`,
		...
		if ( method_exists( $callable[0], 'don_vi_cua' ) ) {
			/* Truyền CẢ TÊN THAM SỐ, không chỉ giá trị. `$id` của `VHCPVP_SoChi` là một dòng
			   sổ, còn `$id` của `VHCPVP_Mk` là một dòng trong đợt — hai thứ tra ở hai bảng
