 ...
 * ⚠️ Chi phí = THỰC CHI theo `VHCPVP_Don::thuc_chi()` (chưa cấp tiền → 0), tính theo NGÀY của dòng
 *    hạng mục rơi trong tháng — cùng tháng với doanh thu, không theo kỳ của đơn.
 */
class VHCPVP_DoanhThu {

	const O_URL  = 'vhcpvp_dt_url';
	const O_KHOA = 'vhcpvp_dt_khoa';
	...
	public static function luu( $d ) {
		...
		if ( null !== $url ) {
			if ( '' !== $url && ! preg_match( '#^https?://[^\s/]+#i', $url ) ) {
				return VHCPVP_Util::err( 'Địa chỉ web Doanh thu phải bắt đầu bằng http:// hoặc https://' );
			}
		...
		self::quen();
		VHCPVP_Log::log_action( array( 'actor' => VHCPVP_Auth::nguoi(), 'role' => VHCPVP_Auth::vai_tro(), 'action' => 'Sửa kết nối web Doanh thu',
	...
	public static function quen() {
		update_option( 'vhcpvp_dt_doi', (string) microtime( true ), false );
	}

	private static function khoa_nho( $tu, $den ) {
		return 'vhcpvp_dt_' . md5( self::url() . '|' . (string) get_option( 'vhcpvp_dt_doi', '' ) . '|' . $tu . '|' . $den );
	}
	...
	public static function kiem_tra() {
		...
		return VHCPVP_Util::ok( array( 'soCuaHang' => count( $kq['cuaHang'] ), 'tu' => $tu, 'den' => $den,
	...
	public static function khoang_thang( $thang ) {
		$t = trim( (string) $thang );
		if ( ! preg_match( '/^(\d{4})-(\d{2})$/', $t, $m ) ) { $t = VHCPVP_Util::now()->format( 'Y-m' ); preg_match( '/^(\d{4})-(\d{2})$/', $t, $m ); }
	 ...
	 * `$khoi` (mb/mn/…) rỗng = cả hệ; đơn chưa đóng dấu khối vẫn tính (cùng luật `VHCPVP_Report::finance`).
	 */
	public static function chi_phi_coso( $tu, $den, $khoi = '' ) {
		...
		foreach ( VHCPVP_Don::cp_rows() as $r ) {
			...
			$ngay = VHCPVP_Util::parse_date( isset( $r['ngay'] ) ? $r['ngay'] : '' );
			if ( null === $ngay || $ngay < $tu || $ngay > $den ) { continue; }
			$eff = VHCPVP_Don::thuc_chi( $r['thanh_tien'], $r['thuc_mua'], $tt_don[ $m ] );
			if ( ! $eff ) { continue; }
	...
	public static function so_sanh( $a = array() ) {
		...
		$cfg  = VHCPVP_Cfg::get_config();
		$coso = isset( $cfg['coso'] ) ? VHCPVP_Cfg::coso_theo_don_vi( $cfg['coso'] ) : array();
	 ...
	 *
	 * Vào: `$items` = mảng của `VHCPVP_DuAn::list_du_an()` (mỗi phần tử có `ten`, `loai`). Ra: CÙNG
	...
	public static function doi_gian( array $items ) {
		try {
			...
			$coso_ds = class_exists( 'VHCPVP_Cfg' ) ? (array) VHCPVP_Cfg::cfg_static()['coso'] : array();
			...
			$can = array(); $hom_nay = VHCPVP_Util::now()->format( 'Y-m-d' );
			foreach ( $items as $i => $x ) {
				...
				$dong_ngay = VHCPVP_Util::parse_date( isset( $c['dongCua'] ) ? $c['dongCua'] : '' );
