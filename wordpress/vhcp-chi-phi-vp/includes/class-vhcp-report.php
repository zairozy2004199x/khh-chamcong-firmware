...
class VHCPVP_Report {
	 ...
	 *
	 * `VHCPVP_Don::thuc_chi()` cần biết đơn đã cấp tiền chưa, mà mấy vòng dưới chỉ cầm DÒNG hạng
	...
	public static function finance( $opts = array() ) {
		...
		foreach ( VHCPVP_Don::don_rows() as $r ) {
			...
			$don_info[ (string) $r['ma_don'] ] = array(
				...
				'dp' => VHCPVP_Util::num( $r['du_phong'] ),
				'bt' => VHCPVP_Util::num( $r['bu_tru'] ),
			);
			$ky_set[ $ky ] = 1;
		}
		...
		foreach ( VHCPVP_Don::cp_rows() as $r ) {
			...
			$tt_  = VHCPVP_Util::num( $r['thanh_tien'] );
			/* 🔴 CỘT "XIN" GIỮ NGUYÊN, CỘT "THỰC CHI" THÌ KHÔNG. Xin là kế hoạch — có từ lúc
			   ...
			   `VHCPVP_Don::thuc_chi()` cùng một luật với màn Quyết toán. */
			$eff  = VHCPVP_Don::thuc_chi( $r['thanh_tien'], $r['thuc_mua'], $info['tt'] );
			$xin  = VHCPVP_Util::is_phat_sinh( $r['phat_sinh'] ) ? 0 : $tt_;
		...
		usort( $ky, function ( $a, $b ) { return VHCPVP_Util::ky_num( $a['ky'] ) <=> VHCPVP_Util::ky_num( $b['ky'] ); } );
		...
		return VHCPVP_Util::ok( array(
			'soDuDauKy'  => VHCPVP_Don::get_so_du_dau_ky(),
			'totals'     => array(
				'xin'     => $tot_xin,
				'thucTe'  => $tot_tt,
				'cl'      => $tot_xin - $tot_tt,
	...
	public static function pending_modules() {
		...
		foreach ( VHCPVP_MK::all_lines() as $r ) {
			$md = (string) $r['ma_don'];
			$agg[ $md ] = ( isset( $agg[ $md ] ) ? $agg[ $md ] : 0 ) + VHCPVP_Util::num( $r['thuc_te'] );
		}
	...
	public static function gian_report( $key ) {
		$key = trim( (string) $key );
		if ( $key === '' ) { return VHCPVP_Util::err( 'Thiếu tên gian/cơ sở' ); }
		...
		$rows = array(); $tot = 0;
		$don = array();
		foreach ( VHCPVP_MK::all_dons() as $r ) { $don[ (string) $r['ma'] ] = array( 'coso' => (string) $r['coso'], 'ten' => (string) $r['ten'] ); }
		foreach ( VHCPVP_MK::all_lines() as $r ) {
		...
		foreach ( VHCPVP_Don::cp_rows() as $r ) {
			if ( mb_strtolower( trim( (string) $r['coso'] ) ) !== $kl ) { continue; }
			$_m = (string) $r['ma_don'];
			$so = VHCPVP_Don::thuc_chi( $r['thanh_tien'], $r['thuc_mua'],
		...
		return VHCPVP_Util::ok( array( 'key' => $key, 'sections' => $sections, 'grand' => $grand ) );
	}
	...
	public static function van_hanh_tuan( $monday_str = '' ) {
		$mon = $monday_str ? VHCPVP_Util::vh_parse_dmy( $monday_str ) : null;
		if ( ! $mon ) { $mon = VHCPVP_Util::now(); }
		$mon = VHCPVP_Util::vh_monday( $mon );
		$sun = clone $mon;
		$sun->modify( '+6 day' );
		$lo = VHCPVP_Util::vh_ymd( $mon );
		$hi = VHCPVP_Util::vh_ymd( $sun );

		$in_wk = function ( $dt ) use ( $lo, $hi ) {
			if ( ! $dt ) { return false; }
			$k = VHCPVP_Util::vh_ymd( $dt );
			return $k >= $lo && $k <= $hi;
		};
		...
		$tt_don = self::tt_theo_don();
		foreach ( VHCPVP_Don::cp_rows() as $r ) {
			$dt = VHCPVP_Util::vh_parse_dmy( $r['ngay'] );
			if ( ! $in_wk( $dt ) ) { continue; }
			$_m = (string) $r['ma_don'];
			$st = VHCPVP_Don::thuc_chi( $r['thanh_tien'], $r['thuc_mua'],
				isset( $tt_don[ $_m ] ) ? $tt_don[ $_m ] : '' );
			if ( ! $st ) { continue; }
			$k = $co( $r['coso'] );
			$map[ $k ]['vh'] += $st;
		...
		return VHCPVP_Util::ok( array(
			...
			'weekNo'     => VHCPVP_Util::vh_week_no( $mon ),
