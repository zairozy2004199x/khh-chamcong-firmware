...
class VHCPVP_Misa {
	...
	public static function tk_mang( $loai_cp, $is_tt, $line_tk_no = '', $line_tk_co = '', $line_ma_dt = '', $coso = '' ) {
		...
		$tk = VHCPVP_Cfg::resolve_tk( $loai_cp, $is_tt ? 'Trực tiếp' : 'Tạm ứng', array(
			'tkCo' => trim( (string) $line_tk_co ),
			'maDt' => trim( (string) $line_ma_dt ),
		), $coso );
		// TK Nợ đi theo LUẬT LÚC XUẤT (xem VHCPVP_Cfg::tkno_xuat) — chung với đơn vận hành
		// và sổ chi phí, để 5 đường xuất không mỗi đường một kiểu.
		$tk['tk_no'] = VHCPVP_Cfg::tkno_xuat( $loai_cp, $coso, $line_tk_no );
		$tk['legacy'] = false;
		return $tk;
	}
	...
	public static function gom_ngay_xau( &$xau, $dmy, $raw, $ma = '' ) {
		if ( ! VHCPVP_Util::ngay_vo_ly( $dmy ) ) { return; }
		$k = $dmy . '|' . (string) $raw;
	...
	private static function clean_nhom( $nhom ) {
		return VHCPVP_Cfg::bo_duoi_nhom( $nhom );
	}
	 ...
	 *  Bắt cả tuần chuẩn 7 ngày (`VHCPVP_Don::nhan_ky`) lẫn tuần cũ bị cắt theo tháng ("1/9-6/9/2026",
	...
	public static function la_tuan_chuan( $ky ) {
		list( $tu, $den ) = VHCPVP_Don::khoang_ky( $ky );
	...
	public static function export_misa( $ky = 'all', $mode = 'chuaxuat', $pl_f = 'all', $mau = self::MAU_CHUAN, $tk_f = 'all', $mang_f = 'all' ) {
		...
		$tk_f = VHCPVP_Util::ma_so( trim( (string) $tk_f ) );
		...
		$pl_f = $pl_f ? $pl_f : 'all';
		$cp   = VHCPVP_Don::cp_rows();              // đọc 1 lần, dùng cho cả cấu hình đối tượng lẫn vòng lặp dưới
		...
		foreach ( VHCPVP_Don::don_rows() as $r ) {
			$ky2 = VHCPVP_Util::fmt( $r['ky'] );
			if ( $ky && $ky !== 'all' && $ky2 !== $ky ) { continue; }
			$qt_cn  = ( (string) $r['nguoi_qt'] !== '' );
			$qt_ncc = ( (string) $r['nguoi_qt_ncc'] !== '' );
			$x_cn   = ( VHCPVP_Util::fmt( $r['ngay_xuat_cn'] ) !== '' );
			$x_ncc  = ( VHCPVP_Util::fmt( $r['ngay_xuat_ncc'] ) !== '' );
			...
			elseif ( $pl_f === 'ncc' )   { $take = $qt_ncc && ( $mode === 'daxuat' ? $x_ncc : ! $x_ncc ); }
				...
				$san = VHCPVP_Don::tt_truoc_misa( isset( $r['khoi'] ) ? $r['khoi'] : '',
					VHCPVP_Don::luong_don( $r ) );
			...
			if ( ! VHCPVP_DonVi::duoc_xem( isset( $r['don_vi'] ) ? $r['don_vi'] : '' ) ) { continue; }
			$ngay = VHCPVP_Util::fmt( $r['ngay_qt'] );
			if ( $ngay === '' ) { $ngay = VHCPVP_Util::fmt( $r['ngay_tao'] ); }
		...
		foreach ( $cp as $r ) {
			...
			$eff_ncc = VHCPVP_Util::is_ncc( $pltt, $r['cn_xu_ly'] );
			...
			$tm     = VHCPVP_Util::blank_or_num( $r['thuc_mua'] );
			...
			$tk_no_ms = VHCPVP_Util::ma_so( $tk_no );
			...
			if ( ! $tk_no ) { $warn[ 'Thiếu TK Nợ cho loại chi phí: ' . VHCPVP_Cfg::bo_duoi_nhom( $nhom ) . ( $pll !== '' ? ' (mảng ' . $pll . ')' : '' ) . ' — khai ở ⚙️ Cấu hình → Loại chi phí' ] = 1; }
			...
			if ( ! $tk_co ) { $warn[ 'Thiếu TK Có cho hình thức chi: ' . ( '' !== trim( (string) $co_key ) ? $co_key : '(trống)' ) . ' — khai ở ⚙️ Cấu hình → Phân loại thanh toán, hoặc khai TK đối ứng riêng cho loại "' . VHCPVP_Cfg::bo_duoi_nhom( $nhom ) . '"' ] = 1; }
			if ( ! $ma_dv ) { $warn[ 'Thiếu Mã đơn vị cho cơ sở: ' . $coso ] = 1; }

			$ngay = VHCPVP_Util::fmt( $r['ngay'] );
			if ( $ngay === '' ) { $ngay = $d['ngay']; }
			VHCPVP_Misa::gom_ngay_xau( $ngay_xau, $ngay, $r['ngay'], $m );
			$nhom_c   = self::clean_nhom( $nhom );
			/* Cột "Tên theo MISA" của loại (bảng Loại chi phí) — khai thì diễn giải dùng tên ấy. */
			$nhom_dg  = VHCPVP_Cfg::ten_misa_loai( $nhom_c );
			...
			$dg1      = VHCPVP_Util::j( array( $nhom_dg, $pll, $ky_dg ) ) . ( $ten1 !== '' ? '_' . $ten1 : '' );
			$dg2      = VHCPVP_Util::j( array( $nhom_dg, $pll, self::ten_coso_gon( $ten_misa, $pll ), $ky_dg ) ) . ( trim( $nd ) !== '' ? '_' . $nd : '' );
			...
			if ( self::MAU_SOCT === $mau ) {
				$r_out = array( $ngay, $ngay, $m, $dg1, $dg2,
					VHCPVP_Util::ma_so( $tk_co ), $sotien, 0, '', '',
					VHCPVP_Util::ma_so( $ma_dt ), VHCPVP_Util::ma_so( $ma_dv ), $ten_misa );
			} else {
				$r_out = array( $ngay, $ngay, '', $dg1, $dg2,
					VHCPVP_Util::ma_so( $tk_no ), VHCPVP_Util::ma_so( $tk_co ), $sotien,
					VHCPVP_Util::ma_so( $ma_dt ), VHCPVP_Util::ma_so( $ma_dv ) );
			}
			$rows_by_nhom[ $gk ][] = array(
				'k'  => ( $_dt = VHCPVP_Util::vh_parse_dmy( $ngay ) ) ? VHCPVP_Util::vh_ymd( $_dt ) : 0,
				'i'  => $stt_chen++,
				'r'  => $r_out,
				'tk' => $tk_no_ms,
	...
	public static function export_ky_thuat() {
		$cfg = VHCPVP_Cfg::cfg_static();   // chỉ cần danh mục cơ sở -> khỏi đọc bảng ChiPhi để gộp đối tượng
		$m_unit = array(); $m_pll = array(); $m_tm = array();
		foreach ( $cfg['coso'] as $x ) { $m_unit[ $x['ten'] ] = $x['maDonVi']; $m_pll[ $x['ten'] ] = $x['phanLoaiLon']; $m_tm[ $x['ten'] ] = $x['tenMisa']; }
		$user_dt = array();
		foreach ( VHCPVP_Cfg::get_users() as $u ) {
		...
		$pay_map = VHCPVP_DuAn::pay_map();
		$app_map = VHCPVP_DuAn::approve_date_map();

		$rows = array(); $warn = array(); $nda = 0; $ngay_xau = array();
		foreach ( VHCPVP_DuAn::all_with_lines() as $r ) {
			...
			if ( $ngay === '' ) { $ngay = VHCPVP_Util::fmt( $r['ngay_tao'] ); }
			VHCPVP_Misa::gom_ngay_xau( $ngay_xau, $ngay, $r['ngay_tao'], $ma_da );
			...
			foreach ( $r['lines'] as $x ) {
				...
				$cap     = trim( (string) $x['cap_cha'] );
				$thuc_te = VHCPVP_Util::num( $x['thuc_te'] );
				...
				if ( ! empty( $tkm['legacy'] ) && ! $is_tt ) {
					...
					$_ung    = VHCPVP_DuAn::pay_ds( $pay, 'tamUng', 'tu' );
				...
				$dg1 = VHCPVP_Util::j( array( $ten_da, $loai, $pll ) ) . ( $is_tt ? '_Trực tiếp NCC' : '_Tạm ứng NV' );
				$dg2 = VHCPVP_Util::j( array( $hang_muc, $ten_misa ) );
				$tail = VHCPVP_Util::j( array( $noi_dung, $ghichu ) );
				if ( $tail !== '' ) { $dg2 .= '_' . $tail; }

				$rows[] = array( $ngay, $ngay, '', $dg1, $dg2, VHCPVP_Util::ma_so( $tk_no ), VHCPVP_Util::ma_so( $tk_co ), $thuc_te, VHCPVP_Util::ma_so( $ma_dt ), VHCPVP_Util::ma_so( $ma_dv ) );
				$used   = true;
			}
			if ( $used ) { $nda++; }
		}
		return array( 'cols' => self::cols(), 'rows' => $rows, 'count' => count( $rows ), 'sodon' => $nda,
			'warn' => array_merge( array_keys( $warn ), VHCPVP_Misa::warn_ngay_xau( $ngay_xau ) ), 'maDons' => array() );
	}
	...
	public static function export_marketing() {
		...
		$don = array();
		foreach ( VHCPVP_MK::all_dons() as $r ) {
			$don[ (string) $r['ma'] ] = array(
				'coso'   => (string) $r['coso'],
				'ten'    => (string) $r['ten'],
				'ky'     => VHCPVP_Util::fmt( $r['ky'] ),
				'kenhCD' => (string) $r['kenh'],
				'ngay'   => VHCPVP_Util::fmt( $r['ngay_tao'] ),
			);
		}
		$rows = array(); $warn = array(); $seen = array(); $ngay_xau = array();
		foreach ( VHCPVP_MK::all_lines() as $r ) {
			$tt = VHCPVP_Util::num( $r['thuc_te'] );
			if ( ! $tt ) { continue; }
			...
			$ngay  = VHCPVP_Util::fmt( $r['ngay'] );
			if ( $ngay === '' ) { $ngay = $d['ngay']; }
			VHCPVP_Misa::gom_ngay_xau( $ngay_xau, $ngay, $r['ngay'], (string) $r['ma_don'] );
			$ma_dv    = isset( $m_unit[ $coso ] ) ? $m_unit[ $coso ] : '';
			$ten_misa = ! empty( $m_tm[ $coso ] ) ? $m_tm[ $coso ] : $coso;
			...
			$dg1 = VHCPVP_Util::j( array( 'MKT', $lc !== '' ? VHCPVP_Cfg::ten_misa_loai( $lc ) : '', $d['ten'], $coso, $kenh, $d['ky'] ) ) . ( $is_tt ? '_Trực tiếp NCC' : '_Tạm ứng NV' );
			$dg2 = VHCPVP_Util::j( array( $nd, $ten_misa ) ) . ( trim( $gc ) !== '' ? '_' . $gc : '' );
			$rows[] = array( $ngay, $ngay, '', $dg1, $dg2, VHCPVP_Util::ma_so( $tkm['tk_no'] ), VHCPVP_Util::ma_so( $tkm['tk_co'] ), $tt, VHCPVP_Util::ma_so( $tkm['ma_dt'] ), VHCPVP_Util::ma_so( $ma_dv ) );
	...
	public static function export_bp( $loai = 'all' ) {
		...
		foreach ( VHCPVP_BP::all_with_lines() as $r ) {
			...
			$ky       = VHCPVP_Util::fmt( $r['ky'] );
			$ngay_dot = VHCPVP_Util::fmt( $r['ngay_tao'] );
			$used     = false;
			foreach ( $r['lines'] as $x ) {
				...
				VHCPVP_Misa::gom_ngay_xau( $ngay_xau, $ngay, $x['ngay'], (string) $r['ma'] );
				$ghichu   = trim( (string) $x['note'] );
				...
				$dg1 = VHCPVP_Util::j( array( $lo, $ten, $nguoi, $ky ) ) . ( $is_tt ? '_Trực tiếp NCC' : '_Tạm ứng NV' );
				$dg2 = VHCPVP_Util::j( array( $nd, $ten_misa ) ) . ( $ghichu !== '' ? '_' . $ghichu : '' );
				$rows[] = array( $ngay, $ngay, '', $dg1, $dg2, VHCPVP_Util::ma_so( $tkm['tk_no'] ), VHCPVP_Util::ma_so( $tkm['tk_co'] ), $tt, VHCPVP_Util::ma_so( $tkm['ma_dt'] ), VHCPVP_Util::ma_so( $ma_dv ) );
				$used   = true;
			}
			if ( $used ) { $ndot++; }
	...
	public static function ma_cua_dong( $r, $khoi, $cf, $da_xuat = false ) {
		...
		$pltt    = (string) $r['phan_loai_tt'];
		$nhom    = (string) $r['nhom'];
		$eff_ncc = VHCPVP_Util::is_ncc( $pltt, isset( $r['cn_xu_ly'] ) ? $r['cn_xu_ly'] : null );
		$co_key  = $eff_ncc ? 'Nhà cung cấp' : $pltt;
		$pll     = isset( $cf['pll'][ $coso ] ) ? $cf['pll'][ $coso ] : '';
		$tk_dong = trim( (string) ( isset( $r['tk_no'] ) ? $r['tk_no'] : '' ) );
		$co_dong = trim( (string) ( isset( $r['tk_co'] ) ? $r['tk_co'] : '' ) );

		if ( $da_xuat && '' !== $tk_dong && ! VHCPVP_Cfg::la_tk_ben_tra( $tk_dong ) ) {
		...
		$tk_no  = VHCPVP_Cfg::tkno_xuat( $nhom, $coso, $tk_dong );
		$nhom_k = mb_strtolower( VHCPVP_Cfg::bo_duoi_nhom( $nhom ) );
		...
		if ( $tk_no === '' && ! empty( $cf['no_mx'][ $mx_k ] ) && ! VHCPVP_Cfg::la_tk_ben_tra( $cf['no_mx'][ $mx_k ] ) ) { $tk_no = $cf['no_mx'][ $mx_k ]; }
		if ( $tk_no === '' && ! empty( $cf['no'][ $nhom ] ) && ! VHCPVP_Cfg::la_tk_ben_tra( $cf['no'][ $nhom ] ) ) { $tk_no = $cf['no'][ $nhom ]; }
	...
	private static function tk_co_cua( $nhom, $khoi, $co_dong, $co_key, $cf ) {
		return (string) VHCPVP_Cfg::tkco_xuat( $nhom, $khoi, $co_dong,
	...
	public static function ban_do_ma( $cfg ) {
		...
		foreach ( (array) $cfg['tkNoMatrix'] as $x ) { $m_no_mx[ trim( (string) $x['nhom'] ) . '|' . trim( (string) $x['pll'] ) ] = $x['tkNo']; }
		foreach ( (array) ( isset( $cfg['loaiChiPhi'] ) ? $cfg['loaiChiPhi'] : array() ) as $x ) {
			if ( trim( (string) $x['tkNo'] ) === '' || VHCPVP_Cfg::la_tk_ben_tra( $x['tkNo'] ) ) { continue; }
			$m_loai[ mb_strtolower( trim( (string) $x['ten'] ) ) ] = (string) $x['tkNo'];
		}
	...
	public static function dong_ma_dong( $ma_don, $pl_f, $cf, $khoi ) {
		global $wpdb;
		$t = VHCPVP_DB::t( 'chiphi' ); $n = 0;
		$rows = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $t WHERE ma_don=%s", (string) $ma_don ), ARRAY_A );
		foreach ( (array) $rows as $r ) {
			$eff_ncc = VHCPVP_Util::is_ncc( (string) $r['phan_loai_tt'], isset( $r['cn_xu_ly'] ) ? $r['cn_xu_ly'] : null );
			if ( $pl_f === 'cn' && $eff_ncc ) { continue; }
			if ( $pl_f === 'ncc' && ! $eff_ncc ) { continue; }
	...
	public static function mark_exported( $ma_dons, $pl_f = 'all' ) {
		...
		if ( ! count( $ma_dons ) ) { return VHCPVP_Util::err( 'Không có đơn để chốt' ); }
		$now = VHCPVP_Util::now_sql();
		$n   = 0; $n_ma = 0;
		/* Bản đồ mã dựng một lần cho cả lô — đóng mã theo đúng bảng đang có ở GIÂY PHÚT chốt. */
		$cf = self::ban_do_ma( VHCPVP_Cfg::get_config( VHCPVP_Don::cp_rows() ) );
		foreach ( $ma_dons as $m ) {
			...
			$data  = array();
			$x_cn  = ( VHCPVP_Util::fmt( $d['ngay_xuat_cn'] ) !== '' );
			$x_ncc = ( VHCPVP_Util::fmt( $d['ngay_xuat_ncc'] ) !== '' );
			if ( ( $pl_f === 'cn' || $pl_f === 'all' ) && ! $x_cn )  { $data['ngay_xuat_cn'] = $now; $x_cn = true; }
			if ( ( $pl_f === 'ncc' || $pl_f === 'all' ) && ! $x_ncc ) { $data['ngay_xuat_ncc'] = $now; $x_ncc = true; }
			$loai = VHCPVP_Don::don_loai( $m );
			if ( ( ! $loai['cn'] || $x_cn ) && ( ! $loai['ncc'] || $x_ncc ) ) { $data['trang_thai'] = 'Đã xuất MISA'; }
			if ( count( $data ) ) { $wpdb->update( VHCPVP_DB::t( 'don' ), $data, array( 'ma_don' => (string) $m ) ); }
			$n++;
		}
		if ( $n_ma ) {
			VHCPVP_Log::log_action( array( 'actor' => VHCPVP_Auth::nguoi(), 'role' => VHCPVP_Auth::vai_tro(), 'action' => 'Đóng mã TK khi xuất MISA',
				'target' => implode( ',', array_map( 'strval', $ma_dons ) ), 'detail' => $n_ma . ' dòng đóng mã TK Nợ/TK Có theo bảng lúc xuất' ) );
		}
		return VHCPVP_Util::ok( array( 'count' => $n, 'dongMa' => $n_ma ) );
	}
}
