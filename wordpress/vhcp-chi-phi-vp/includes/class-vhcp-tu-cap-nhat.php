 ...
 *
 *   Lớp này chỉ lọc tag theo TIỀN TỐ của chính plugin mình (`vhcp-chi-phi-vp-v…`), nên chín plugin
...
class VHCPVP_TuCapNhat {
	...
	public static function init() {
		...
		/* 🔴 ĐỔI TÊN THƯ MỤC SAU KHI GIẢI NÉN. Tệp .zip của Releases giải ra thành thư mục
		   `vhcp-chi-phi-vp/` — đúng tên plugin đang cài, nên WordPress đặt lại đúng chỗ. Nhưng nếu
		...
		add_filter( 'vhcpvp_tu_cap_nhat_ds', array( __CLASS__, 'khai_ds' ) );
	}
	 ...
	 * danh sách plugin nào cả — nó dựng bảng từ bộ lọc `vhcpvp_tu_cap_nhat_ds` lúc chạy. Nên plugin
	...
	private static function ban_hien() {
		return defined( 'VHCPVP_VERSION' ) ? (string) VHCPVP_VERSION : '0';
	}
	...
	/** Đường dẫn plugin dạng `vhcp-chi-phi-vp/vhcp-chi-phi-vp.php` — khoá WordPress dùng để nhận plugin. */
	private static function duong() {
		return plugin_basename( VHCPVP_DIR . 'vhcp-chi-phi-vp.php' );
	}
	...
	public static function ban_moi( $bo_qua_nho = false ) {
		...
		$hien  = defined( 'VHCPVP_VERSION' ) ? VHCPVP_VERSION : '0';
		$tot   = null;
		foreach ( $ds as $rel ) {
			...
			/* 🔴 LỌC THEO TIỀN TỐ CỦA CHÍNH PLUGIN NÀY. Không có bước này thì bản chấm công
			   `vhcpvp-cham-cong-v3.76.0` hiện ra như bản mới của chi phí. */
