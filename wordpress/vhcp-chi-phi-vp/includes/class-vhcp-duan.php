...
class VHCPVP_DuAn {
	...
	public static function create_du_an( $loai, $ten, $nguoi, $tu = '', $den = '' ) {
		...
		$wpdb->insert( VHCPVP_DB::t( 'da_index' ), array(
			...
			'loai'       => $loai,
			'trang_thai' => 'Đang làm',
			/* Mảng đóng dấu ngay lúc lập — xem chốt ở `VHCPVP_Don::create_don()`. */
			'khoi'       => VHCPVP_DB::khoi(),
	...
	public static function list_du_an() {
		...
		if ( class_exists( 'VHCPVP_DoanhThu' ) ) { $out = VHCPVP_DoanhThu::doi_gian( $out ); }
	...
	public static function get_du_an( $ma_da ) {
		...
		foreach ( self::lines_of( $ma_da ) as $r ) {
			...
			$lines[] = array(
				...
				'taoLuc'    => VHCPVP_Util::fmt( isset( $r['tao_luc'] ) ? $r['tao_luc'] : '' ),
		...
		return VHCPVP_Util::ok( array(
			...
			'ngayTao'         => VHCPVP_Util::fmt( $f['ngay_tao'] ),
			'kyDA'            => self::get_ky_da( $ma_da ),
			'lenh'            => $lenh,
			'lenhQT'          => $lenhQT,
	...
	public static function cap_tien_phan( $ma_da, $dot, $them = array() ) {
		...
		if ( $het ) {
			...
			/* `$nay = 0` chỉ xảy ra khi lệnh mất hàng (sổ cũ) — `update_line` không cho gõ cả dự
			   ...
			   số đã đưa. */
			$du_nay = self::da_cap_tong( $moi ) - ( $nay > 0 ? $nay : VHCPVP_Util::num( $moi['soTien'] ) );
	...
	public static function them_dong_muc_con_cu( $ma_da, $rec ) {
		...
		$data['tao_luc'] = VHCPVP_Util::now_sql();   // xem 🔴 ở `add_line()`
		$ok_dl = $wpdb->insert( VHCPVP_DB::t( 'da_line' ), $data );
		/* 🔴 SOI KẾT QUẢ — xem `kiem-ghi-so-phai-soi-ket-qua.php`. */
		if ( ! $ok_dl ) { $ct = trim( (string) $wpdb->last_error );
			return VHCPVP_Util::err( 'Không ghi được dòng dự án' . ( '' !== $ct ? ' — ' . $ct : '.' ) ); }
	 ...
	 * ═════════════════════════════════════════════════════════════════════════════════════════
	 * Loại chi phí quyết định TK Nợ / TK Có / mã đối tượng (`VHCPVP_Cfg::resolve_tk`). Chọn sai là
	 * dòng ấy vào sai tài khoản, và cái sai chỉ lộ ra lúc xuất MISA — khi đã quá muộn để hỏi lại
	 * người nhập họ mua cái gì.
	 *
	 * ⚠️ CHỈ KẾ TOÁN — cùng luật với `VHCPVP_Don::set_line_nhom()` bên đơn tuần. Gắn mã hạch toán
	...
	public static function dat_loai_cp_line( $ma_da, $row, $loai_cp ) {
		...
		if ( '' === $loai_cp ) { return VHCPVP_Util::err( 'Chọn loại chi phí' ); }

		$tk = VHCPVP_Cfg::resolve_tk( $loai_cp, (string) $cur['hinh_thuc'],
		...
		return VHCPVP_Util::ok( array( 'loaiCp' => $loai_cp, 'tkNo' => $tk['tk_no'], 'tkCo' => $tk['tk_co'] ) );
	}
