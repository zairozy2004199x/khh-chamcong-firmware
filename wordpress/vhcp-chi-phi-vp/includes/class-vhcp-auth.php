...
class VHCPVP_Auth {
	 ...
	 *
	 * Bảng hàm gọi thẳng vào VHCPVP_Cfg::save_config() nên bên đó không biết ai đang lưu.
	...
	public static function dat_vai_tro( $r, $ten = '', $coso = '', $bo_phan = '', $ma_nv = '' ) {
		...
		self::$vai_tro  = class_exists( 'VHCPVP_Cfg' ) ? VHCPVP_Cfg::vai_goc( (string) $r ) : (string) $r;
	 ...
	 * dài ở `VHCPVP_Cfg::bo_phan_cua_nguoi()`.
	 */
	/* ══════════════════════════════════════════════════════════════════════════════════════
	...
	public static function xem_duoc_loai( $ten_loai ) {
		...
		return VHCPVP_Cfg::loai_thuoc_bo_phan( $ten_loai, $bo );
	}
	...
	public static function so_ma_tat() {
		...
		if ( ! class_exists( 'VHCPVP_Meta' ) || ! method_exists( 'VHCPVP_Meta', 'get' ) ) { return self::$so_tat; }
		$raw = (string) call_user_func( array( 'VHCPVP_Meta', 'get' ), self::O_MA_TAT, '' );
		foreach ( preg_split( '/\r\n|\r|\n/', $raw ) as $dong ) {
	...
	public static function dat_ma_tat( $raw ) {
		...
		/* ⚠️ Gác CÙNG HÀM với lời gọi — luật `tools/test/kiem-goi-cheo.php`. */
		if ( class_exists( 'VHCPVP_Meta' ) && method_exists( 'VHCPVP_Meta', 'set' ) ) {
			call_user_func( array( 'VHCPVP_Meta', 'set' ), self::O_MA_TAT, implode( "\n", $sach ) );
		}
	...
	public static function so_coso() {
		...
		/* ⚠️ ĐỌC `cfg_static()`, KHÔNG ĐỌC `get_config()`. Hàm này chạy trên MỌI lượt lọc đơn,
		   ...
		   ⚠️ Gác CÙNG HÀM với lời gọi — luật `tools/test/kiem-goi-cheo.php`. */
		if ( class_exists( 'VHCPVP_Cfg' ) && method_exists( 'VHCPVP_Cfg', 'cfg_static' ) ) {
			$cfg = (array) call_user_func( array( 'VHCPVP_Cfg', 'cfg_static' ) );
	 ...
	 *
	 * 🔴 CHỈ TRẢ LỜI "CÓ ĐƯỢC ĐỌC KHÔNG". Chốt đơn vị nằm chỗ khác (`VHCPVP_DonVi::duoc_xem`),
	...
	public static function coso_cua_nguoi( $ten ) {
		...
		if ( '' === $k ) { return array(); }
		/* ⚠️ Gác CÙNG HÀM với lời gọi — luật `tools/test/kiem-goi-cheo.php`. */
		if ( ! class_exists( 'VHCPVP_Cfg' ) || ! method_exists( 'VHCPVP_Cfg', 'get_users' ) ) { return array(); }
		foreach ( (array) call_user_func( array( 'VHCPVP_Cfg', 'get_users' ) ) as $u ) {
			$u = (array) $u;
	...
	public static function login( $pin ) {
		...
		$u = null;
		foreach ( VHCPVP_Cfg::get_users() as $x ) {   // get_users() tự seed nếu cấu hình còn trống
		...
		if ( $u ) {
			...
			return array(
				...
				'roleGoc' => VHCPVP_Cfg::vai_goc( $u['vaiTro'] !== '' ? $u['vaiTro'] : 'Nhân viên' ),
	...
	private static function qua_nhan_su( $pin ) {
		...
		$theo_ten = null;
		foreach ( VHCPVP_Cfg::get_users() as $x ) {
	...
	public static function change_pin( $name, $old, $new ) {
		...
		$old  = trim( (string) $old );
		$new  = trim( (string) $new );
		if ( ! preg_match( '/^\d{4,8}$/', $new ) ) { return VHCPVP_Util::err( 'PIN mới phải gồm 4–8 chữ số' ); }
		VHCPVP_Cfg::seed();
		$rows = VHCPVP_Cfg::read( VHCPVP_Cfg::USER );
		$my   = -1;
		foreach ( $rows as $i => $r ) {
			if ( trim( (string) $r[0] ) === $name && trim( (string) $r[1] ) === $old ) { $my = $i; break; }
		}
		if ( $my < 0 ) { return VHCPVP_Util::err( 'PIN hiện tại không đúng' ); }
		foreach ( $rows as $j => $r ) {
			if ( $j !== $my && trim( (string) $r[1] ) === $new ) { return VHCPVP_Util::err( 'PIN này đã có người dùng khác — chọn PIN khác' ); }
		}
		VHCPVP_Cfg::set_cell( VHCPVP_Cfg::USER, $my, 1, $new );
	...
	public static function issue_token( $ten, $role, $coso, $bo_phan ) {
		...
		$wpdb->insert( VHCPVP_DB::t( 'session' ), array(
			'token'   => $tok,
			'ten'     => (string) $ten,
			'vai_tro' => (string) $role,
			'coso'    => (string) $coso,
	...
	public static function user_by_token( $token ) {
		...
		if ( '' !== $khoa && class_exists( 'VHCPVP_Cfg' ) ) {
		...
		return array( 'name' => $ten, 'role' => $vai,
			'roleGoc' => VHCPVP_Cfg::vai_goc( $vai ),
	...
	public static function logout( $token ) {
		global $wpdb;
		$wpdb->delete( VHCPVP_DB::t( 'session' ), array( 'token' => (string) $token ) );
		return VHCPVP_Util::ok();
	}
	...
	public static function mo_khoa() {
		...
		delete_transient( self::fail_key() );
		$n = 0;
		$rows = $wpdb->get_col( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE '_transient_vhcpvp\\_fail\\_%'" );
	...
	public static function sso_secret() {
		$s = VHCPVP_Meta::get( 'SSO_SECRET' );
		if ( ! $s ) { $s = get_option( 'vhcpvp_sso_secret' ); }
		return $s ? $s : '';
	}
	...
	public static function resolve_sso_user( $ident ) {
		...
		return array( 'name' => (string) ( isset( $ident['n'] ) ? $ident['n'] : '' ), 'role' => $role,
			'roleGoc' => VHCPVP_Cfg::vai_goc( $role ), 'coso' => self::coso_hien( $coso ) );
	}

	private static function sso_overrides() {
		$hit = get_transient( 'vhcpvp_ssomap' );
		if ( is_array( $hit ) ) { return $hit; }
		$out = array();
		foreach ( VHCPVP_Cfg::read( VHCPVP_Cfg::SSO ) as $r ) {
			$em = strtolower( trim( (string) $r[0] ) );
			if ( $em === '' ) { continue; }
			$out[ $em ] = array( 'role' => trim( (string) $r[1] ), 'coso' => trim( (string) $r[2] ) );
		}
		set_transient( 'vhcpvp_ssomap', $out, 300 );
		return $out;
	}
