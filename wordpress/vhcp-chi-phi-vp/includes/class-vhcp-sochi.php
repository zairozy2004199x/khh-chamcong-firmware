...
class VHCPVP_SoChi {
	...
	public static function resolve_tk( $rec ) {
		...
		return VHCPVP_Cfg::resolve_tk( $g( 'loai' ), $g( 'hinhThuc' ), array( 'tkNo' => $g( 'tkNo' ), 'tkCo' => $g( 'tkCo' ), 'maDt' => $g( 'maDt' ) ), $g( 'coso' ) );
	}
	...
	private static function data( $rec, $nguoi = '' ) {
		...
		$st = VHCPVP_Util::blank_or_num( $g( 'soTien' ) );
		if ( $st === null ) { $st = VHCPVP_Util::num( $sl ) * VHCPVP_Util::num( $dg ); }

		$ts    = VHCPVP_Util::blank_or_num( $g( 'thueSuat' ) );
		$tthue = ( $ts === null ) ? null : round( $st * $ts / 100 );

		$ngay = VHCPVP_Util::parse_date( $g( 'ngay' ) );
		if ( ! $ngay ) { $ngay = VHCPVP_Util::today_sql(); }
		$ky = VHCPVP_Util::st( $g( 'ky' ) );
		...
		$d = array(
			'ngay'       => $ngay,
			'ky'         => $ky,
			'coso'       => VHCPVP_Util::st( $g( 'coso' ) ),
			'loai'       => VHCPVP_Util::st( $g( 'loai' ) ),
			'tk_no'      => $tk['tk_no'],
			'tk_co'      => $tk['tk_co'],
			'ma_dt'      => $tk['ma_dt'],
			'doi_tuong'  => VHCPVP_Util::st( $g( 'doiTuong' ) ),
			'noi_dung'   => VHCPVP_Util::st( $g( 'noiDung' ) ),
			'dvt'        => VHCPVP_Util::st( $g( 'dvt' ) ),
			'so_luong'   => $sl,
			'don_gia'    => $dg,
			'so_tien'    => $st,
			...
			'thue_suat'  => $ts,
			'tien_thue'  => $tthue,
			'ghi_chu'    => VHCPVP_Util::st( $g( 'ghiChu' ) ),
			...
			'ma_du_an'   => VHCPVP_Util::st( $g( 'maDuAn' ) ),
			'hang_muc'   => VHCPVP_Util::st( $g( 'hangMuc' ) ),
			'du_toan'    => VHCPVP_Util::blank_or_num( $g( 'duToan' ) ),
		...
		$nx = VHCPVP_Util::parse_date( $g( 'ngayXuat' ) );
	...
	public static function add( $rec, $nguoi = '' ) {
		global $wpdb;
		$rec = (array) $rec;
		if ( VHCPVP_Util::st( isset( $rec['loai'] ) ? $rec['loai'] : '' ) === '' ) { return VHCPVP_Util::err( 'Chọn loại chi phí' ); }
		$data               = self::data( $rec, $nguoi );
		$data['id']         = VHCPVP_Util::uid( 'C' );
		$data['tao_luc']    = VHCPVP_Util::now_sql();
	...
	public static function update( $id, $rec, $nguoi = '' ) {
		global $wpdb;
		$cur = self::row( $id );
		if ( ! $cur ) { return VHCPVP_Util::err( 'Không tìm thấy dòng chi' ); }
		if ( $cur['ngay_xuat'] ) { return VHCPVP_Util::err( 'Dòng đã xuất MISA — không sửa (bỏ chốt xuất trước nếu cần)' ); }
		$rec  = (array) $rec;
		$data = self::data( $rec, $nguoi !== '' ? $nguoi : (string) $cur['nguoi_nhap'] );
		$wpdb->update( VHCPVP_DB::t( 'so_chi' ), $data, array( 'id' => (string) $id ) );
		return VHCPVP_Util::ok( array( 'tkNo' => $data['tk_no'], 'tkCo' => $data['tk_co'] ) );
	}
	...
	public static function delete( $id ) {
		...
		$cur = self::row( $id );
		if ( ! $cur ) { return VHCPVP_Util::err( 'Không tìm thấy dòng chi' ); }
		if ( $cur['ngay_xuat'] ) { return VHCPVP_Util::err( 'Dòng đã xuất MISA — không xóa' ); }
		$wpdb->delete( VHCPVP_DB::t( 'so_chi' ), array( 'id' => (string) $id ) );
	...
	private static function out( $r ) {
		return array(
			...
			'dvt'       => (string) $r['dvt'],
			'soLuong'   => VHCPVP_Util::out_num( $r['so_luong'] ),
			'donGia'    => VHCPVP_Util::out_num( $r['don_gia'] ),
			'soTien'    => VHCPVP_Util::num( $r['so_tien'] ),
			'hinhThuc'  => (string) $r['hinh_thuc'],
			'vat'       => (string) $r['vat'],
			'thueSuat'  => VHCPVP_Util::out_num( $r['thue_suat'] ),
			'tienThue'  => VHCPVP_Util::num( $r['tien_thue'] ),
			'maDuAn'    => isset( $r['ma_du_an'] ) ? (string) $r['ma_du_an'] : '',
			'hangMuc'   => isset( $r['hang_muc'] ) ? (string) $r['hang_muc'] : '',
			'duToan'    => isset( $r['du_toan'] ) ? VHCPVP_Util::out_num( $r['du_toan'] ) : '',
			'hoSo'      => isset( $r['ho_so'] ) ? (string) $r['ho_so'] : '',
			'ghiChu'    => (string) $r['ghi_chu'],
			'anh'       => (string) $r['anh'],
			'nguoiNhap' => (string) $r['nguoi_nhap'],
			'daXuat'    => ( VHCPVP_Util::fmt( $r['ngay_xuat'] ) !== '' ),
			'ngayXuat'  => VHCPVP_Util::fmt( $r['ngay_xuat'] ),
		);
	}
	...
	public static function list_chi( $opts = array() ) {
		...
		foreach ( self::all_rows() as $r ) {
			...
			if ( ! VHCPVP_Auth::xem_duoc_loai( isset( $r['loai'] ) ? $r['loai'] : '' ) ) { continue; }
			/* ══════════════════════════════════════════════════════════════════════════════
			...
			if ( '' !== $cs_dong && ! VHCPVP_Auth::trong_tam(
			...
			if ( $f_tk !== 'all' && ! VHCPVP_Cfg::tk_thuoc_cay( $r['tk_no'], $f_tk ) ) { continue; }
			if ( $f_da !== 'all' ) {
				if ( $f_da === '(khong)' ) { if ( $mda !== '' ) { continue; } }
				elseif ( $mda !== $f_da ) { continue; }
			}
			$da_xuat = ( VHCPVP_Util::fmt( $r['ngay_xuat'] ) !== '' );
			...
			if ( $mda !== '' ) {
				...
				$by_da[ $mda ]['duToan'] += VHCPVP_Util::num( $x['duToan'] );
				$by_da[ $mda ]['n']++;
			}
		}
		...
		usort( $by_ky, function ( $a, $b ) { return VHCPVP_Util::ky_num( $b['ky'] ) <=> VHCPVP_Util::ky_num( $a['ky'] ); } );

		$ky_list = array_keys( $ky_set );
		usort( $ky_list, function ( $a, $b ) { return VHCPVP_Util::ky_num( $b ) <=> VHCPVP_Util::ky_num( $a ); } );
		...
		$tk_cha  = VHCPVP_Cfg::tk_cha_ds( $tk_list );
	...
	public static function theo_du_an( $ma_du_an ) {
		...
		$rows = VHCPVP_DB::rows( $wpdb->prepare( "SELECT * FROM $t WHERE ma_du_an=%s ORDER BY stt ASC", $ma ) );
		if ( ! count( $rows ) ) {
			$rows = VHCPVP_DB::rows( $wpdb->prepare( "SELECT * FROM $t WHERE LOWER(ma_du_an)=LOWER(%s) ORDER BY stt ASC", $ma ) );
		}
		$out = array();
	...
	public static function gan_ma_tai_khoan( $all = false ) {
		...
		$n = 0; $chua = array();
		foreach ( self::all_rows() as $r ) {
			if ( VHCPVP_Util::fmt( $r['ngay_xuat'] ) !== '' ) { continue; }
			...
			$giu = VHCPVP_Cfg::ma_con_hop_le( $r['loai'], $r['coso'], $r['tk_no'] );
		...
		return VHCPVP_Util::ok( array( 'updated' => $n, 'thieuMa' => array_keys( $chua ) ) );
	}
	...
	public static function export_misa( $ky = 'all', $mode = 'chuaxuat' ) {
		...
		foreach ( self::all_rows() as $r ) {
			...
			$so_tien = VHCPVP_Util::num( $r['so_tien'] );
			if ( ! $so_tien ) { continue; }

			$coso  = (string) $r['coso'];
			$loai  = (string) $r['loai'];
			// TK Nợ theo LUẬT LÚC XUẤT (VHCPVP_Cfg::tkno_xuat) — chung với 4 đường xuất kia:
			...
			$tk_no = VHCPVP_Cfg::tkno_xuat( $loai, $coso, $r['tk_no'] );
			$tk_co = trim( (string) $r['tk_co'] );
			$ma_dv = isset( $m_unit[ $coso ] ) ? $m_unit[ $coso ] : '';
			$tenm  = ! empty( $m_tm[ $coso ] ) ? $m_tm[ $coso ] : $coso;
			$ngay  = VHCPVP_Util::fmt( $r['ngay'] );
			VHCPVP_Misa::gom_ngay_xau( $ngay_xau, $ngay, $r['ngay'], (string) $r['id'] );
			...
			$ten_xuat = VHCPVP_Cfg::ten_misa_loai( $loai );
			$mda   = isset( $r['ma_du_an'] ) ? trim( (string) $r['ma_du_an'] ) : '';
			$dg1   = VHCPVP_Util::j( array( $ten_xuat, $coso, $r['ky'] ) ) . ( $is_tt ? '_Trực tiếp NCC' : '_Tạm ứng NV' );
			// Dòng thuộc dự án thì ghi mã dự án vào diễn giải để bên kế toán dò lại được
			$dg2   = VHCPVP_Util::j( array( $mda, (string) $r['noi_dung'], $tenm ) );
			$gc    = trim( (string) $r['ghi_chu'] );
			if ( $gc !== '' ) { $dg2 .= '_' . $gc; }

			$rows[] = array( $ngay, $ngay, '', $dg1, $dg2, VHCPVP_Util::ma_so( $tk_no ), VHCPVP_Util::ma_so( $tk_co ), $so_tien, VHCPVP_Util::ma_so( (string) $r['ma_dt'] ), VHCPVP_Util::ma_so( $ma_dv ) );
			$ids[]  = (string) $r['id'];
		}

		return array(
			'cols'   => VHCPVP_Misa::cols(),
			'rows'   => $rows,
			'count'  => count( $rows ),
			'sodon'  => count( $ids ),
			'warn'   => array_merge( array_keys( $warn ), VHCPVP_Misa::warn_ngay_xau( $ngay_xau ) ),
			'maDons' => $ids,
		);
	}
	...
	public static function mark_exported( $ids ) {
		global $wpdb;
		$ids = (array) $ids;
		if ( ! count( $ids ) ) { return VHCPVP_Util::err( 'Không có dòng nào để chốt' ); }
		$now = VHCPVP_Util::now_sql();
		$n   = 0;
		foreach ( $ids as $id ) {
			$r = self::row( $id );
			if ( ! $r || VHCPVP_Util::fmt( $r['ngay_xuat'] ) !== '' ) { continue; }
			$wpdb->update( VHCPVP_DB::t( 'so_chi' ), array( 'ngay_xuat' => $now ), array( 'id' => (string) $id ) );
			$n++;
		}
	...
	public static function unmark_exported( $ids ) {
		...
		foreach ( (array) $ids as $id ) {
			$wpdb->update( VHCPVP_DB::t( 'so_chi' ), array( 'ngay_xuat' => null ), array( 'id' => (string) $id ) );
			$n++;
		}
		return VHCPVP_Util::ok( array( 'count' => $n ) );
	}
