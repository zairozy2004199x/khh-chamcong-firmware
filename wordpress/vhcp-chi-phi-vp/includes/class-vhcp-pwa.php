 ...
 *    quản được đường nằm dưới thư mục chứa nó. Để ở `/wp-content/plugins/vhcp-chi-phi-vp/assets/js/`
 ...
 *    nào đè cái nào. Tên bày dưới biểu tượng lấy từ `VHCPVP_App::ten_trang()`, thứ script ấy cũng
 *    đã đổi sẵn.
 */
...
class VHCPVP_Pwa {
	...
	public static function init() {
		foreach ( VHCPVP_App::cac_slug() as $s ) {
			...
			add_rewrite_rule( '^' . $s . '/manifest\.json$', 'index.php?vhcpvp_pwa=manifest', 'top' );
			add_rewrite_rule( '^' . $s . '/sw\.js$', 'index.php?vhcpvp_pwa=sw', 'top' );
		}
		add_filter( 'query_vars', array( __CLASS__, 'query_vars' ) );
		/* Ưu tiên 4 — TRƯỚC `VHCPVP_App::maybe_render()` (mặc định 10). Nó nhận cả `?vhcpvp=app`,
	...
	public static function query_vars( $v ) { $v[] = 'vhcpvp_pwa'; return $v; }

	public static function maybe_render() {
		$viec = (string) get_query_var( 'vhcpvp_pwa' );
		/* Đường lui khi hosting chưa bật đường dẫn đẹp, hoặc bảng luật chưa được nạp lại. */
		if ( '' === $viec && isset( $_GET['vhcpvp_pwa'] ) ) {
			$viec = sanitize_key( wp_unslash( $_GET['vhcpvp_pwa'] ) );
		}
	...
	public static function goc() { return trailingslashit( VHCPVP_App::app_url() ); }
	...
	public static function url_phu( $viec ) {
		...
		return add_query_arg( 'vhcpvp_pwa', $viec, home_url( '/' ) );
	}
	...
	public static function ra_manifest() {
		...
		$m   = array(
			...
			'icons'            => array(
				array(
					'src'     => VHCPVP_URL . 'assets/img/app/bieu-tuong-192.png',
					'sizes'   => '192x192',
					'type'    => 'image/png',
					'purpose' => 'any',
				),
				array(
					'src'     => VHCPVP_URL . 'assets/img/app/bieu-tuong-512.png',
					'sizes'   => '512x512',
					'type'    => 'image/png',
					'purpose' => 'any',
				),
				...
				array(
					'src'     => VHCPVP_URL . 'assets/img/app/bieu-tuong-512-maskable.png',
					'sizes'   => '512x512',
					'type'    => 'image/png',
					'purpose' => 'maskable',
				),
	...
	public static function ra_sw() {
		$tep = VHCPVP_DIR . 'assets/js/sw.js';
		nocache_headers();
		...
		if ( ! is_readable( $tep ) ) {
			...
			echo "// thiếu assets/js/sw.js trong bản cài\nthrow new Error('vhcp-chi-phi-vp: thiếu assets/js/sw.js');\n";
			exit;
		}
		...
		$js = str_replace( '__BAN__', VHCPVP_VERSION, $js );
		/* Thư mục tệp tĩnh của CHÍNH bản này. Thợ nền so địa chỉ với nó để biết tệp nào của mình
		   ...
		   hỏng im lặng, vì script tách đổi mọi chuỗi `vhcpvp` trong tệp ấy. */
		$js = str_replace( '__GOC_TINH__', VHCPVP_URL, $js );
	...
	public static function khoi_head() {
		$ten = VHCPVP_App::ten_trang();
		$h   = "\n<!-- vhcpvp-pwa " . esc_html( VHCPVP_VERSION ) . " -->\n";
		 ...
		 * ────────────────────────────────────────────────────────────────────────────────── */
		$h .= '<link rel="apple-touch-icon" href="' . esc_url( VHCPVP_URL . 'assets/img/app/bieu-tuong-180.png' ) . "\">\n";
		...
		$h .= '<script src="' . esc_url( VHCPVP_URL . 'assets/js/pwa.js?ver=' . rawurlencode( VHCPVP_VERSION ) )
