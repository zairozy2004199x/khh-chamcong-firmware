...
require_once VHCPVP_DIR . 'includes/class-vhcp-doanh-thu.php';
/* Chi phí Vending HCMC kéo về thành đơn thật — dùng khoang_thang của lớp Doanh thu nên nạp sau nó. */
require_once VHCPVP_DIR . 'includes/class-vhcp-vending.php';
...
// Điểm nhận chi phí web Vending HCMC đẩy sang (máy chủ khác, gác bằng khoá chung) — xem VHCPVP_Vending::routes()
add_action( 'rest_api_init', array( 'VHCPVP_Vending', 'routes' ) );
// …và cửa dự phòng admin-ajax.php cho chính điểm nhận ấy (hosting chặn /wp-json/), không cần đăng nhập — khoá là cửa
add_action( 'wp_ajax_nopriv_vhcpvp_vending_nhan', array( 'VHCPVP_Vending', 'ajax_nhan' ) );
add_action( 'wp_ajax_vhcpvp_vending_nhan', array( 'VHCPVP_Vending', 'ajax_nhan' ) );
