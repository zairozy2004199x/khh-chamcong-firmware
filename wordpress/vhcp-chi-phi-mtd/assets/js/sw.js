 ...
 *
 * ⚠️ Tệp này được PHP phục vụ ở địa chỉ `/<slug>/sw.js` (xem `VHCPMTD_Pwa::ra_sw`), và `__BAN__` bị
 ...
 * =================================================================================================
 * 1. KHÔNG BAO GIỜ NHỚ MỘT LƯỢT GỌI DỮ LIỆU. Mọi đường hỏi/ghi của app đều mang `vhcpmtd_api`,
...
var BAN    = '__BAN__';
var KHO_VO = 'vhcpmtd-vo-' + BAN;      // vỏ app: trang /<slug>/
var KHO_TT = 'vhcpmtd-tt-' + BAN;      // tệp tĩnh: css, js, ảnh
var TT_TOI_DA = 120;
...
self.addEventListener('install', function (e) {
  e.waitUntil(
    caches.open(KHO_VO).then(function (kho) {
      /* Nạp sẵn đúng TRANG APP. Trang này giống hệt nhau với mọi người — `VHCPMTD_App::render()`
  ...
  e.waitUntil(
    caches.keys().then(function (ten) {
      return Promise.all(ten.map(function (t) {
        if (t.indexOf('vhcpmtd-') !== 0) { return null; }       // kho của bộ khác, không đụng
 ...
 *
 * 🔴 DÒ BA ĐƯỜNG, KHÔNG PHẢI MỘT. App có ba cổng gọi máy chủ (xem `VHCPMTD_App::head_block`):
 *    `/wp-json/`, `admin-ajax.php`, và chính URL trang kèm `vhcpmtd_api` — cổng cuối sinh ra vì
 ...
 */
function laDuLieu(u) {
  return u.searchParams.has('vhcpmtd_api')
