 ...
 * được dịch thành 1 request POST tới /wp-json/vhcpmtd/v1/call {fn:'getDon', args:[ma]}.
 *
...
(function () {
	...
	var CFG        = window.VHCPMTD_CFG || {};
	var TOKEN_KEY  = 'vhcpmtd_token';
	var USER_KEY   = 'vhcpmtd_user';
	var reloading  = false;
	 ...
	 */
	var DUONG_KEY = 'vhcpmtd_duong';
	var duong = 'rest';
	try {
		var d = localStorage.getItem( DUONG_KEY );
		if ( d === 'ajax' || d === 'trang' ) { duong = d; }
		else if ( localStorage.getItem( 'vhcpmtd_ajax' ) === '1' ) { duong = 'ajax'; }
	} catch ( e ) {}
	function nhoDuong( d ) {
		duong = d;
	...
	function guiRest( fn, args, tok ) {
		return fetch( CFG.endpoint, {
			...
			headers: { 'Content-Type': 'application/json', 'X-VHCPMTD-Token': tok },
	...
	/**
	 * Đường CUỐI: gửi vào chính URL của app (…/chi-phi/?vhcpmtd_api=1).
	 *
	...
	function call( fn, args, onOk, onErr ) {
		...
		function thu( d ) {
			...
			return boGui( d )( fn, args, tok ).then( docKetQua ).then( function ( r ) {
				...
				try { localStorage.removeItem( DUONG_KEY ); localStorage.removeItem( 'vhcpmtd_ajax' ); } catch ( e ) {}
				return r;
			} );
		}
	...
	window.addEventListener( 'load', function () {
		if ( typeof window.logout === 'function' && ! window.logout.__vhcpmtd ) {
			var orig = window.logout;
			window.logout = function () {
				var tok = getToken();
				if ( tok ) { call( 'vhcpmtdLogout', [ tok ], null, null ); }
				clearSession();
				return orig.apply( this, arguments );
			};
			window.logout.__vhcpmtd = true;
		}
	} );
})();
