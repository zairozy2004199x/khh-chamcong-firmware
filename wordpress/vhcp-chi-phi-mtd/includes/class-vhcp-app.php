...
class VHCPMTD_App {
	 ...
	 *
	 *      ô Cài đặt trên host đang lưu sẵn 'chi-phi-mtd' (anh Thắng dùng link ấy từ đầu)
	 *        -> slug() trả 'chi-phi-mtd'
	...
	public static function init() {
		foreach ( self::cac_slug() as $s ) {
			add_rewrite_rule( '^' . $s . '/?$', 'index.php?vhcpmtd_app=1', 'top' );
		}
	...
	public static function head_block( $tieu_de = null, $trang = '', $fns = null ) {
		...
		$cfg = array(
			...
			'layCoSoGhe' => VHCPMTD_Cfg::lay_coso_ghe() ? 1 : 0,
		);
		 ...
		 *
		 * ⚠️ DÒ CẢ LỚP LẪN HÀM. Shortcode `[vhcpmtd_app]` gọi `head_block()` từ ngữ cảnh khác, và
		 ...
		 * ══════════════════════════════════════════════════════════════════════════════════ */
		if ( class_exists( 'VHCPMTD_Pwa' ) && method_exists( 'VHCPMTD_Pwa', 'khoi_head' ) ) {
			$out .= VHCPMTD_Pwa::khoi_head();
		}
		return $out;
	}
