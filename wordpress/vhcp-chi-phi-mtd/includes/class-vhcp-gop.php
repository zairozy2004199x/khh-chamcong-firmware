...
class VHCPMTD_Gop {
	 ...
	 *     đang nằm trong hằng ấy — nó quét mọi chữ `vhcpmtd` — thành một tiền tố KHÔNG TỒN TẠI.
	 *     Bản mới bày ra bảng đối chiếu trống rỗng rồi nói "bên kia không có gì", mà sự thật là
	 *     nó đang dò một cái tên bịa.
	 *
	 * Nên hỏi thẳng cơ sở dữ liệu: bảng `…vhcpmtd<mã>_don` nào đang có thì đó là một kho. Cách này
	 ...
	 *    Bản gốc có mã rỗng, khối của nó đọc từ `VHCPMTD_DB::khoi()`.
	 *
	...
	public static function kho() {
		...
		/* ⚠️ ĐỘT BIẾN TƯƠNG ĐƯƠNG — hai lớp gác dưới đây gỡ RIÊNG từng lớp thì bài kiểm vẫn xanh,
		   ...
		   con mồi `wpxvhcpmtdzz_don` trong `kiem-soat-gop.php`. Giữ cả hai là cố ý — lớp SQL lọc
		   ...
		   🔴 CHE `_` CỦA TIỀN TỐ. Trong SQL, `_` là ký tự thay thế MỘT ký tự bất kỳ — `wp_vhcpmtd…`
		...
		$mau   = $wpdb->esc_like( $wpdb->prefix . 'vhcpmtd' ) . '%' . $wpdb->esc_like( '_don' );
		foreach ( (array) $wpdb->get_col( $wpdb->prepare( 'SHOW TABLES LIKE %s', $mau ) ) as $ten ) {
			...
			if ( ! preg_match( '#^' . preg_quote( $wpdb->prefix, '#' ) . 'vhcpmtd([a-z0-9]*)_don$#', (string) $ten, $m ) ) { continue; }
			$tien_to = 'vhcpmtd' . $m[1] . '_';
			if ( '' === $m[1] ) { $rieng[ VHCPMTD_DB::khoi() ] = $tien_to; continue; }
			$khac[ $m[1] ] = $tien_to;
		}
		ksort( $khac );
		return $rieng + $khac;
	...
	const DANH_MUC = array(
		...
		/* Cơ sở: khóa là TÊN gian hàng, so Mã đơn vị MISA + Tên MISA. Cơ sở được nhận ra bằng
		   chuỗi tên (xem `VHCPMTD_Cfg::coso_la()`) nên trùng tên là trùng thật — hoặc là tai nạn. */
		...
		/* `CH_VaiTro` KHÔNG có trong `VHCPMTD_Cfg::headers()` — bảng ấy đọc theo chỉ số trần
	...
	/** Tên bảng đầy đủ của một kho. Không dùng `VHCPMTD_DB::t()` vì nó khóa cứng vào kho KVC. */
	...
	/** Đọc một bảng cấu hình của MỘT kho bất kỳ (bản sao của `VHCPMTD_Cfg::read()` có tiền tố). */
	private static function cfg_cua( $khoi, $bang ) {
		global $wpdb;
		$t = self::bang( $khoi, 'cfg' );
		if ( ! self::co_bang( $t ) ) { return null; }
		$n    = count( VHCPMTD_Cfg::headers( $bang ) );
	...
	public static function doi_chieu( $khoi, $bang ) {
		...
		foreach ( $ben_nay as $r ) {
			...
			if ( '' === $k ) { continue; }
			/* Giữ dòng ĐẦU khi KVC lỡ có hai dòng cùng tên — cùng luật với `VHCPMTD_Cfg::write()`. */
		...
		foreach ( $ben_kia as $r ) {
			...
			foreach ( $luat['so'] as $c ) {
				...
				if ( '' === $a || '' === $b ) { continue; }
				$hd = isset( $luat['nhan'] ) ? $luat['nhan'] : VHCPMTD_Cfg::headers( $bang );
				$lech[] = array(
	...
	public static function mo_coi( $khoi ) {
		...
		$ten_loai = array();
		foreach ( (array) self::cfg_cua( self::kho_nha(), VHCPMTD_Cfg::LOAI ) as $r ) {
			$k = self::chuan( isset( $r[0] ) ? $r[0] : '' );
			if ( '' !== $k ) { $ten_loai[ $k ] = 1; }
		}
		$ten_coso = array();
		foreach ( (array) self::cfg_cua( self::kho_nha(), VHCPMTD_Cfg::COSO ) as $r ) {
			$k = self::chuan( isset( $r[0] ) ? $r[0] : '' );
			if ( '' !== $k ) { $ten_coso[ $k ] = 1; }
		}
