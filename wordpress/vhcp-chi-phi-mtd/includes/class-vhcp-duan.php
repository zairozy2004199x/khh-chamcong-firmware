 ...
 *
 * App cũ mỗi dự án là 1 tab Google Sheet; ở đây là các dòng trong vhcpmtd_da_line khóa theo
...
class VHCPMTD_DuAn {
	...
	public static function find( $ma_da ) {
		global $wpdb;
		$t = VHCPMTD_DB::t( 'da_index' );
		return VHCPMTD_DB::row( $wpdb->prepare( "SELECT * FROM $t WHERE ma_da=%s", (string) $ma_da ) );
	}
	...
	public static function don_vi_cua( $khoa, $kieu = 'ma_da' ) {
		...
		$r = self::find( $khoa );
		if ( ! $r ) { return null; }
		return VHCPMTD_DonVi::cua_nguoi( isset( $r['nguoi_tao'] ) ? $r['nguoi_tao'] : '' );
	}

	private static function lines_of( $ma_da ) {
		global $wpdb;
		$t = VHCPMTD_DB::t( 'da_line' );
		return VHCPMTD_DB::rows( $wpdb->prepare( "SELECT * FROM $t WHERE ma_da=%s ORDER BY row_no ASC", (string) $ma_da ) );
	}
	...
	private static function is_real( $r ) {
		return ! ( trim( (string) $r['noi_dung'] ) === '' && ! ( VHCPMTD_Util::num( $r['du_toan'] ) || VHCPMTD_Util::num( $r['thuc_te'] ) ) );
	}
	...
	public static function create_du_an( $loai, $ten, $nguoi, $tu = '', $den = '' ) {
		...
		if ( $loai === 'Chi phí cơ sở' ) {
			/* Tên rỗng = mở lại sổ CHUNG cũ (nút "🔧 Mở sổ chung (cũ)" vẫn gọi đúng như trước).
			   ...
			   MỘT ĐƠN" bên dưới. */
			$ten_cs = VHCPMTD_Util::san( $ten );
			return ( '' === $ten_cs ) ? self::ensure_co_so_chung( $nguoi ) : self::tao_don_coso( $ten_cs, $nguoi, $tu, $den );
		}
		$ten = VHCPMTD_Util::san( $ten );
		if ( ! in_array( $loai, self::LOAI_DU_AN, true ) ) {
			return VHCPMTD_Util::err( 'Loại dự án không hợp lệ: "' . $loai . '". Hợp lệ: '
				. implode( ' · ', self::LOAI_DU_AN ) . '.' );
		}
		if ( $ten === '' ) { return VHCPMTD_Util::err( 'Nhập tên dự án' ); }
		$ma = VHCPMTD_Util::uid( 'DA' );
		$wpdb->insert( VHCPMTD_DB::t( 'da_index' ), array(
			'ma_da'      => $ma,
			'ten'        => $ten,
			'loai'       => $loai,
			'trang_thai' => 'Đang làm',
		...
		return VHCPMTD_Util::ok( array( 'maDA' => $ma, 'ten' => $ten, 'loai' => $loai, 'sheet' => '', 'trangThai' => 'Đang làm', 'url' => '' ) );
	}
	...
	public static function ma_coso_chung() {
		global $wpdb;
		$m = trim( (string) VHCPMTD_Meta::get( self::MK_COSO_CHUNG, '' ) );
		if ( '-' === $m ) { return ''; }
		if ( '' !== $m ) { return self::find( $m ) ? $m : ''; }
		$t = VHCPMTD_DB::t( 'da_index' );
		$r = VHCPMTD_DB::row( $wpdb->prepare( "SELECT * FROM $t WHERE loai=%s ORDER BY stt ASC LIMIT 1", 'Chi phí cơ sở' ) );
		if ( ! $r ) { return ''; }   // 🔴 KHÔNG tự tạo: hàm này bị gọi cả trong delete()
		VHCPMTD_Meta::set( self::MK_COSO_CHUNG, (string) $r['ma_da'] );
	...
	public static function tao_don_coso( $ten, $nguoi, $tu = '', $den = '' ) {
		global $wpdb;
		$ten = VHCPMTD_Util::san( $ten );
		if ( '' === $ten ) { return VHCPMTD_Util::err( 'Nhập tên đợt chi phí cơ sở' ); }
		$tu  = trim( (string) $tu );
		$den = trim( (string) $den );
		if ( '' !== $tu && '' !== $den && $den < $tu ) {
			return VHCPMTD_Util::err( 'Ngày kết thúc không được trước ngày bắt đầu.' );
		}
		/* Ghim trạng thái "đơn chung" TRƯỚC khi thêm đơn mới — xem chốt ⚠️ ở trên. */
		if ( '' === self::ma_coso_chung() ) { VHCPMTD_Meta::set( self::MK_COSO_CHUNG, '-' ); }
		$ma = VHCPMTD_Util::uid( 'DA' );
		$wpdb->insert( VHCPMTD_DB::t( 'da_index' ), array(
			'ma_da'      => $ma,
			'ten'        => $ten,
		...
		if ( '' !== $tu || '' !== $den ) { VHCPMTD_Meta::set_json( 'daKy_' . $ma, array( 'tu' => $tu, 'den' => $den ) ); }
		VHCPMTD_Log::log_action( array(
			'actor'  => (string) ( '' !== $nguoi ? $nguoi : VHCPMTD_Auth::nguoi() ),
			'role'   => VHCPMTD_Auth::vai_tro(),
			'action' => 'Lập đơn chi phí cơ sở',
			'target' => $ma,
			'detail' => $ten . ( ( '' !== $tu || '' !== $den ) ? ( ' · ' . $tu . ' → ' . $den ) : '' ),
		) );
		return VHCPMTD_Util::ok( array( 'maDA' => $ma, 'ten' => $ten, 'loai' => 'Chi phí cơ sở', 'sheet' => '', 'trangThai' => 'Đang làm', 'url' => '', 'kyDA' => array( 'tu' => $tu, 'den' => $den ) ) );
	}
	...
	public static function ensure_co_so_chung( $nguoi ) {
		...
		VHCPMTD_Meta::set( self::MK_COSO_CHUNG, $ma );
		return VHCPMTD_Util::ok( array( 'maDA' => $ma, 'ten' => $ten, 'loai' => 'Chi phí cơ sở', 'sheet' => '', 'trangThai' => 'Đang làm', 'url' => '' ) );
	}
	...
	public static function list_don_hm() {
		...
		foreach ( self::all_with_lines() as $r ) {
			if ( null !== $dv_xem
				&& ! VHCPMTD_DonVi::duoc_xem( VHCPMTD_DonVi::cua_nguoi( isset( $r['nguoi_tao'] ) ? $r['nguoi_tao'] : '' ) ) ) { continue; }
			...
			foreach ( $r['lines'] as $x ) {
				...
				if ( '' !== $cap && '(Phát sinh)' !== $cap ) {
					$con[ $cap ] = ( isset( $con[ $cap ] ) ? $con[ $cap ] : 0 ) + VHCPMTD_Util::num( $x['thuc_te'] );
				}
			}
			foreach ( $r['lines'] as $x ) {
				...
				$out[] = array(
					...
					'duToan'    => VHCPMTD_Util::num( $x['du_toan'] ),
					'thucTe'    => ( isset( $con[ $nd ] ) && $con[ $nd ] > 0 ) ? $con[ $nd ] : VHCPMTD_Util::num( $x['thuc_te'] ),
					'hm'        => $h,
					'dotHien'   => $bd_tu,
					'qtDotHien' => $bd_qt,
	...
	public static function list_du_an() {
		$out = array();
		$sc_tong = VHCPMTD_SoChi::tong_theo_du_an();   // 1 lệnh DB cho mọi dự án
		$dv_xem  = VHCPMTD_DonVi::xem_duoc();
		...
		foreach ( self::all_with_lines() as $r ) {
			...
			foreach ( $lines as $x ) {
				...
				if ( $cap === '' ) {
					$dt += VHCPMTD_Util::num( $x['du_toan'] );
					if ( ! ( isset( $child[ $nd ] ) && $child[ $nd ] > 0 ) ) { $tt += VHCPMTD_Util::num( $x['thuc_te'] ); }
				} else {
		...
		/* 🔴 Ô CHỌN CƠ SỞ KHÔNG ĐƯỢC BÀY GIAN CỦA BÊN KIA — anh Thắng 11/09/2026: *"Thêm đơn vị
		   ...
		   dựng công phu ở `VHCPMTD_DonVi` đều vô nghĩa ngay tại ô người ta gõ hằng ngày: chọn
		...
		$coso = VHCPMTD_DonVi::coso_xem_duoc();
		if ( null === $coso ) {
			$coso = array();
	...
	public static function rename_du_an( $ma_da, $ten ) {
		global $wpdb;
		$ten = VHCPMTD_Util::san( $ten );
		if ( $ten === '' ) { return VHCPMTD_Util::err( 'Tên trống' ); }
		$f = self::find( $ma_da );
		if ( ! $f ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		if ( self::la_coso_chung( $ma_da ) ) { return VHCPMTD_Util::err( 'Sổ Chi phí cơ sở CHUNG không đổi tên — đơn cơ sở theo đợt thì đổi được' ); }
		$wpdb->update( VHCPMTD_DB::t( 'da_index' ), array( 'ten' => $ten ), array( 'ma_da' => (string) $ma_da ) );
		return VHCPMTD_Util::ok( array( 'ten' => $ten ) );
	}
	...
	public static function get_du_an( $ma_da ) {
		...
		foreach ( self::lines_of( $ma_da ) as $r ) {
			...
			$lines[] = array(
				...
				'donGia'    => VHCPMTD_Util::num( $r['don_gia'] ),
				'thanhTien' => VHCPMTD_Util::num( $r['thanh_tien'] ),
				'vat'       => (string) $r['vat'],
				'anh'       => (string) $r['anh'],
				'gian'      => (string) $r['gian'],
				'note'      => (string) $r['note'],
		...
		foreach ( self::ds_dot( $ma_da ) as $d ) {
			...
			if ( in_array( $d['tt'], array( 'xin', 'duyet', 'ung' ), true ) ) {
				...
				foreach ( (array) $d['lich'] as $lx ) {
					$tong_lich += VHCPMTD_Util::num( isset( $lx['soTien'] ) ? $lx['soTien'] : 0 );
				}
				if ( $tong_lich > 0 ) {
					$da_xin += $tong_lich;
		...
		$sc_lines = VHCPMTD_SoChi::theo_du_an( (string) $ma_da );
		if ( ! count( $sc_lines ) ) { $sc_lines = VHCPMTD_SoChi::theo_du_an( (string) $f['ten'] ); }
		$sc_tien = 0; $sc_du_toan = 0;
		foreach ( $sc_lines as $x ) {
			$sc_tien    += VHCPMTD_Util::num( $x['soTien'] );
			$sc_du_toan += VHCPMTD_Util::num( $x['duToan'] );
		}
	...
	private static function nd_list( $loai ) {
		$o = VHCPMTD_Meta::get_json( 'da_ndlist_v1', array() );
	...
	private static function push_nd( $loai, $nd ) {
		...
		$o[ $loai ] = $a;
		VHCPMTD_Meta::set_json( 'da_ndlist_v1', $o );
	}

	// ---------------------------------------------------------------- kế toán chi tiền

	public static function get_pay( $ma_da ) {
		return VHCPMTD_Meta::get_json( 'daPay_' . $ma_da, array() );
	}
	...
	public static function get_du_toan_da( $ma_da ) {
		return VHCPMTD_Util::num( VHCPMTD_Meta::get( 'daDuToan_' . $ma_da, 0 ) );
	}
	...
	public static function get_ky_da( $ma_da ) {
		$o = VHCPMTD_Meta::get_json( 'daKy_' . $ma_da, array() );
		return array(
	...
	public static function set_ky_da( $ma_da, $tu, $den, $nguoi = '' ) {
		if ( ! self::find( $ma_da ) ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		$tu  = trim( (string) $tu );
		$den = trim( (string) $den );
		...
		VHCPMTD_Meta::set_json( 'daKy_' . $ma_da, array( 'tu' => $tu, 'den' => $den ) );
		VHCPMTD_Log::log_action( array(
			'actor'  => (string) ( '' !== $nguoi ? $nguoi : VHCPMTD_Auth::nguoi() ),
			'role'   => VHCPMTD_Auth::vai_tro(),
		...
		return VHCPMTD_Util::ok( array( 'ky' => array( 'tu' => $tu, 'den' => $den ) ) );
	}

	public static function set_du_toan_da( $ma_da, $so, $nguoi = '' ) {
		if ( ! self::find( $ma_da ) ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		$n = VHCPMTD_Util::num( $so );
		/* 🔴 SỐ ÂM LÀ VÔ NGHĨA, và nó lặng lẽ làm phần "còn dự phòng" phình ra. Chối thẳng. */
		if ( $n < 0 ) { return VHCPMTD_Util::err( 'Tổng dự toán không được âm.' ); }
		VHCPMTD_Meta::set( 'daDuToan_' . $ma_da, $n );
		VHCPMTD_Log::log_action( array(
			'actor'  => (string) ( '' !== $nguoi ? $nguoi : VHCPMTD_Auth::nguoi() ),
			'role'   => VHCPMTD_Auth::vai_tro(),
		...
		return VHCPMTD_Util::ok( array( 'duToanDA' => $n ) );
	}

	public static function approve_date( $ma_da ) {
		return (string) VHCPMTD_Meta::get( 'daApp_' . $ma_da, '' );
	}
	...
	public static function pay_tong( $p, $phase, $loai ) {
		$t = 0;
		foreach ( self::pay_ds( $p, $phase, $loai ) as $x ) { $t += VHCPMTD_Util::num( isset( $x['amount'] ) ? $x['amount'] : 0 ); }
		return $t;
	}
	...
	public static function confirm_pay( $ma_da, $phase, $loai, $amount, $nguoi, $ghi_chu = '' ) {
		$f = self::find( $ma_da );
		if ( ! $f ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		if ( ! in_array( (string) $phase, array( 'tamUng', 'quyetToan' ), true ) ) { return VHCPMTD_Util::err( 'Giai đoạn không hợp lệ' ); }
		if ( ! in_array( (string) $loai, array( 'tu', 'tt' ), true ) ) { return VHCPMTD_Util::err( 'Loại chi không hợp lệ' ); }
		$st = (string) $f['trang_thai'];
		if ( $st !== 'Đã duyệt' && $st !== 'Đã đóng' ) { return VHCPMTD_Util::err( 'Chỉ chi tiền khi dự án đã kế toán duyệt tạm ứng' ); }
		...
		$so = VHCPMTD_Util::num( $amount );
		if ( $so <= 0 ) { return VHCPMTD_Util::err( 'Số tiền phải lớn hơn 0.' ); }
		...
		$ds[] = array(
			'done'   => true,
			'amount' => $so,
			'date'   => VHCPMTD_Util::now()->format( 'd/m/Y H:i' ),
			'by'     => (string) $nguoi,
			'ghiChu' => trim( (string) $ghi_chu ),
		);
		if ( ! isset( $p[ $phase ] ) || ! is_array( $p[ $phase ] ) ) { $p[ $phase ] = array(); }
		$p[ $phase ][ $loai ] = $ds;
		VHCPMTD_Meta::set_json( 'daPay_' . $ma_da, $p );
		return VHCPMTD_Util::ok( array( 'pay' => $p ) );
	}
	...
	public static function unconfirm_pay( $ma_da, $phase, $loai, $idx = null ) {
		...
		$p  = self::get_pay( $ma_da );
		$ds = self::pay_ds( $p, $phase, $loai );
		if ( ! $ds ) { return VHCPMTD_Util::err( 'Chưa có lần chi nào để hoàn tác.' ); }
		$i = ( null === $idx || '' === $idx ) ? ( count( $ds ) - 1 ) : (int) $idx;
		if ( $i < 0 || $i >= count( $ds ) ) { return VHCPMTD_Util::err( 'Không có lần chi thứ ' . ( (int) $idx + 1 ) . '.' ); }
		array_splice( $ds, $i, 1 );
		if ( ! isset( $p[ $phase ] ) || ! is_array( $p[ $phase ] ) ) { $p[ $phase ] = array(); }
	...
	public static function get_hm( $ma_da ) {
		$o = VHCPMTD_Meta::get_json( 'daHM_' . $ma_da, array() );
	...
	private static function hm_ghi_( $ma_da, $row, $sua ) {
		...
		$moi['moc'] = $cu['moc'];
		$moi['moc'][ $moi['tt'] ] = VHCPMTD_Util::now()->format( 'd/m/Y H:i' ) . ' · ' . VHCPMTD_Auth::nguoi();
		$o[ $k ] = $moi;
		VHCPMTD_Meta::set_json( 'daHM_' . $ma_da, $o );
		VHCPMTD_Log::log_action( array(
	...
	public static function get_dot( $ma_da, $loai = 'tu' ) {
		$o = VHCPMTD_Meta::get_json( self::dot_meta_( $loai ) . $ma_da, array() );
		return is_array( $o ) ? $o : array();
	}
	...
	public static function dot_cua( $ma_da, $dot, $loai = 'tu' ) {
		...
		return array(
			...
			'soTien' => isset( $x['soTien'] ) ? VHCPMTD_Util::num( $x['soTien'] ) : 0,
			'unc'    => isset( $x['unc'] ) ? (string) $x['unc'] : '',
	...
	public static function da_cap_tong( $d ) {
		...
		foreach ( (array) ( isset( $d['daCap'] ) ? $d['daCap'] : array() ) as $x ) {
			$t += VHCPMTD_Util::num( isset( $x['soTien'] ) ? $x['soTien'] : 0 );
		}
		return $t;
	}
	...
	private static function dot_ghi_( $ma_da, $dot, $sua, $viec, $loai = 'tu' ) {
		...
		$moi['moc'][ $moi['tt'] ] = VHCPMTD_Util::now()->format( 'd/m/Y H:i' ) . ' · ' . VHCPMTD_Auth::nguoi();
		$o[ $k ] = $moi;
		VHCPMTD_Meta::set_json( self::dot_meta_( $loai ) . $ma_da, $o );
		VHCPMTD_Log::log_action( array(
			'actor'  => VHCPMTD_Auth::nguoi(),
			'role'   => VHCPMTD_Auth::vai_tro(),
			'action' => (string) $viec,
	...
	public static function tien_hm( $ma_da, $row ) {
		...
		foreach ( $lines as $l ) {
			if ( (int) $l['row_no'] === (int) $row ) {
				...
				$tu_than = VHCPMTD_Util::num( $l['thuc_te'] );
			}
		}
		if ( '' === $nd ) { return 0; }
		$con = 0; $co_con = false;
		foreach ( $lines as $l ) {
			if ( trim( (string) $l['cap_cha'] ) === $nd ) { $co_con = true; $con += VHCPMTD_Util::num( $l['thuc_te'] ); }
		}
	...
	public static function tien_hm_du_kien( $ma_da, $row ) {
		...
		foreach ( $lines as $l ) {
			if ( (int) $l['row_no'] === (int) $row ) {
				$nd      = trim( (string) $l['noi_dung'] );
				$tu_than = VHCPMTD_Util::num( $l['thuc_te'] );
				$dt      = VHCPMTD_Util::num( $l['du_toan'] );
				$tht     = VHCPMTD_Util::num( $l['thanh_tien'] );
	...
	public static function xin_tam_ung_dot( $ma_da, $rows, $lich = array(), $ghi_chu = '' ) {
		...
		if ( ! $rows ) { return VHCPMTD_Util::err( 'Chưa tích hạng mục nào để xin tạm ứng.' ); }
		...
		foreach ( $rows as $r ) {
			if ( ! isset( $lon[ $r ] ) ) {
				return VHCPMTD_Util::err( 'Dòng ' . $r . ' không phải hạng mục lớn của dự án này.' );
			}
			if ( self::hm_la_ncc( $ma_da, $r ) ) {
				return VHCPMTD_Util::err( '"' . $lon[ $r ] . '" là khoản kế toán trả thẳng nhà cung cấp — không xin tạm ứng.' );
			}
			$h = self::hm_cua( $ma_da, $r );
			if ( ! in_array( $h['tt'], array( 'nhap', 'tra' ), true ) ) {
				return VHCPMTD_Util::err( '"' . $lon[ $r ] . '" đã nằm trong một lệnh tạm ứng rồi.' );
			}
			$nhan[] = $r;
		...
		if ( $tong <= 0 ) {
			return VHCPMTD_Util::err( 'Mấy hạng mục vừa tích đều chưa có số tiền nào — Dự toán, '
		...
		foreach ( (array) $lich as $x ) {
			...
			foreach ( (array) ( isset( $x['rows'] ) ? $x['rows'] : array() ) as $rr ) {
				...
				if ( ! isset( $trong_lenh[ $rr ] ) ) {
					return VHCPMTD_Util::err( 'Dòng ' . $rr . ' được xếp vào một lần nhận tiền nhưng '
						. 'không nằm trong lệnh này.' );
				}
				if ( isset( $da_gan[ $rr ] ) ) {
					return VHCPMTD_Util::err( '"' . $lon[ $rr ] . '" bị xếp vào hai lần nhận tiền — '
		...
		return VHCPMTD_Util::ok( array( 'dot' => $moi, 'soTien' => $tong, 'so' => count( $nhan ) ) );
	}
	...
	public static function xoa_dot_tra( $ma_da, $dot, $loai = 'tu' ) {
		if ( 'Admin' !== VHCPMTD_Auth::vai_tro() ) { return VHCPMTD_Util::err( 'Chỉ Admin dọn được lệnh bị trả.' ); }
		if ( ! self::find( $ma_da ) ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		$d = self::dot_cua( $ma_da, $dot, $loai );
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy lệnh ' . (int) $dot ); }
		if ( 'tra' !== $d['tt'] ) {
			return VHCPMTD_Util::err( 'Chỉ dọn được lệnh ĐÃ BỊ TRẢ LẠI. Lệnh này đang ở "' . $d['tt']
		...
		foreach ( self::lines_of( $ma_da ) as $l ) {
			...
			if ( $h && (int) $h['dot'] === (int) $dot ) {
				return VHCPMTD_Util::err( 'Hạng mục "' . trim( (string) $l['noi_dung'] ) . '" vẫn trỏ vào lệnh này — chưa dọn được.' );
			}
		}
		VHCPMTD_Log::log_action( array(
			'actor'  => VHCPMTD_Auth::nguoi(), 'role' => VHCPMTD_Auth::vai_tro(),
		...
		VHCPMTD_Meta::set_json( self::dot_meta_( $loai ) . $ma_da, $o );
		return VHCPMTD_Util::ok( array( 'daXoa' => (int) $dot, 'conLai' => count( $o ) ) );
	}

	public static function cap_tien_phan( $ma_da, $dot, $them = array() ) {
		if ( ! self::find( $ma_da ) ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		$d = self::dot_cua( $ma_da, $dot );
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy lệnh tạm ứng ' . (int) $dot ); }
		$vai = VHCPMTD_Auth::vai_tro();
		if ( ! in_array( $vai, array( 'Admin', 'Kế toán cá nhân', 'Kế toán NCC' ), true ) ) {
			return VHCPMTD_Util::err( 'Chỉ kế toán cấp tạm ứng được.' );
		}
		if ( 'ung' === $d['tt'] ) { return VHCPMTD_Util::err( 'Lệnh này đã cấp đủ tiền rồi.' ); }
		if ( 'duyet' !== $d['tt'] ) {
			return VHCPMTD_Util::err( 'Chỉ cấp tiền cho lệnh ĐÃ DUYỆT. Lệnh này đang ở bước "'
				. $d['tt'] . '".' );
		}

		$them   = (array) $them;
		$so     = VHCPMTD_Util::num( isset( $them['soTien'] ) ? $them['soTien'] : 0 );
		$con    = self::con_phai_cap( $d );
		if ( $so <= 0 ) {
			return VHCPMTD_Util::err( 'Nhập số tiền thật sự đưa lần này (lớn hơn 0).' );
		}
		/* ══════════════════════════════════════════════════════════════════════════════════
		...
		if ( $cho_lan > 0 ) {
			...
			if ( $so > $con_lan ) {
				...
				foreach ( (array) ( isset( $d['lich'] ) ? $d['lich'] : array() ) as $y ) {
					...
					$c = VHCPMTD_Util::num( isset( $y['soTien'] ) ? $y['soTien'] : 0 ) - ( isset( $da_lan[ $l ] ) ? $da_lan[ $l ] : 0 );
					if ( $c > 0 ) { $con_khac += $c; }
				}
				if ( $con_khac > 0 ) {
		...
		if ( $het ) {
			...
			/* `$nay = 0` chỉ xảy ra khi lệnh mất hàng (sổ cũ) — `update_line` không cho gõ cả dự
			   ...
			   số đã đưa. */
			$du_nay = self::da_cap_tong( $moi ) - ( $nay > 0 ? $nay : VHCPMTD_Util::num( $moi['soTien'] ) );
			return VHCPMTD_Util::ok( array( 'dot' => $moi, 'daCap' => self::da_cap_tong( $moi ),
	...
	public static function dat_tt_dot( $ma_da, $dot, $tt, $them = array() ) {
		if ( ! self::find( $ma_da ) ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		$tt = (string) $tt;
		if ( ! in_array( $tt, self::TT_DOT, true ) ) { return VHCPMTD_Util::err( 'Trạng thái không hợp lệ' ); }
		$d = self::dot_cua( $ma_da, $dot );
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy lệnh tạm ứng ' . (int) $dot ); }
		$them = (array) $them;
		$vai  = VHCPMTD_Auth::vai_tro();
		...
		return VHCPMTD_Util::ok( array( 'dot' => $moi, 'tuQuyetToan' => (int) $tu_qt['so'] ) );
	}
	...
	public static function xin_quyet_toan_dot( $ma_da, $rows, $ghi_chu = '' ) {
		if ( ! self::find( $ma_da ) ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		$rows = array_values( array_unique( array_map( 'intval', (array) $rows ) ) );
		if ( ! $rows ) { return VHCPMTD_Util::err( 'Chưa tích hạng mục nào để gửi quyết toán.' ); }
		...
		foreach ( $rows as $r ) {
			...
			$h = self::hm_cua( $ma_da, $r );
			if ( 'xong' !== $h['tt'] ) {
				return VHCPMTD_Util::err( '"' . $lon[ $r ] . '" chưa chốt hoàn thành — chốt hoá đơn xong mới gửi quyết toán được.' );
			}
			if ( $h['qtDot'] > 0 ) {
				return VHCPMTD_Util::err( '"' . $lon[ $r ] . '" đã nằm trong một lệnh quyết toán rồi.' );
			}
			$nhan[] = $r;
	...
	public static function dat_tt_qt( $ma_da, $dot, $tt, $them = array() ) {
		if ( ! self::find( $ma_da ) ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		$tt = (string) $tt;
		if ( ! in_array( $tt, array( 'xong', 'tra' ), true ) ) { return VHCPMTD_Util::err( 'Trạng thái không hợp lệ' ); }
		$d = self::dot_cua( $ma_da, $dot, 'qt' );
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy lệnh quyết toán ' . (int) $dot ); }
		$vai = VHCPMTD_Auth::vai_tro();
		if ( ! in_array( $vai, array( 'Admin', 'Quản lý', 'Kế toán cá nhân', 'Kế toán NCC' ), true ) ) {
			return VHCPMTD_Util::err( 'Chỉ quản lý hoặc kế toán chốt / trả lại quyết toán được.' );
		}
		if ( 'xong' === $d['tt'] ) { return VHCPMTD_Util::err( 'Lệnh quyết toán ' . $d['dot'] . ' đã chốt sổ rồi.' ); }
		...
		return VHCPMTD_Util::ok( array( 'dot' => $moi ) );
	}
	...
	public static function dat_hm( $ma_da, $row, $tt, $them = array() ) {
		if ( ! self::find( $ma_da ) ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		$tt = (string) $tt;
		if ( ! in_array( $tt, self::TT_HM, true ) ) { return VHCPMTD_Util::err( 'Trạng thái không hợp lệ' ); }
		$them = (array) $them;
		$vai  = VHCPMTD_Auth::vai_tro();
		$duyet_duoc = in_array( $vai, array( 'Admin', 'Quản lý', 'Kế toán cá nhân', 'Kế toán NCC' ), true );
		$ke_toan    = in_array( $vai, array( 'Admin', 'Kế toán cá nhân', 'Kế toán NCC' ), true );
		...
		if ( 'xong' === $cu['tt'] && 'nhap' !== $tt ) {
			return VHCPMTD_Util::err( 'Hạng mục đã chốt là chi thực tế — không đổi được nữa.' );
		}
		if ( 'xong' === $cu['tt'] && 'nhap' === $tt && ! $ke_toan ) {
			return VHCPMTD_Util::err( 'Chỉ kế toán mở lại được hạng mục đã chốt.' );
		}
		if ( in_array( $tt, array( 'duyet', 'tra' ), true ) && ! $duyet_duoc ) {
			return VHCPMTD_Util::err( 'Chỉ quản lý hoặc kế toán duyệt / trả lại được.' );
		}
		...
		if ( $la_ncc && in_array( $tt, array( 'xong', 'ung' ), true ) && ! $ke_toan ) {
			return VHCPMTD_Util::err( 'Hạng mục này kế toán trả thẳng nhà cung cấp — chỉ kế toán xác nhận đã chi và khoá đơn.' );
		}
		if ( $la_ncc && 'xin' === $tt ) {
			return VHCPMTD_Util::err( 'Hạng mục này kế toán trả thẳng nhà cung cấp — không xin tạm ứng. Kế toán tích xác nhận đã chi.' );
		}
		...
		if ( ! $la_ncc && in_array( $tt, array( 'xin', 'duyet', 'ung' ), true ) ) {
			return VHCPMTD_Util::err( 'Bước này nay đi theo LỆNH tạm ứng của cả dự án — '
		...
		if ( ! $bo_qua_chuoi ) {
			if ( 'xin' === $tt && ! in_array( $cu['tt'], array( 'nhap', 'tra' ), true ) ) {
				return VHCPMTD_Util::err( 'Hạng mục này đã qua bước xin tạm ứng rồi.' );
			}
			if ( 'duyet' === $tt && 'xin' !== $cu['tt'] ) {
				return VHCPMTD_Util::err( 'Chỉ duyệt được hạng mục đang xin tạm ứng.' );
			}
			if ( 'ung' === $tt && 'duyet' !== $cu['tt'] ) {
				return VHCPMTD_Util::err( 'Chỉ cấp tạm ứng cho hạng mục đã duyệt.' );
			}
		}
		...
		if ( 'xong' === $tt ) {
			...
			if ( self::tien_hm( $ma_da, $row ) <= 0 ) {
				return VHCPMTD_Util::err( 'Hạng mục này chưa có số tiền thực tế nào (đang là 0đ) — '
		...
		if ( 'xin' === $tt && isset( $them['lich'] ) && is_array( $them['lich'] ) ) {
			...
			foreach ( $them['lich'] as $x ) {
				...
				$ds[] = array(
					'lan'    => $lan,
					'ngay'   => $ngay,
					'soTien' => VHCPMTD_Util::num( isset( $x['soTien'] ) ? $x['soTien'] : 0 ),
				);
			}
			$sua['lich'] = $ds;
		}
		...
		return VHCPMTD_Util::ok( array( 'hm' => $moi, 'tuQuyetToan' => (int) $tu_qt['so'] ) );
	}
	...
	private static function line_data( $rec ) {
		...
		$sl  = VHCPMTD_Util::num( $g( 'soLuong' ) );
		$dg  = VHCPMTD_Util::num( $g( 'donGia' ) );
		$cap = VHCPMTD_Util::st( $g( 'capCha' ) );
		// Gắn mã tài khoản theo LOẠI CHI PHÍ ngay lúc nhập (giống sổ chi phí).
		$loai_cp = VHCPMTD_Util::st( $g( 'loaiCp' ) );
		// Gian hàng đóng vai "cơ sở" ở mảng kỹ thuật -> mã theo mảng kinh doanh của gian đó.
		$tk      = VHCPMTD_Cfg::resolve_tk( $loai_cp, VHCPMTD_Util::st( $g( 'hinhThuc' ) ), array( 'tkNo' => VHCPMTD_Util::st( $g( 'tkNo' ) ), 'tkCo' => VHCPMTD_Util::st( $g( 'tkCo' ) ), 'maDt' => VHCPMTD_Util::st( $g( 'maDt' ) ) ), VHCPMTD_Util::st( $g( 'gian' ) ) );
		return array(
			'loai_cp'    => $loai_cp,
			'tk_no'      => $loai_cp !== '' ? $tk['tk_no'] : '',
			'tk_co'      => $loai_cp !== '' ? $tk['tk_co'] : '',
			'ma_dt'      => $loai_cp !== '' ? $tk['ma_dt'] : '',
			'noi_dung'   => VHCPMTD_Util::st( $g( 'noiDung' ) ),
			'du_toan'    => ( $cap === '' ) ? VHCPMTD_Util::num( $g( 'duToan' ) ) : 0,   // chỉ hạng mục lớn có dự toán
			...
			'thuc_te'    => ( ( null === $g( 'thucTe' ) || '' === $g( 'thucTe' ) ) && $sl * $dg > 0 )
				? ( $sl * $dg ) : VHCPMTD_Util::num( $g( 'thucTe' ) ),
			'so_luong'   => $sl,
			'don_gia'    => $dg,
			'thanh_tien' => $sl * $dg,
			'vat'        => VHCPMTD_Util::st( $g( 'vat' ) ),
			'anh'        => VHCPMTD_Util::st( $g( 'anh' ) ),
			'gian'       => VHCPMTD_Util::st( $g( 'gian' ) ),
			'note'       => VHCPMTD_Util::st( $g( 'note' ) ),
			'cap_cha'    => $cap,
			'hinh_thuc'  => VHCPMTD_Util::st( $g( 'hinhThuc' ) ),
			'ho_so'      => VHCPMTD_Util::st( $g( 'hoSo' ) ),
		);
	}
	...
	public static function add_line( $ma_da, $rec ) {
		...
		$st = (string) ( $f['trang_thai'] !== '' ? $f['trang_thai'] : 'Đang làm' );
		if ( $st === 'Đã đóng' ) {
			return VHCPMTD_Util::err( 'Dự án đã đóng — bấm "Mở lại" rồi nhập' );
		}
		$data           = self::line_data( $rec );
	...
	private static function sua_o_line_( $ma_da, $row, $cot, $url, $cong_them, $viec ) {
		...
		if ( ! $cu ) { return VHCPMTD_Util::err( 'Không tìm thấy dòng ' . $row ); }
	...
	public static function update_line( $ma_da, $row, $rec ) {
		...
		if ( ! $f ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		$st = (string) ( $f['trang_thai'] !== '' ? $f['trang_thai'] : 'Đang làm' );
		if ( $st === 'Đã đóng' ) { return VHCPMTD_Util::err( 'Dự án đã đóng — bấm "Mở lại" rồi sửa' ); }
		$row = (int) $row;
		/* ⚠️ SỬA CHẶN NHƯ XOÁ. Anh Thắng nói *"không cho xoá dòng"*, nhưng để hở nút sửa thì gõ
		   tiền về 0 là xoá trá hình, chỉ khác cái tên. */
		$_c = self::loi_hang_muc_da_chot_( $ma_da, $row, 'sửa dòng' );
		if ( '' !== $_c ) { return VHCPMTD_Util::err( $_c ); }
		if ( $row < self::DATA_ROW ) { return VHCPMTD_Util::err( 'Dòng không hợp lệ' ); }
		$t   = VHCPMTD_DB::t( 'da_line' );
		$cur = VHCPMTD_DB::row( $wpdb->prepare( "SELECT * FROM $t WHERE ma_da=%s AND row_no=%d", (string) $ma_da, $row ) );
		if ( ! $cur ) { return VHCPMTD_Util::err( 'Dòng không hợp lệ' ); }
	...
	public static function nhat_ky_du_an( $ma_da, $limit = 50 ) {
		global $wpdb;
		$ma_da = trim( (string) $ma_da );
		if ( '' === $ma_da ) { return VHCPMTD_Util::ok( array( 'items' => array() ) ); }
		$limit = (int) $limit;
	...
	public static function delete_line( $ma_da, $row ) {
		...
		if ( ! $f ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		$st = (string) ( $f['trang_thai'] !== '' ? $f['trang_thai'] : 'Đang làm' );
		if ( $st === 'Đã đóng' ) { return VHCPMTD_Util::err( 'Dự án đã đóng — bấm "Mở lại" rồi xóa' ); }
		$row = (int) $row;
		if ( $row < self::DATA_ROW ) { return VHCPMTD_Util::err( 'Dòng không hợp lệ' ); }
		$t   = VHCPMTD_DB::t( 'da_line' );
		$cur = VHCPMTD_DB::row( $wpdb->prepare( "SELECT * FROM $t WHERE ma_da=%s AND row_no=%d", (string) $ma_da, $row ) );
	...
	private static function set_status( $ma_da, $status ) {
		global $wpdb;
		$wpdb->update( VHCPMTD_DB::t( 'da_index' ), array( 'trang_thai' => (string) $status ), array( 'ma_da' => (string) $ma_da ) );
	}

	public static function submit( $ma_da ) {
		$f = self::find( $ma_da );
		if ( ! $f ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		if ( self::la_coso_chung( $ma_da ) ) { return VHCPMTD_Util::err( 'Sổ Chi phí cơ sở CHUNG không cần duyệt' ); }
		if ( (string) ( $f['trang_thai'] !== '' ? $f['trang_thai'] : 'Đang làm' ) !== 'Đang làm' ) { return VHCPMTD_Util::err( 'Chỉ gửi khi đang làm' ); }
	...
	public static function approve( $ma_da, $nguoi ) {
		$f = self::find( $ma_da );
		if ( ! $f ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		if ( (string) $f['trang_thai'] !== 'Chờ kế toán duyệt' ) { return VHCPMTD_Util::err( 'Dự án không ở trạng thái chờ duyệt' ); }
		self::set_status( $ma_da, 'Đã duyệt' );
		VHCPMTD_Meta::set( 'daApp_' . $ma_da, VHCPMTD_Util::now()->format( 'd/m/Y' ) );   // ngày chứng từ khi xuất MISA
		return VHCPMTD_Util::ok();
	}
	...
	public static function ret( $ma_da ) {
		$f = self::find( $ma_da );
		if ( ! $f ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		if ( (string) $f['trang_thai'] !== 'Chờ kế toán duyệt' ) { return VHCPMTD_Util::err( 'Chỉ trả khi đang chờ duyệt' ); }
	...
	public static function close( $ma_da ) {
		$f = self::find( $ma_da );
		if ( ! $f ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án' ); }
		if ( self::la_coso_chung( $ma_da ) ) { return VHCPMTD_Util::err( 'Sổ Chi phí cơ sở CHUNG không đóng — nó chạy xuyên suốt' ); }
		if ( (string) $f['trang_thai'] === 'Đã đóng' ) { return VHCPMTD_Util::err( 'Dự án đã đóng' ); }
	...
	public static function delete( $ma_da ) {
		...
		if ( $la_chung ) {
			...
			$n_sc = count( VHCPMTD_SoChi::theo_du_an( (string) $ma_da ) );
			if ( ! $n_sc ) { $n_sc = count( VHCPMTD_SoChi::theo_du_an( (string) $f['ten'] ) ); }
			if ( $n_dong || $n_sc ) {
				return VHCPMTD_Util::err( 'Sổ Chi phí cơ sở CHUNG còn ' . ( $n_dong + $n_sc )
					. ' dòng chi — không xoá được. Dòng ở đây có thể đã xuất MISA; xoá hết dòng trước rồi mới xoá sổ.' );
			}
		}
		if ( (string) $f['trang_thai'] === 'Đã đóng' ) { return VHCPMTD_Util::err( 'Dự án đã đóng — Admin "Mở lại" trước khi xóa' ); }
		$wpdb->delete( VHCPMTD_DB::t( 'da_line' ), array( 'ma_da' => (string) $ma_da ) );
		$wpdb->delete( VHCPMTD_DB::t( 'da_index' ), array( 'ma_da' => (string) $ma_da ) );
		VHCPMTD_Meta::del( 'daPay_' . $ma_da );
		VHCPMTD_Meta::del( 'daApp_' . $ma_da );
		VHCPMTD_Meta::del( 'daKy_' . $ma_da );
		...
		if ( $la_chung ) { VHCPMTD_Meta::set( self::MK_COSO_CHUNG, '-' ); }
		return VHCPMTD_Util::ok();
	}
	...
	public static function gan_ma_tai_khoan( $all = false ) {
		...
		foreach ( self::all_with_lines() as $p ) {
			...
			foreach ( $p['lines'] as $x ) {
				...
				$giu    = VHCPMTD_Cfg::ma_con_hop_le( $loai, $gian_x, $x['tk_no'] );
				$tk  = VHCPMTD_Cfg::resolve_tk( $loai, $ht, array( 'tkNo' => $giu ), $gian_x );
		...
		return VHCPMTD_Util::ok( array( 'updated' => $n, 'thieuMa' => array_keys( $thieu ), 'khongSuyDuoc' => $khong_suy ) );
	}
	...
	public static function all_with_lines() {
		$ti = VHCPMTD_DB::t( 'da_index' );
		$tl = VHCPMTD_DB::t( 'da_line' );
		$rows = VHCPMTD_DB::rows( "SELECT * FROM $ti ORDER BY stt ASC" );
		$by   = array();
		foreach ( VHCPMTD_DB::rows( "SELECT * FROM $tl ORDER BY ma_da ASC, row_no ASC" ) as $l ) {
			$by[ (string) $l['ma_da'] ][] = $l;
		}
		foreach ( $rows as $i => $r ) {
			$k = (string) $r['ma_da'];
	...
	public static function pay_map() {
		$out = array();
		foreach ( VHCPMTD_Meta::get_prefix( 'daPay_' ) as $k => $v ) {
	...
	public static function approve_date_map() {
		$out = array();
		foreach ( VHCPMTD_Meta::get_prefix( 'daApp_' ) as $k => $v ) {
