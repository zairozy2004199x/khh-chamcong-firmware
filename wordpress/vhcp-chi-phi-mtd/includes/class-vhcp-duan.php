...
class VHCPMTD_DuAn {
	...
	public static function dat_tt_dot( $ma_da, $dot, $tt, $them = array() ) {
		...
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy lệnh tạm ứng đợt ' . (int) $dot ); }
		$them = (array) $them;
		...
		if ( 'ung' === $d['tt'] ) { return VHCPMTD_Util::err( 'Lệnh đợt ' . $d['dot'] . ' đã cấp tiền rồi.' ); }
	...
	public static function dat_tt_qt( $ma_da, $dot, $tt, $them = array() ) {
		...
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy lệnh quyết toán đợt ' . (int) $dot ); }
		$vai = VHCPMTD_Auth::vai_tro();
		...
		if ( 'xong' === $d['tt'] ) { return VHCPMTD_Util::err( 'Lệnh quyết toán đợt ' . $d['dot'] . ' đã chốt sổ rồi.' ); }
	...
	private static function line_data( $rec ) {
		...
		return array(
			...
			'thuc_te'    => VHCPMTD_Util::num( $g( 'thucTe' ) ),
			'so_luong'   => $sl,
			'don_gia'    => $dg,
			'thanh_tien' => $sl * $dg,
