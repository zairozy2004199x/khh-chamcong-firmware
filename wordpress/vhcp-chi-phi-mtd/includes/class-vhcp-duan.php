...
class VHCPMTD_DuAn {
	...
	public static function cap_tien_phan( $ma_da, $dot, $them = array() ) {
		...
		if ( $so > $con ) {
			return VHCPMTD_Util::err( 'Lệnh này chỉ còn ' . number_format( (float) $con, 0, ',', '.' )
	...
	public static function dat_hm( $ma_da, $row, $tt, $them = array() ) {
		...
		if ( 'xong' === $tt ) {
			...
			if ( ! $co_ct && ! $khai_ko ) {
				return VHCPMTD_Util::err( 'Hạng mục này chưa có chứng từ nào: ô hoá đơn trống, mà cột ẢNH / '
