 ...
 * nay là các dòng trong vhcpmtd_bp_line khóa theo (ma, row_no), row_no bắt đầu từ 5.
 */
...
class VHCPMTD_BP {
	...
	public static function find( $ma ) {
		...
		return VHCPMTD_DB::row( $wpdb->prepare( "SELECT * FROM $t WHERE ma=%s", (string) $ma ) );
	}

	public static function lines_of( $ma ) {
		global $wpdb;
		$t = VHCPMTD_DB::t( 'bp_line' );
		return VHCPMTD_DB::rows( $wpdb->prepare( "SELECT * FROM $t WHERE ma=%s ORDER BY row_no ASC", (string) $ma ) );
	}
	 ...
	 * lập thì luôn có đúng một nhà — xem chốt dài ở `VHCPMTD_DuAn::don_vi_cua()`.
	 */
	...
	public static function all_with_lines() {
		$ti = VHCPMTD_DB::t( 'bp_index' );
		$tl = VHCPMTD_DB::t( 'bp_line' );
		$rows = VHCPMTD_DB::rows( "SELECT * FROM $ti ORDER BY stt ASC" );
		$by   = array();
		foreach ( VHCPMTD_DB::rows( "SELECT * FROM $tl ORDER BY ma ASC, row_no ASC" ) as $l ) {
			$by[ (string) $l['ma'] ][] = $l;
		}
		foreach ( $rows as $i => $r ) {
			$k = (string) $r['ma'];
	...
	public static function create_voi_ma( $ma, $loai, $ten, $nguoi, $dia_diem, $ky, $creator ) {
		global $wpdb;
		$ma = trim( (string) $ma );
		if ( $ma === '' ) { return VHCPMTD_Util::err( 'Thiếu mã đợt' ); }
		if ( self::find( $ma ) ) { return VHCPMTD_Util::ok( array( 'ma' => $ma, 'daCo' => 1 ) ); }
		$loai = in_array( trim( (string) $loai ), array( 'Công tác', 'Setup' ), true ) ? trim( (string) $loai ) : 'Công tác';
		$ky   = trim( (string) $ky );
		if ( $ky === '' ) { $ky = VHCPMTD_Util::now()->format( 'm/Y' ); }
		$wpdb->insert( VHCPMTD_DB::t( 'bp_index' ), array(
			/* Mảng đóng dấu lúc lập — xem chốt ở `VHCPMTD_SoChi::add()`. */
			'khoi'       => VHCPMTD_DB::khoi(),
			'ma'         => $ma,
			'loai'       => $loai,
			'ten'        => VHCPMTD_Util::san( $ten ) !== '' ? VHCPMTD_Util::san( $ten ) : $ma,
			'nguoi'      => trim( (string) $nguoi ),
			'dia_diem'   => trim( (string) $dia_diem ),
			'ky'         => $ky,
			'trang_thai' => 'Đang xử lý',
			'ngay_tao'   => VHCPMTD_Util::now()->format( 'd/m/Y' ),
			'nguoi_tao'  => (string) $creator,
		) );
		return VHCPMTD_Util::ok( array( 'ma' => $ma ) );
	}

	public static function create( $loai, $ten, $nguoi, $dia_diem, $ky, $creator ) {
		global $wpdb;
		$loai = trim( (string) $loai );
		if ( ! in_array( $loai, array( 'Công tác', 'Setup' ), true ) ) { return VHCPMTD_Util::err( 'Loại không hợp lệ' ); }
		$ten = VHCPMTD_Util::san( $ten );
		if ( $ten === '' ) { return VHCPMTD_Util::err( 'Tên đợt trống' ); }
		$ky = trim( (string) $ky );
		if ( $ky === '' ) { $ky = VHCPMTD_Util::now()->format( 'm/Y' ); }
		$ma = VHCPMTD_Util::uid( 'BP' );
		$wpdb->insert( VHCPMTD_DB::t( 'bp_index' ), array(
			/* Mảng đóng dấu lúc lập — xem chốt ở `VHCPMTD_SoChi::add()`. */
			'khoi'       => VHCPMTD_DB::khoi(),
			'ma'         => $ma,
	...
	public static function get( $ma ) {
		$f = self::find( $ma );
		if ( ! $f ) { return VHCPMTD_Util::err( 'Không tìm thấy đợt chi phí' ); }
		$lines = array();
	...
	private static function line_data( $rec, $coso = '' ) {
		...
		$tk      = VHCPMTD_Cfg::resolve_tk( $loai_cp, VHCPMTD_Util::st( $g( 'hinhThuc' ) ), array( 'tkNo' => VHCPMTD_Util::st( $g( 'tkNo' ) ), 'tkCo' => VHCPMTD_Util::st( $g( 'tkCo' ) ), 'maDt' => VHCPMTD_Util::st( $g( 'maDt' ) ) ), $coso );
		return array(
			'loai_cp'    => $loai_cp,
	...
	public static function add_line( $ma, $rec ) {
		global $wpdb;
		$dot = self::find( $ma );
		if ( ! $dot ) { return VHCPMTD_Util::err( 'Không tìm thấy' ); }
		$rec = (array) $rec;
		if ( ! isset( $rec['ngay'] ) || trim( (string) $rec['ngay'] ) === '' ) { $rec['ngay'] = VHCPMTD_Util::now()->format( 'd/m/Y' ); }
		$data           = self::line_data( $rec, (string) $dot['dia_diem'] );
		$data['ma']     = (string) $ma;
		$data['row_no'] = self::next_row( $ma );
		$wpdb->insert( VHCPMTD_DB::t( 'bp_line' ), $data );
	...
	public static function update_line( $ma, $row, $rec ) {
		...
		$t = VHCPMTD_DB::t( 'bp_line' );
		if ( ! VHCPMTD_DB::row( $wpdb->prepare( "SELECT id FROM $t WHERE ma=%s AND row_no=%d", (string) $ma, $row ) ) ) { return VHCPMTD_Util::err( 'Dòng không hợp lệ' ); }
	...
	public static function delete_line( $ma, $row ) {
		global $wpdb;
		if ( ! self::find( $ma ) ) { return VHCPMTD_Util::err( 'Không tìm thấy' ); }
		$row = (int) $row;
		if ( $row < self::DATA_ROW ) { return VHCPMTD_Util::err( 'Dòng không hợp lệ' ); }
		$wpdb->delete( VHCPMTD_DB::t( 'bp_line' ), array( 'ma' => (string) $ma, 'row_no' => $row ) );
		return VHCPMTD_Util::ok();
	}
	...
	public static function rename( $ma, $ten ) {
		...
		if ( ! self::find( $ma ) ) { return VHCPMTD_Util::err( 'Không tìm thấy' ); }
		$wpdb->update( VHCPMTD_DB::t( 'bp_index' ), array( 'ten' => $ten ), array( 'ma' => (string) $ma ) );
	...
	public static function close( $ma ) {
		global $wpdb;
		if ( ! self::find( $ma ) ) { return VHCPMTD_Util::err( 'Không tìm thấy' ); }
		$wpdb->update( VHCPMTD_DB::t( 'bp_index' ), array( 'trang_thai' => 'Đã đóng' ), array( 'ma' => (string) $ma ) );
		return VHCPMTD_Util::ok();
	}
	...
	public static function reopen( $ma ) {
		global $wpdb;
		if ( ! self::find( $ma ) ) { return VHCPMTD_Util::err( 'Không tìm thấy' ); }
		$wpdb->update( VHCPMTD_DB::t( 'bp_index' ), array( 'trang_thai' => 'Đang xử lý' ), array( 'ma' => (string) $ma ) );
		return VHCPMTD_Util::ok();
	}
	...
	public static function gan_ma_tai_khoan( $all = false ) {
		...
		foreach ( self::all_with_lines() as $b ) {
			foreach ( $b['lines'] as $r ) {
				...
				$giu = VHCPMTD_Cfg::ma_con_hop_le( $loai, (string) $b['dia_diem'], $r['tk_no'] );
				$tk = VHCPMTD_Cfg::resolve_tk( $loai, trim( (string) $r['hinh_thuc'] ), array( 'tkNo' => $giu ), (string) $b['dia_diem'] );
				if ( $tk['tk_no'] === '' ) { $thieu[ $loai ] = 1; }
				$wpdb->update( $t, array( 'tk_no' => $tk['tk_no'], 'tk_co' => $tk['tk_co'], 'ma_dt' => $tk['ma_dt'] ), array( 'id' => (int) $r['id'] ) );
				$n++;
			}
		}
		return VHCPMTD_Util::ok( array( 'updated' => $n, 'thieuMa' => array_keys( $thieu ), 'chuaChonLoai' => $chua ) );
	}

	public static function delete( $ma ) {
		global $wpdb;
		if ( ! self::find( $ma ) ) { return VHCPMTD_Util::err( 'Không tìm thấy' ); }
		$wpdb->delete( VHCPMTD_DB::t( 'bp_line' ), array( 'ma' => (string) $ma ) );
		$wpdb->delete( VHCPMTD_DB::t( 'bp_index' ), array( 'ma' => (string) $ma ) );
		return VHCPMTD_Util::ok();
	}
}
