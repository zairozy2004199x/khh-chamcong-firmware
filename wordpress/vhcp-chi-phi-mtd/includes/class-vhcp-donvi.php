...
class VHCPMTD_DonVi {
	 ...
	 *
	 * Đổi tên nhà mẹ (hay tắt hẳn luật này) bằng khoá `vhcpmtd_dv_me`; để trống khoá = tắt.
	 * ══════════════════════════════════════════════════════════════════════════════════════ */
	...
	public static function don_vi_me() {
		$x = get_option( 'vhcpmtd_dv_me', null );
	...
	private static function ds_trong_don() {
		global $wpdb;
		$t = VHCPMTD_DB::t( 'don' );
		$c = VHCPMTD_DB::rows( "SELECT DISTINCT don_vi FROM $t" );
		$out = array();
	 ...
	 *
	 * ⚠️ KHAI THÊM ĐƠN VỊ thì đặt khoá `vhcpmtd_dv_nhieu_coso` (danh sách ngăn bằng dấu phẩy).
	...
	public static function nhieu_coso( $don_vi ) {
		$ds = get_option( 'vhcpmtd_dv_nhieu_coso', null );
	...
	public static function cua_toi() {
		return self::cua_nguoi( VHCPMTD_Auth::nguoi() );
	}
	 ...
	 *
	 * ⚠️ VÀ HAI TRỤC NÀY VỐN TRÙNG NHAU: `DON_VI_GHE = 'POSH'`, mà mã ở `VHCPMTD_Cfg` ghi rõ *"Máy
	...
	const KHOI_THEO_DON_VI = array(
		...
		/* 'vending' (25/09/2026): chi phí máy bán hàng kéo từ web Vending HCMC — đơn vị VENDING, xem `VHCPMTD_Vending`. */
	...
	public static function khoi_ds() {
		...
		foreach ( array_keys( self::KHOI_THEO_DON_VI ) as $ma ) {
			$ra[] = array( 'ma' => $ma, 'ten' => VHCPMTD_Cfg::ten_khoi( $ma ) );
		}
		return $ra;
	}
	...
	public static function khoi_xem_duoc() {
		/* ═══════════════════════════════════════════════════════════════════════════
		   ...
		   ══════════════════════════════════════════════════════════════════════════ */
		$vai = trim( (string) VHCPMTD_Auth::vai_hien() );
		if ( 'Admin' !== $vai ) {
			$tay = self::khoi_khai_tay( VHCPMTD_Auth::nguoi() );
			if ( $tay ) { return $tay; }
		}
		$ds = self::xem_duoc();
	...
	/**
	 * CHỐT MỘT LƯỢT CHO MỌI HÀM CÓ MÃ ĐƠN — gọi từ `VHCPMTD_Api::handle()`.
	 *
	 * 🔴 VÌ SAO KHÔNG RẢI CHỐT VÀO TỪNG HÀM. Riêng `VHCPMTD_Don` đã có 15 hàm đụng vào một đơn mà
	...
	public static function chan_theo_ham( $callable, $args ) {
		...
		/* ⚠️ HAI NHÁNH DƯỚI CHỈ DÀNH CHO `VHCPMTD_Don`. `VHCPMTD_Mk::add_line()` cũng nhận `$ma_don`,
		...
		if ( method_exists( $callable[0], 'don_vi_cua' ) ) {
			/* Truyền CẢ TÊN THAM SỐ, không chỉ giá trị. `$id` của `VHCPMTD_SoChi` là một dòng
			   sổ, còn `$id` của `VHCPMTD_Mk` là một dòng trong đợt — hai thứ tra ở hai bảng
