...
class VHCPMTD_MK {
	...
	public static function lines_of( $ma ) {
		...
		return VHCPMTD_DB::rows( $wpdb->prepare( "SELECT * FROM $t WHERE ma_don=%s ORDER BY stt ASC", (string) $ma ) );
	}
	...
	public static function don_vi_cua( $khoa, $kieu = 'ma' ) {
		if ( 'id' === $kieu ) {
			global $wpdb;
			$t = VHCPMTD_DB::t( 'mk_line' );
			$l = VHCPMTD_DB::row( $wpdb->prepare( "SELECT ma_don FROM $t WHERE id=%s", (string) $khoa ) );
			if ( ! $l ) { return null; }
	...
	public static function create_don( $coso, $ten, $ky, $kenh, $nguoi ) {
		...
		if ( $coso === '' && $ten === '' ) { return VHCPMTD_Util::err( 'Nhập Cơ sở/Gian hoặc Tên chiến dịch/sự kiện' ); }
		$ma = VHCPMTD_Util::uid( 'MKD' );
		$ky = trim( (string) $ky );
		if ( $ky === '' ) { $ky = VHCPMTD_Util::now()->format( 'm/Y' ); }
		$wpdb->insert( VHCPMTD_DB::t( 'mk_don' ), array(
	...
	private static function agg() {
		...
		foreach ( self::all_lines() as $r ) {
			...
			$agg[ $md ]['dt'] += VHCPMTD_Util::num( $r['du_toan'] );
			$agg[ $md ]['tt'] += VHCPMTD_Util::num( $r['thuc_te'] );
			$agg[ $md ]['kq'] += VHCPMTD_Util::num( $r['ket_qua'] );
		}
		return $agg;
	}
	...
	public static function list_don( $coso = 'all' ) {
		...
		$cs = VHCPMTD_DonVi::coso_xem_duoc();
		if ( null === $cs ) {
			$cs = array();
			foreach ( VHCPMTD_Cfg::cfg_static()['coso'] as $x ) { $cs[] = $x['ten']; }
		}
		return VHCPMTD_Util::ok( array( 'items' => array_reverse( $out ), 'coso' => $cs ) );
	}

	public static function get_don( $ma ) {
		$h = self::don_row( $ma );
		if ( ! $h ) { return VHCPMTD_Util::err( 'Không tìm thấy đơn' ); }
		$lines = array();
		$dt = 0; $tt = 0; $kq = 0; $dt_tu = 0; $dt_tt = 0; $tt_tu = 0; $tt_tt = 0; $vat = 0; $novat = 0;
		foreach ( self::lines_of( $ma ) as $r ) {
			$d = VHCPMTD_Util::num( $r['du_toan'] );
			$t = VHCPMTD_Util::num( $r['thuc_te'] );
			$k = VHCPMTD_Util::num( $r['ket_qua'] );
	...
	private static function line_data( $rec, $coso = '' ) {
		...
		return array(
			...
			'ket_qua'   => VHCPMTD_Util::num( $g( 'ketQua' ) ),
	...
	public static function update_line( $id, $rec ) {
		global $wpdb;
		$t = VHCPMTD_DB::t( 'mk_line' );
		$cur = VHCPMTD_DB::row( $wpdb->prepare( "SELECT * FROM $t WHERE id=%s", (string) $id ) );
		if ( ! $cur ) { return VHCPMTD_Util::err( 'Không tìm thấy hạng mục' ); }
	...
	public static function delete_line( $id ) {
		global $wpdb;
		$t = VHCPMTD_DB::t( 'mk_line' );
		$cur = VHCPMTD_DB::row( $wpdb->prepare( "SELECT id FROM $t WHERE id=%s", (string) $id ) );
		if ( ! $cur ) { return VHCPMTD_Util::err( 'Không tìm thấy hạng mục' ); }
	...
	public static function edit_don( $ma, $coso, $ten, $ky = null, $kenh = null ) {
		global $wpdb;
		if ( ! self::don_row( $ma ) ) { return VHCPMTD_Util::err( 'Không tìm thấy đơn' ); }
		$data = array( 'coso' => trim( (string) $coso ), 'ten' => VHCPMTD_Util::san( $ten ) );
		if ( $ky !== null )   { $data['ky']   = trim( (string) $ky ); }
		if ( $kenh !== null ) { $data['kenh'] = trim( (string) $kenh ); }
		$wpdb->update( VHCPMTD_DB::t( 'mk_don' ), $data, array( 'ma' => (string) $ma ) );
		return VHCPMTD_Util::ok();
	}

	public static function close_don( $ma ) {
		global $wpdb;
		if ( ! self::don_row( $ma ) ) { return VHCPMTD_Util::err( 'Không tìm thấy' ); }
		$wpdb->update( VHCPMTD_DB::t( 'mk_don' ), array( 'trang_thai' => 'Đã đóng' ), array( 'ma' => (string) $ma ) );
		return VHCPMTD_Util::ok();
	}
	...
	public static function reopen_don( $ma ) {
		global $wpdb;
		if ( ! self::don_row( $ma ) ) { return VHCPMTD_Util::err( 'Không tìm thấy' ); }
		$wpdb->update( VHCPMTD_DB::t( 'mk_don' ), array( 'trang_thai' => 'Đang chạy' ), array( 'ma' => (string) $ma ) );
		return VHCPMTD_Util::ok();
	}
	...
	public static function gan_ma_tai_khoan( $all = false ) {
		...
		foreach ( self::all_lines() as $r ) {
			...
			$giu  = VHCPMTD_Cfg::ma_con_hop_le( $loai, $cs, $r['tk_no'] );
			$tk = VHCPMTD_Cfg::resolve_tk( $loai, trim( (string) $r['hinh_thuc'] ), array( 'tkNo' => $giu ), $cs );
	...
	public static function delete_don( $ma ) {
		global $wpdb;
		if ( ! self::don_row( $ma ) ) { return VHCPMTD_Util::err( 'Không tìm thấy đơn' ); }
		$wpdb->delete( VHCPMTD_DB::t( 'mk_line' ), array( 'ma_don' => (string) $ma ) );
		$wpdb->delete( VHCPMTD_DB::t( 'mk_don' ), array( 'ma' => (string) $ma ) );
		return VHCPMTD_Util::ok();
	}
}
