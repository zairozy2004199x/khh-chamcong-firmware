 ...
 *
 *   Lớp này chỉ lọc tag theo TIỀN TỐ của chính plugin mình (`vhcp-chi-phi-mtd-v…`), nên chín plugin
...
class VHCPMTD_TuCapNhat {
	...
	public static function init() {
		...
		/* 🔴 ĐỔI TÊN THƯ MỤC SAU KHI GIẢI NÉN. Tệp .zip của Releases giải ra thành thư mục
		   `vhcp-chi-phi-mtd/` — đúng tên plugin đang cài, nên WordPress đặt lại đúng chỗ. Nhưng nếu
		...
		add_filter( 'vhcpmtd_tu_cap_nhat_ds', array( __CLASS__, 'khai_ds' ) );
	}
	 ...
	 * danh sách plugin nào cả — nó dựng bảng từ bộ lọc `vhcpmtd_tu_cap_nhat_ds` lúc chạy. Nên plugin
	...
	private static function ban_hien() {
		return defined( 'VHCPMTD_VERSION' ) ? (string) VHCPMTD_VERSION : '0';
	}
	...
	/** Đường dẫn plugin dạng `vhcp-chi-phi-mtd/vhcp-chi-phi-mtd.php` — khoá WordPress dùng để nhận plugin. */
	private static function duong() {
		return plugin_basename( VHCPMTD_DIR . 'vhcp-chi-phi-mtd.php' );
	}
	...
	public static function ban_moi( $bo_qua_nho = false ) {
		...
		$dau = array( 'headers' => array(
			...
			'User-Agent' => 'vhcpmtd-tu-cap-nhat',
		), 'timeout' => 15 );
		...
		$hien  = defined( 'VHCPMTD_VERSION' ) ? VHCPMTD_VERSION : '0';
		$tot   = null;
		foreach ( $ds as $rel ) {
			...
			/* 🔴 LỌC THEO TIỀN TỐ CỦA CHÍNH PLUGIN NÀY. Không có bước này thì bản chấm công
			   `vhcpmtd-cham-cong-v3.76.0` hiện ra như bản mới của chi phí. */
