...
class VHCPMTD_Import {
	...
	private static function nap_theo_tieu_de( $bang, $rows, $replace, $ma_chon, $coso_mac_dinh = '' ) {
		global $wpdb;
		$k = VHCPMTD_Nap::khop( $bang, $rows );
		if ( ! empty( $k['loi'] ) ) { return VHCPMTD_Util::err( $k['loi'] ); }
		$hd = $k['hd'];
		$o  = function ( $r, $f ) use ( $hd ) { return VHCPMTD_Nap::o( $r, $hd, $f ); };
		...
		switch ( $bang ) {
			...
			case 'chiphi':
				...
				foreach ( $k['rows'] as $r ) {
					...
					$tt  = $o( $r, 'thanh_tien' ) !== '' ? self::n0( $o( $r, 'thanh_tien' ) ) : ( VHCPMTD_Util::num( $sl ) * VHCPMTD_Util::num( $dg ) );
					$cs  = $o( $r, 'coso' );
					$loai = $o( $r, 'loai_cp' );
					$tk  = ( $loai !== '' )
						? VHCPMTD_Cfg::resolve_tk( $loai, $o( $r, 'phan_loai_tt' ), array(), $cs )
						: VHCPMTD_Don::tk_of_line( $o( $r, 'nhom' ), $o( $r, 'phan_loai_tt' ) );
					if ( trim( (string) $tk['tk_no'] ) === '' ) { $thieu_ma++; }
					$id = $o( $r, 'id' ) !== '' ? $o( $r, 'id' ) : VHCPMTD_Util::uid( 'CP' );
			...
			case 'bp_line':
				$t = VHCPMTD_DB::t( 'bp_line' );
				if ( $replace ) { $wpdb->query( "DELETE FROM $t" ); }
				$dia = array();
				foreach ( VHCPMTD_BP::all_with_lines() as $b ) { $dia[ (string) $b['ma'] ] = (string) $b['dia_diem']; }
				$row_no = array();
				foreach ( $k['rows'] as $r ) {
					...
					if ( ! isset( $row_no[ $m ] ) ) {
						$row_no[ $m ] = max( VHCPMTD_DB::DATA_ROW - 1, (int) $wpdb->get_var( $wpdb->prepare( "SELECT MAX(row_no) FROM $t WHERE ma=%s", $m ) ) );
					}
					$row_no[ $m ]++;
					$sl   = self::n0( $o( $r, 'so_luong' ) );
					$dg   = self::n0( $o( $r, 'don_gia' ) );
					$loai = $o( $r, 'loai_cp' );
					$tk   = ( $loai !== '' ) ? VHCPMTD_Cfg::resolve_tk( $loai, $o( $r, 'hinh_thuc' ), array(), $dia[ $m ] ) : array( 'tk_no' => '', 'tk_co' => '', 'ma_dt' => '' );
			...
			case 'da_line':
				...
				foreach ( $k['rows'] as $r ) {
					...
					$nd = $o( $r, 'noi_dung' );
					if ( $da === '' ) { return VHCPMTD_Util::err( 'File không có cột "Mã dự án" — chọn dự án ở ô "Dự án / Đợt nhận dòng" rồi nạp lại.' ); }
					if ( $nd === '' && ! self::n0( $o( $r, 'thuc_te' ) ) && ! self::n0( $o( $r, 'du_toan' ) ) ) { $skipped++; continue; }
					if ( ! VHCPMTD_DuAn::find( $da ) ) { $chua_co_cha[ $da ] = 1; $skipped++; continue; }
					if ( ! isset( $row_no[ $da ] ) ) {
						if ( $replace ) { $wpdb->delete( $t, array( 'ma_da' => $da ) ); }
						$row_no[ $da ] = max( VHCPMTD_DB::DATA_ROW - 1, (int) $wpdb->get_var( $wpdb->prepare( "SELECT MAX(row_no) FROM $t WHERE ma_da=%s", $da ) ) );
					}
					$row_no[ $da ]++;
					$sl   = self::n0( $o( $r, 'so_luong' ) );
					$dg   = self::n0( $o( $r, 'don_gia' ) );
					$gian = $o( $r, 'gian' );
					$loai = $o( $r, 'loai_cp' );
					$tk   = ( $loai !== '' ) ? VHCPMTD_Cfg::resolve_tk( $loai, $o( $r, 'hinh_thuc' ), array(), $gian ) : array( 'tk_no' => '', 'tk_co' => '', 'ma_dt' => '' );
					if ( $tk['tk_no'] === '' ) { $thieu_ma++; }
					$wpdb->insert( $t, array(
						'ma_da'      => $da,
						'row_no'     => $row_no[ $da ],
			...
			case 'mk_line':
				$t = VHCPMTD_DB::t( 'mk_line' );
				if ( $replace ) { $wpdb->query( "DELETE FROM $t" ); }
				$cs_don = array();
				foreach ( VHCPMTD_MK::all_dons() as $d ) { $cs_don[ (string) $d['ma'] ] = (string) $d['coso']; }
				foreach ( $k['rows'] as $r ) {
					$md = $o( $r, 'ma_don' );
					if ( $md === '' ) { $skipped++; continue; }
					if ( ! isset( $cs_don[ $md ] ) ) { $chua_co_cha[ $md ] = 1; $skipped++; continue; }
					$loai = $o( $r, 'loai_cp' );
					$tk   = ( $loai !== '' ) ? VHCPMTD_Cfg::resolve_tk( $loai, $o( $r, 'hinh_thuc' ), array(), $cs_don[ $md ] ) : array( 'tk_no' => '', 'tk_co' => '', 'ma_dt' => '' );
					if ( $tk['tk_no'] === '' ) { $thieu_ma++; }
					$id = $o( $r, 'id' ) !== '' ? $o( $r, 'id' ) : VHCPMTD_Util::uid( 'MKL' );
					$wpdb->delete( $t, array( 'id' => $id ) );
					$wpdb->insert( $t, array(
						'id'        => $id,
						'ma_don'    => $md,
			...
			case 'sochi':
				...
				foreach ( $k['rows'] as $r ) {
					...
					$res = VHCPMTD_SoChi::add( array(
						...
						'soTien'   => ( isset( $la_hang_muc[ mb_strtolower( $nd ) ] ) ? 0 : self::n( VHCPMTD_Nap::o_so( $r, $k, 'so_tien' ) ) ),
						...
						'duToan'   => self::n( VHCPMTD_Nap::o_so( $r, $k, 'du_toan' ) ),
						'hoSo'     => $o( $r, 'ho_so' ),
					...
					$tien_dong = VHCPMTD_Util::num( isset( $res['soTien'] ) ? $res['soTien'] : 0 );
					$tong_tien += $tien_dong;
				...
				$them_loai = VHCPMTD_Cfg::them_loai_neu_thieu( array_keys( $loai_moi ) );
				break;

			default:
				return VHCPMTD_Util::err( 'Bảng đích chưa hỗ trợ tự dò' );
		}
	...
	private static function d( $v ) {
		return VHCPMTD_Util::parse_date( $v );
	}
	...
	public static function run( $type, $text, $opts = array() ) {
		...
		global $wpdb;
		$types = self::types();
		if ( ! isset( $types[ $type ] ) ) { return VHCPMTD_Util::err( 'Loại tab không hợp lệ' ); }

		// Chưa chọn file / chưa dán gì: nói đúng chuyện đó, đừng bắt người dùng đi soi file.
		if ( trim( (string) $text ) === '' ) {
			return VHCPMTD_Util::err( 'Chưa chọn file CSV (và ô "dán nội dung" cũng đang trống). Bấm "Choose File" chọn file .csv rồi nạp lại.' );
		}
		...
		if ( ! count( $rows ) ) {
			return VHCPMTD_Util::err( 'File có ' . number_format( strlen( $text ) ) . ' ký tự nhưng không tách được dòng nào. '
		...
		$tho = count( $rows );
		if ( $skip > 0 && $tho <= $skip ) {
			return VHCPMTD_Util::err( 'Tab này bỏ ' . $skip . ' dòng đầu (dòng tiêu đề của bảng tính) nhưng file chỉ có ' . $tho . ' dòng. '
		...
		elseif ( $header ) {
			if ( $tho === 1 ) {
				return VHCPMTD_Util::err( 'File chỉ có 1 dòng và đang tích "Dòng đầu là tiêu đề" nên không còn dòng dữ liệu nào. '
					. 'Bỏ tích ô đó nếu dòng đầu đã là dữ liệu.' );
			}
			$rows = array_slice( $rows, 1 );
		}
		if ( ! count( $rows ) ) { return VHCPMTD_Util::err( 'Không còn dòng dữ liệu nào sau khi bỏ dòng tiêu đề — kiểm tra lại file.' ); }
		...
		if ( strpos( $type, 'CH_' ) === 0 ) {
			$want = count( VHCPMTD_Cfg::headers( $type ) );
			$clean = array();
			foreach ( $rows as $r ) {
			...
			if ( $replace ) { VHCPMTD_Cfg::write( $type, $clean, false ); }
			else { foreach ( $clean as $row ) { VHCPMTD_Cfg::append( $type, $row ); } }
			...
			$dong_bo = 0;
			if ( $type === VHCPMTD_Cfg::NHOM || $type === VHCPMTD_Cfg::TKNO ) {
				$r       = VHCPMTD_Cfg::dong_bo_tk_loai();
				$dong_bo = (int) $r['updated'];
				$thieu_ma = (int) $r['thieuMa'];
			}
			return VHCPMTD_Util::ok( array( 'inserted' => $n, 'skipped' => $skipped, 'dongBoLoai' => $dong_bo, 'thieuMa' => $thieu_ma ) );
		}
		...
		switch ( $type ) {
			...
			case 'ChiPhi':
				...
				foreach ( $rows as $r ) {
					...
					$data = array(
						...
						'cn_xu_ly'     => VHCPMTD_Util::cn_flag( self::c( $r, 18 ) ) ? 1 : 0,
						'phat_sinh'    => VHCPMTD_Util::is_phat_sinh( self::c( $r, 19 ) ) ? 1 : 0,
					);
					...
					$tk = VHCPMTD_Don::tk_of_line( $data['nhom'], $data['phan_loai_tt'] );
					$data['tk_no'] = $tk['tk_no'];
					$data['tk_co'] = $tk['tk_co'];
					if ( $tk['tk_no'] === '' ) { $thieu_ma++; }
					$wpdb->delete( $t, array( 'id' => $id ) );
			...
			case 'DA_Sheet':
				if ( $ma === '' ) { return VHCPMTD_Util::err( 'Chọn dự án kỹ thuật để nạp các dòng hạng mục vào' ); }
				$da_rec = VHCPMTD_DuAn::find( $ma );
				if ( ! $da_rec ) { return VHCPMTD_Util::err( 'Không tìm thấy dự án: ' . $ma ); }
				$loai_mac_dinh = VHCPMTD_DuAn::loai_cp_mac_dinh( $da_rec['loai'] );
				$t = VHCPMTD_DB::t( 'da_line' );
				if ( $replace ) { $wpdb->delete( $t, array( 'ma_da' => $ma ) ); }
				$row_no = (int) $wpdb->get_var( $wpdb->prepare( "SELECT MAX(row_no) FROM $t WHERE ma_da=%s", $ma ) );
				$row_no = max( VHCPMTD_DB::DATA_ROW - 1, $row_no );
				foreach ( $rows as $r ) {
					$nd = self::c( $r, 0 );
					...
					$tk = ( $loai_cp !== '' ) ? VHCPMTD_Cfg::resolve_tk( $loai_cp, self::c( $r, 11 ), array(), self::c( $r, 8 ) ) : array( 'tk_no' => '', 'tk_co' => '', 'ma_dt' => '' );
					if ( $tk['tk_no'] === '' ) { $thieu_ma++; }
					$wpdb->insert( $t, array(
						'ma_da'      => $ma,
						'row_no'     => $row_no,
			...
			case 'MK_Line':
				...
				if ( $replace ) { $wpdb->query( "DELETE FROM $t" ); }
				$coso_don = array();
				foreach ( VHCPMTD_MK::all_dons() as $d ) { $coso_don[ (string) $d['ma'] ] = (string) $d['coso']; }
				foreach ( $rows as $r ) {
					$k = self::c( $r, 0 );
					if ( $k === '' || self::c( $r, 1 ) === '' ) { $skipped++; continue; }
					$loai_cp = self::c( $r, 12 );
					$md      = self::c( $r, 1 );
					$tk = ( $loai_cp !== '' ) ? VHCPMTD_Cfg::resolve_tk( $loai_cp, self::c( $r, 6 ), array(), isset( $coso_don[ $md ] ) ? $coso_don[ $md ] : '' ) : array( 'tk_no' => '', 'tk_co' => '', 'ma_dt' => '' );
					if ( $tk['tk_no'] === '' ) { $thieu_ma++; }
			...
			case 'BP_Sheet':
				if ( $ma === '' ) { return VHCPMTD_Util::err( 'Chọn đợt Công tác/Setup để nạp các dòng vào' ); }
				if ( ! VHCPMTD_BP::find( $ma ) ) { return VHCPMTD_Util::err( 'Không tìm thấy đợt: ' . $ma ); }
				$t = VHCPMTD_DB::t( 'bp_line' );
				if ( $replace ) { $wpdb->delete( $t, array( 'ma' => $ma ) ); }
				$row_no = (int) $wpdb->get_var( $wpdb->prepare( "SELECT MAX(row_no) FROM $t WHERE ma=%s", $ma ) );
				$row_no = max( VHCPMTD_DB::DATA_ROW - 1, $row_no );
				$dot_rec  = VHCPMTD_BP::find( $ma );
				...
				foreach ( $rows as $r ) {
					...
					$loai_cp = self::c( $r, 11 );
					$tk = ( $loai_cp !== '' ) ? VHCPMTD_Cfg::resolve_tk( $loai_cp, self::c( $r, 6 ), array(), $dia_diem ) : array( 'tk_no' => '', 'tk_co' => '', 'ma_dt' => '' );
					if ( $tk['tk_no'] === '' ) { $thieu_ma++; }
					$wpdb->insert( $t, array(
						'ma'         => $ma,
						'loai_cp'    => $loai_cp,
			...
			default:
				return VHCPMTD_Util::err( 'Chưa hỗ trợ loại tab này' );
		}

		VHCPMTD_Cfg::clear_cache();
		return VHCPMTD_Util::ok( array( 'inserted' => $n, 'skipped' => $skipped, 'thieuMa' => $thieu_ma ) );
	}
