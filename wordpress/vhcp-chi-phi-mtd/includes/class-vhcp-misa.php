...
class VHCPMTD_Misa {
	...
	public static function export_misa( $ky = 'all', $mode = 'chuaxuat', $pl_f = 'all', $mau = self::MAU_CHUAN, $tk_f = 'all' ) {
		...
		foreach ( $cp as $r ) {
			...
			if ( $tk_no === '' && ! empty( $m_no_mx[ $mx_k ] ) && ! VHCPMTD_Cfg::la_tk_ben_tra( $m_no_mx[ $mx_k ] ) ) { $tk_no = $m_no_mx[ $mx_k ]; }
			if ( $tk_no === '' && ! empty( $m_no[ $nhom ] ) && ! VHCPMTD_Cfg::la_tk_ben_tra( $m_no[ $nhom ] ) ) { $tk_no = $m_no[ $nhom ]; }
			/* TK Có = BÊN TRẢ TIỀN. Ba bậc, và bậc đầu là mới (anh Thắng 21/09/2026: *"MTĐ tùy
			   ...
			   Bậc 1 bỏ trống thì hai bậc sau y như cũ — xem chốt dài ở `VHCPMTD_Cfg::tkco_xuat()`.
			...
			$ten1     = $d['nguoiDuyet'];
			$dg1      = VHCPMTD_Util::j( array( $nhom_c, $pll, $d['ky'] ) ) . ( $ten1 !== '' ? '_' . $ten1 : '' );
			$dg2      = VHCPMTD_Util::j( array( $nhom_c, $pll, $ten_misa ) ) . ( trim( $nd ) !== '' ? '_' . $nd : '' );
