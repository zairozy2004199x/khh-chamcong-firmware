...
class VHCPMTD_Don {
	...
	/* ══════════════════════════════════════════════════════════════════════════════════════════
	 * LUỒNG TRỰC TIẾP — GỬI ĐƠN ĐẦY ĐỦ THẲNG CHO KẾ TOÁN, KHÔNG QUA TẠM ỨNG.
	 * ══════════════════════════════════════════════════════════════════════════════════════════
	 * Anh Thắng 22/09/2026: *"Bên anh đó có 2 luồng. 1 luồng trực tiếp, 1 luồng gián tiếp. Trực
	 * tiếp là gửi đơn đầy đủ cho kế toán và quyết toán. Còn gián tiếp là tạm ứng, duyệt tạm
	 * ứng… cái đang chạy"*, rồi chốt bốn bước: Tạo đơn → Chờ quyết toán → Đã quyết toán → Đã
	 * thanh toán → Xuất MISA.
	 *
	 * 🔴 KHÁC `LUONG_CHI` Ở CHỖ NÀO. `LUONG_CHI` (của Máy tự động / Văn phòng) vẫn có khâu
	 *    DUYỆT CHI trước khi tiêu — đơn phải được duyệt rồi mới chi. Luồng này thì tiền ĐÃ CHI
	 *    RỒI: người lập gửi lên một bộ chứng từ đầy đủ, kế toán chỉ còn quyết toán và trả tiền.
	 *    Nên nó bỏ luôn cả ba bước tạm ứng, không chỉ bỏ `Chờ cấp tạm ứng`.
	 *
	 * ⚠️ VẪN KHÔNG ĐẶT CHUỖI TRẠNG THÁI MỚI — dùng lại đúng bộ chuỗi đang lưu trong sổ, chỉ bỏ
	 *    bước và đổi chữ. Lý do đầy đủ ở khối chú thích của `LUONG_CHI` ngay trên: tên trạng
	 *    thái đang được đọc ở hàng trăm chỗ, thêm một bộ song song là mỗi chốt phải học thêm
	 *    mấy tên, và chỗ nào quên thì đơn lọt lưới im lặng.
	 * ══════════════════════════════════════════════════════════════════════════════════════════ */
	const LUONG_TT = array(
		'Nháp'           => 'Tạo đơn',
		'Chờ quyết toán' => 'Chờ quyết toán',
		'Đã quyết toán'  => 'Đã quyết toán',
		'Đã thanh toán'  => 'Đã thanh toán',
	...
	/**
	 * MÃ LUỒNG LƯU TRONG CỘT `don.luong`.
	 *
	 * ⚠️ MÃ NGẮN, KHÔNG LƯU TÊN TIẾNG VIỆT. Tên hiện trên màn còn đổi — anh Thắng đã đổi "Chờ
	 *    duyệt tạm ứng" thành "Chờ duyệt chi" một lần rồi; lưu tên là mỗi lượt đổi chữ thành
	 *    một lượt sửa dữ liệu.
	 * ⚠️ RỖNG KHÔNG PHẢI MỘT MÃ — nó nghĩa là "chưa chọn, cứ theo khối như cũ". Xem `luong_cua()`.
	 */
	...
	/**
	 * Luồng của một đơn: [chuỗi lưu trong sổ => chữ hiện trên màn], đúng thứ tự.
	 *
	 * 🔴 ĐƠN NÓI TRƯỚC, KHỐI NÓI SAU. Từ 22/09/2026 luồng là thuộc tính của TỪNG ĐƠN (cột
	 *    `don.luong`), không phải của cả khối: cùng một cơ sở Hà Nội thì hôm nay chi thẳng nhà
	 *    cung cấp, mai xin tiền trước.
	 *
	 * ⚠️ RỖNG = THEO KHỐI NHƯ CŨ, và đó là cả điểm của lượt đổi này. Hàng trăm đơn đang chạy
	 *    đều rỗng; bắt chúng theo một mã nào đó là đổi luồng giữa chừng cho đơn đã duyệt một
	 *    nửa — đơn Máy tự động đang ở "Đã duyệt chi" sẽ hoá thành một bước không có trong luồng
	 *    mới, và màn không biết vẽ nó ở đâu.
	 * ⚠️ MÃ LẠ CŨNG NGÃ VỀ THEO KHỐI. Thà chạy đúng như hôm qua còn hơn rơi vào một luồng thứ
	 *    ba do gõ sai mà ra.
	 */
	public static function luong_cua( $khoi, $luong = '' ) {
	...
	/** Mã luồng đã chuẩn hoá của một hàng đơn — mã lạ về rỗng ("theo khối"). */
	public static function luong_don( $d ) {
		$d = (array) $d;
	...
	public static function la_truc_tiep( $d ) {
		return 'tt' === self::luong_don( $d );
	}

	/** Chữ hiện trên màn cho một trạng thái, theo khối. Trạng thái lạ trả về NGUYÊN VĂN. */
	public static function ten_tt( $st, $khoi = '', $luong = '' ) {
		$st = trim( (string) $st );
		if ( '' === $st ) { $st = 'Nháp'; }
		$l = self::luong_cua( $khoi, $luong );
	...
	public static function tt_trong_luong( $st, $khoi = '', $luong = '' ) {
		return isset( self::luong_cua( $khoi, $luong )[ trim( (string) $st ) ] );
	}
	 ...
	 */
	public static function tt_truoc_misa( $khoi = '', $luong = '' ) {
		$ds = array_keys( self::luong_cua( $khoi, $luong ) );
	...
	public static function thuc_chi( $thanh_tien, $thuc_mua, $trang_thai_don ) {
		...
		$tt = VHCPMTD_Util::num( $thanh_tien );
		$tm = VHCPMTD_Util::blank_or_num( $thuc_mua );
	...
	private static function ghi_vet( $ma_don, $hanh_dong, $chi_tiet ) {
		if ( ! class_exists( 'VHCPMTD_Log' ) || ! method_exists( 'VHCPMTD_Log', 'log_action' ) ) { return; }
	...
	private static function bao_noi_bo( $ma_don, $chu ) {
		...
		/* ⚠️ TIỀN TỐ BẢNG ĐI VÀO KHOÁ GỘP, và đây KHÔNG phải chuyện làm cho đẹp.
		   ...
		   `VHCPMTD_DB::t('')` trả tiền tố bảng của CHÍNH bản đang chạy, mà mỗi bản một tiền tố
		...
		/* 🔴 DÒNG BẢNG TIN: TRUNG TÍNH, KHÔNG TIỀN KHÔNG TÊN — anh Thắng chốt 08/09/2026.
		   `$chu` bên dưới mang số tiền (`VHCPMTD_Util::tien`), lý do trả lại, và đi vào chuông
	...
	private static function ta_dong( $r ) {
		...
		$tt = VHCPMTD_Util::num( isset( $r['thanh_tien'] ) ? $r['thanh_tien'] : 0 );
		$tm = ( isset( $r['thuc_mua'] ) && '' !== $r['thuc_mua'] && null !== $r['thuc_mua'] )
			? VHCPMTD_Util::num( $r['thuc_mua'] ) : null;
	...
	public static function nhat_ky_don( $ma_don, $limit = 50 ) {
		...
		if ( '' === $ma_don ) { return VHCPMTD_Util::ok( array( 'items' => array() ) ); }
		$limit = (int) $limit;
		...
		foreach ( (array) $rows as $r ) {
			$tg = VHCPMTD_Util::fmt_dt( $r['tg'] );
			/* Thời điểm vô lý thì nói là không biết, đừng hiện ngày bịa — xem chú thích cùng
			   kiểu ở `VHCPMTD_Log::get_log()`. */
			if ( '' !== $tg && VHCPMTD_Util::ngay_vo_ly( substr( $tg, 0, 10 ) ) ) { $tg = ''; }
			$items[] = array(
				'tg'      => $tg,
	...
	public static function tim_don( $q = '', $coso = '', $nhom = '', $limit = 60 ) {
		...
		if ( '' === $q && '' === $coso && '' === $nhom ) {
			return VHCPMTD_Util::ok( array( 'items' => array(), 'chuaNhap' => true ) );
		}

		$td = VHCPMTD_DB::t( 'don' );
		$tc = VHCPMTD_DB::t( 'chiphi' );
		...
		/* Lọc đơn vị NGAY TRONG SQL, trước `LIMIT` — xem `VHCPMTD_DonVi::dieu_kien_sql()`. */
		$dv = VHCPMTD_DonVi::dieu_kien_sql( 'd.don_vi' );
		...
		if ( '' !== $q && $rows ) {
			...
			foreach ( VHCPMTD_DB::rows( $wpdb->prepare(
				...
				/* 🔴 SỐ HIỆN RA LÀ THỰC CHI, KHÔNG PHẢI THÀNH TIỀN.
				   ...
				   ⚠️ Ô THỰC CHI ĐỂ TRỐNG khác hẳn ô ghi số 0: trống = chưa ai chốt lại, nên lui
				      ...
				      thành 0. */
				$tm = VHCPMTD_Util::blank_or_num( $l['thuc_mua'] );
				$dong_khop[ $m2 ][] = array(
					'noiDung' => trim( (string) $l['noi_dung'] ),
					'tien'    => ( null === $tm ) ? VHCPMTD_Util::num( $l['thanh_tien'] ) : $tm,
				);
			}
		}
		...
		foreach ( (array) $rows as $r ) {
			...
			$items[] = array(
				...
				'ky'        => (string) $r['ky'],
				'coso'      => implode( ', ', $cs ),
				'donVi'     => VHCPMTD_DonVi::chuan( isset( $r['don_vi'] ) ? $r['don_vi'] : '' ),
				'nguoiLap'  => (string) $r['nguoi_lap'],
				'trangThai' => (string) $r['trang_thai'],
				'loai'      => $loai,
				'soDong'    => (int) $r['so_dong'],
				'tongTien'  => VHCPMTD_Util::num( $r['tong_tien'] ),
			);
		}
		return VHCPMTD_Util::ok( array( 'items' => $items, 'chuaNhap' => false ) );
	}
	...
	public static function ds_loai_chi_phi() {
		...
		return VHCPMTD_Util::ok( array( 'items' => array_values( array_filter( (array) $rows ) ) ) );
	}
	...
	public static function cp_rows() {
		...
		return VHCPMTD_DB::rows( "SELECT * FROM $t ORDER BY stt ASC" );
	}
	...
	public static function tu_rows() {
		global $wpdb;
		$t = VHCPMTD_DB::t( 'tamung' );
		return VHCPMTD_DB::rows( "SELECT * FROM $t ORDER BY id ASC" );
	}

	public static function don_row( $ma_don ) {
		global $wpdb;
		$t = VHCPMTD_DB::t( 'don' );
		return VHCPMTD_DB::row( $wpdb->prepare( "SELECT * FROM $t WHERE ma_don=%s", (string) $ma_don ) );
	}

	public static function line_row( $id ) {
		global $wpdb;
		$t = VHCPMTD_DB::t( 'chiphi' );
		return VHCPMTD_DB::row( $wpdb->prepare( "SELECT * FROM $t WHERE id=%s", (string) $id ) );
	}

	private static function upd_don( $ma_don, $data ) {
		global $wpdb;
		return $wpdb->update( VHCPMTD_DB::t( 'don' ), $data, array( 'ma_don' => (string) $ma_don ) );
	}
	...
	private static function dem_loai_chua_bo_phan( $cp ) {
		...
		foreach ( (array) $cp as $r ) {
			...
			$da[ $k ] = ! VHCPMTD_Cfg::bo_phan_ds_cua_loai( $ten );
		}
		$n = 0;
	...
	public static function get_bootstrap() {
		...
		/* 🔴 Ô CHỌN CƠ SỞ CHỈ BÀY GIAN CỦA BÊN MÌNH (anh Thắng 08/09/2026: *"2 bộ phận không
		   nhìn thấy nhau"*). Không lọc lại ở đây: `VHCPMTD_Cfg::get_config()` đã cắt theo đơn vị
		...
		$ten_tk = array();
		if ( count( $can ) ) {
			foreach ( VHCPMTD_Cfg::tai_khoan() as $x ) {
		...
		return array(
			...
			'vending'    => VHCPMTD_Vending::cau_hinh(),
			...
			/* 🔴 Khối có lọc loại chi phí không — xem chốt dài ở `VHCPMTD_Cfg::LOC_LOAI_THEO_KHOI`.
			...
			/* 🔴 TÊN NHÀ MẸ PHẢI XUỐNG TỚI GIAO DIỆN. Luật "nhà mẹ đọc cả hệ" trước nay CHỈ có
			   ở máy chủ (`VHCPMTD_DonVi::la_don_vi_me()`), nên hộp chọn cơ sở bên kia đành so
			...
			/* 🔴 DANH SÁCH KHỐI — MỘT NGUỒN DUY NHẤT. Màn từng gõ cứng `[kvc, mtd, vp]`, nên
			   ...
			   nó biến mất. Xem chốt dài ở `VHCPMTD_DonVi::khoi_ds()`. */
			'khoiDs'     => VHCPMTD_DonVi::khoi_ds(),
			...
			'vaiTuyBien' => VHCPMTD_Cfg::vai_tuy_bien(),
			...
			'donViCuaToi'=> VHCPMTD_DonVi::cua_toi(),
			'xemDonVi'   => VHCPMTD_DonVi::xem_duoc(),
			'nhieuDonVi' => VHCPMTD_DonVi::nhieu_don_vi(),
			'cosoPll'    => $coso_ml,
			/* ═══════════════════════════════════════════════════════════════════════════
			   ...
			   (`VHCPMTD_DonVi::cua_coso()`) mà chưa bao giờ được gửi xuống.
	...
	public static function product_suggestions( $cp = null ) {
		...
		foreach ( $cp as $r ) {
			...
			if ( ! isset( $map[ $k ] ) || $t >= $map[ $k ]['_t'] ) {
				$map[ $k ] = array( 'ten' => $ten, 'gia' => VHCPMTD_Util::num( $r['don_gia'] ), 'dvt' => (string) $r['dvt'], 'nhom' => (string) $r['nhom'], '_t' => $t );
			}
		}
		$out = array();
	...
	public static function list_dons( $cp = null ) {
		...
		foreach ( $tu as $r ) {
			$m = (string) $r['ma_don'];
			if ( $m === '' ) { continue; }
			$tu_sum[ $m ] = ( isset( $tu_sum[ $m ] ) ? $tu_sum[ $m ] : 0 ) + VHCPMTD_Util::num( $r['so'] );
			$tu_has[ $m ] = true;
		...
		/* 🔴 ĐƠN NÀO CHẠM TỚI BỘ PHẬN CỦA MÌNH — dựng một lượt, không hỏi lại từng đơn.
		   ...
		   ⚠️ Đơn KHÔNG có dòng chi nào (đơn xin ứng trước — luật 01/09/2026 cho gửi như thế)
		      ...
		      đơn đang treo tiền. */
		$bo_phan_bo = VHCPMTD_Auth::bo_phan_bo();
		...
		foreach ( $cp as $r ) {
			...
			$_dm = VHCPMTD_Cfg::dau_muc_cua_loai( isset( $r['nhom'] ) ? $r['nhom'] : '', isset( $khoi_don[ $m ] ) ? $khoi_don[ $m ] : '' );
			if ( ! isset( $dm_by[ $m ] ) ) { $dm_by[ $m ] = array(); }
			$dm_by[ $m ][ $_dm ] = 1;
			if ( '' !== $bo_phan_bo ) {
				if ( ! isset( $don_cham[ $m ] ) ) { $don_cham[ $m ] = false; }
				if ( VHCPMTD_Auth::xem_duoc_loai( isset( $r['nhom'] ) ? $r['nhom'] : '' ) ) { $don_cham[ $m ] = true; }
				...
				if ( VHCPMTD_Cfg::bo_phan_ds_cua_loai( $_nhom ) && VHCPMTD_Cfg::loai_thuoc_bo_phan( $_nhom, $bo_phan_bo ) ) {
					$don_ro[ $m ] = true;
				}
			}
			...
			if ( ! VHCPMTD_Util::is_phat_sinh( $r['phat_sinh'] ) ) { $xin[ $m ] = ( isset( $xin[ $m ] ) ? $xin[ $m ] : 0 ) + $tt; }
			if ( VHCPMTD_Util::is_ncc( $r['phan_loai_tt'], $r['cn_xu_ly'] ) ) { $tt_ncc[ $m ] = ( isset( $tt_ncc[ $m ] ) ? $tt_ncc[ $m ] : 0 ) + $eff; }
		...
		foreach ( $dons as $r ) {
			...
			if ( '' !== $bo_phan_bo && isset( $don_cham[ $m ] ) && ! $don_cham[ $m ] ) {
				...
				if ( ! VHCPMTD_Auth::nv_co_coso()
					|| ! VHCPMTD_Auth::trong_tam( (string) $r['nguoi_lap'], $_cs_m ) ) {
			...
			$du_phong = VHCPMTD_Util::num( $r['du_phong'] );
			$bu_tru   = VHCPMTD_Util::num( $r['bu_tru'] );
			/* NULL = chưa ai chốt số; 0 = đã chốt và chốt là không đồng nào. Xem khối 🔴 ở
			   `get_don()` — `num()` nghiền hai thứ ấy thành một. */
			$tu_d     = VHCPMTD_Util::blank_or_num( $r['tam_ung_duyet'] );
			...
			$out[] = array(
				...
				/* Mảng của đơn — thanh KVC · MTĐ · VP lọc theo ô này. Đơn cũ (trước khi có cột)
				   đã được `VHCPMTD_DB::lap_mang()` lấp sẵn, nên ở đây không cần đoán gì. */
				'khoi'        => trim( (string) ( isset( $r['khoi'] ) ? $r['khoi'] : '' ) ),
				/* Luồng của đơn — màn vẽ thanh bước và bày nút gửi theo ô này. Rỗng = theo khối
				   như cũ; xem `luong_cua()`. */
				...
				'ngayDuyet'   => VHCPMTD_Util::fmt( $r['ngay_duyet'] ),
				'nguoiQT'     => (string) $r['nguoi_qt'],
				'ngayQT'      => VHCPMTD_Util::fmt( $r['ngay_qt'] ),
				'chenhLechQT' => VHCPMTD_Util::num( $r['chenh_lech_qt'] ),
				'xuLy'        => (string) $r['xu_ly'],
				'soThucMua'   => $mua_cn,
				...
				'ngayQTNCC'   => VHCPMTD_Util::fmt( $r['ngay_qt_ncc'] ),
				'qtCN'        => ( (string) $r['nguoi_qt'] !== '' ),
				'qtNCC'       => ( (string) $r['nguoi_qt_ncc'] !== '' ),
				'xuatCN'      => VHCPMTD_Util::fmt( $r['ngay_xuat_cn'] ),
				'xuatNCC'     => VHCPMTD_Util::fmt( $r['ngay_xuat_ncc'] ),
				'nguoiCap'    => (string) $r['nguoi_cap'],
				'ngayCap'     => VHCPMTD_Util::fmt( $r['ngay_cap'] ),
				...
				'ngayTatToan' => VHCPMTD_Util::fmt( $r['ngay_tat_toan'] ),
				'tamUng'      => $tam_ung,
				'thucChi'     => $tc,
				'thucChiCN'   => $mua_cn,
				'thucChiNCC'  => $tc_ncc,
		 ...
		 *
		 * ⚠️ LUẬT ĐẦY ĐỦ nằm trong `VHCPMTD_Auth::trong_tam()` — kể cả chỗ hai vai hiểu ô Cơ
		 *    sở RỖNG theo hai nghĩa ngược nhau. Để nó ở Auth vì `VHCPMTD_Duan` và `VHCPMTD_Bp` cũng
		...
		$out = array_values( array_filter( $out, function ( $x ) {
			return VHCPMTD_Auth::trong_tam( (string) $x['nguoiLap'], (string) $x['coso'] );
		} ) );
		...
		$dv_xem = VHCPMTD_DonVi::xem_duoc();
		if ( null !== $dv_xem ) {
			$truoc2 = count( $out );
			$out = array_values( array_filter( $out, function ( $x ) {
				return VHCPMTD_DonVi::duoc_xem( isset( $x['donVi'] ) ? $x['donVi'] : '' );
			} ) );
	 ...
	 *                   cùng cơ sở khai với mình (`VHCPMTD_Auth::trong_tam`)
	...
	private static function loi_khong_phai_don_minh( $ma_don ) {
		...
		$loi_dv = VHCPMTD_DonVi::vi_sao_khong_dung( $ma_don );
		...
		if ( ! VHCPMTD_Auth::la_nhan_vien() && 'Quản lý' !== VHCPMTD_Auth::vai_tro() ) { return ''; }
		$d = self::don_row( $ma_don );
		if ( ! $d ) { return 'Không tìm thấy đơn ' . $ma_don . ' trong sổ.'; }
		$cua = mb_strtolower( trim( (string) $d['nguoi_lap'] ) );
		$toi = mb_strtolower( trim( VHCPMTD_Auth::nguoi() ) );
		...
		if ( VHCPMTD_Auth::trong_tam( (string) $d['nguoi_lap'], implode( ', ', self::cac_coso_cua_don( $ma_don ) ) ) ) { return ''; }
	 ...
	 * theo NGƯỜI LẬP.
	 *
	 * 🔴 TRƯỚC BẢN NÀY ĐƠN NÀO CŨNG MANG DẤU CỦA BẢN CÀI (`VHCPMTD_DB::khoi()`): một bản cài = một
	 ...
	 * ⚠️ VẪN KHÔNG PHẢI TRỤC PHÂN QUYỀN — giữ nguyên chốt ở `VHCPMTD_DonVi::khoi_xem_duoc()`: khối
	...
	public static function khoi_cho_don( $nguoi_lap, $khoi_man = '' ) {
		$tay = VHCPMTD_DonVi::khoi_khai_tay( $nguoi_lap );
		if ( 1 === count( $tay ) ) { return $tay[0]; }
		$m = mb_strtolower( trim( (string) $khoi_man ) );
		if ( count( $tay ) > 1 && '' !== $m && in_array( $m, $tay, true ) ) { return $m; }
		return VHCPMTD_DB::khoi();
	}

	public static function tao_don_moi( $ky, $nguoi_lap, $luong = '', $khoi_man = '' ) {
		$dv = VHCPMTD_DonVi::cua_nguoi( $nguoi_lap );
		if ( ! VHCPMTD_DonVi::duoc_xem( $dv ) ) {
			$ds = VHCPMTD_DonVi::xem_duoc();
			return VHCPMTD_Util::err(
	...
	public static function create_don( $ky, $nguoi_lap, $luong = '', $khoi_man = '' ) {
		...
		/* 🔴 LUỒNG ĐÓNG DẤU NGAY LÚC LẬP, và chuẩn hoá tại đây. Mã lạ về rỗng = "theo khối như
		   cũ" — thà chạy đúng như hôm qua còn hơn rơi vào một luồng thứ ba do gõ sai mà ra.
		   ⚠️ KHÁC `khoi` và `don_vi` ngay dưới ở một điểm: hai cột kia KHÔNG có ô cho người dùng
		      chọn, còn cột này thì CÓ — vì anh Thắng chốt 22/09/2026 người lập chọn trên từng
		...
		$ok = $wpdb->insert( VHCPMTD_DB::t( 'don' ), array(
			...
			/* 🔴 MẢNG ĐÓNG DẤU NGAY LÚC LẬP, và không đổi nữa. Anh Thắng 19/09/2026 chốt gộp
			   ...
			   `NOT NULL DEFAULT` ở `VHCPMTD_DB`: rỗng nghĩa là mất tích, không phải "chưa khai".
			   ...
			   🔴 Từ 23/09/2026 dấu lấy theo NGƯỜI LẬP trước (Bắc–Nam dùng chung một bản), lui về
			      `VHCPMTD_DB::khoi()` khi người ấy chưa tích khối — xem `khoi_cho_don()`. */
		...
		if ( ! $ok ) {
			...
			return VHCPMTD_Util::err( 'Không ghi được đơn mới vào sổ'
				. ( '' !== $chi_tiet ? ' — ' . $chi_tiet : '.' ) );
		}
		return VHCPMTD_Util::ok( array( 'maDon' => $m ) );
	}
	...
	public static function get_don( $ma_don, $with_products = true ) {
		...
		if ( ! $r ) { return VHCPMTD_Util::err( 'Không tìm thấy đơn' ); }
		...
		$don = array(
			...
			'chenhLechQT' => VHCPMTD_Util::num( $r['chenh_lech_qt'] ),
			'xuLy'        => (string) $r['xu_ly'],
			'soThucMua'   => VHCPMTD_Util::out_num( $r['so_tien_thuc_mua'] ),
			'httt'        => (string) $r['hinh_thuc_tt'],
			'anhHoaDon'   => (string) $r['hoa_don_qt'],
				/* Gác isset: cột này thêm ở bản 1.31.0. Site nâng cấp plugin xong mà bảng chưa kịp
				   nới (dbDelta chạy ở lượt tải trang sau) thì đọc thẳng là cảnh báo tràn nhật ký
			...
			'nguoiQTNCC'  => (string) $r['nguoi_qt_ncc'],
			'ngayQTNCC'   => VHCPMTD_Util::fmt( $r['ngay_qt_ncc'] ),
			'tamUngDuyet' => VHCPMTD_Util::out_num( $r['tam_ung_duyet'] ),
			'nguoiCap'    => (string) $r['nguoi_cap'],
			'ngayCap'     => VHCPMTD_Util::fmt( $r['ngay_cap'] ),
			'htCap'       => (string) $r['ht_cap'],
			'anhCap'      => (string) $r['anh_cap'],
			'duPhong'     => VHCPMTD_Util::num( $r['du_phong'] ),
			'buTru'       => VHCPMTD_Util::num( $r['bu_tru'] ),
		);
		...
		$don['nhieuCoSo'] = VHCPMTD_DonVi::don_nhieu_coso( $ma_don );

		$bt_auto = self::chot_bu_tru( $ma_don );
		if ( (string) $don['trangThai'] === 'Nháp' ) { $don['buTru'] = VHCPMTD_Util::num( $bt_auto['so'] ); }
		$don['buTruAuto'] = $bt_auto;

		global $wpdb;
		$tt = VHCPMTD_DB::t( 'tamung' );
		$tam_ung = array(); $has_tu_rows = false;
		foreach ( VHCPMTD_DB::rows( $wpdb->prepare( "SELECT * FROM $tt WHERE ma_don=%s ORDER BY id ASC", (string) $ma_don ) ) as $x ) {
			$tam_ung[ (string) $x['coso'] ] = VHCPMTD_Util::num( $x['so'] );
			$has_tu_rows = true;
		}

		$tcp = VHCPMTD_DB::t( 'chiphi' );
		if ( $with_products ) {
			...
			$cp_all = null;
			$cp     = VHCPMTD_DB::rows( $wpdb->prepare( "SELECT * FROM $tcp WHERE ma_don=%s ORDER BY stt ASC", (string) $ma_don ) );
		}
		$lines = array();
		foreach ( $cp as $x ) {
			$dong = array(
				...
				'dvt'        => (string) $x['dvt'],
				'soLuong'    => VHCPMTD_Util::num( $x['so_luong'] ),
				'donGia'     => VHCPMTD_Util::num( $x['don_gia'] ),
				'thanhTien'  => VHCPMTD_Util::num( $x['thanh_tien'] ),
				'ghiChu'     => (string) $x['ghi_chu'],
				'anh'        => (string) $x['anh'],
				'thueSuat'   => VHCPMTD_Util::out_num( $x['thue_suat'] ),
				'tienThue'   => VHCPMTD_Util::num( $x['tien_thue'] ),
				'thucMua'    => VHCPMTD_Util::out_num( $x['thuc_mua'] ),
				'cnXuLy'     => VHCPMTD_Util::cn_flag( $x['cn_xu_ly'] ),
				'phatSinh'   => VHCPMTD_Util::is_phat_sinh( $x['phat_sinh'] ),
				'tkNo'       => (string) $x['tk_no'],
				'tkCo'       => (string) $x['tk_co'],
				...
				'taoLuc'     => VHCPMTD_Util::fmt( $x['tao_luc'] ),
			);
			/* TRỤC PHÂN TÍCH — đọc qua khung, và ĐỌC chứ không đưa nguyên chuỗi trong sổ xuống:
			   ...
			   thì bên kia lặng lẽ nói khác. Xem `VHCPMTD_Truc::doc()`. */
			$lines[] = array_merge( $dong, VHCPMTD_Truc::doc_dong( $x ) );
		}
		...
		$tu_tay_sum = 0;
		foreach ( $tam_ung as $v ) { $tu_tay_sum += VHCPMTD_Util::num( $v ); }
		...
		$tu_tay_sum += VHCPMTD_Util::num( $don['duPhong'] );
		...
		$tu_duyet = VHCPMTD_Util::blank_or_num( $don['tamUngDuyet'] );
	...
	public static function set_tam_ung( $ma_don, $coso, $so ) {
		...
		global $wpdb;
		if ( ! $coso ) { return VHCPMTD_Util::err( 'Thiếu cơ sở' ); }
		$d = self::don_row( $ma_don );
		if ( $d && (string) $d['trang_thai'] !== 'Nháp' ) { return VHCPMTD_Util::err( 'Tạm ứng đã khóa (chỉ sửa khi đơn ở "Nháp")' ); }
		...
		if ( $d && self::la_truc_tiep( $d ) && VHCPMTD_Util::num( $so ) > 0 ) {
			return VHCPMTD_Util::err( 'Đơn này đi luồng TRỰC TIẾP — tiền đã chi, không có bước tạm ứng nên không nhập số xin. Cần tạm ứng thì nhờ kế toán bấm "🔁 Đổi luồng" sang Qua tạm ứng.' );
		}
		...
		if ( $cs_don !== '' && strcasecmp( $cs_don, (string) $coso ) !== 0 ) {
			return VHCPMTD_Util::err( 'Đơn này của cơ sở "' . $cs_don . '" — muốn tạm ứng cơ sở khác thì tạo đơn mới.' );
		}
		 ...
		 * được (`VHCPMTD_DonVi::don_nhieu_coso()`). Nhưng lưu một dòng tạm ứng vào là đơn LẬP TỨC
		 ...
		 *    họ quyết.
		 * ══════════════════════════════════════════════════════════════════════════════════════ */
		if ( VHCPMTD_DonVi::MO_KHI_KHONG_TAM_UNG ) {
			$khac = array();
	...
	public static function set_du_phong( $ma_don, $so ) {
		...
		$d = self::don_row( $ma_don );
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy đơn' ); }
		if ( (string) $d['trang_thai'] !== 'Nháp' ) { return VHCPMTD_Util::err( 'Chỉ sửa dự phòng khi đơn ở "Nháp"' ); }
		self::upd_don( $ma_don, array( 'du_phong' => VHCPMTD_Util::num( $so ) ) );
		return VHCPMTD_Util::ok();
	}
	...
	public static function bu_tru_tu_dong( $ma_don, $ds = null ) {
		...
		foreach ( $ds as $x ) {
			...
			foreach ( self::tach_coso( (string) $x['coso'] ) as $cs ) {
				$nd = VHCPMTD_Cfg::coso_dong_cua( $cs );
			...
			$tong += VHCPMTD_Util::num( $x['chenhLech'] );
			$so_don++;
		}
		...
		if ( $so > 0 )     { $ly = 'tuần trước (' . $ky_truoc . ') của ' . $ql . ' — ' . $goi . ' — còn THIẾU ' . VHCPMTD_Util::tien( $so ) . $duoi; }
		elseif ( $so < 0 ) { $ly = 'tuần trước (' . $ky_truoc . ') của ' . $ql . ' — ' . $goi . ' — còn DƯ ' . VHCPMTD_Util::tien( -$so ) . $duoi; }
	...
	private static function chot_bu_tru( $ma_don, $ds = null ) {
		...
		if ( VHCPMTD_Util::num( $d['bu_tru'] ) !== VHCPMTD_Util::num( $bt['so'] ) ) {
			self::upd_don( $ma_don, array( 'bu_tru' => VHCPMTD_Util::num( $bt['so'] ) ) );
		}
		return $bt;
	}
	...
	public static function set_tu_extra( $ma_don, $du_phong, $bu_tru ) {
		...
		$d = self::don_row( $ma_don );
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy đơn' ); }
		if ( (string) $d['trang_thai'] !== 'Nháp' ) { return VHCPMTD_Util::err( 'Chỉ sửa khi đơn ở "Nháp"' ); }
		...
		self::upd_don( $ma_don, array( 'du_phong' => VHCPMTD_Util::num( $du_phong ) ) );
		self::chot_bu_tru( $ma_don );
		return VHCPMTD_Util::ok( array( 'buTru' => VHCPMTD_Util::num( self::don_row( $ma_don )['bu_tru'] ) ) );
	}
	...
	private static function line_data( $id, $ma_don, $rec ) {
		...
		$sl = VHCPMTD_Util::num( $get( 'soLuong' ) );
		$dg = VHCPMTD_Util::num( $get( 'donGia' ) );
		$tt = $sl * $dg;
		$tt_in = $get( 'thanhTien' );
		if ( $tt_in !== null && $tt_in !== '' && is_numeric( str_replace( array( ',', ' ' ), '', (string) $tt_in ) ) ) {
			$tt = VHCPMTD_Util::num( $tt_in );
		}
		$ts    = VHCPMTD_Util::blank_or_num( $get( 'thueSuat' ) );
		$tthue = ( $ts === null ) ? null : round( $tt * $ts / 100 );
		$tm    = VHCPMTD_Util::blank_or_num( $get( 'thucMua' ) );
		$cnv   = $get( 'cnXuLy' );
		$cn    = ( $cnv === 0 || $cnv === false || $cnv === '0' ) ? 0 : 1;
		$ps    = VHCPMTD_Util::is_phat_sinh( $get( 'phatSinh' ) ) ? 1 : 0;
		$ngay  = VHCPMTD_Util::parse_date( $get( 'ngay' ) );
		if ( ! $ngay ) { $ngay = VHCPMTD_Util::today_sql(); }   // không nhập ngày -> lấy ngày nhập
		...
		$row = array(
			...
			'coso'         => VHCPMTD_Util::st( $get( 'coso' ) ),
			'ngay'         => $ngay,
			'phan_loai_tt' => VHCPMTD_Util::st( $get( 'phanLoaiTT' ) ),
			'doi_tuong'    => VHCPMTD_Util::st( $get( 'doiTuong' ) ),
			'nhom'         => VHCPMTD_Util::st( $get( 'nhom' ) ),
			'noi_dung'     => VHCPMTD_Util::st( $get( 'noiDung' ) ),
			'dvt'          => VHCPMTD_Util::st( $get( 'dvt' ) ),
			'so_luong'     => VHCPMTD_Util::blank_or_num( $get( 'soLuong' ) ),
			'don_gia'      => VHCPMTD_Util::blank_or_num( $get( 'donGia' ) ),
			'thanh_tien'   => $tt,
			'ghi_chu'      => VHCPMTD_Util::st( $get( 'ghiChu' ) ),
			'anh'          => VHCPMTD_Util::st( $get( 'anh' ) ),
			'tao_luc'      => VHCPMTD_Util::now_sql(),
			'thue_suat'    => $ts,
			'tien_thue'    => $tthue,
			'thuc_mua'     => $tm,
			'cn_xu_ly'     => $cn,
		...
		/* ══════════════════════════════════════════════════════════════════════════════════
		 * TRỤC PHÂN TÍCH — mỗi trục một cột, khai một chỗ (`VHCPMTD_Truc::ds()`).
		 * ══════════════════════════════════════════════════════════════════════════════════
		...
		foreach ( VHCPMTD_Truc::ghi_dong( $get ) as $cot => $gt ) {
			$row[ $cot ] = $gt;
		}
		return $row;
	}
	...
	public static function giai_doan_doc( $v ) {
		/* Từ 22/09/2026 luật nằm ở khung TRỤC PHÂN TÍCH (`VHCPMTD_Truc`) — hai hàm này ở lại làm
		   lối gọi quen cho mã cũ, KHÔNG giữ bản luật thứ hai. */
		return VHCPMTD_Truc::doc( 'giaiDoan', $v );
	}

	public static function giai_doan_chuan( $v ) {
		return VHCPMTD_Truc::chuan( 'giaiDoan', $v );
	}
	...
	public static function tk_of_line( $nhom, $phan_loai_tt, $coso = '' ) {
		...
		$tk_no = VHCPMTD_Cfg::tkno_loai( $nhom, $coso );
		/* Mã danh mục mà là 141/331 thì không đem gắn lên dòng — xem `VHCPMTD_Cfg::bo_ma_ben_tra_`. */
		if ( $tk_no === '' && ! VHCPMTD_Cfg::la_tk_ben_tra( $cat['tkNo'] ) ) { $tk_no = $cat['tkNo']; }
		$tk_co = $cat['tkCo'];
		if ( $tk_co === '' ) {
	...
	public static function gan_ma_tai_khoan( $all = false ) {
		...
		return VHCPMTD_Util::ok( array( 'updated' => $n, 'thieuMa' => array_keys( $thieu ), 'boQuaDaXuat' => $bo_qua ) );
	}
	...
	public static function coso_cua_don( $ma_don ) {
		...
		if ( VHCPMTD_DonVi::don_nhieu_coso( $ma_don ) ) { return ''; }
	...
	private static function loi_thieu_so_luong( $rec ) {
		...
		$sl  = VHCPMTD_Util::blank_or_num( $g( 'soLuong' ) );
		if ( $sl !== null && $sl > 0 ) { return ''; }
		$dg  = VHCPMTD_Util::blank_or_num( $g( 'donGia' ) );
		$tt  = VHCPMTD_Util::blank_or_num( $g( 'thanhTien' ) );
	...
	public static function add_line( $ma_don, $rec ) {
		...
		if ( $loi !== '' ) { return VHCPMTD_Util::err( $loi ); }
		// Gian đã đóng thì không nhận chi mới — đóng gian là đã chốt sổ với cơ sở đó.
		$cs_moi = trim( (string) ( isset( $rec['coso'] ) ? $rec['coso'] : '' ) );
		if ( $cs_moi !== '' ) {
			$nd = VHCPMTD_Cfg::coso_dong_cua( $cs_moi );
			if ( $nd !== '' ) {
				return VHCPMTD_Util::err( 'Cơ sở "' . $cs_moi . '" đã đóng cửa ngày ' . $nd . ' — không nhập chi phí mới cho gian đã đóng.' );
			}
		}
		$loi_sl = self::loi_thieu_so_luong( $rec );
		if ( $loi_sl !== '' ) { return VHCPMTD_Util::err( $loi_sl ); }
		$rec['phatSinh'] = $ps;
		$id   = VHCPMTD_Util::uid( 'L' );
		 ...
		 * ══════════════════════════════════════════════════════════════════════════════════ */
		$ok = $wpdb->insert( VHCPMTD_DB::t( 'chiphi' ), $data );
		if ( ! $ok ) {
			$chi_tiet = trim( (string) $wpdb->last_error );
			return VHCPMTD_Util::err( 'Không ghi được dòng chi vào sổ'
				. ( '' !== $chi_tiet ? ' — ' . $chi_tiet : '.' ) );
		}
		self::ghi_vet( $ma_don, ( $ps ? 'Thêm dòng phát sinh' : 'Thêm hạng mục xin' ),
			self::ta_dong( $data ) . ' · lúc đơn ở "' . $st . '"' );
		return VHCPMTD_Util::ok( array( 'id' => $id, 'phatSinh' => $ps ) );
	}
	...
	public static function update_line( $id, $rec ) {
		...
		if ( ! $cur ) { return VHCPMTD_Util::err( 'Không tìm thấy dòng' ); }
		$ma_don = (string) $cur['ma_don'];
		$ps     = VHCPMTD_Util::is_phat_sinh( $cur['phat_sinh'] );
		...
		if ( isset( $rec['coso'] ) && trim( (string) $rec['coso'] ) !== '' ) {
			...
			if ( $khac > 0 ) {
				...
				if ( $loi2 !== '' ) { return VHCPMTD_Util::err( $loi2 ); }
			}
		}
		if ( ! array_key_exists( 'thucMua', $rec ) ) { $rec['thucMua'] = VHCPMTD_Util::out_num( $cur['thuc_mua'] ); }
		...
		$data = self::line_data( $id, $ma_don, $rec );
		$data['tao_luc'] = $cur['tao_luc'] ? $cur['tao_luc'] : VHCPMTD_Util::now_sql();
		...
		if ( $data['tk_no'] === '' && trim( (string) $cur['tk_no'] ) !== ''
			&& ! VHCPMTD_Cfg::la_tk_ben_tra( $cur['tk_no'] ) ) { $data['tk_no'] = $cur['tk_no']; }
		if ( $data['tk_co'] === '' && trim( (string) $cur['tk_co'] ) !== '' ) { $data['tk_co'] = $cur['tk_co']; }
		unset( $data['id'] );
		$wpdb->update( VHCPMTD_DB::t( 'chiphi' ), $data, array( 'id' => (string) $id ) );
	...
	public static function set_line_thuc_mua( $id, $val, $actor = '' ) {
		...
		if ( $st !== 'Đã cấp tạm ứng' && $st !== 'Chờ quyết toán' ) {
			return VHCPMTD_Util::err( 'Chỉ nhập Thực chi khi "Đã cấp tạm ứng" (hoặc kế toán sửa khi "Chờ quyết toán")' );
		}
		...
		if ( 'Chờ quyết toán' === $st && VHCPMTD_Auth::la_nhan_vien() ) {
			return VHCPMTD_Util::err( 'Đơn đã gửi kế toán — nhân viên thôi sửa được Thực chi. '
				. 'Cần sửa thì nhờ kế toán trả đơn lại.' );
		}
		$v = VHCPMTD_Util::blank_or_num( $val );
		$cu_tm = ( '' !== $cur['thuc_mua'] && null !== $cur['thuc_mua'] )
			? VHCPMTD_Util::num( $cur['thuc_mua'] ) : null;
		$wpdb->update( VHCPMTD_DB::t( 'chiphi' ), array( 'thuc_mua' => $v ), array( 'id' => (string) $id ) );
		if ( $cu_tm !== ( null === $v ? null : VHCPMTD_Util::num( $v ) ) ) {
			...
			self::ghi_vet( $ma_don, 'Nhập thực chi',
				trim( (string) $cur['noi_dung'] ) . ': ' . $_ts( $cu_tm ) . '  →  '
					. $_ts( null === $v ? null : VHCPMTD_Util::num( $v ) ) );
		}
		$kt_sua = false;
		if ( $st === 'Chờ quyết toán' ) { self::mark_kt_sua( $ma_don, $actor ); $kt_sua = true; }
		return VHCPMTD_Util::ok( array( 'ktSua' => $kt_sua ) );
	}

	public static function set_line_cn( $id, $on ) {
		global $wpdb;
		if ( ! self::line_row( $id ) ) { return VHCPMTD_Util::err( 'Không tìm thấy dòng' ); }
		$wpdb->update( VHCPMTD_DB::t( 'chiphi' ), array( 'cn_xu_ly' => $on ? 1 : 0 ), array( 'id' => (string) $id ) );
		return VHCPMTD_Util::ok();
	}
	...
	public static function set_line_ngay( $id, $ngay ) {
		...
		$la_admin = ( VHCPMTD_Auth::vai_tro() === 'Admin' );
		if ( ! $la_admin ) {
			$_c = self::vi_sao_khong_sua( (string) $cur['ma_don'] );
			if ( $_c !== '' ) { return VHCPMTD_Util::err( $_c . ' (nhờ Admin sửa)' ); }
		}
		$moi = VHCPMTD_Util::parse_date( $ngay );
		if ( ! $moi ) { return VHCPMTD_Util::err( 'Ngày không đọc được: ' . $ngay ); }
		if ( VHCPMTD_Util::ngay_vo_ly( VHCPMTD_Util::fmt( $moi ) ) ) {
			return VHCPMTD_Util::err( 'Ngày vô lý: ' . VHCPMTD_Util::fmt( $moi ) );
		}
		$wpdb->update( VHCPMTD_DB::t( 'chiphi' ), array( 'ngay' => $moi ), array( 'id' => (string) $id ) );
		return VHCPMTD_Util::ok( array( 'ngay' => VHCPMTD_Util::fmt( $moi ) ) );
	}
	 ...
	 * ═════════════════════════════════════════════════════════════════════════════════════════
	 * Loại chi phí ở đơn tuần chính là NHÓM MẶT HÀNG: nó suy ra TK Nợ (`VHCPMTD_Cfg::tkno_loai`).
	...
	public static function set_line_nhom( $id, $nhom ) {
		...
		$nhom = trim( (string) $nhom );
		if ( '' === $nhom ) { return VHCPMTD_Util::err( 'Chọn loại chi phí' ); }

		$tk = self::tk_of_line( $nhom, (string) $cur['phan_loai_tt'], (string) $cur['coso'] );
		$wpdb->update( VHCPMTD_DB::t( 'chiphi' ), array(
			'nhom'  => $nhom,
			'tk_no' => $tk['tk_no'],
			'tk_co' => $tk['tk_co'],
		), array( 'id' => (string) $id ) );
		...
		return VHCPMTD_Util::ok( array( 'nhom' => $nhom, 'tkNo' => $tk['tk_no'], 'tkCo' => $tk['tk_co'] ) );
	}
	...
	public static function set_line_tk_no( $id, $tk, $them = 0 ) {
		...
		$vai = VHCPMTD_Auth::vai_tro();
		if ( ! in_array( $vai, array( 'Admin', 'Kế toán cá nhân', 'Kế toán NCC' ), true ) ) {
			return VHCPMTD_Util::err( 'Chỉ kế toán chỉnh TK Nợ được — đây là mã hạch toán, '
		...
		if ( $da_xuat && 'Admin' !== $vai ) {
			return VHCPMTD_Util::err( 'Đơn đã xuất MISA — mã TK của dòng đã đóng theo tệp đã nộp. Chỉ Admin sửa được, và phải sửa tay cả trong MISA.' );
		}
		$tk  = trim( (string) $tk );
		...
		$wpdb->update( VHCPMTD_DB::t( 'chiphi' ), array( 'tk_no' => $tk ), array( 'id' => (string) $id ) );
		VHCPMTD_Log::log_action( array(
			'actor'  => VHCPMTD_Auth::nguoi(),
			'role'   => $vai,
		...
		return VHCPMTD_Util::ok( array( 'tkNo' => $tk, 'daXuat' => $da_xuat ? 1 : 0 ) );
	}
	...
	public static function sua_ky_hong( $chot = false ) {
		...
		foreach ( self::don_rows() as $d ) {
			...
			$iso = VHCPMTD_Util::parse_date( $ky );
			$moi = $iso ? self::nhan_ky( $iso ) : '';
			$ds[] = array( 'maDon' => (string) $d['ma_don'], 'cu' => $ky,
				'ngay' => $iso ? VHCPMTD_Util::fmt( $iso ) : '', 'moi' => $moi );
			if ( $chot && $moi !== '' ) {
				$wpdb->update( $t, array( 'ky' => $moi ), array( 'ma_don' => (string) $d['ma_don'] ) );
				$sua++;
			}
		}
		return VHCPMTD_Util::ok( array( 'items' => array_slice( $ds, 0, 400 ), 'tong' => count( $ds ), 'daSua' => $sua ) );
	}
	...
	public static function sua_ngay_hong( $cach = 'ky', $nam = 0, $chot = false, $ma_don = '' ) {
		...
		$nam  = (int) $nam;
		if ( $cach === 'nam' && $chot && ( $nam < 2000 || $nam > 2100 ) ) { return VHCPMTD_Util::err( 'Năm phải trong khoảng 2000–2100' ); }
		...
		foreach ( VHCPMTD_DB::rows( $sql ) as $r ) {
			$dmy = VHCPMTD_Util::fmt( $r['ngay'] );
			if ( ! VHCPMTD_Util::ngay_vo_ly( $dmy ) ) { continue; }
			$m_don = (string) $r['ma_don'];
	...
	public static function set_don_ngay( $ma_don, $truong, $ngay ) {
		if ( VHCPMTD_Auth::vai_tro() !== 'Admin' ) { return VHCPMTD_Util::err( 'Chỉ Admin sửa được ngày của đơn' ); }
		$cot = array(
			'ngayTao'   => 'ngay_tao',
		...
		$k = (string) $truong;
		if ( ! isset( $cot[ $k ] ) ) { return VHCPMTD_Util::err( 'Không rõ cột ngày: ' . $k ); }
		$d = self::don_row( $ma_don );
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy đơn' ); }
		...
		if ( $s === '' ) {                      // xoá cho về trống là hợp lệ
			$wpdb->update( VHCPMTD_DB::t( 'don' ), array( $cot[ $k ] => null ), array( 'ma_don' => (string) $ma_don ) );
			return VHCPMTD_Util::ok( array( 'ngay' => '' ) );
		}
		$iso = VHCPMTD_Util::parse_date( $s );
		if ( ! $iso ) { return VHCPMTD_Util::err( 'Ngày không đọc được: ' . $s ); }
		if ( VHCPMTD_Util::ngay_vo_ly( VHCPMTD_Util::fmt( $iso ) ) ) { return VHCPMTD_Util::err( 'Ngày vô lý: ' . VHCPMTD_Util::fmt( $iso ) ); }
		$wpdb->update( VHCPMTD_DB::t( 'don' ), array( $cot[ $k ] => $iso . ' 00:00:00' ), array( 'ma_don' => (string) $ma_don ) );
		return VHCPMTD_Util::ok( array( 'ngay' => VHCPMTD_Util::fmt( $iso ) ) );
	}
	...
	public static function set_line_anh( $id, $url ) {
		...
		if ( ! $chua_chot && ! ( $la_ke_toan && $da_chot ) ) {
			return VHCPMTD_Util::err( $da_chot
		...
		$wpdb->update( VHCPMTD_DB::t( 'chiphi' ), array( 'anh' => (string) $url ), array( 'id' => (string) $id ) );
	...
	public static function delete_line( $id ) {
		...
		if ( ! $la_admin ) {
			if ( ! $ps && ! $xin_edit ) { return VHCPMTD_Util::err( 'Hạng mục đã xin không được xóa (để Thực chi = 0 nếu không mua; đơn bị trả lại mới mở khóa)' ); }
			if ( $ps ) {
				$_c = self::vi_sao_khong_sua( (string) $cur['ma_don'] );
				if ( $_c !== '' ) { return VHCPMTD_Util::err( $_c ); }
			}
		...
		$wpdb->delete( VHCPMTD_DB::t( 'chiphi' ), array( 'id' => (string) $id ) );
		...
		if ( $la_admin && ! $binh_thuong ) {
			VHCPMTD_Log::log_action( array(
				...
				'detail' => 'dòng ' . (string) $id . ' · trạng thái đơn: ' . $st
					...
					. ' · ' . (string) $cur['noi_dung']
					. ' · thành tiền ' . VHCPMTD_Util::out_num( $cur['thanh_tien'] )
					. ' · thực mua ' . VHCPMTD_Util::out_num( $cur['thuc_mua'] ),
			) );
		}
		return VHCPMTD_Util::ok();
	}
	...
	public static function duplicate_line( $id, $coso, $actor = '' ) {
		...
		global $wpdb;
		$v = self::line_row( $id );
		if ( ! $v ) { return VHCPMTD_Util::err( 'Không tìm thấy dòng' ); }
		...
		$rec = array(
			...
			'noiDung'    => $v['noi_dung'],
			'dvt'        => $v['dvt'],
			'soLuong'    => VHCPMTD_Util::out_num( $v['so_luong'] ),
			'donGia'     => VHCPMTD_Util::out_num( $v['don_gia'] ),
			'thanhTien'  => $v['thanh_tien'],
			'ghiChu'     => $v['ghi_chu'],
			'anh'        => $v['anh'],
			'thueSuat'   => VHCPMTD_Util::out_num( $v['thue_suat'] ),
			'thucMua'    => VHCPMTD_Util::out_num( $v['thuc_mua'] ),
			'cnXuLy'     => (int) $v['cn_xu_ly'],
			'phatSinh'   => (int) $v['phat_sinh'],
		);
		$nid = VHCPMTD_Util::uid( 'L' );
		/* Cùng luật với `add_line()`: ghi hỏng thì NÓI RA, đừng báo xong. */
		$ok_nb = $wpdb->insert( VHCPMTD_DB::t( 'chiphi' ), self::line_data( $nid, $ma_don, $rec ) );
		if ( ! $ok_nb ) {
			$ct = trim( (string) $wpdb->last_error );
			return VHCPMTD_Util::err( 'Không ghi được dòng chi vào sổ' . ( '' !== $ct ? ' — ' . $ct : '.' ) );
		}
		if ( $st === 'Chờ quyết toán' ) { self::mark_kt_sua( $ma_don, $actor ); }
		return VHCPMTD_Util::ok( array( 'id' => $nid ) );
	}
	...
	public static function gui_duyet_tam_ung( $ma_don ) {
		...
		$d = self::don_row( $ma_don );
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy đơn' ); }
		if ( (string) $d['trang_thai'] !== 'Nháp' ) { return VHCPMTD_Util::err( 'Chỉ gửi xin tạm ứng khi đơn ở "Nháp"' ); }
		/* 🔴 ĐƠN TRỰC TIẾP KHÔNG ĐI ĐƯỜNG NÀY. Luồng trực tiếp không có bước tạm ứng nào cả —
		   cho qua là đơn rơi vào `Chờ duyệt tạm ứng`, một trạng thái KHÔNG CÓ trong luồng của
		   nó: thanh bước không biết vẽ ở đâu, màn duyệt tạm ứng thì bày ra một đơn mà tiền đã
		   chi rồi. Chối ở đây, và NÓI RA phải bấm nút nào. */
		if ( self::la_truc_tiep( $d ) ) {
			return VHCPMTD_Util::err( 'Đơn này đi luồng TRỰC TIẾP — không qua tạm ứng. Bấm "Gửi quyết toán" để gửi thẳng cho kế toán.' );
		}
		$t = VHCPMTD_DB::t( 'chiphi' );
		...
		$tt_bang = VHCPMTD_DB::t( 'tamung' );
		$tu_sum  = VHCPMTD_Util::num( $wpdb->get_var( $wpdb->prepare(
			"SELECT SUM(so) FROM $tt_bang WHERE ma_don=%s", (string) $ma_don ) ) );
		if ( ! $n && ! ( $tu_sum > 0 ) ) {
			return VHCPMTD_Util::err( 'Chưa có gì để gửi — thêm ít nhất 1 hạng mục, hoặc nhập số tạm ứng.' );
		}
		...
		self::upd_don( $ma_don, array( 'ngay_gui' => VHCPMTD_Util::now_sql() ) );
		return VHCPMTD_Util::ok();
	}
	...
	public static function gui_quyet_toan( $ma_don ) {
		...
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy đơn' ); }
		/* ══════════════════════════════════════════════════════════════════════════════════
		 * HAI LUỒNG, HAI CHỖ XUẤT PHÁT.
		 * ══════════════════════════════════════════════════════════════════════════════════
		 * · Qua tạm ứng: gửi quyết toán khi đơn `Đã cấp tạm ứng` — tiền đã ra, giờ báo tiêu hết
		 *   bao nhiêu.
		 * · Trực tiếp (22/09/2026): gửi thẳng từ `Nháp` — anh Thắng: *"Trực tiếp là gửi đơn đầy
		 *   đủ cho kế toán và quyết toán"*. Tiền đã chi rồi, không có gì để tạm ứng.
		 *
		 * 🔴 ĐỌC BƯỚC XUẤT PHÁT TỪ CHÍNH BẢNG LUỒNG, không gõ cứng hai chuỗi. Bảng luồng là nơi
		 *    khai thứ tự bước; hỏi nó "bước ngay trước Chờ quyết toán là gì" thì thêm/bớt bước ở
		 *    `LUONG_*` là hàm này tự đúng theo — một chỗ khai, không có chỗ thứ hai để quên.
		 *    Cùng lối với `tt_truoc_misa()`.
		 * ══════════════════════════════════════════════════════════════════════════════════ */
		$buoc = array_keys( self::luong_cua( isset( $d['khoi'] ) ? $d['khoi'] : '', self::luong_don( $d ) ) );
		$i_qt = array_search( 'Chờ quyết toán', $buoc, true );
		$tu   = ( false !== $i_qt && $i_qt > 0 ) ? $buoc[ $i_qt - 1 ] : 'Đã cấp tạm ứng';
		if ( (string) $d['trang_thai'] !== $tu ) {
			return VHCPMTD_Util::err( 'Chỉ gửi khi đơn "' . self::ten_tt( $tu, isset( $d['khoi'] ) ? $d['khoi'] : '', self::luong_don( $d ) ) . '"' );
		}
		...
		if ( self::la_truc_tiep( $d ) ) {
			...
			if ( null !== $xin && $xin > 0 ) {
				return VHCPMTD_Util::err( 'Đơn TRỰC TIẾP không có bước tạm ứng, mà đơn này đang xin tạm ứng ' . VHCPMTD_Util::tien( $xin )
		...
		self::upd_don( $ma_don, array(
			...
			'ngay_gui_qt' => VHCPMTD_Util::now_sql(),
		) );
		return VHCPMTD_Util::ok();
	}
	...
	public static function save_quyet_toan( $ma_don, $obj ) {
		...
		$d = self::don_row( $ma_don );
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy đơn' ); }
		if ( (string) $d['trang_thai'] === 'Đã quyết toán' ) { return VHCPMTD_Util::err( 'Đơn đã quyết toán — không sửa' ); }
		$obj = (array) $obj;
		self::upd_don( $ma_don, array(
			'so_tien_thuc_mua' => VHCPMTD_Util::blank_or_num( isset( $obj['soThucMua'] ) ? $obj['soThucMua'] : '' ),
	...
	public static function set_hoa_don_qt( $ma_don, $url, $nguoi = '', $o = 1 ) {
		...
		return VHCPMTD_Util::ok( array( 'o' => $o, 'anhHoaDon' => $u ) );
	}
	...
	public static function duyet_tam_ung( $ma_don, $nguoi, $so_tam_ung = '' ) {
		$d = self::don_row( $ma_don );
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy đơn' ); }
		if ( (string) $d['trang_thai'] !== 'Chờ duyệt tạm ứng' ) { return VHCPMTD_Util::err( 'Đơn không ở "Chờ duyệt tạm ứng"' ); }
		$khoi   = isset( $d['khoi'] ) ? $d['khoi'] : '';
		/* ⚠️ HỎI LUỒNG CỦA CHÍNH ĐƠN, không hỏi mỗi khối. Từ 22/09/2026 người lập chọn luồng
		   trên từng đơn; hỏi mỗi khối là đơn trực tiếp của một khối "qua tạm ứng" vẫn bị đẩy
		   sang `Chờ cấp tạm ứng` — một bước không có trong luồng của nó. */
		$qua_tu = self::tt_trong_luong( 'Chờ cấp tạm ứng', $khoi, self::luong_don( $d ) );
		self::upd_don( $ma_don, array(
			'trang_thai'    => $qua_tu ? 'Chờ cấp tạm ứng' : 'Đã cấp tạm ứng',
			'nguoi_duyet'   => (string) $nguoi,
			'ngay_duyet'    => VHCPMTD_Util::now_sql(),
			'tam_ung_duyet' => VHCPMTD_Util::blank_or_num( $so_tam_ung ),
		) );
	...
	public static function danh_dau_thanh_toan( $ma_don, $nguoi ) {
		...
		$khoi = isset( $d['khoi'] ) ? $d['khoi'] : '';
		$lg   = self::luong_don( $d );
		if ( ! self::tt_trong_luong( 'Đã thanh toán', $khoi, $lg ) ) {
			return VHCPMTD_Util::err( 'Luồng của đơn này không có bước "Đã thanh toán" riêng — tiền đã đưa từ lúc cấp tạm ứng.' );
		}
		if ( (string) $d['trang_thai'] !== 'Đã quyết toán' ) {
			return VHCPMTD_Util::err( 'Chỉ đánh dấu thanh toán cho đơn đã duyệt quyết toán. Đơn này đang ở "'
				. self::ten_tt( (string) $d['trang_thai'], $khoi, $lg ) . '".' );
		}
	...
	public static function tong_de_duyet( $ma_don ) {
		...
		if ( ! VHCPMTD_DonVi::MO_KHI_KHONG_TAM_UNG ) { return $t; }
		/* Có dòng tạm ứng (kể cả 0đ — nó vẫn là lời chốt gian) thì con số của đơn là số xin. */
		if ( VHCPMTD_DonVi::don_co_tam_ung( $ma_don ) ) { return $t; }
		return self::tong_thanh_tien( $ma_don ) + VHCPMTD_Util::num( self::du_phong_cua( $ma_don ) );
	}
	...
	public static function tong_xin_hien_tai( $ma_don ) {
		...
		foreach ( (array) ( isset( $g['tamUng'] ) ? $g['tamUng'] : array() ) as $v ) {
			$t += VHCPMTD_Util::num( $v );
		}
		return $t + VHCPMTD_Util::num( $g['don']['duPhong'] );
	}
	...
	public static function doi_luong_don( $ma_don, $luong_moi, $ly_do = '' ) {
		...
		if ( '' === $moi ) {
			return VHCPMTD_Util::err( 'Mã luồng "' . trim( (string) $luong_moi ) . '" không có — chỉ nhận: ' . implode( ' · ', array_keys( self::LUONG_MA ) ) . '.' );
		}
		$cu = self::luong_don( $d );
		$khoi = isset( $d['khoi'] ) ? $d['khoi'] : '';
		...
		if ( self::luong_cua( $khoi, $cu ) === self::luong_cua( $khoi, $moi ) ) {
			return VHCPMTD_Util::err( 'Đơn đang đi đúng luồng "' . self::LUONG_MA[ $moi ] . '" rồi — không có gì để đổi.' );
		}
		$st = (string) $d['trang_thai'];
		if ( '' === $st ) { $st = 'Nháp'; }
		if ( self::da_chot( $st ) || 'Đã xuất MISA' === $st ) {
			return VHCPMTD_Util::err( 'Đơn đã "' . self::ten_tt( $st, $khoi, $cu ) . '" — sổ đã chốt, không đổi luồng được nữa.' );
		}
		...
		if ( $tien_da_ra ) {
			return VHCPMTD_Util::err( 'Đơn đang "' . self::ten_tt( $st, $khoi, $cu ) . '" — tiền đã cấp, không đổi luồng được. Muốn đổi thì Trả lại đơn rồi đi lại từ đầu.' );
		}
		$data = array( 'luong' => $moi, 'trang_thai' => 'Nháp', 'ngay_gui' => null, 'ngay_gui_qt' => null );
		$duyet_cu = VHCPMTD_Util::num( $d['tam_ung_duyet'] );
		if ( $duyet_cu > 0 ) {
		...
		self::ghi_vet( $ma_don, 'Đổi luồng đơn',
			...
			. ( $duyet_cu > 0 ? ' · gỡ số đã duyệt ' . VHCPMTD_Util::tien( $duyet_cu ) : '' )
		...
		return VHCPMTD_Util::ok( array( 'luongCu' => $cu, 'luongMoi' => $moi, 'trangThaiCu' => $st, 'trangThai' => 'Nháp', 'goDuyet' => $duyet_cu > 0 ) );
	}
	...
	public static function duyet_lai_tam_ung( $ma_don, $nguoi = '', $so = '' ) {
		...
		if ( 'Chờ cấp tạm ứng' !== $st ) {
			return VHCPMTD_Util::err( 'Chỉ duyệt lại được đơn ĐÃ duyệt mà CHƯA cấp tiền. Đơn này đang ở "'
				. $st . '"'
		...
		$cu  = VHCPMTD_Util::num( $d['tam_ung_duyet'] );
		$moi = ( '' === trim( (string) $so ) ) ? self::tong_xin_hien_tai( $ma_don ) : VHCPMTD_Util::num( $so );
		if ( null === $moi ) { return VHCPMTD_Util::err( 'Không đọc ra được tổng xin hiện tại của đơn.' ); }
		if ( $moi < 0 ) { return VHCPMTD_Util::err( 'Số tạm ứng không âm được.' ); }
		if ( VHCPMTD_Util::num( $moi ) === $cu ) {
			return VHCPMTD_Util::err( 'Số duyệt đang đúng bằng ' . VHCPMTD_Util::tien( $cu ) . ' rồi — không có gì để đổi.' );
		}

		$nguoi = trim( (string) $nguoi );
		if ( '' === $nguoi ) { $nguoi = VHCPMTD_Auth::nguoi(); }

		self::upd_don( $ma_don, array(
			'tam_ung_duyet' => VHCPMTD_Util::num( $moi ),
			'nguoi_duyet'   => $nguoi,
			'ngay_duyet'    => VHCPMTD_Util::now_sql(),
		) );
		self::ghi_vet( $ma_don, 'Duyệt lại số tạm ứng',
			VHCPMTD_Util::tien( $cu ) . '  →  ' . VHCPMTD_Util::tien( $moi ) );
		self::bao_noi_bo( $ma_don, 'đã được duyệt LẠI số tạm ứng: ' . VHCPMTD_Util::tien( $moi ) );
		return VHCPMTD_Util::ok( array( 'cu' => $cu, 'moi' => VHCPMTD_Util::num( $moi ) ) );
	}
	...
	public static function don_bu_tru_cu( $chot = false, $chi_ma = null ) {
		...
		if ( $chot && ! $chon ) {
			return VHCPMTD_Util::err( 'Chưa chọn đơn nào. Tích vào những đơn cần dọn rồi bấm lại — '
		...
		foreach ( self::don_rows() as $d ) {
			$st  = (string) $d['trang_thai'];
			$ma  = (string) $d['ma_don'];
			$duy = VHCPMTD_Util::num( $d['tam_ung_duyet'] );
			...
			if ( 'Nháp' === $st || 'Chờ duyệt tạm ứng' === $st ) {
				...
				if ( $chot && isset( $chon[ $ma ] ) ) {
					...
					self::ghi_vet( $ma, 'Gỡ số duyệt mồ côi (đơn đang ở "' . $st . '")',
						VHCPMTD_Util::tien( $duy ) . '  →  chưa duyệt' );
					$da_sua++;
				}
				continue;
			}
			...
			if ( $chot && isset( $chon[ $ma ] ) ) {
				self::upd_don( $ma, array( 'tam_ung_duyet' => VHCPMTD_Util::num( $xin ) ) );
				self::ghi_vet( $ma, 'Dọn bù trừ cũ khỏi số duyệt',
					VHCPMTD_Util::tien( $duy ) . '  →  ' . VHCPMTD_Util::tien( $xin )
					. ' (gỡ phần bù trừ ' . VHCPMTD_Util::tien( $bt ) . ' theo luật trước 1.53.0)' );
				$da_sua++;
			}
		}
		$so_mocoi = 0;
	...
	public static function cap_tam_ung( $ma_don, $nguoi, $ht_cap = 'Tiền mặt', $anh_cap = '' ) {
		$d = self::don_row( $ma_don );
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy đơn' ); }
		if ( (string) $d['trang_thai'] !== 'Chờ cấp tạm ứng' ) { return VHCPMTD_Util::err( 'Đơn chưa được duyệt tạm ứng' ); }
		$ht_cap = $ht_cap ? $ht_cap : 'Tiền mặt';
		if ( $ht_cap === 'Chuyển khoản' && ! $anh_cap ) { return VHCPMTD_Util::err( 'Chuyển khoản phải đính ảnh chứng từ' ); }
		self::upd_don( $ma_don, array(
			'trang_thai' => 'Đã cấp tạm ứng',
			'nguoi_cap'  => (string) $nguoi,
			'ngay_cap'   => VHCPMTD_Util::now_sql(),
	...
	public static function go_cap_tam_ung( $ma_don, $ly_do = '' ) {
		...
		if ( (string) $d['trang_thai'] !== 'Đã cấp tạm ứng' ) {
			return VHCPMTD_Util::err( 'Chỉ trả ngược được đơn đang "Đã cấp tạm ứng". Đơn này đang ở "' . (string) $d['trang_thai'] . '".' );
		}
		$ly_do = trim( (string) $ly_do );
		if ( '' === $ly_do ) { return VHCPMTD_Util::err( 'Phải nêu lý do — đây là lượt gỡ cấp tiền của đơn đã đánh dấu cấp.' ); }
		self::upd_don( $ma_don, array(
	...
	public static function don_loai( $ma_don ) {
		...
		$t   = VHCPMTD_DB::t( 'chiphi' );
		$cn  = false; $ncc = false;
		foreach ( VHCPMTD_DB::rows( $wpdb->prepare( "SELECT phan_loai_tt, cn_xu_ly FROM $t WHERE ma_don=%s", (string) $ma_don ) ) as $r ) {
			if ( VHCPMTD_Util::is_ncc( $r['phan_loai_tt'], $r['cn_xu_ly'] ) ) { $ncc = true; } else { $cn = true; }
		}
	...
	public static function xac_nhan_quyet_toan_cn( $ma_don, $nguoi, $xu_ly, $chenh_lech ) {
		$d = self::don_row( $ma_don );
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy đơn' ); }
		if ( (string) $d['trang_thai'] !== 'Chờ quyết toán' ) { return VHCPMTD_Util::err( 'Đơn không ở "Chờ quyết toán"' ); }
		self::upd_don( $ma_don, array(
			'nguoi_qt'      => (string) $nguoi,
			'ngay_qt'       => VHCPMTD_Util::now_sql(),
			'chenh_lech_qt' => VHCPMTD_Util::num( $chenh_lech ),
			'xu_ly'         => (string) $xu_ly,
	...
	public static function xac_nhan_quyet_toan_ncc( $ma_don, $nguoi ) {
		...
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy đơn' ); }
		$st = (string) $d['trang_thai'];
		if ( $st === 'Nháp' || $st === 'Đã xuất MISA' ) { return VHCPMTD_Util::err( 'Đơn chưa gửi hoặc đã xuất' ); }
		$data = array( 'nguoi_qt_ncc' => (string) $nguoi, 'ngay_qt_ncc' => VHCPMTD_Util::now_sql() );
	...
	public static function tra_lai_don( $ma_don, $ly_do = '' ) {
		...
		if ( $go_duyet ) {
			self::ghi_vet( $ma_don, 'Gỡ số đã duyệt (đơn bị trả về Nháp)',
				VHCPMTD_Util::tien( $duyet_cu ) . '  →  chưa duyệt' );
		}
		self::bao_noi_bo( $ma_don, 'bị TRẢ LẠI' . ( $ly_do ? ' — ' . $ly_do : '' ) . '. Sửa rồi gửi lại nhé.' );
		return VHCPMTD_Util::ok( array( 'target' => $target, 'goDuyet' => $go_duyet ) );
	}
	...
	public static function delete_don( $ma_don ) {
		...
		if ( ! in_array( $st, array( 'Nháp', 'Chờ duyệt tạm ứng', 'Chờ cấp tạm ứng' ), true ) ) {
			return VHCPMTD_Util::err( 'Chỉ xóa được đơn CHƯA cấp tạm ứng. Đơn đã cấp tiền: dùng "🚫 Không dùng" hoặc Trả lại.' );
		}
	...
	public static function delete_don_admin( $ma_don ) {
		...
		VHCPMTD_Log::log_action( array(
			...
			'detail' => 'trạng thái: ' . (string) $d['trang_thai']
				...
				. ' · ' . $so_dong . ' dòng'
				. ' · tổng thành tiền ' . VHCPMTD_Util::out_num( isset( $sum['tt'] ) ? $sum['tt'] : 0 )
				. ' · tổng thực mua ' . VHCPMTD_Util::out_num( isset( $sum['tm'] ) ? $sum['tm'] : 0 )
				. ' · tạm ứng duyệt ' . VHCPMTD_Util::out_num( $d['tam_ung_duyet'] ),
		) );
	...
	private static function purge_don( $ma_don ) {
		...
		$wpdb->delete( VHCPMTD_DB::t( 'chiphi' ), array( 'ma_don' => (string) $ma_don ) );
		$wpdb->delete( VHCPMTD_DB::t( 'tamung' ), array( 'ma_don' => (string) $ma_don ) );
		$wpdb->delete( VHCPMTD_DB::t( 'don' ), array( 'ma_don' => (string) $ma_don ) );
		return VHCPMTD_Util::ok();
	}
	...
	private static function vao_thung_rac_don( $ma_don ) {
		...
		$d = self::don_row( $ma_don );
		if ( ! $d ) { return 0; }
		$chi = VHCPMTD_DB::rows( $wpdb->prepare(
			'SELECT * FROM ' . VHCPMTD_DB::t( 'chiphi' ) . ' WHERE ma_don=%s ORDER BY stt ASC', $ma_don ) );
		$tu  = VHCPMTD_DB::rows( $wpdb->prepare(
			'SELECT * FROM ' . VHCPMTD_DB::t( 'tamung' ) . ' WHERE ma_don=%s ORDER BY id ASC', $ma_don ) );
		$tien = 0;
		foreach ( $chi as $r ) { $tien += VHCPMTD_Util::num( $r['thanh_tien'] ); }
		return self::vao_thung_rac( 'don', $ma_don,
			'Đơn ' . $ma_don . ' · ' . (string) $d['ky'] . ' · ' . (string) $d['nguoi_lap']
				. ' · ' . count( $chi ) . ' dòng · ' . VHCPMTD_Util::out_num( $tien )
	...
	private static function vao_thung_rac_dong( $row ) {
		...
		return self::vao_thung_rac( 'dong', (string) $row['id'],
			...
			array( 'chiphi' => $row ),
			VHCPMTD_DonVi::cua_don( (string) $row['ma_don'] ) );
	}

	private static function vao_thung_rac( $loai, $khoa, $nhan, $du_lieu, $don_vi = '' ) {
		global $wpdb;
		$wpdb->insert( VHCPMTD_DB::t( 'thungrac' ), array(
			...
			'nguoi'   => VHCPMTD_Auth::nguoi(),
			'vai_tro' => VHCPMTD_Auth::vai_tro(),
	...
	public static function ds_thung_rac( $limit = 50, $ca = false ) {
		...
		$limit = (int) $limit;
		if ( $limit <= 0 || $limit > 300 ) { $limit = 50; }
		$rows = VHCPMTD_DB::rows( $wpdb->prepare(
			'SELECT * FROM ' . VHCPMTD_DB::t( 'thungrac' ) . ' ORDER BY id DESC LIMIT %d', $limit * 3 ) );
		$dv_toi = VHCPMTD_DonVi::cua_toi();
		$nv     = VHCPMTD_Auth::la_nhan_vien();
		$toi    = mb_strtolower( trim( (string) VHCPMTD_Auth::nguoi() ) );
		$out = array();
		foreach ( $rows as $r ) {
			if ( ! $ca && (int) $r['da_hoan'] ) { continue; }
			$dv = trim( (string) $r['don_vi'] );
			if ( '' !== $dv && '' !== $dv_toi && ! VHCPMTD_DonVi::bang( $dv, $dv_toi ) ) { continue; }
			...
			$out[] = array(
				...
				'hoanLuc' => VHCPMTD_Util::fmt_dt( $r['hoan_luc'] ),
			);
			if ( count( $out ) >= $limit ) { break; }
		}
		return VHCPMTD_Util::ok( array( 'items' => $out ) );
	}
	...
	public static function hoan_tac( $id ) {
		...
		$r  = $wpdb->get_row( $wpdb->prepare(
			'SELECT * FROM ' . VHCPMTD_DB::t( 'thungrac' ) . ' WHERE id=%d', $id ), ARRAY_A );
		if ( ! $r ) { return VHCPMTD_Util::err( 'Không tìm thấy mục này trong thùng rác.' ); }
		if ( (int) $r['da_hoan'] ) {
			return VHCPMTD_Util::err( 'Mục này đã được hoàn lúc ' . VHCPMTD_Util::fmt_dt( $r['hoan_luc'] )
				. ( '' !== trim( (string) $r['hoan_nguoi'] ) ? ' bởi ' . $r['hoan_nguoi'] : '' )
				. ' — hoàn lần nữa là nhân đôi số.' );
		}
		$dv = trim( (string) $r['don_vi'] );
		$dv_toi = VHCPMTD_DonVi::cua_toi();
		if ( '' !== $dv && '' !== $dv_toi && ! VHCPMTD_DonVi::bang( $dv, $dv_toi ) ) {
			return VHCPMTD_Util::err( 'Mục này thuộc đơn vị ' . $dv . ' — không phải sổ của bạn.' );
		}
		if ( VHCPMTD_Auth::la_nhan_vien()
			&& mb_strtolower( trim( (string) $r['nguoi'] ) ) !== mb_strtolower( trim( (string) VHCPMTD_Auth::nguoi() ) ) ) {
			return VHCPMTD_Util::err( 'Chỉ hoàn lại được thao tác của chính mình. Nhờ kế toán hoàn giúp.' );
		}

		$d = json_decode( (string) $r['du_lieu'], true );
		if ( ! is_array( $d ) ) { return VHCPMTD_Util::err( 'Bản sao trong thùng rác đọc không ra — không dựng lại được.' ); }

		if ( 'don' === (string) $r['loai'] ) {
			$don = isset( $d['don'] ) ? (array) $d['don'] : array();
			$ma  = isset( $don['ma_don'] ) ? (string) $don['ma_don'] : '';
			if ( '' === $ma ) { return VHCPMTD_Util::err( 'Bản sao thiếu mã đơn.' ); }
			if ( self::don_row( $ma ) ) {
				return VHCPMTD_Util::err( 'Mã đơn ' . $ma . ' nay đã có một đơn khác đang dùng — '
			...
			if ( ! isset( $don['khoi'] ) || '' === trim( (string) $don['khoi'] ) ) {
				...
				if ( '' === $don['khoi'] ) { $don['khoi'] = VHCPMTD_DB::khoi(); }
			}
			/* ══════════════════════════════════════════════════════════════════════════
			...
			foreach ( (array) ( isset( $d['tamung'] ) ? $d['tamung'] : array() ) as $x ) {
				if ( ! $wpdb->insert( VHCPMTD_DB::t( 'tamung' ), (array) $x ) ) {
					$hong[] = 'tạm ứng (' . trim( (string) $wpdb->last_error ) . ')';
				}
			}
			if ( $hong ) {
				return VHCPMTD_Util::err( 'Hoàn tác chưa xong — không ghi lại được: '
			...
			return VHCPMTD_Util::ok( array( 'loai' => 'don', 'maDon' => $ma, 'soDong' => $so ) );
		}
		...
		if ( '' === $ma ) { return VHCPMTD_Util::err( 'Bản sao thiếu mã đơn.' ); }
		if ( ! self::don_row( $ma ) ) {
			return VHCPMTD_Util::err( 'Đơn ' . $ma . ' của dòng này đã bị xoá — '
				. 'hoàn lại ĐƠN trước, dòng chi nằm sẵn trong đó.' );
		}
		if ( isset( $row['id'] ) && self::line_row( (string) $row['id'] ) ) {
			return VHCPMTD_Util::err( 'Dòng này đã có lại trong đơn rồi.' );
		}
		$ok_row = $wpdb->insert( VHCPMTD_DB::t( 'chiphi' ), $row );
		if ( ! $ok_row ) {
			$ct = trim( (string) $wpdb->last_error );
			return VHCPMTD_Util::err( 'Không ghi được dòng chi vào sổ' . ( '' !== $ct ? ' — ' . $ct : '.' ) );
		}
		...
		return VHCPMTD_Util::ok( array( 'loai' => 'dong', 'maDon' => $ma, 'id' => (string) $row['id'] ) );
	}

	private static function danh_dau_hoan( $id ) {
		global $wpdb;
		$wpdb->update( VHCPMTD_DB::t( 'thungrac' ), array(
			'da_hoan'    => 1,
			'hoan_luc'   => current_time( 'mysql' ),
			'hoan_nguoi' => VHCPMTD_Auth::nguoi(),
		), array( 'id' => (int) $id ) );
	}
	...
	public static function chuyen_don_vi( $ma_don, $ids, $don_vi, $nguoi = '' ) {
		...
		$dv_moi = VHCPMTD_DonVi::chuan( $don_vi );
		$dv_cu  = VHCPMTD_DonVi::chuan( isset( $d['don_vi'] ) ? $d['don_vi'] : '' );
		if ( VHCPMTD_DonVi::bang( $dv_moi, $dv_cu ) ) {
			return VHCPMTD_Util::err( 'Đơn này đã thuộc đơn vị ' . $dv_cu . ' rồi.' );
		}
		...
		if ( ! in_array( $dv_moi, VHCPMTD_DonVi::ds(), true ) ) {
			/* Chỉ đúng chỗ KHAI, không phải một chỗ nào đó cũng có chữ "đơn vị": danh mục cơ
			   sở là nơi ranh giới được vạch (`VHCPMTD_DonVi::cua_coso()`). Chỉ sai chỗ thì người
			   ta đi khai ở bảng người dùng, khai xong vẫn báo đúng câu này. */
			return VHCPMTD_Util::err( 'Chưa có đơn vị "' . $dv_moi . '" — khai ở Cấu hình → Danh mục cơ sở, cột "Đơn vị" trước.' );
		}
		$_c = self::vi_sao_khong_sua( $ma_don );
		if ( '' !== $_c ) { return VHCPMTD_Util::err( $_c ); }

		$tat_ca = self::rows_of_don( $ma_don );
		if ( ! $tat_ca ) { return VHCPMTD_Util::err( 'Đơn này chưa có dòng chi nào để chuyển.' ); }
		...
		if ( ! $chon ) { return VHCPMTD_Util::err( 'Không có dòng nào khớp để chuyển.' ); }
		...
		$wpdb->update( VHCPMTD_DB::t( 'don' ), array( 'don_vi' => $dv_moi ), array( 'ma_don' => $ma_moi ) );

		$tien = 0;
		foreach ( $chon as $r ) {
			$wpdb->update( VHCPMTD_DB::t( 'chiphi' ), array( 'ma_don' => $ma_moi ), array( 'id' => (string) $r['id'] ) );
			$tien += VHCPMTD_Util::num( $r['thanh_tien'] );
		}
	...
	public static function doi_moc_ky( $ky_cu, $tu, $den ) {
		...
		if ( '' === $ky_cu ) { return VHCPMTD_Util::err( 'Chưa chọn kỳ cần đổi' ); }

		$t1 = self::ngay_iso_( $tu );
		$t2 = self::ngay_iso_( $den );
		if ( '' === $t1 || '' === $t2 ) { return VHCPMTD_Util::err( 'Ngày không hợp lệ' ); }
		if ( $t1 > $t2 ) { return VHCPMTD_Util::err( 'Ngày bắt đầu phải trước ngày kết thúc' ); }
		...
		if ( ( strtotime( $t2 ) - strtotime( $t1 ) ) / 86400 > 31 ) {
			return VHCPMTD_Util::err( 'Khoảng này dài hơn 31 ngày — kiểm lại ngày tháng' );
		}

		$ky_moi = self::ky_tu_khoang_( $t1, $t2 );
		$t      = VHCPMTD_DB::t( 'don' );
		$dem_cu = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $t WHERE ky=%s", $ky_cu ) );
		if ( ! $dem_cu ) { return VHCPMTD_Util::err( 'Không có đơn nào ở kỳ này' ); }
		if ( $ky_moi === $ky_cu ) { return VHCPMTD_Util::err( 'Mốc mới trùng đúng mốc đang có — không có gì để đổi' ); }
		...
		return VHCPMTD_Util::ok( array( 'doi' => $dem_cu, 'kyMoi' => $ky_moi, 'gop' => $gop ) );
	}
	...
	public static function ds_ky_dang_co() {
		...
		$t  = VHCPMTD_DB::t( 'don' );
		$ra = array();
		foreach ( VHCPMTD_DB::rows( "SELECT ky, COUNT(*) AS n FROM $t WHERE ky <> '' GROUP BY ky" ) as $r ) {
			$k = (string) $r['ky'];
		...
		return VHCPMTD_Util::ok( array( 'items' => $ra ) );
	}
	...
	public static function dat_khoang_ky( $ma_don, $tu, $den, $ly_do = '', $nguoi = '' ) {
		...
		$a = self::ngay_iso_( $tu );
		$z = self::ngay_iso_( $den );
		if ( '' === $a || '' === $z ) { return VHCPMTD_Util::err( 'Chọn đủ ngày bắt đầu và ngày kết thúc.' ); }
		if ( $z < $a ) { return VHCPMTD_Util::err( 'Ngày kết thúc phải sau ngày bắt đầu.' ); }
		$ky_moi = self::ky_tu_khoang( $a, $z );
		if ( '' === $ky_moi ) { return VHCPMTD_Util::err( 'Khoảng ngày không dựng ra được kỳ.' ); }

		$ky_cu = trim( (string) $d['ky'] );
		if ( $ky_moi === $ky_cu ) { return VHCPMTD_Util::err( 'Đơn này đang ở đúng khoảng ngày đó rồi.' ); }
	...
	public static function chuyen_ky( $ma_don, $ky_moi = '', $ly_do = '', $nguoi = '' ) {
		...
		$ky_cu = trim( (string) $d['ky'] );
		if ( '' === $ky_cu ) {
			return VHCPMTD_Util::err( 'Đơn này chưa có kỳ nên không biết nhảy từ đâu. '
		...
		if ( '' === $ky_moi ) {
			return VHCPMTD_Util::err( 'Không đọc ra được tuần kế tiếp của kỳ "' . $ky_cu . '".' );
		}
		...
		if ( '' === $tu_m || '' === $den_m ) {
			return VHCPMTD_Util::err( 'Kỳ "' . $ky_moi . '" không đọc ra được khoảng ngày — '
				. 'chọn tuần trong danh sách, đừng gõ tay.' );
		}
		if ( $ky_moi === $ky_cu ) {
			return VHCPMTD_Util::err( 'Đơn này đang ở kỳ ' . $ky_cu . ' rồi.' );
		}
	...
	public static function doi_coso_don( $ma_don, $coso_moi = '', $coso_cu = '', $ly_do = '' ) {
		...
		if ( 'Admin' !== VHCPMTD_Auth::vai_tro() ) {
			return VHCPMTD_Util::err( 'Đổi cơ sở của cả đơn là việc của Admin — '
		...
		if ( '' === $coso_cu ) {
			...
			if ( count( $ds_cu ) > 1 ) {
				return VHCPMTD_Util::err( 'Đơn này đang có ' . count( $ds_cu ) . ' cơ sở ('
					. implode( ' · ', $ds_cu ) . ') — phải chỉ rõ đổi cơ sở nào.' );
			}
			$coso_cu = $ds_cu ? $ds_cu[0] : '';
		}
		if ( '' === $coso_cu ) { return VHCPMTD_Util::err( 'Đơn này chưa có dòng nào mang cơ sở.' ); }
		if ( mb_strtolower( $coso_cu ) === mb_strtolower( $coso_moi ) ) {
			return VHCPMTD_Util::err( 'Đơn này đang ở cơ sở "' . $coso_cu . '" rồi.' );
		}

		$t_cp = VHCPMTD_DB::t( 'chiphi' );
		$t_tu = VHCPMTD_DB::t( 'tamung' );

		/* ── tạm ứng: cộng dồn nếu tên đích đã có dòng, vì (ma_don, coso) là khoá duy nhất ── */
		$cu_row = VHCPMTD_DB::row( $wpdb->prepare(
			"SELECT * FROM $t_tu WHERE ma_don=%s AND coso=%s", $ma_don, $coso_cu ) );
		$moi_row = VHCPMTD_DB::row( $wpdb->prepare(
			"SELECT * FROM $t_tu WHERE ma_don=%s AND coso=%s", $ma_don, $coso_moi ) );
		$n_tu = 0;
		if ( $cu_row ) {
			if ( $moi_row ) {
				$wpdb->update( $t_tu,
					array( 'so' => VHCPMTD_Util::num( $moi_row['so'] ) + VHCPMTD_Util::num( $cu_row['so'] ) ),
					array( 'id' => $moi_row['id'] ) );
	...
	private static function rows_of_don( $ma_don ) {
		global $wpdb;
		$t = VHCPMTD_DB::t( 'chiphi' );
		return VHCPMTD_DB::rows( $wpdb->prepare( "SELECT * FROM $t WHERE ma_don=%s ORDER BY stt ASC", (string) $ma_don ) );
	}
	...
	public static function set_tat_toan_tuan( $ma_don, $on, $actor = '' ) {
		...
		self::upd_don( $ma_don, array(
			...
			'ngay_tat_toan' => $on ? VHCPMTD_Util::now_sql() : null,
		) );
		return VHCPMTD_Util::ok( array( 'tatToan' => (bool) $on ) );
	}
	...
	public static function ghi_lenh_tu( $ma_dons, $nguoi, $luc = '' ) {
		global $wpdb;
		$luc = trim( (string) $luc );
		if ( '' === $luc ) { $luc = VHCPMTD_Util::now_sql(); }
		...
		foreach ( self::list_dons() as $d ) {
			...
			$so_don++;
			$tu = (int) VHCPMTD_Util::num( isset( $d['tamUng'] ) ? $d['tamUng'] : 0 );
			$tong += $tu;
		...
		$id = VHCPMTD_Util::uid( 'LTU' );
		$ok = $wpdb->insert( VHCPMTD_DB::t( 'lenh_tu' ), array(
			'id'       => $id,
			/* Mảng đóng dấu lúc lập lệnh — xem chốt ở `VHCPMTD_SoChi::add()`. Lệnh tạm ứng là tờ
	...
	private static function sql_tu_dmy_( $dmy ) {
		$d = VHCPMTD_Util::vh_parse_dmy( (string) $dmy );
	...
	public static function don_duyet_chua_co_lenh() {
		...
		foreach ( self::list_dons() as $d ) {
			...
			$ra[] = array(
				...
				'tamUng'     => (int) VHCPMTD_Util::num( isset( $d['tamUng'] ) ? $d['tamUng'] : 0 ),
			);
		}
		return $ra;
	}
	...
	public static function dung_lenh_bu() {
		...
		if ( ! count( $sot ) ) {
			return VHCPMTD_Util::ok( array( 'soLenh' => 0, 'soDon' => 0, 'tong' => 0 ) );
		}
		$nhom = array();
		foreach ( $sot as $d ) {
		...
		if ( count( $loi ) ) { return VHCPMTD_Util::err( implode( ' · ', array_unique( $loi ) ) ); }
		return VHCPMTD_Util::ok( array( 'soLenh' => $so_lenh, 'soDon' => $so_don, 'tong' => $tong ) );
	}
	...
	public static function ds_lenh_tu( $gioi_han = 50 ) {
		...
		$t = VHCPMTD_DB::t( 'lenh_tu' );
		$out = array();
		foreach ( (array) VHCPMTD_DB::rows( "SELECT * FROM $t ORDER BY stt DESC LIMIT $n" ) as $r ) {
			...
			$dv = (string) $r['don_vi'];
			if ( $dv !== '' && ! VHCPMTD_DonVi::duoc_xem( $dv ) ) { continue; }
			$ct = json_decode( (string) $r['chi_tiet'], true );
			$out[] = array(
				'id'      => (string) $r['id'],
				'luc'     => VHCPMTD_Util::fmt( $r['luc'] ),
				'nguoi'   => (string) $r['nguoi'],
				'donVi'   => $dv,
				'ky'      => (string) $r['ky'],
				'tong'    => VHCPMTD_Util::num( $r['tong'] ),
				'soDon'   => (int) $r['so_don'],
	...
	public static function ha_tam_ung_ve_0( $ma_don, $ly_do = '' ) {
		...
		if ( ! in_array( $st, array( 'Đã cấp tạm ứng', 'Chờ quyết toán' ), true ) ) {
			return VHCPMTD_Util::err( 'Chỉ hạ được tạm ứng của đơn ĐÃ CẤP mà chưa chốt sổ. Đơn này đang ở "'
				. $st . '"'
		...
		$cu = VHCPMTD_Util::num( $d['tam_ung_duyet'] );
		if ( 0.0 === (float) $cu && null === VHCPMTD_Util::blank_or_num( $d['tam_ung_duyet'] ) ) {
			/* Chưa ai chốt số: hạ về 0 vẫn có nghĩa (0 khác "chưa chốt"), nên cứ chạy tiếp. */
			$cu = 0.0;
		} elseif ( 0.0 === (float) $cu ) {
			return VHCPMTD_Util::err( 'Tạm ứng của đơn này đang là 0 rồi — không có gì để hạ.' );
		}

		$ly_do = trim( (string) $ly_do );
		if ( '' === $ly_do ) {
			return VHCPMTD_Util::err( 'Phải nêu lý do — đây là lượt sửa tiền trên đơn đã đánh dấu cấp.' );
		}

		self::upd_don( $ma_don, array( 'tam_ung_duyet' => 0 ) );
		self::ghi_vet( $ma_don, 'Hạ tạm ứng về 0 (tiền chưa ra khỏi két)',
			VHCPMTD_Util::tien( $cu ) . '  →  0đ · ' . $ly_do );
		self::bao_noi_bo( $ma_don, 'được hạ tạm ứng về 0đ (tiền chưa ra khỏi két) — ' . $ly_do );
		return VHCPMTD_Util::ok( array( 'cu' => $cu ) );
	}
	...
	public static function sua_tam_ung_da_cap( $ma_don, $so_moi = '', $ly_do = '' ) {
		...
		$st = (string) $d['trang_thai'];
		if ( ! in_array( $st, array( 'Đã cấp tạm ứng', 'Chờ quyết toán' ), true ) ) {
			return VHCPMTD_Util::err( 'Chỉ sửa được tạm ứng của đơn ĐÃ CẤP mà chưa chốt sổ. Đơn này đang ở "' . $st . '"'
				. ( 'Chờ cấp tạm ứng' === $st ? ' — chưa cấp thì bấm "Duyệt lại theo số mới".'
					: ( self::da_chot( $st ) ? ' — đã chốt sổ, số đã vào báo cáo và MISA.' : '.' ) ) );
		}
		$moi = VHCPMTD_Util::blank_or_num( $so_moi );
		if ( null === $moi || $moi < 0 ) { return VHCPMTD_Util::err( 'Số tạm ứng mới không hợp lệ.' ); }
		$ly_do = trim( (string) $ly_do );
		if ( '' === $ly_do ) { return VHCPMTD_Util::err( 'Phải nêu lý do — đây là lượt sửa tiền trên đơn đã đánh dấu cấp.' ); }
		$cu = VHCPMTD_Util::num( $d['tam_ung_duyet'] );
		if ( (float) $cu === (float) $moi ) { return VHCPMTD_Util::err( 'Số mới trùng số cũ — không có gì thay đổi.' ); }
		self::upd_don( $ma_don, array( 'tam_ung_duyet' => $moi ) );
		self::ghi_vet( $ma_don, 'Admin sửa số tạm ứng (đơn đã cấp)',
			VHCPMTD_Util::tien( $cu ) . '  →  ' . VHCPMTD_Util::tien( $moi ) . 'đ · ' . $ly_do );
		self::bao_noi_bo( $ma_don, 'được Admin sửa tạm ứng ' . VHCPMTD_Util::tien( $cu ) . 'đ → ' . VHCPMTD_Util::tien( $moi ) . 'đ — ' . $ly_do );
		return VHCPMTD_Util::ok( array( 'cu' => $cu, 'moi' => $moi ) );
	}
	...
	public static function khong_dung_tam_ung( $ma_don ) {
		...
		$d = self::don_row( $ma_don );
		if ( ! $d ) { return VHCPMTD_Util::err( 'Không tìm thấy đơn' ); }
		if ( (string) $d['trang_thai'] !== 'Đã cấp tạm ứng' ) { return VHCPMTD_Util::err( 'Chỉ đánh dấu khi đơn "Đã cấp tạm ứng"' ); }
		$old  = (string) $d['ghi_chu'];
	...
	public static function get_so_du_dau_ky() {
		return VHCPMTD_Util::num( VHCPMTD_Meta::get( 'soDuDauKy', 0 ) );
	}

	public static function set_so_du_dau_ky( $v ) {
		VHCPMTD_Meta::set( 'soDuDauKy', (string) VHCPMTD_Util::num( $v ) );
		return VHCPMTD_Util::ok( array( 'value' => VHCPMTD_Util::num( $v ) ) );
	}
	...
	public static function don_folder_meta( $ma_don ) {
		...
		if ( $ma_don ) {
			...
			$t    = VHCPMTD_DB::t( 'chiphi' );
			$cnt  = array();
			foreach ( VHCPMTD_DB::rows( $wpdb->prepare( "SELECT coso FROM $t WHERE ma_don=%s ORDER BY stt ASC", (string) $ma_don ) ) as $r ) {
				$c = trim( (string) $r['coso'] );
				if ( $c === '' ) { continue; }
