 ...
 *
 *   DonHang   -> vhcpmtd_don            TamUng  -> vhcpmtd_tamung        ChiPhi  -> vhcpmtd_chiphi
 *   DA_Index  -> vhcpmtd_da_index       <sheet dự án> -> vhcpmtd_da_line (khóa theo ma_da + row_no)
 *   MK_Don    -> vhcpmtd_mk_don         MK_Line -> vhcpmtd_mk_line
 *   BP_Index  -> vhcpmtd_bp_index       <sheet đợt>   -> vhcpmtd_bp_line (khóa theo ma + row_no)
 *   CH_*      -> vhcpmtd_cfg (bảng dùng chung, mỗi dòng là 1 hàng sheet dạng JSON)
 *   NhatKy    -> vhcpmtd_log
 *   Document/Script Properties -> vhcpmtd_meta
 *
...
class VHCPMTD_DB {
	 ...
	 * Bản 1.266.0 thêm cột `chiphi.giai_doan` mà quên nâng số này. `vhcpmtd_maybe_upgrade()` chỉ
	 * gọi `install()` khi số này KHÁC `vhcpmtd_db_version` đang lưu — nên `dbDelta()` không chạy,
	...
	public static function t( $name ) {
		global $wpdb;
		return $wpdb->prefix . 'vhcpmtd_' . $name;
	}
	...
	public static function xoa_du_lieu() {
		...
		foreach ( array( 'daApp_', 'daPay_' ) as $p ) {
			foreach ( VHCPMTD_Meta::get_prefix( $p ) as $k => $v ) { VHCPMTD_Meta::del( $k ); }
		}
	...
	public static function install() {
		...
		/* Cột `don_vi` = ĐƠN VỊ (K&H · POSH) — xem `VHCPMTD_DonVi`. Ghi MỘT LẦN lúc lập đơn, theo
		 ...
		 *    lượt đổi chữ thành một lượt sửa dữ liệu. Xem `VHCPMTD_Don::LUONG_MA`.
		 * ══════════════════════════════════════════════════════════════════════════════════ */
		...
		update_option( 'vhcpmtd_db_version', self::SCHEMA_VERSION );
		update_option( 'vhcpmtd_flush_rewrite', 1 );   // để vhcpmtd_flush_rewrite() nạp lại /chi-phi/ và /hop-dong/
		VHCPMTD_Cfg::seed();
		/* Bật "Xác nhận quyết toán" cho các vai kế toán trên bảng quyền ĐANG LƯU — xem chú
		   thích dài ở `VHCPMTD_Cfg::va_quyen_quyet_toan()`. Đặt SAU `seed()` vì bảng quyền phải
		   có mặt trước đã. */
	 ...
	 *      · `VHCPMTD_Cfg::MANG` (= 'CH_MangTK') — MẢNG KINH DOANH: Event · Farm · Funzone · TuTu,
	 *        thứ quyết định mã tài khoản 641x của một dòng chi;
	 *      · `VHCPMTD_TraMa` — NGUỒN của một dòng: 'sochi' · 'don' · 'kt' · 'mkt'.
	 *    Bản 1.217.x thêm nghĩa thứ BA cho cùng chữ ấy (KVC · MTĐ · VP), và hằng mới
	 *    `VHCPMTD_DB::MANG` chỉ khác `VHCPMTD_Cfg::MANG` đúng một tên lớp. Ở đây đọc nhầm nghĩa là
