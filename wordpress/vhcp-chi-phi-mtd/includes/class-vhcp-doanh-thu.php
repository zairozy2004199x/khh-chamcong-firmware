 ...
 * ⚠️ Chi phí = THỰC CHI theo `VHCPMTD_Don::thuc_chi()` (chưa cấp tiền → 0), tính theo NGÀY của dòng
 *    hạng mục rơi trong tháng — cùng tháng với doanh thu, không theo kỳ của đơn.
 */
class VHCPMTD_DoanhThu {

	const O_URL  = 'vhcpmtd_dt_url';
	const O_KHOA = 'vhcpmtd_dt_khoa';
	...
	public static function luu( $d ) {
		...
		if ( null !== $url ) {
			if ( '' !== $url && ! preg_match( '#^https?://[^\s/]+#i', $url ) ) {
				return VHCPMTD_Util::err( 'Địa chỉ web Doanh thu phải bắt đầu bằng http:// hoặc https://' );
			}
		...
		self::quen();
		VHCPMTD_Log::log_action( array( 'actor' => VHCPMTD_Auth::nguoi(), 'role' => VHCPMTD_Auth::vai_tro(), 'action' => 'Sửa kết nối web Doanh thu',
	...
	public static function quen() {
		update_option( 'vhcpmtd_dt_doi', (string) microtime( true ), false );
	}

	private static function khoa_nho( $tu, $den ) {
		return 'vhcpmtd_dt_' . md5( self::url() . '|' . (string) get_option( 'vhcpmtd_dt_doi', '' ) . '|' . $tu . '|' . $den );
	}
	...
	public static function kiem_tra() {
		...
		return VHCPMTD_Util::ok( array( 'soCuaHang' => count( $kq['cuaHang'] ), 'tu' => $tu, 'den' => $den,
	...
	public static function khoang_thang( $thang ) {
		$t = trim( (string) $thang );
		if ( ! preg_match( '/^(\d{4})-(\d{2})$/', $t, $m ) ) { $t = VHCPMTD_Util::now()->format( 'Y-m' ); preg_match( '/^(\d{4})-(\d{2})$/', $t, $m ); }
	 ...
	 * `$khoi` (mb/mn/…) rỗng = cả hệ; đơn chưa đóng dấu khối vẫn tính (cùng luật `VHCPMTD_Report::finance`).
	 */
	public static function chi_phi_coso( $tu, $den, $khoi = '' ) {
		...
		foreach ( VHCPMTD_Don::cp_rows() as $r ) {
			...
			$ngay = VHCPMTD_Util::parse_date( isset( $r['ngay'] ) ? $r['ngay'] : '' );
			if ( null === $ngay || $ngay < $tu || $ngay > $den ) { continue; }
			$eff = VHCPMTD_Don::thuc_chi( $r['thanh_tien'], $r['thuc_mua'], $tt_don[ $m ] );
			if ( ! $eff ) { continue; }
	...
	public static function so_sanh( $a = array() ) {
		...
		$cfg  = VHCPMTD_Cfg::get_config();
		$coso = isset( $cfg['coso'] ) ? VHCPMTD_Cfg::coso_theo_don_vi( $cfg['coso'] ) : array();
	 ...
	 *
	 * Vào: `$items` = mảng của `VHCPMTD_DuAn::list_du_an()` (mỗi phần tử có `ten`, `loai`). Ra: CÙNG
	...
	public static function doi_gian( array $items ) {
		try {
			...
			$coso_ds = class_exists( 'VHCPMTD_Cfg' ) ? (array) VHCPMTD_Cfg::cfg_static()['coso'] : array();
			...
			$can = array(); $hom_nay = VHCPMTD_Util::now()->format( 'Y-m-d' );
			foreach ( $items as $i => $x ) {
				...
				$dong_ngay = VHCPMTD_Util::parse_date( isset( $c['dongCua'] ) ? $c['dongCua'] : '' );
