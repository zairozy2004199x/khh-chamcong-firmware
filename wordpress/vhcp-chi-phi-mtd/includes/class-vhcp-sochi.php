...
class VHCPMTD_SoChi {
	...
	public static function don_vi_cua( $khoa, $kieu = 'id' ) {
		...
		return VHCPMTD_DonVi::cua_coso( isset( $r['coso'] ) ? $r['coso'] : '' );
	}
	...
	public static function resolve_tk( $rec ) {
		...
		return VHCPMTD_Cfg::resolve_tk( $g( 'loai' ), $g( 'hinhThuc' ), array( 'tkNo' => $g( 'tkNo' ), 'tkCo' => $g( 'tkCo' ), 'maDt' => $g( 'maDt' ) ), $g( 'coso' ) );
	}
	...
	private static function data( $rec, $nguoi = '' ) {
		...
		$st = VHCPMTD_Util::blank_or_num( $g( 'soTien' ) );
		if ( $st === null ) { $st = VHCPMTD_Util::num( $sl ) * VHCPMTD_Util::num( $dg ); }

		$ts    = VHCPMTD_Util::blank_or_num( $g( 'thueSuat' ) );
		$tthue = ( $ts === null ) ? null : round( $st * $ts / 100 );

		$ngay = VHCPMTD_Util::parse_date( $g( 'ngay' ) );
		if ( ! $ngay ) { $ngay = VHCPMTD_Util::today_sql(); }
		$ky = VHCPMTD_Util::st( $g( 'ky' ) );
		...
		$d = array(
			'ngay'       => $ngay,
			'ky'         => $ky,
			'coso'       => VHCPMTD_Util::st( $g( 'coso' ) ),
			'loai'       => VHCPMTD_Util::st( $g( 'loai' ) ),
			'tk_no'      => $tk['tk_no'],
			'tk_co'      => $tk['tk_co'],
			'ma_dt'      => $tk['ma_dt'],
			'doi_tuong'  => VHCPMTD_Util::st( $g( 'doiTuong' ) ),
			'noi_dung'   => VHCPMTD_Util::st( $g( 'noiDung' ) ),
			'dvt'        => VHCPMTD_Util::st( $g( 'dvt' ) ),
			'so_luong'   => $sl,
			'don_gia'    => $dg,
			'so_tien'    => $st,
			...
			'thue_suat'  => $ts,
			'tien_thue'  => $tthue,
			'ghi_chu'    => VHCPMTD_Util::st( $g( 'ghiChu' ) ),
			...
			'ma_du_an'   => VHCPMTD_Util::st( $g( 'maDuAn' ) ),
			'hang_muc'   => VHCPMTD_Util::st( $g( 'hangMuc' ) ),
			'du_toan'    => VHCPMTD_Util::blank_or_num( $g( 'duToan' ) ),
		...
		$nx = VHCPMTD_Util::parse_date( $g( 'ngayXuat' ) );
	...
	public static function add( $rec, $nguoi = '' ) {
		global $wpdb;
		$rec = (array) $rec;
		if ( VHCPMTD_Util::st( isset( $rec['loai'] ) ? $rec['loai'] : '' ) === '' ) { return VHCPMTD_Util::err( 'Chọn loại chi phí' ); }
		...
		$ok = $wpdb->insert( VHCPMTD_DB::t( 'so_chi' ), $data );
		if ( ! $ok ) {
			$ct = trim( (string) $wpdb->last_error );
			return VHCPMTD_Util::err( 'Không ghi được dòng sổ chi' . ( '' !== $ct ? ' — ' . $ct : '.' ) );
		}
	...
	public static function update( $id, $rec, $nguoi = '' ) {
		global $wpdb;
		$cur = self::row( $id );
		if ( ! $cur ) { return VHCPMTD_Util::err( 'Không tìm thấy dòng chi' ); }
		if ( $cur['ngay_xuat'] ) { return VHCPMTD_Util::err( 'Dòng đã xuất MISA — không sửa (bỏ chốt xuất trước nếu cần)' ); }
		$rec  = (array) $rec;
		$data = self::data( $rec, $nguoi !== '' ? $nguoi : (string) $cur['nguoi_nhap'] );
		$wpdb->update( VHCPMTD_DB::t( 'so_chi' ), $data, array( 'id' => (string) $id ) );
		return VHCPMTD_Util::ok( array( 'tkNo' => $data['tk_no'], 'tkCo' => $data['tk_co'] ) );
	}
	...
	public static function delete( $id ) {
		...
		$cur = self::row( $id );
		if ( ! $cur ) { return VHCPMTD_Util::err( 'Không tìm thấy dòng chi' ); }
		if ( $cur['ngay_xuat'] ) { return VHCPMTD_Util::err( 'Dòng đã xuất MISA — không xóa' ); }
		$wpdb->delete( VHCPMTD_DB::t( 'so_chi' ), array( 'id' => (string) $id ) );
	...
	private static function out( $r ) {
		return array(
			...
			'dvt'       => (string) $r['dvt'],
			'soLuong'   => VHCPMTD_Util::out_num( $r['so_luong'] ),
			'donGia'    => VHCPMTD_Util::out_num( $r['don_gia'] ),
			'soTien'    => VHCPMTD_Util::num( $r['so_tien'] ),
			'hinhThuc'  => (string) $r['hinh_thuc'],
			'vat'       => (string) $r['vat'],
			'thueSuat'  => VHCPMTD_Util::out_num( $r['thue_suat'] ),
			'tienThue'  => VHCPMTD_Util::num( $r['tien_thue'] ),
			'maDuAn'    => isset( $r['ma_du_an'] ) ? (string) $r['ma_du_an'] : '',
			'hangMuc'   => isset( $r['hang_muc'] ) ? (string) $r['hang_muc'] : '',
			'duToan'    => isset( $r['du_toan'] ) ? VHCPMTD_Util::out_num( $r['du_toan'] ) : '',
			'hoSo'      => isset( $r['ho_so'] ) ? (string) $r['ho_so'] : '',
			'ghiChu'    => (string) $r['ghi_chu'],
			'anh'       => (string) $r['anh'],
			'nguoiNhap' => (string) $r['nguoi_nhap'],
			'daXuat'    => ( VHCPMTD_Util::fmt( $r['ngay_xuat'] ) !== '' ),
			'ngayXuat'  => VHCPMTD_Util::fmt( $r['ngay_xuat'] ),
		);
	}
	...
	public static function list_chi( $opts = array() ) {
		...
		foreach ( self::all_rows() as $r ) {
			if ( null !== $dv_xem && ! VHCPMTD_DonVi::xem_duoc_coso( isset( $r['coso'] ) ? $r['coso'] : '' ) ) { continue; }
			...
			if ( ! VHCPMTD_Auth::xem_duoc_loai( isset( $r['loai'] ) ? $r['loai'] : '' ) ) { continue; }
			/* ══════════════════════════════════════════════════════════════════════════════
			...
			if ( '' !== $cs_dong && ! VHCPMTD_Auth::trong_tam(
			...
			if ( $f_tk !== 'all' && ! VHCPMTD_Cfg::tk_thuoc_cay( $r['tk_no'], $f_tk ) ) { continue; }
			if ( $f_da !== 'all' ) {
				if ( $f_da === '(khong)' ) { if ( $mda !== '' ) { continue; } }
				elseif ( $mda !== $f_da ) { continue; }
			}
			$da_xuat = ( VHCPMTD_Util::fmt( $r['ngay_xuat'] ) !== '' );
			...
			if ( $mda !== '' ) {
				...
				$by_da[ $mda ]['duToan'] += VHCPMTD_Util::num( $x['duToan'] );
				$by_da[ $mda ]['n']++;
			}
		}
		...
		usort( $by_ky, function ( $a, $b ) { return VHCPMTD_Util::ky_num( $b['ky'] ) <=> VHCPMTD_Util::ky_num( $a['ky'] ); } );

		$ky_list = array_keys( $ky_set );
		usort( $ky_list, function ( $a, $b ) { return VHCPMTD_Util::ky_num( $b ) <=> VHCPMTD_Util::ky_num( $a ); } );
		...
		$tk_cha  = VHCPMTD_Cfg::tk_cha_ds( $tk_list );
	...
	public static function theo_du_an( $ma_du_an ) {
		...
		$rows = VHCPMTD_DB::rows( $wpdb->prepare( "SELECT * FROM $t WHERE ma_du_an=%s ORDER BY stt ASC", $ma ) );
		if ( ! count( $rows ) ) {
			$rows = VHCPMTD_DB::rows( $wpdb->prepare( "SELECT * FROM $t WHERE LOWER(ma_du_an)=LOWER(%s) ORDER BY stt ASC", $ma ) );
		}
		$out = array();
	...
	public static function gan_ma_tai_khoan( $all = false ) {
		...
		$n = 0; $chua = array();
		foreach ( self::all_rows() as $r ) {
			if ( VHCPMTD_Util::fmt( $r['ngay_xuat'] ) !== '' ) { continue; }
			...
			$giu = VHCPMTD_Cfg::ma_con_hop_le( $r['loai'], $r['coso'], $r['tk_no'] );
		...
		return VHCPMTD_Util::ok( array( 'updated' => $n, 'thieuMa' => array_keys( $chua ) ) );
	}
	...
	public static function export_misa( $ky = 'all', $mode = 'chuaxuat' ) {
		...
		foreach ( self::all_rows() as $r ) {
			...
			$so_tien = VHCPMTD_Util::num( $r['so_tien'] );
			if ( ! $so_tien ) { continue; }

			$coso  = (string) $r['coso'];
			$loai  = (string) $r['loai'];
			// TK Nợ theo LUẬT LÚC XUẤT (VHCPMTD_Cfg::tkno_xuat) — chung với 4 đường xuất kia:
			...
			$tk_no = VHCPMTD_Cfg::tkno_xuat( $loai, $coso, $r['tk_no'] );
			$tk_co = trim( (string) $r['tk_co'] );
			$ma_dv = isset( $m_unit[ $coso ] ) ? $m_unit[ $coso ] : '';
			$tenm  = ! empty( $m_tm[ $coso ] ) ? $m_tm[ $coso ] : $coso;
			$ngay  = VHCPMTD_Util::fmt( $r['ngay'] );
			VHCPMTD_Misa::gom_ngay_xau( $ngay_xau, $ngay, $r['ngay'], (string) $r['id'] );
			...
			$ten_xuat = VHCPMTD_Cfg::ten_misa_loai( $loai );
			$mda   = isset( $r['ma_du_an'] ) ? trim( (string) $r['ma_du_an'] ) : '';
			$dg1   = VHCPMTD_Util::j( array( $ten_xuat, $coso, $r['ky'] ) ) . ( $is_tt ? '_Trực tiếp NCC' : '_Tạm ứng NV' );
			// Dòng thuộc dự án thì ghi mã dự án vào diễn giải để bên kế toán dò lại được
			$dg2   = VHCPMTD_Util::j( array( $mda, (string) $r['noi_dung'], $tenm ) );
			$gc    = trim( (string) $r['ghi_chu'] );
			if ( $gc !== '' ) { $dg2 .= '_' . $gc; }

			$rows[] = array( $ngay, $ngay, '', $dg1, $dg2, VHCPMTD_Util::ma_so( $tk_no ), VHCPMTD_Util::ma_so( $tk_co ), $so_tien, VHCPMTD_Util::ma_so( (string) $r['ma_dt'] ), VHCPMTD_Util::ma_so( $ma_dv ) );
			$ids[]  = (string) $r['id'];
		}

		return array(
			'cols'   => VHCPMTD_Misa::cols(),
			'rows'   => $rows,
			'count'  => count( $rows ),
			'sodon'  => count( $ids ),
			'warn'   => array_merge( array_keys( $warn ), VHCPMTD_Misa::warn_ngay_xau( $ngay_xau ) ),
			'maDons' => $ids,
		);
	}
	...
	public static function mark_exported( $ids ) {
		global $wpdb;
		$ids = (array) $ids;
		if ( ! count( $ids ) ) { return VHCPMTD_Util::err( 'Không có dòng nào để chốt' ); }
		$now = VHCPMTD_Util::now_sql();
		$n   = 0;
		foreach ( $ids as $id ) {
			$r = self::row( $id );
			if ( ! $r || VHCPMTD_Util::fmt( $r['ngay_xuat'] ) !== '' ) { continue; }
			$wpdb->update( VHCPMTD_DB::t( 'so_chi' ), array( 'ngay_xuat' => $now ), array( 'id' => (string) $id ) );
			$n++;
		}
		return VHCPMTD_Util::ok( array( 'count' => $n ) );
	}
	...
	public static function unmark_exported( $ids ) {
		...
		foreach ( (array) $ids as $id ) {
			$wpdb->update( VHCPMTD_DB::t( 'so_chi' ), array( 'ngay_xuat' => null ), array( 'id' => (string) $id ) );
			$n++;
		}
		return VHCPMTD_Util::ok( array( 'count' => $n ) );
	}
