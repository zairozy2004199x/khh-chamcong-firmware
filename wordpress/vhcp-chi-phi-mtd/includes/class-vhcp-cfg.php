 ...
 *
 * Mỗi "sheet cấu hình" là các dòng trong bảng vhcpmtd_cfg (cột `bang` = tên sheet cũ,
...
class VHCPMTD_Cfg {
	...
	public static function dong_cua_coso( $ten, $dong = true, $nguoi = '' ) {
		$ten = trim( (string) $ten );
		if ( $ten === '' ) { return VHCPMTD_Util::err( 'Chọn cơ sở' ); }
		$k = mb_strtolower( $ten );

		$rows = array(); $thay = false;
		$ngay = $dong ? VHCPMTD_Util::now()->format( 'd/m/Y' ) : '';
		...
		if ( ! $thay ) { return VHCPMTD_Util::err( 'Không thấy cơ sở "' . $ten . '" trong bảng cơ sở' ); }
		...
		$n = 0;
		if ( $dong ) {
			foreach ( VHCPMTD_Don::list_dons() as $d ) {
				if ( mb_strtolower( trim( (string) $d['coso'] ) ) !== $k ) { continue; }
				if ( ! empty( $d['tatToan'] ) ) { continue; }
				VHCPMTD_Don::set_tat_toan_tuan( $d['maDon'], true, ( $nguoi !== '' ? $nguoi : 'Đóng gian hàng' ) );
				$n++;
			}
		}
		...
		return VHCPMTD_Util::ok( array( 'coso' => $ten, 'ngay' => $ngay, 'soDonTatToan' => $n ) );
	}
	 ...
	 *    Mã lạ cũng về rỗng: `VHCPMTD_Don::luong_don()` là nơi duy nhất quyết định mã nào có thật,
	...
	public static function luong_cua_bo_phan( $bp ) {
		...
		foreach ( self::read( self::BP ) as $r ) {
			...
			return VHCPMTD_Don::luong_don( array( 'luong' => isset( $r[1] ) ? $r[1] : '' ) );
		}
		return '';
	}
	 ...
	 *    đó là bộ phận sẽ đi duyệt đơn này. `VHCPMTD_Auth::bo_phan_bo()` trả bộ phận của phiên
	...
	public static function vai_tuy_bien() {
		...
		foreach ( self::read( self::VAI ) as $r ) {
			 ...
			 *    (`VHCPMTD_Auth::dat_vai_tro()` vốn đã đọc đúng chỗ ấy — cột của vai chưa từng
	...
	/* ⚠️ `bo_phan_cua_nguoi( $ten_vai )` ĐÃ BỎ — anh Thắng 21/09/2026: *"bỏ vai trò đi, cho
	   ...
	   (`VHCPMTD_Auth::dat_vai_tro()`), một nơi duy nhất. */
		...
		return array(
			...
			/* 🔴 AI BẤM ĐƯỢC NÚT "＋ TẠO ĐƠN MỚI" — anh Thắng 26/09/2026, đơn chi phí Vận Hành:
			   *"giờ check kế toán sao chưa có nút tạo đơn"*. Trước bản này nút gác cứng theo
			   ...
			   vai nào ở `createDon` (`VHCPMTD_Api::required_roles()`), nên đây chỉ là màn hình —
	...
	public static function read_all() {
		$t    = VHCPMTD_DB::t( 'cfg' );
		$out  = array();
		foreach ( VHCPMTD_DB::rows( "SELECT bang, cols FROM $t ORDER BY bang ASC, stt ASC, id ASC" ) as $r ) {
			$bang = (string) $r['bang'];
			$a    = json_decode( $r['cols'], true );
			if ( ! is_array( $a ) ) { $a = array(); }
			$n = count( self::headers( $bang ) );
	...
	public static function write( $bang, $rows, $snapshot = true ) {
		...
		$t = VHCPMTD_DB::t( 'cfg' );
		if ( $snapshot ) {
			VHCPMTD_Meta::set_json( 'cfg_undo', array( 'name' => $bang, 'data' => self::read( $bang ) ) );
		}
	...
	public static function nhan_coso_ngoai( $ten, $don_vi ) {
		...
		self::append( self::COSO, array( $ten, '', '', '', '', VHCPMTD_DonVi::chuan( $don_vi ) ) );
		return true;
	}
	 ...
	 *
	 * ⚠️ MẶC ĐỊNH RỖNG NGHĨA LÀ "NHÀ MẶC ĐỊNH CỦA CHÍNH BẢN NÀY" (`VHCPMTD_DonVi::chuan()` lo phần
	 *    ấy), chứ không phải "không có nhà". Bản gốc giữ 'POSH'; script tách đặt rỗng cho bản
	 *    vùng. Khai lại được bằng khoá `vhcpmtd_dv_ghe` nếu ngày nào cần khác.
	 * ══════════════════════════════════════════════════════════════════════════════════════════ */
	const DON_VI_GHE = '';
	 ...
	 *    đời site (xem cờ `vhcpmtd_hut_coso_ghe` ở `vhcp-chi-phi-mtd.php`). Xóa là ở yên; muốn hút lại thì
	...
	public static function bao_coso_ghe() {
		$v = get_option( 'vhcpmtd_bao_coso_ghe', null );
	...
	public static function don_vi_ghe() {
		$v = get_option( 'vhcpmtd_dv_ghe', null );
		return is_string( $v ) ? trim( $v ) : self::DON_VI_GHE;
	}

	/**
	 * Tai nghe cho móc `vhg_coso_da_luu` của plugin Ghế — xem chỗ đăng ký ở `vhcp-chi-phi-mtd.php`.
	 *
	...
	public static function hut_coso_ghe_api() {
		...
		return array( 'ok' => true, 'them' => $n, 'thongBao' => $n
			? ( 'Đã thêm ' . $n . ' cơ sở từ bên Ghế, gắn đơn vị ' . VHCPMTD_DonVi::chuan( self::don_vi_ghe() ) . '.' )
	...
	private static function bao_coso_posh_( $rows ) {
		...
		foreach ( (array) $rows as $r ) {
			...
			$dv = VHCPMTD_DonVi::chuan( isset( $r[5] ) ? $r[5] : '' );
			...
			if ( '' === $tn || VHCPMTD_DonVi::chuan( self::don_vi_ghe() ) !== $dv ) { continue; }
			do_action( 'vhcpmtd_coso_posh_da_luu', $tn );
		}
	}
	...
	public static function clear_cache() {
		...
		self::$memo = null;
		self::$dm_memo = array();
		wp_cache_delete( 'vhcpmtd_cfgstatic', 'vhcpmtd' );
		wp_cache_delete( 'vhcpmtd_quyen', 'vhcpmtd' );
		wp_cache_delete( 'vhcpmtd_ssomap', 'vhcpmtd' );
		delete_transient( 'vhcpmtd_cfgstatic' );
		delete_transient( 'vhcpmtd_quyen' );
		delete_transient( 'vhcpmtd_ssomap' );
	}
	...
	private static function seed_from( $all ) {
		...
		if ( ! VHCPMTD_Meta::get( 'seeded_thaodo_setup_v2' ) ) {
			$did  = true;
			...
			VHCPMTD_Meta::set( 'seeded_thaodo_setup_v2', '1' );
		}
		...
		/* LOẠI "Chi phí cơ sở" MỞ THÊM CHO BỘ PHẬN KỸ THUẬT — anh Thắng 11/09/2026:
		   *"bổ sung loại chi phí ( chi phí cơ sở )"*, sau khi ô Loại chi phí của đơn Kỹ thuật
		   ...
		   ⚠️ LOẠI CHƯA KHAI BỘ PHẬN NÀO THÌ ĐỂ YÊN: bỏ trống nghĩa là DÙNG CHUNG cho mọi bộ
		      ...
		      ngược điều đang cần. */
		if ( ! VHCPMTD_Meta::get( 'seeded_coso_kythuat_v1' ) ) {
			$did  = true;
			...
			VHCPMTD_Meta::set( 'seeded_coso_kythuat_v1', '1' );
		}
		 ...
		 *
		 * Khối mặc định là khối của CHÍNH BẢN ĐANG CHẠY (`VHCPMTD_DB::khoi()`): dữ liệu đang có ở
	...
	public static function cfg_static() {
		...
		$hit = get_transient( 'vhcpmtd_cfgstatic' );
		...
		foreach ( self::rows_of( $all, self::COSO ) as $r ) {
			...
			$out['coso'][] = array( 'ten' => $r[0], 'maDonVi' => VHCPMTD_Util::ma_so( $r[1] ), 'phanLoaiLon' => $r[2], 'tenMisa' => $r[3], 'dongCua' => isset( $r[4] ) ? (string) $r[4] : '',
				'donVi' => VHCPMTD_DonVi::chuan( isset( $r[5] ) ? $r[5] : '' ),
		...
		foreach ( self::rows_of( $all, self::NHOM ) as $r ) {
			if ( trim( (string) $r[0] ) === '' ) { continue; }
			$out['nhom'][] = array( 'ten' => $r[0], 'loai' => ( $r[1] !== '' ? $r[1] : 'canhan' ), 'tkNo' => VHCPMTD_Util::ma_so( $r[2] ), 'boPhan' => $r[3] );
		}
		foreach ( self::rows_of( $all, self::LOAI ) as $r ) {
			...
			$out['loaiChiPhi'][] = array( 'ten' => $r[0], 'tkNo' => VHCPMTD_Util::ma_so( $r[1] ), 'tkCo' => VHCPMTD_Util::ma_so( $r[2] ), 'maDt' => VHCPMTD_Util::ma_so( $r[3] ), 'boPhan' => $r[4], 'note' => $r[5], 'tenMisa' => isset( $r[6] ) ? $r[6] : '', 'loaiTt' => isset( $r[7] ) ? $r[7] : '', 'donVi' => isset( $r[8] ) ? $r[8] : '', 'khoi' => isset( $r[9] ) ? $r[9] : '', 'vaiTro' => isset( $r[10] ) ? $r[10] : '', 'dauMuc' => isset( $r[11] ) ? $r[11] : '', 'cha' => isset( $r[12] ) ? $r[12] : '' );
		}
		foreach ( self::rows_of( $all, self::TKNO ) as $r ) {
			if ( trim( (string) $r[0] ) === '' ) { continue; }
			$out['tkNoMatrix'][] = array( 'nhom' => $r[0], 'pll' => $r[1], 'tkNo' => VHCPMTD_Util::ma_so( $r[2] ) );
		}
		...
		foreach ( self::rows_of( $all, self::BP ) as $r ) {
			$t = trim( (string) ( isset( $r[0] ) ? $r[0] : '' ) );
			if ( '' === $t ) { continue; }
			$lg = VHCPMTD_Don::luong_don( array( 'luong' => isset( $r[1] ) ? $r[1] : '' ) );
		...
		foreach ( self::rows_of( $all, self::USER ) as $r ) {
			...
			$out['users'][] = array( 'ten' => $r[0], 'pin' => VHCPMTD_Util::pin_sach( $r[1] ), 'vaiTro' => ( $r[2] !== '' ? $r[2] : 'Nhân viên' ), 'coso' => $r[3], 'tkCo' => VHCPMTD_Util::ma_so( $r[4] ), 'maDt' => VHCPMTD_Util::ma_so( $r[5] ), 'boPhan' => $r[6],
				'donVi' => isset( $r[7] ) ? trim( (string) $r[7] ) : '',
				'xemDonVi' => isset( $r[8] ) ? trim( (string) $r[8] ) : '',
				/* MÃ NV — khoá thứ hai, xem khối dài ở `VHCPMTD_Auth::login()`. Ô cuối cùng nên
				   dòng cũ chín ô không có nó; `isset` lo phần ấy. */
				'maNv' => isset( $r[9] ) ? VHCPMTD_Util::ma_so( $r[9] ) : '',
				/* KHỐI — anh Thắng 21/09/2026: *"chỗ đơn vị thay bằng khối, tích nếu 1 người làm 2
				   ...
				   "không thuộc khối nào" — xem `VHCPMTD_DonVi::khoi_xem_duoc()`. */
		...
		/* Cơ sở -> ĐƠN VỊ. Tra nhanh, khoá đã hạ chữ thường — mọi màn hỏi "dòng chi này của
		   bên nào" đều đi qua bảng này, xem `VHCPMTD_DonVi::cua_coso()`. */
		$out['cosoDonVi'] = array();
		...
		foreach ( $out['coso'] as $x ) {
			...
			if ( $k !== '' ) {
				...
				$out['cosoDonVi'][ $k ] = VHCPMTD_DonVi::chuan( isset( $x['donVi'] ) ? $x['donVi'] : '' );
				$bp = isset( $x['boPhan'] ) ? (string) $x['boPhan'] : '';
				if ( '' === $bp ) {
					$kh = VHCPMTD_DonVi::khoi_cua( isset( $x['donVi'] ) ? $x['donVi'] : '' );
		...
		set_transient( 'vhcpmtd_cfgstatic', $out, 300 );
		self::$memo = $out;
		return $out;
	}
	...
	public static function get_config( $cp_data = null ) {
		...
		if ( $cp_data === null ) { $cp_data = VHCPMTD_Don::cp_rows(); }
	...
	public static function coso_theo_don_vi( $ds ) {
		if ( ! class_exists( 'VHCPMTD_DonVi' ) || null === VHCPMTD_DonVi::xem_duoc() ) { return $ds; }
		$ra = array();
		foreach ( (array) $ds as $x ) {
			if ( VHCPMTD_DonVi::duoc_xem( isset( $x['donVi'] ) ? $x['donVi'] : '' ) ) { $ra[] = $x; }
		}
		return $ra;
	}
	...
	public static function save_config( $cfg ) {
		...
		if ( isset( $cfg['users'] ) && is_array( $cfg['users'] ) ) {
			...
			foreach ( $cfg['users'] as $x0 ) {
				...
				if ( '' !== $t0 ) {
					if ( isset( $da_ten[ $t0 ] ) ) {
						return VHCPMTD_Util::err( 'Không lưu: có HAI dòng cùng tên "'
				...
				if ( '' !== $m0 ) {
					$mk = mb_strtolower( $m0 );
					if ( isset( $da_mnv[ $mk ] ) ) {
						return VHCPMTD_Util::err( 'Không lưu: mã NV "' . $m0 . '" đang khai cho HAI '
		...
		if ( isset( $cfg['coso'] ) && is_array( $cfg['coso'] ) ) {
			 ...
			 * ══════════════════════════════════════════════════════════════════════════════ */
			if ( class_exists( 'VHCPMTD_DonVi' ) && null !== VHCPMTD_DonVi::xem_duoc() ) {
				$giu = array();
				foreach ( self::read( self::COSO ) as $r0 ) {
					$r0 = array_values( (array) $r0 );
					$t0 = trim( (string) ( isset( $r0[0] ) ? $r0[0] : '' ) );
					if ( '' === $t0 ) { continue; }
					if ( ! VHCPMTD_DonVi::duoc_xem( isset( $r0[5] ) ? $r0[5] : '' ) ) { $giu[] = $r0; }
				}
		...
		if ( isset( $cfg['loaiChiPhi'] ) && is_array( $cfg['loaiChiPhi'] ) ) {
			...
			foreach ( $cfg['loaiChiPhi'] as $x ) {
				...
				if ( '' === $kh ) { $kh = VHCPMTD_DB::khoi(); }
				$rows[] = array( $tn, VHCPMTD_Util::ma_so( $g( $x, 'tkNo' ) ), VHCPMTD_Util::ma_so( $g( $x, 'tkCo' ) ), VHCPMTD_Util::ma_so( $g( $x, 'maDt' ) ), $g( $x, 'boPhan' ), $nt, $g( $x, 'tenMisa' ), $g( $x, 'loaiTt' ), $g( $x, 'donVi' ), $kh, $g( $x, 'vaiTro' ), $g( $x, 'dauMuc' ), $g( $x, 'cha' ) );
			}
		...
		if ( isset( $cfg['tkNoMatrix'] ) && is_array( $cfg['tkNoMatrix'] ) ) {
			$rows = array();
			foreach ( $cfg['tkNoMatrix'] as $x ) { $x = (array) $x; $rows[] = array( $g( $x, 'nhom' ), $g( $x, 'pll' ), VHCPMTD_Util::ma_so( $g( $x, 'tkNo' ) ) ); }
		...
		if ( isset( $cfg['users'] ) && is_array( $cfg['users'] ) ) {
			...
			if ( VHCPMTD_Auth::vai_tro() !== '' && VHCPMTD_Auth::vai_tro() !== 'Admin' ) {
				$admin_cu = array();
			...
			foreach ( $cfg['users'] as $x ) {
				...
				$rows[] = array(
					$g( $x, 'ten' ),
					VHCPMTD_Util::pin_sach( $g( $x, 'pin' ) ),
					( $g( $x, 'vaiTro' ) !== '' ? $g( $x, 'vaiTro' ) : 'Nhân viên' ),
					$g( $x, 'coso' ),
					VHCPMTD_Util::ma_so( $g( $x, 'tkCo' ) ),
					VHCPMTD_Util::ma_so( $g( $x, 'maDt' ) ),
					$g( $x, 'boPhan' ),
					$g( $x, 'donVi' ),
					$g( $x, 'xemDonVi' ),
					VHCPMTD_Util::ma_so( $g( $x, 'maNv' ) ),
					$g( $x, 'khoi' )
				);
			}
		...
		if ( isset( $cfg['vaiTro'] ) && is_array( $cfg['vaiTro'] ) ) {
			...
			if ( 'Admin' !== VHCPMTD_Auth::vai_tro() ) {
				return VHCPMTD_Util::err( 'Chỉ Admin mới thêm/sửa vai trò được.' );
			}
			$rows = array();
		...
		if ( isset( $cfg['boPhanDs'] ) && is_array( $cfg['boPhanDs'] ) ) {
			...
			if ( 'Admin' !== VHCPMTD_Auth::vai_tro() ) {
				return VHCPMTD_Util::err( 'Chỉ Admin mới thêm/sửa bộ phận được.' );
			}
			$rows = array(); $da = array();
			foreach ( $cfg['boPhanDs'] as $x ) {
				...
				/* 🔴 CỘT LUỒNG PHẢI ĐI THEO, và phải chịu được CẢ HAI dạng gửi lên. Màn cũ (và
				   ...
				   ⚠️ Chuẩn hoá qua `VHCPMTD_Don::luong_don()` — một nơi duy nhất biết mã nào có
				      thật, và mã lạ về rỗng = "theo khối như cũ". */
				$lg = is_array( $x ) && isset( $x['luong'] )
					? VHCPMTD_Don::luong_don( array( 'luong' => $x['luong'] ) )
			...
			if ( ! $rows ) {
				return VHCPMTD_Util::err( 'Phải còn ít nhất một bộ phận. Xoá hết thì hệ tự dùng lại danh sách mặc định, không phải "không có bộ phận nào".' );
			}
		...
		if ( isset( $cfg['vending'] ) && is_array( $cfg['vending'] ) ) {
			/* 🔴 CHỈ ADMIN — địa chỉ + khoá chia sẻ sang web Vending HCMC. Khoá rỗng = giữ (xem `VHCPMTD_Vending::luu`). */
			if ( 'Admin' !== VHCPMTD_Auth::vai_tro() ) { return VHCPMTD_Util::err( 'Chỉ Admin mới đổi kết nối web Vending được.' ); }
			$kq_vd = VHCPMTD_Vending::luu( $cfg['vending'] );
		...
		if ( isset( $cfg['tinhNang'] ) && is_array( $cfg['tinhNang'] ) ) {
			...
			if ( 'Admin' !== VHCPMTD_Auth::vai_tro() ) {
				return VHCPMTD_Util::err( 'Chỉ Admin mới bật/tắt tính năng được.' );
			}
			$luu = self::tinh_nang_luu();
			foreach ( $cfg['tinhNang'] as $ma => $tt ) {
				$ma = (string) $ma; $tt = mb_strtolower( trim( (string) $tt ) );
				if ( ! isset( self::TINH_NANG[ $ma ] ) ) { return VHCPMTD_Util::err( 'Không có tính năng mã "' . $ma . '" trong bản này.' ); }
				if ( ! in_array( $tt, self::TN_TRANG_THAI, true ) ) { return VHCPMTD_Util::err( 'Trạng thái "' . $tt . '" không hợp lệ — chỉ tat · admin · bat.' ); }
				$luu[ $ma ] = $tt;
			}
			$rows = array();
			foreach ( $luu as $ma => $tt ) { $rows[] = array( $ma, $tt ); }
			self::write( self::TN, $rows );
			VHCPMTD_Log::log_action( array( 'actor' => VHCPMTD_Auth::nguoi(), 'role' => VHCPMTD_Auth::vai_tro(), 'action' => 'Đổi cờ tính năng',
	...
	public static function undo_config() {
		$snap = VHCPMTD_Meta::get_json( 'cfg_undo', null );
		if ( ! $snap || empty( $snap['name'] ) ) { return VHCPMTD_Util::err( 'Chưa có lần lưu nào để hồi lại' ); }
		self::write( $snap['name'], isset( $snap['data'] ) ? $snap['data'] : array(), false );
		VHCPMTD_Meta::del( 'cfg_undo' );
		self::clear_cache();
		return VHCPMTD_Util::ok( array( 'name' => $snap['name'] ) );
	}
	...
	private static function quyen_cot_dang_luu() {
		$m = VHCPMTD_Meta::get_json( self::QUYEN_COT_O, array() );
	...
	public static function va_cot_quyen_them_vai() {
		...
		if ( ! $rows ) {
			VHCPMTD_Meta::set_json( self::QUYEN_COT_O, $roles );
			return 0;
		}
	...
	public static function get_quyen() {
		$hit = get_transient( 'vhcpmtd_quyen' );
		...
		foreach ( self::read( self::QUYEN ) as $r ) {
			...
			foreach ( $roles as $i => $role ) {
				...
				$o[ $role ] = $co ? VHCPMTD_Util::quyen_truthy( $r[ 2 + $i ] ) : null;   // null = chưa khai
			}
			$saved[ $key ] = $o;
		}
		$out = array();
		...
		set_transient( 'vhcpmtd_quyen', $out, 300 );
		return $out;
	}
	 ...
	 */
	const CO_VA_QT = 'vhcpmtd_va_qt_ke_toan';
	...
	public static function reset_quyen() {
		...
		self::write( self::QUYEN, $rows, false );
		VHCPMTD_Meta::set_json( self::QUYEN_COT_O, $roles );   // xem `va_cot_quyen_them_vai()`
		self::clear_cache();
		return VHCPMTD_Util::ok( array( 'soHanhDong' => count( $rows ) ) );
	}
	...
	public static function tinh_nang_bat( $ma, $vai = null ) {
		...
		$vai = null === $vai ? VHCPMTD_Auth::vai_tro() : (string) $vai;
		return 'Admin' === $vai;
	}
	...
	public static function loc_loai_theo_khoi() {
		$v = get_option( 'vhcpmtd_loc_loai_theo_khoi', null );
	...
	public static function loc_loai_theo_vai() {
		$v = get_option( 'vhcpmtd_loc_loai_theo_vai', null );
	...
	public static function xuat_misa_duoc() {
		if ( 'Admin' === VHCPMTD_Auth::vai_tro() ) { return true; }
		$k = self::khoi_cua_vai( VHCPMTD_Auth::vai_hien() );
	...
	public static function loai_row( $ten, $khoi = '' ) {
		...
		/* ═══════════════════════════════════════════════════════════════════════════════════
		   ...
		   🔴 VÌ Ô KHỐI CỦA BẢNG NÀY KHÔNG BAO GIỜ RỖNG. `VHCPMTD_Cfg::write()` lấp ô trống thành
	...
	public static function loai_tk( $ten, $khoi = '' ) {
		...
		return array(
			...
			/* Tên loại CHA — dòng này là chi phí con của nó. Rỗng = dòng ở tầng trên cùng.
			   Xem chốt ở `VHCPMTD_Cfg::DAU_MUC_MAC_DINH` về ba tầng của cây. */
	...
	public static function loai_thuoc_don_vi( $ten_loai, $don_vi ) {
		...
		foreach ( self::cfg_static_raw_loai() as $x ) {
			...
			foreach ( explode( ',', $dv ) as $d ) {
				if ( '' !== trim( $d ) && VHCPMTD_DonVi::bang( $d, $don_vi ) ) { return true; }
			}
			return false;
		}
	 ...
	 *    đúng kiểu lỗi không ai truy ra. `VHCPMTD_Util::ma_so()` là chỗ dọn duy nhất.
	 *
	...
	public static function tk_thuoc_cay( $tk, $loc ) {
		...
		$a = VHCPMTD_Util::ma_so( $tk );
		$b = VHCPMTD_Util::ma_so( $loc );
		if ( '' === $a || '' === $b ) { return false; }
		if ( $a === $b ) { return true; }
		return 0 === strpos( $a, $b );
	}
	...
	public static function tkco_xuat( $loai, $khoi, $tk_dong, $tk_phan_loai = '' ) {
		...
		/* ⚠️ ĐỘT BIẾN TƯƠNG ĐƯƠNG, ghi lại để lần sau khỏi đuổi theo: bỏ `trim()` ở dòng dưới
		   KHÔNG đổi kết quả — `VHCPMTD_Cfg::write()` đã cho ô này qua `VHCPMTD_Util::ma_so()`, nên giá
	...
	public static function loai_cua_coso( $coso ) {
		...
		if ( '' === $coso ) { return VHCPMTD_Util::err( 'Chưa chọn cơ sở' ); }
	...
	public static function khai_cho_coso( $rec ) {
		...
		$ten  = $g( 'ten' );
		$tkno = $g( 'tkNo' );
		if ( $ten === '' ) { return VHCPMTD_Util::err( 'Nhập tên gọi chi phí' ); }
		if ( $tkno === '' ) { return VHCPMTD_Util::err( 'Nhập số tài khoản (TK Nợ)' ); }
		...
		if ( ! count( $cosos ) && ! count( $mangs ) ) { return VHCPMTD_Util::err( 'Tích ít nhất 1 cơ sở (hoặc 1 mảng) để áp mã' ); }
	...
	public static function get_tai_khoan() {
		return VHCPMTD_Util::ok( array( 'taiKhoan' => self::tai_khoan(), 'mangTk' => self::mang_tk() ) );
	}
	...
	public static function do_mang_tu_tk( $goc = '641' ) {
		...
		if ( ! count( $chart ) ) { return VHCPMTD_Util::err( 'Chưa nạp hệ thống tài khoản (wp-admin → Vận Hành Chi Phí → Nhập dữ liệu → CH_TaiKhoan)' ); }
		$goc = trim( (string) $goc );
		...
		if ( ! count( $cha ) ) { return VHCPMTD_Util::err( 'Không thấy nhóm tài khoản nào dưới ' . $goc . ' có tài khoản con' ); }
	...
	public static function ghep_he_thong_tk( $opts = array() ) {
		...
		$chart = self::tai_khoan();
		$mang  = self::mang_tk();
		if ( ! count( $chart ) ) { return VHCPMTD_Util::err( 'Chưa nạp hệ thống tài khoản (⚙️ Cấu hình → nạp CSV → CH_TaiKhoan)' ); }
		if ( ! count( $mang ) ) { return VHCPMTD_Util::err( 'Chưa khai bảng "Mảng kinh doanh → nhóm TK" nên chưa biết tài khoản nào thuộc mảng nào' ); }
	...
	public static function dong_bo_tk_loai() {
		...
		if ( ! count( $loai ) ) { return VHCPMTD_Util::ok( array( 'updated' => 0, 'thieuMa' => 0, 'tong' => 0 ) ); }
		...
		return VHCPMTD_Util::ok( array( 'updated' => $upd, 'thieuMa' => $thieu, 'tong' => count( $rows ) ) );
	}
	 ...
	 *    hội chợ), gõ tự do, không lấy từ danh mục cơ sở — xem chốt ở `VHCPMTD_BP::don_vi_cua()`.
	...
	public static function coso_la() {
		...
		foreach ( self::COSO_BANG as $b => $cot ) {
			$t = VHCPMTD_DB::t( $b );
			foreach ( VHCPMTD_DB::rows( "SELECT $cot AS coso, COUNT(*) AS n FROM $t WHERE $cot<>'' GROUP BY $cot" ) as $r ) {
	...
	public static function doi_ten_coso( $cu, $moi ) {
		...
		$cu  = trim( (string) $cu );
		$moi = trim( (string) $moi );
		if ( '' === $cu || '' === $moi ) { return VHCPMTD_Util::err( 'Thiếu tên cũ hoặc tên mới' ); }
		if ( mb_strtolower( $cu ) === mb_strtolower( $moi ) ) { return VHCPMTD_Util::err( 'Hai tên giống nhau' ); }

		$dem = array();
		foreach ( self::COSO_BANG as $b => $cot ) {
			$n = $wpdb->update( VHCPMTD_DB::t( $b ), array( $cot => $moi ), array( $cot => $cu ) );
			$dem[ $b ] = (int) $n;
		}
		...
		self::clear_cache();
		return VHCPMTD_Util::ok( array( 'dong' => $dem, 'nguoi' => $u_doi, 'cauHinh' => $c_doi ) );
	}
	...
	private static function sao_luu_users() {
		...
		$ds = VHCPMTD_Meta::get_json( 'users_bak', array() );
		...
		array_unshift( $ds, array(
			...
			'boi'   => (string) VHCPMTD_Auth::nguoi(),
			'so'    => count( $rows ),
			'rows'  => $rows,
		) );
		VHCPMTD_Meta::set_json( 'users_bak', array_slice( $ds, 0, self::BAK_MAX ) );
	}
	...
	public static function khoi_phuc_users( $i = 0 ) {
		$ds = VHCPMTD_Meta::get_json( 'users_bak', array() );
		$i  = (int) $i;
		if ( ! isset( $ds[ $i ]['rows'] ) ) { return VHCPMTD_Util::err( 'Không có bản lưu số ' . $i ); }
		...
		if ( ! $them ) { return VHCPMTD_Util::ok( array( 'them' => 0, 'tong' => count( $hien ) ) ); }
		self::write( self::USER, $hien );
		self::clear_cache();
		return VHCPMTD_Util::ok( array( 'them' => $them, 'tong' => count( $hien ) ) );
	}
	...
	public static function soat_nhan_su() {
		...
		if ( null === $hs ) {
			return VHCPMTD_Util::ok( array( 'coNhanSu' => false, 'nhom' => array(), 'tong' => array() ) );
		}
	...
	public static function doi_ten_nguoi( $cu, $moi, $gop = false ) {
		...
		$moi = trim( (string) $moi );
		if ( '' === $cu || '' === $moi ) { return VHCPMTD_Util::err( 'Thiếu tên cũ hoặc tên mới' ); }
		if ( $cu === $moi ) { return VHCPMTD_Util::err( 'Hai tên giống hệt nhau — không có gì để đổi' ); }
		...
		if ( $co_dich && $k_cu !== $k_moi && ! $gop ) {
			$dem = self::dem_don_theo_ten();
			return VHCPMTD_Util::err( 'Tên "' . $moi . '" đã có tài khoản. Đổi "' . $cu . '" thành tên đó '
		...
		$doi_cot( VHCPMTD_DB::t( 'session' ), 'ten' );
		...
		self::clear_cache();
		return VHCPMTD_Util::ok( array( 'dong' => $dem, 'doiDong' => array_sum( $dem ),
