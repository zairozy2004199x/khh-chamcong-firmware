 ...
 *
 * 🔴 KHÔNG NHẬP HAI LẦN. Bản đồ id bên Vending → mã đơn giữ ở `VHCPMTD_Meta` (`vending_map`). Kéo lại
...
class VHCPMTD_Vending {
	...
	public static function dong_bo( $a = array() ) {
		...
		foreach ( $kq['chiPhi'] as $x ) {
			...
			$wpdb->update( $t_don, array( 'trang_thai' => $tt_moi, 'ngay_qt' => $ngay, 'nguoi_qt' => $x['approver'], 'ngay_gui_qt' => VHCPMTD_Util::now_sql(),
		...
		VHCPMTD_Meta::set( self::META_LAN, VHCPMTD_Util::now_sql() . ' · ' . $thang . ' · mới ' . $moi . ' · cập nhật ' . $cap );
		VHCPMTD_Log::log_action( array( 'actor' => VHCPMTD_Auth::nguoi(), 'role' => VHCPMTD_Auth::vai_tro(), 'action' => 'Kéo chi phí Vending về',
		...
		return VHCPMTD_Util::ok( array( 'thang' => $thang, 'tu' => $tu, 'den' => $den, 'web' => $kq['web'], 'tong' => count( $kq['chiPhi'] ),
