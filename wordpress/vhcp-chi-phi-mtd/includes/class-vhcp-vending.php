 ...
 *
 * 🔴 KHÔNG NHẬP HAI LẦN. Bản đồ id bên Vending → mã đơn giữ ở `VHCPMTD_Meta` (`vending_map`). Đẩy lại
 ...
 * 🔴 KHOÁ LÀ BÍ MẬT — cùng luật `VHCPMTD_DoanhThu`: không trả xuống màn, ô trống = giữ, đi trong header.
 * ⚠️ Đơn vị 'VENDING' — kế toán bị bó đơn vị khác không thấy các đơn này; Admin / nhà mẹ thấy đủ.
 */
class VHCPMTD_Vending {

	const O_URL   = 'vhcpmtd_vd_url';
	const DUONG_NHAN = '/vhcpmtd/v1/vending-nhan';
	const O_KHOA  = 'vhcpmtd_vd_khoa';
	...
	public static function cau_hinh() {
		...
		$admin = class_exists( 'VHCPMTD_Auth' ) && 'Admin' === VHCPMTD_Auth::vai_tro();
		$map = VHCPMTD_Meta::get_json( self::META_MAP, array() );
		...
		return array( 'san' => $co, 'url' => $admin ? $url : '', 'khoaCo' => $co,
			...
			'diaChiNhanAjax' => function_exists( 'admin_url' ) ? admin_url( 'admin-ajax.php?action=vhcpmtd_vending_nhan' ) : 'admin-ajax.php?action=vhcpmtd_vending_nhan',
			'soDaNhap' => count( (array) $map ), 'lanCuoi' => (string) VHCPMTD_Meta::get( self::META_LAN, '' ),
	...
	public static function kiem_tra() {
		$nay = VHCPMTD_Util::now();
		$den = $nay->format( 'Y-m-d' ); $tu = ( clone $nay )->modify( '-30 days' )->format( 'Y-m-d' );
		$kq  = self::goi( $tu, $den );
		if ( ! $kq['ok'] ) { return VHCPMTD_Util::err( $kq['error'] ); }
		$tt = array();
		foreach ( $kq['chiPhi'] as $x ) { $k = '' !== $x['status'] ? $x['status'] : '(trống)'; $tt[ $k ] = ( isset( $tt[ $k ] ) ? $tt[ $k ] : 0 ) + 1; }
		return VHCPMTD_Util::ok( array( 'soKhoan' => count( $kq['chiPhi'] ), 'tu' => $tu, 'den' => $den, 'web' => $kq['web'], 'theoTrangThai' => $tt ) );
	}
	...
	public static function gieo_danh_muc() {
		...
		$rows = VHCPMTD_Cfg::read( VHCPMTD_Cfg::COSO ); $co = array();
		...
		if ( $them_cs ) { VHCPMTD_Cfg::write( VHCPMTD_Cfg::COSO, $rows, false ); }
		$rows = VHCPMTD_Cfg::read( VHCPMTD_Cfg::LOAI ); $co = array();
		...
		if ( $them_loai ) { VHCPMTD_Cfg::write( VHCPMTD_Cfg::LOAI, $rows, false ); }
		if ( $them_cs || $them_loai ) { VHCPMTD_Cfg::clear_cache(); }
	...
	public static function routes() {
		register_rest_route( 'vhcpmtd/v1', '/vending-nhan', array(
			'methods'             => 'POST',
	...
	public static function duoc_nhan( $req ) {
		...
		if ( '' === $luu || '' === $gui || ! hash_equals( $luu, $gui ) ) {
			return new WP_Error( 'vhcpmtd_vd_khoa', 'Chưa có khoá chia sẻ hoặc khoá không khớp.', array( 'status' => 401 ) );
		}
		return true;
	}
	...
	public static function rest_nhan( $req ) {
		...
		if ( is_array( $body ) && ! empty( $body['ping'] ) ) {
			return VHCPMTD_Util::ok( array( 'ping' => true, 'web' => function_exists( 'get_bloginfo' ) ? (string) get_bloginfo( 'name' ) : '', 'banChiPhi' => defined( 'VHCPMTD_VERSION' ) ? VHCPMTD_VERSION : '' ) );
		}
		$ds = ( is_array( $body ) && isset( $body['khoan'] ) ) ? (array) $body['khoan'] : array();
		if ( ! $ds ) { return new WP_Error( 'vhcpmtd_vd_rong', 'Không có khoản nào trong gói gửi.', array( 'status' => 400 ) ); }
		$web = ( is_array( $body ) && isset( $body['web'] ) ) ? (string) $body['web'] : 'Vending HCMC';
		VHCPMTD_Auth::dat_vai_tro( 'Admin', 'Vending HCMC (đẩy tự động)' );
		$chuan = array();
		foreach ( $ds as $x ) {
	...
	public static function dong_bo( $a = array() ) {
		$a = (array) $a;
		list( $thang, $tu, $den ) = VHCPMTD_DoanhThu::khoang_thang( isset( $a['thang'] ) ? $a['thang'] : '' );
		$kq = self::goi( $tu, $den );
		if ( ! $kq['ok'] ) { return VHCPMTD_Util::err( $kq['error'] ); }
	...
	public static function nhan_khoan( $ds, $nguon = '' ) {
		...
		$map  = (array) VHCPMTD_Meta::get_json( self::META_MAP, array() );
		global $wpdb;
		$t_don = VHCPMTD_DB::t( 'don' ); $t_cp = VHCPMTD_DB::t( 'chiphi' );
		...
		foreach ( (array) $ds as $x ) {
			...
			$tt_moi = self::TT_DON;
			$ngay   = VHCPMTD_Util::parse_date( $x['date'] );
			...
			if ( isset( $map[ $x['id'] ] ) ) {
				...
				if ( ! $d ) { unset( $map[ $x['id'] ] ); }   // đơn đã bị xoá bên này → lập lại như mới
					...
					if ( VHCPMTD_Don::da_chot( (string) $d['trang_thai'] ) ) { $da_chot++; continue; }
			...
			$r = VHCPMTD_Don::create_don( VHCPMTD_Don::nhan_ky( $ngay ), ( $x['requester'] ?: 'Vending HCMC' ), 'tt' );
			...
			$l = VHCPMTD_Don::add_line( $m, array( 'coso' => $x['department'], 'ngay' => $ngay, 'phanLoaiTT' => 'Thanh toán cá nhân', 'nhom' => $x['type'],
			...
			$wpdb->update( $t_don, array( 'trang_thai' => $tt_moi, 'nguoi_duyet' => $x['approver'], 'ngay_duyet' => $ngay, 'ngay_gui_qt' => VHCPMTD_Util::now_sql(),
				'so_tien_thuc_mua' => $x['amount'], 'hinh_thuc_tt' => 'Vending', 'hoa_don_qt' => $x['receiptLink'], 'ghi_chu' => $ghi ), array( 'ma_don' => $m ) );
			$map[ $x['id'] ] = $m; $moi++;
		}
		VHCPMTD_Meta::set_json( self::META_MAP, $map );
		VHCPMTD_Meta::set( self::META_LAN, VHCPMTD_Util::now_sql() . ' · ' . $nguon . ' · mới ' . $moi . ' · cập nhật ' . $cap );
		VHCPMTD_Log::log_action( array( 'actor' => VHCPMTD_Auth::nguoi(), 'role' => VHCPMTD_Auth::vai_tro(), 'action' => 'Nhận chi phí Vending',
		...
		return VHCPMTD_Util::ok( array( 'nguon' => $nguon, 'tong' => count( (array) $ds ),
