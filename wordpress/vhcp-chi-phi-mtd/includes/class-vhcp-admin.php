...
class VHCPMTD_Admin {
	...
	public static function handle_post() {
		if ( ! isset( $_POST['vhcpmtd_action'] ) ) { return; }
		if ( ! current_user_can( self::CAP ) ) { wp_die( 'Không đủ quyền.' ); }
		$action = sanitize_text_field( wp_unslash( $_POST['vhcpmtd_action'] ) );
		...
		if ( $action === 'settings' ) {
			...
			update_option( 'vhcpmtd_slug', $slug );
			if ( $old !== $slug ) { update_option( 'vhcpmtd_flush_rewrite', 1 ); }

			$tz = isset( $_POST['vhcpmtd_timezone'] ) ? sanitize_text_field( wp_unslash( $_POST['vhcpmtd_timezone'] ) ) : 'Asia/Bangkok';
			if ( in_array( $tz, timezone_identifiers_list(), true ) ) { update_option( 'vhcpmtd_timezone', $tz ); }

			$secret = isset( $_POST['vhcpmtd_sso_secret'] ) ? trim( (string) wp_unslash( $_POST['vhcpmtd_sso_secret'] ) ) : '';
			VHCPMTD_Meta::set( 'SSO_SECRET', $secret );
			 ...
			 * việc để trống một ô.
			 * ══════════════════════════════════════════════════════════════════════════════ */
			if ( ! empty( $_POST['vhcpmtd_gh_xoa'] ) ) {
				VHCPMTD_TuCapNhat::xoa_khoa();
			} elseif ( isset( $_POST['vhcpmtd_gh_token'] ) ) {
				VHCPMTD_TuCapNhat::dat_khoa( wp_unslash( $_POST['vhcpmtd_gh_token'] ) );
			}

			VHCPMTD_Cfg::clear_cache();
			wp_safe_redirect( add_query_arg( array( 'page' => 'vhcpmtd-settings', 'vhcpmtd_msg' => 'saved' ), admin_url( 'admin.php' ) ) );
			exit;
		}

		if ( $action === 'import' ) {
			$type    = isset( $_POST['vhcpmtd_type'] ) ? sanitize_text_field( wp_unslash( $_POST['vhcpmtd_type'] ) ) : '';
			$text    = isset( $_POST['vhcpmtd_csv'] ) ? (string) wp_unslash( $_POST['vhcpmtd_csv'] ) : '';
			$opts    = array(
				'header'  => ! empty( $_POST['vhcpmtd_header'] ),
				'replace' => ! empty( $_POST['vhcpmtd_replace'] ),
				'ma'      => isset( $_POST['vhcpmtd_ma'] ) ? sanitize_text_field( wp_unslash( $_POST['vhcpmtd_ma'] ) ) : '',
			);

			if ( ! empty( $_FILES['vhcpmtd_file']['tmp_name'] ) && is_uploaded_file( $_FILES['vhcpmtd_file']['tmp_name'] ) ) {
				$raw = file_get_contents( $_FILES['vhcpmtd_file']['tmp_name'] );
				if ( $raw !== false && $raw !== '' ) { $text = $raw; }
			}

			$res = VHCPMTD_Import::run( $type, $text, $opts );
			set_transient( 'vhcpmtd_import_res_' . get_current_user_id(), $res, 60 );
			wp_safe_redirect( add_query_arg( array( 'page' => 'vhcpmtd-import' ), admin_url( 'admin.php' ) ) );
			exit;
		}

		if ( $action === 'sheet' ) {
			$url = isset( $_POST['vhcpmtd_url'] ) ? esc_url_raw( wp_unslash( $_POST['vhcpmtd_url'] ) ) : '';
			$tabs_txt = isset( $_POST['vhcpmtd_tabs'] ) ? (string) wp_unslash( $_POST['vhcpmtd_tabs'] ) : '';
			$tabs     = array();
			foreach ( preg_split( '/[\r\n]+/', $tabs_txt ) as $t ) {
				$t = sanitize_text_field( $t );
				if ( trim( $t ) !== '' ) { $tabs[] = trim( $t ); }
			}
			$res = VHCPMTD_Sheet::nap_ca_file( $url, array(
				'thu'     => ! empty( $_POST['vhcpmtd_thu'] ),
				'replace' => ! empty( $_POST['vhcpmtd_replace'] ),
				'taoCha'  => ! empty( $_POST['vhcpmtd_taocha'] ),
				'tabs'    => $tabs,
			) );
			set_transient( 'vhcpmtd_sheet_tabs_' . get_current_user_id(), $tabs_txt, 3600 );
			set_transient( 'vhcpmtd_sheet_res_' . get_current_user_id(), $res, 120 );
			set_transient( 'vhcpmtd_sheet_url_' . get_current_user_id(), $url, 3600 );
			wp_safe_redirect( add_query_arg( array( 'page' => 'vhcpmtd-sheet' ), admin_url( 'admin.php' ) ) );
			exit;
		}
		if ( $action === 'doiten' ) {
			$cu  = isset( $_POST['vhcpmtd_cu'] ) ? sanitize_text_field( wp_unslash( $_POST['vhcpmtd_cu'] ) ) : '';
			$moi = isset( $_POST['vhcpmtd_moi'] ) ? sanitize_text_field( wp_unslash( $_POST['vhcpmtd_moi'] ) ) : '';
			$thu = ! empty( $_POST['vhcpmtd_thu'] );
			$res = VHCPMTD_Upload::doi_ten_mien( $cu, $moi, $thu );
			set_transient( 'vhcpmtd_doiten_res_' . get_current_user_id(), $res, 60 );
			wp_safe_redirect( add_query_arg( array( 'page' => 'vhcpmtd' ), admin_url( 'admin.php' ) ) );
			exit;
		}
		if ( $action === 'xoadl' ) {
			$xn = isset( $_POST['vhcpmtd_xacnhan'] ) ? trim( (string) wp_unslash( $_POST['vhcpmtd_xacnhan'] ) ) : '';
			if ( mb_strtoupper( $xn ) !== 'XOA' ) {
				set_transient( 'vhcpmtd_xoadl_' . get_current_user_id(), array( 'loi' => 'Chưa xóa — phải gõ đúng chữ XOA để xác nhận.' ), 60 );
			} else {
				set_transient( 'vhcpmtd_xoadl_' . get_current_user_id(), array( 'ok' => VHCPMTD_DB::xoa_du_lieu() ), 60 );
			}
			wp_safe_redirect( add_query_arg( array( 'page' => 'vhcpmtd' ), admin_url( 'admin.php' ) ) );
			exit;
		}
		if ( $action === 'mokhoa' ) {
			$n = VHCPMTD_Auth::mo_khoa();
			wp_safe_redirect( add_query_arg( array( 'page' => 'vhcpmtd', 'vhcpmtd_msg' => 'mokhoa', 'vhcpmtd_n' => $n ), admin_url( 'admin.php' ) ) );
			exit;
		}
		if ( $action === 'flush' ) {
			update_option( 'vhcpmtd_flush_rewrite', 1 );
			VHCPMTD_DB::install();
			wp_safe_redirect( add_query_arg( array( 'page' => 'vhcpmtd', 'vhcpmtd_msg' => 'flushed' ), admin_url( 'admin.php' ) ) );
			exit;
		}
	}
	...
	public static function page_main() {
		...
		$dt = get_transient( 'vhcpmtd_doiten_res_' . get_current_user_id() );
		if ( $dt ) {
			delete_transient( 'vhcpmtd_doiten_res_' . get_current_user_id() );
			if ( empty( $dt['success'] ) ) {
		...
		echo '<form method="post" action="' . esc_url( admin_url( 'admin.php?page=vhcpmtd' ) ) . '">';
		wp_nonce_field( 'vhcpmtd_doiten' );
		echo '<input type="hidden" name="vhcpmtd_action" value="doiten">';
		echo '<table class="form-table"><tr><th scope="row">Tên miền cũ</th><td><input name="vhcpmtd_cu" class="regular-text" placeholder="khaki-scorpion-706230.hostingersite.com"></td></tr>';
		echo '<tr><th scope="row">Tên miền mới</th><td><input name="vhcpmtd_moi" class="regular-text" placeholder="' . esc_attr( (string) $host ) . '"> <span class="description">để trống = tên miền hiện tại</span></td></tr>';
		echo '<tr><th scope="row">Chỉ thử</th><td><label><input type="checkbox" name="vhcpmtd_thu" value="1" checked> chỉ đếm, chưa ghi</label></td></tr></table>';
		submit_button( 'Đổi tên miền trong link ảnh', 'secondary', 'submit', false );
		echo '</form>';

		$xd = get_transient( 'vhcpmtd_xoadl_' . get_current_user_id() );
		if ( $xd ) {
			delete_transient( 'vhcpmtd_xoadl_' . get_current_user_id() );
			if ( ! empty( $xd['loi'] ) ) {
		...
		echo '<form method="post" action="' . esc_url( admin_url( 'admin.php?page=vhcpmtd' ) ) . '">';
		wp_nonce_field( 'vhcpmtd_xoadl' );
		echo '<input type="hidden" name="vhcpmtd_action" value="xoadl">';
		echo '<table class="form-table"><tr><th scope="row">Gõ chữ <code>XOA</code> để xác nhận</th><td>'
			. '<input name="vhcpmtd_xacnhan" class="regular-text" placeholder="XOA" autocomplete="off"></td></tr></table>';
		...
		echo '<form method="post" action="' . esc_url( admin_url( 'admin.php?page=vhcpmtd' ) ) . '">';
		wp_nonce_field( 'vhcpmtd_mokhoa' );
		echo '<input type="hidden" name="vhcpmtd_action" value="mokhoa">';
		...
		echo '<p>Nhúng vào 1 trang WordPress bằng shortcode: <code>[vhcpmtd_app height="900"]</code>. ';
		...
		foreach ( VHCPMTD_Import::counts() as $label => $c ) {
		...
		wp_nonce_field( 'vhcpmtd_flush' );
		echo '<input type="hidden" name="vhcpmtd_action" value="flush">';
	...
	public static function page_sheet() {
		...
		$res = get_transient( 'vhcpmtd_sheet_res_' . $uid );
		if ( $res ) { delete_transient( 'vhcpmtd_sheet_res_' . $uid ); }
		$url  = (string) get_transient( 'vhcpmtd_sheet_url_' . $uid );
		$tabs = (string) get_transient( 'vhcpmtd_sheet_tabs_' . $uid );
		...
		echo '<form method="post" action="' . esc_url( admin_url( 'admin.php?page=vhcpmtd-sheet' ) ) . '">';
		wp_nonce_field( 'vhcpmtd_sheet' );
		echo '<input type="hidden" name="vhcpmtd_action" value="sheet">';
		echo '<table class="form-table"><tr><th scope="row">Link bảng tính</th><td>'
			. '<input name="vhcpmtd_url" class="large-text" value="' . esc_attr( $url ) . '" placeholder="https://docs.google.com/spreadsheets/d/…"></td></tr>';
		echo '<tr><th scope="row">Tên các tab</th><td>'
			. '<textarea name="vhcpmtd_tabs" rows="6" class="large-text" placeholder="VH_Index&#10;VH_Line&#10;CT_ChiTiet&#10;DA SNOW NHÀ TUYẾT BÌNH DƯƠNG">' . esc_textarea( $tabs ) . '</textarea>'
		...
		echo '<tr><th scope="row">Chỉ thử</th><td><label><input type="checkbox" name="vhcpmtd_thu" value="1" checked>'
			. ' chỉ xem sẽ nạp gì, chưa ghi vào database</label></td></tr>';
		echo '<tr><th scope="row">Xóa dữ liệu cũ</th><td><label><input type="checkbox" name="vhcpmtd_replace" value="1">'
			. ' xóa sạch bảng tương ứng trước khi nạp (chỉ tích cho lượt nạp đầu)</label></td></tr>';
		echo '<tr><th scope="row">Tự tạo dòng cha</th><td><label><input type="checkbox" name="vhcpmtd_taocha" value="1" checked>'
	...
	public static function page_import() {
		$res = get_transient( 'vhcpmtd_import_res_' . get_current_user_id() );
		if ( $res ) { delete_transient( 'vhcpmtd_import_res_' . get_current_user_id() ); }
		...
		wp_nonce_field( 'vhcpmtd_import' );
		echo '<input type="hidden" name="vhcpmtd_action" value="import">';
		echo '<table class="form-table"><tbody>';

		echo '<tr><th scope="row"><label for="vhcpmtd_type">Tab đang nạp</label></th><td><select name="vhcpmtd_type" id="vhcpmtd_type" required>';
		foreach ( VHCPMTD_Import::types() as $k => $v ) {
			echo '<option value="' . esc_attr( $k ) . '">' . esc_html( $v['label'] ) . '</option>';
		}
		echo '</select></td></tr>';

		echo '<tr><th scope="row"><label for="vhcpmtd_ma">Dự án / Đợt nhận dòng</label></th><td><select name="vhcpmtd_ma" id="vhcpmtd_ma"><option value="">— chỉ cần khi nạp tab của 1 dự án / 1 đợt —</option>';
		$da = VHCPMTD_DuAn::list_du_an();
		foreach ( (array) $da['items'] as $x ) {
			echo '<option value="' . esc_attr( $x['maDA'] ) . '">🔧 ' . esc_html( $x['ten'] . ' · ' . $x['loai'] ) . '</option>';
		}
		$bp = VHCPMTD_BP::list_bp( 'all' );
		foreach ( (array) $bp['items'] as $x ) {
			echo '<option value="' . esc_attr( $x['ma'] ) . '">' . ( $x['loai'] === 'Công tác' ? '✈️ ' : '🛠️ ' ) . esc_html( $x['ten'] ) . '</option>';
		}
		echo '</select></td></tr>';

		echo '<tr><th scope="row">File CSV</th><td><input type="file" name="vhcpmtd_file" accept=".csv,.tsv,.txt,text/csv">'
		...
		echo '<tr><th scope="row"><label for="vhcpmtd_csv">…hoặc dán nội dung</label></th><td><textarea name="vhcpmtd_csv" id="vhcpmtd_csv" rows="8" style="width:100%;font-family:monospace"></textarea></td></tr>';
		echo '<tr><th scope="row">Tùy chọn</th><td>';
		echo '<label><input type="checkbox" name="vhcpmtd_header" value="1" checked> Dòng đầu là tiêu đề (bỏ qua)</label><br>';
		echo '<label><input type="checkbox" name="vhcpmtd_replace" value="1"> Xóa dữ liệu cũ của bảng này trước khi nạp</label>';
		echo '</td></tr>';
		echo '</tbody></table>';
	...
	public static function page_settings() {
		$slug    = get_option( 'vhcpmtd_slug', 'chi-phi' );
		$tz     = get_option( 'vhcpmtd_timezone', 'Asia/Bangkok' );
		$secret = (string) VHCPMTD_Meta::get( 'SSO_SECRET', '' );

		echo '<div class="wrap"><h1>Cài đặt Vận Hành Chi Phí</h1>';
		if ( isset( $_GET['vhcpmtd_msg'] ) && $_GET['vhcpmtd_msg'] === 'saved' ) {
		 ...
		 * ══════════════════════════════════════════════════════════════════════════════════ */
		$co_khoa = VHCPMTD_TuCapNhat::co_khoa();
		echo '<tr><th scope="row"><label for="vhcpmtd_gh_token">Khoá GitHub</label></th><td>';
		echo '<input type="password" name="vhcpmtd_gh_token" id="vhcpmtd_gh_token" value="" autocomplete="new-password" class="regular-text code" placeholder="'
		...
		echo '<p class="description">'
			...
			. esc_html( VHCPMTD_TuCapNhat::REPO ) . '</code>, mục <b>Contents</b> để <b>Read-only</b>. '
		...
		if ( $co_khoa ) {
			echo '<p><label><input type="checkbox" name="vhcpmtd_gh_xoa" value="1"> Xoá hẳn khoá đang lưu</label></p>';
		}
		echo '</td></tr>';
