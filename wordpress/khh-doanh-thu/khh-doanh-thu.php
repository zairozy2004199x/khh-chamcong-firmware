 ...
 *
 * ⚠️ `Plugin URI` trỏ đúng kho mã, y như Chấm Công và Ghế Massage — bộ tự cập nhật ở trang IT
 *    (khmatrix.com/it) nhận mặt plugin qua thư mục trên nhánh đã ghim, và số ở dòng `Version`
 *    trên kia là thứ nó đem so với bản đang chạy. Đánh số lùi hay trùng là nó im lặng không mời
 *    cập nhật, hoặc tệ hơn: mời cài đè một bản thiếu tính năng.
 *
...
/* Tự cập nhật từ GitHub Releases — hiện nút "Cập nhật" ngay ở màn Plugin.
   `tools/test/kiem-tu-cap-nhat.php` quét đủ MỌI plugin, nên bộ mới quên nối là bộ thử đỏ.
   ⚠️ Tiền tố tag phải bằng đúng tên thư mục (`khh-doanh-thu-v`) — đó là chỗ các plugin phân
      biệt bản của nhau; chép lớp từ bộ khác mà quên đổi là bộ này đi nhận bản của bộ kia. */
require_once KHH_DT_DIR . 'includes/class-khhdt-tu-cap-nhat.php';
KHHDT_TuCapNhat::init();
...
function khh_dt_rest_nap( $req ) {
	...
	$loai = in_array( $loai, array( 'sao_ke', 'momo_pos', 'momo_sk', 'bes' ), true ) ? $loai : 'pos';
...
function khh_dt_nap_tep( $dich, $ten, $loai, $them = array() ) {
	...
	if ( 'bes' === $loai ) {
		...
		try {
			...
			return new WP_Error(
				'khh_dt_bes',
				'Đọc file Bes không xong: ' . $t->getMessage() . ' (' . basename( $t->getFile() ) . ' dòng ' . $t->getLine() . ')',
				array( 'status' => 500 )
			);
		}
		wp_delete_file( $dich );
		...
		/* Ghi qua ĐÚNG hàm mà đường FABi dùng: khoá (ngày, cơ sở) nên nạp lại cùng ngày là ghi
		   đè, không cộng dồn — và mọi màn phía sau không cần biết số này từ hệ nào tới. */
		$n = khh_dt_ghi_kho( $kq['dong'] );
		return array(
			'xong'       => true,
			'loai'       => 'bes',
			'da_ghi'     => $n,
			'ngay'       => $kq['ngay'],
			'cua_hang'   => $kq['cua_hang'],
			'ten_file'   => $kq['ten_file'],
			'thanh_tien' => $kq['thanh_tien'],
			'so_mon'     => $kq['so_mon'],
			'so_ve'      => $kq['so_ve'],
		);
	}
