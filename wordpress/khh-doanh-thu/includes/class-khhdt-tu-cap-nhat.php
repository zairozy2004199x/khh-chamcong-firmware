...
class KHHDT_TuCapNhat {
	...
	private static function duong() {
		return plugin_basename( KHH_DT_DIR . 'khh-doanh-thu.php' );
	}
	...
	public static function ban_moi( $bo_qua_nho = false ) {
		...
		$hien  = defined( 'KHH_DT_VERSION' ) ? KHH_DT_VERSION : '0';
		$tot   = null;
		foreach ( $ds as $rel ) {
	...
	public static function chi_tiet( $ket_qua, $viec, $args ) {
		...
		$o = new stdClass();
		$o->name          = 'Báo cáo doanh thu FABi (K&H)';
		$o->slug          = $args->slug;
		$o->version       = $moi['ver'];
