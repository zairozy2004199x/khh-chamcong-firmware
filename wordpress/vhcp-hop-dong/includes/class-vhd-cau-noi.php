<?php
/**
 * CHUYỂN TIẾP LỆNH sang app hợp đồng trên Apps Script.
 *
...
class VHD_CauNoi {
	...
	public static function url()  { return trim( (string) get_option( 'vhd_exec_url', '' ) ); }
	public static function khoa() { return trim( (string) get_option( 'vhd_web_key', '' ) ); }
	...
	public static function goi( $fn, $args = array() ) {
		$url = self::url();
		if ( $url === '' ) {
			return array( 'ok' => false, 'error' => 'Chưa khai địa chỉ /exec của app hợp đồng trong Cài đặt.' );
		}
		$khoa = self::khoa();
		if ( $khoa === '' ) {
			return array( 'ok' => false, 'error' => 'Chưa có khoá cầu nối. Vào Cài đặt → Thư viện hợp đồng để sinh khoá.' );
		}
		...
		if ( is_wp_error( $r ) ) {
			return array( 'ok' => false, 'error' => 'Không gọi được app hợp đồng: ' . $r->get_error_message() );
		}
		...
		if ( $body !== '' && $body[0] !== '{' && $body[0] !== '[' ) {
			...
			if ( stripos( $body, 'Script function not found' ) !== false || stripos( $body, 'doPost' ) !== false ) {
				return array( 'ok' => false, 'error' => 'App hợp đồng chưa có hàm doPost — chưa dán file CauNoi.gs, '
		...
		if ( ! is_array( $j ) ) {
			return array( 'ok' => false, 'error' => 'Không đọc được phản hồi của app hợp đồng (mã ' . $code . ').' );
		}
		if ( empty( $j['ok'] ) ) {
			return array( 'ok' => false, 'error' => isset( $j['error'] ) ? (string) $j['error'] : 'Lỗi không rõ từ app hợp đồng' );
		}
	...
	public static function giao_dien( $bo_qua_cache = false ) {
		...
		$html = (string) $r['data'];
		if ( trim( $html ) === '' ) {
			return array( 'ok' => false, 'error' => 'App hợp đồng trả về giao diện rỗng — kiểm biến CN_FILE_GIAO_DIEN trong CauNoi.gs.' );
		}
