/**
 * google.script.run  →  WordPress  →  app hợp đồng trên Apps Script
 * ============================================================================
 *
...
(function () {
	...
	var CFG       = window.VHD_CFG || {};
	var TOKEN_KEY = 'vhd_token';          // khoá riêng, KHÔNG dùng chung với app Vận hành chi phí:
	...
	function moGate() {
		...
		wrap.innerHTML =
			...
			+ '<h2 style="font-size:17px;color:#1d4ed8;margin:6px 0 3px;font-weight:700">Thư viện Hợp đồng</h2>'
		...
		function vao() {
			...
			guiCoDuPhong( { fn: 'login', args: [ pin ], token: '' } ).then( function ( r ) {
				...
				datToken( d.token );
				try { sessionStorage.setItem( 'vhd_user', JSON.stringify( { name: d.name, role: d.role, coso: d.coso } ) ); } catch ( e ) {}
	...
	function themThanhNguoiDung( d ) {
		...
		document.getElementById( 'vhdThoat' ).onclick = function () {
			var tok = layToken();
			if ( tok ) { guiCoDuPhong( { fn: 'vhdLogout', args: [], token: tok } ); }
			xoaToken();
			try { sessionStorage.removeItem( 'vhd_user' ); } catch ( e ) {}
			location.reload();
		};
	}
	...
	window.addEventListener( 'DOMContentLoaded', function () {
		...
		var u = null;
		try { u = JSON.parse( sessionStorage.getItem( 'vhd_user' ) || 'null' ); } catch ( e ) {}
