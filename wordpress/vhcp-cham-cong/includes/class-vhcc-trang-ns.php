...
class VHCC_TrangNS {
	...
	private static function trang_chinh( $toi ) {
		...
		/* Tab nào đang mở. Mặc định là Nhân sự — người mở trang này hằng ngày là để tìm một
		   người, không phải để khai quyền. */
		...
		self::the_tab( $tab );
		/* 🔴 CẢNH BÁO ĐỨNG ĐÚNG TAB CỦA NÓ.
		   `canh_nguon()` nói về cột VAI TRÒ — mà cột ấy chỉ khai được ở tab Nhân sự; dán nó lên
		   tab Quyền là một dải vàng nói về một cột không có mặt trên màn.
		   `canh_ghe()` thì ngược lại: nó nói về cột Ghế massage, chỉ có ở tab Quyền.
		   Cảnh báo đứng nhầm tab còn tệ hơn không có: người đọc đi tìm cái cột nó nhắc tới, không
		   thấy, rồi thôi không đọc dải vàng nào nữa. */
		...
		if ( 'quyen' === $tab ) {
			...
			echo '</div>';
			/* Ngay DƯỚI bảng: khai theo NHÓM là đường nhanh hơn hẳn bấm từng người, nên nó phải
			   là thứ tiếp theo mắt chạm tới — không bắt người ta đi tìm ở cuối trang. */
	 ...
	 * THANH TAB.
	 *
	 * 🔴 GIỮ NGUYÊN BỘ LỌC KHI ĐỔI TAB. Đang lọc "cơ sở FARM_PT" ở tab Nhân sự mà bấm sang tab
	 *    Quyền lại thấy cả 261 người thì coi như phải lọc lại từ đầu — và người ta sẽ thôi dùng
	 *    tab thứ hai. Nên mọi tham số lọc đi theo, chỉ đổi mỗi `ntab`.
	 *
	 ...
	 *    trang"*. Anh đúng, và con số nói rõ: năm trang × ba nút = mười lăm nút mỗi hàng, cộng ô
	 *    cơ sở bốn dòng và ô vai — bảng đã tràn. Thêm trang thứ sáu, thứ bảy thì không còn là
	 *    "hơi rối" nữa mà là không dùng được.
	 *
	 ...
	 *
	 * 🔴 MỖI TAB CHỈ GỬI LÊN NHÓM Ô CỦA CHÍNH NÓ, và đó là chỗ dễ mất dữ liệu nhất của cả bản
	 *    này. `viec_luu()` đã lường trước từ lâu — chú thích của nó viết nguyên văn: *"hôm nay
	 *    trang luôn vẽ cột quyền nên `o` luôn có mặt và lỗi không lộ; ngày cột ấy ẩn đi với một
	 *    vai nào đó thì đổi cơ sở im lặng không ăn"*. Hôm nay chính là ngày ấy. Đã soát lại cả
	 *    năm hàm lưu (`luu_nhieu` · `luu_vai` · `luu_coso` · `luu_mang_bp` · `luu_ghe`): thiếu
	 *    nhóm nào thì nhóm ấy KHÔNG chạy, không phải chạy với mảng rỗng rồi xoá sạch. Bài kiểm
	 *    canh điều này ở cả hai chiều.
	 */
	private static function the_bang( $toi, $ds_trang, $cs, $q, $vai, $p, $mang = '', $nbp = '', $tab = 'nhan_su' ) {
		...
		/* 🔴 TÊN BẢNG PHẢI THEO NỘI DUNG BẢNG. Từ lúc tách tab, tab Nhân sự không còn một ô
		   quyền trang nào — để nguyên tên "Ai vào được trang nào" là mời người ta mở ra tìm
		   quyền rồi thấy một bảng khác hẳn thứ mình định tìm. Đúng cái lỗi anh Thắng từng bắt
		   ở khối Lương ("chưa bỏ bảng này đi à em"), lặp lại ở chỗ khác. */
		...
		foreach ( $lat as $r ) {
			...
			/* ⚠️ ĐẾM ĐÚNG SỐ CỘT ĐANG VẼ, không đếm số cột "lẽ ra có". Lệch một cột thì hàng sửa
			   tràn ra ngoài bảng hoặc thụt vào — trông như vỡ giao diện, mà nguyên do chỉ là
			   một con số ở đây. Tab Nhân sự: Mã · Tên · Cơ sở · Vai = 4. Tab Quyền: Mã · Tên ·
			   Vai = 3, cộng số trang, cộng ghế và chi phí nếu có. */
