<?php
/**
 * MỘT CỬA cho giao diện: POST /wp-json/vhcc/v1/call  {fn, args:[…], token}
 *
 * Mọi lệnh (trừ login) phải kèm token phiên còn hạn và vai trò được vào hệ thống chấm công.
...
class VHCC_API {
	...
	private static $noi_bo = array( 'login', 'vhccLogout' );

	public static function register_routes() {
		register_rest_route( 'vhcc/v1', '/call', array(
			'methods'             => 'POST',
	...
	/** Nhận lệnh ngay trên URL của trang (…/cham-cong/?vhcc_api=1) — đường dự phòng khi hosting chặn /wp-json/. */
	public static function trang() {
		...
		if ( $fn === '' ) {
			status_header( 200 );
			echo wp_json_encode( array( 'ok' => true, 'data' => array( 'song' => true, 'ver' => VHCC_VERSION ) ) );
			exit;
		}
		$res = self::chay( $fn, $args, $tok );
		status_header( (int) $res['status'] );
	...
	public static function handle( WP_REST_Request $req ) {
		...
		if ( $tok === '' ) { $tok = (string) $req->get_header( 'x_vhcc_token' ); }
	...
	private static function chay( $fn, $args, $token ) {
		...
		if ( ! in_array( $fn, self::$cong_khai, true ) ) {
			$user = VHCC_Auth::user_by_token( $token );
		...
		if ( $fn === 'login' ) {
			$r = VHCC_Auth::login( isset( $args[0] ) ? $args[0] : '' );
			return array( 'status' => 200, 'body' => array( 'ok' => true, 'data' => $r ) );
		}
		if ( $fn === 'vhccLogout' ) {
			return array( 'status' => 200, 'body' => array( 'ok' => true, 'data' => VHCC_Auth::logout( $token ) ) );
		}

		// Còn lại: chuyển sang app gốc. Danh sách hàm được phép do CauNoiChamCong.gs quyết định — để một
		// chỗ duy nhất, khỏi phải khai hai nơi rồi lệch nhau.
		$r = VHCC_CauNoi::goi( $fn, $args );
		if ( empty( $r['ok'] ) ) {
