...
class VHCC_TuCapNhat {
	...
	const TIEN_TO = 'vhcp-cham-cong-v';
	...
	public static function init() {
		...
		/* Gắn khoá vào TIÊU ĐỀ cho lượt tải gói — xem `goi_tai()` để biết vì sao không
		   được nhét vào địa chỉ. */
	...
	private static function duong() {
		return plugin_basename( VHCC_DIR . 'vhcp-cham-cong.php' );
	}
	...
	public static function ban_moi( $bo_qua_nho = false ) {
		...
		$hien  = defined( 'VHCC_VERSION' ) ? VHCC_VERSION : '0';
		$tot   = null;
		foreach ( $ds as $rel ) {
	 ...
	 * hợp lệ"* — và trong chính câu báo lỗi ấy là NGUYÊN CÁI KHOÁ, in ra màn hình, rồi đi vào
	 * ảnh chụp, rồi đi vào chat. Khoá coi như lộ, phải thu hồi.
	 *
	 * Hai lỗi cùng một nguyên nhân:
	 *   1. KHÔNG TẢI ĐƯỢC. `wp_http_validate_url()` của WordPress CHỐI mọi địa chỉ có phần
	 *      "người dùng" (`user@host`) — đó là chốt chống SSRF, có từ lâu và sẽ không bỏ. Nên
	 *      đường tải ấy chưa bao giờ chạy được, chỉ là tới hôm nay mới có người bấm.
	 ...
	 *      không chặn được điều đó. Chỉ có cách KHÔNG ĐỂ BÍ MẬT TRONG ĐỊA CHỈ mới chặn được.
	 *
	 ...
	 *
	 * ⚠️ Và kho này CÔNG KHAI (CLAUDE.md §4), nên tải gói vốn KHÔNG CẦN khoá. Khoá chỉ để nới
	 *    trần số lượt gọi API. Ai chưa khai khoá thì mọi thứ vẫn chạy y nguyên.
	 */
	 ...
	 *
	 * ⚠️ CHỈ gắn cho địa chỉ của CHÍNH kho này. Gắn cho mọi lượt gọi là gửi khoá GitHub của anh
	 *    Thắng tới bất cứ máy chủ nào plugin khác trên site gọi tới.
	 */
