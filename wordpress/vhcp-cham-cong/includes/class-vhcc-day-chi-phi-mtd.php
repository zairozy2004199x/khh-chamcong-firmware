<?php
/**
 * ĐẨY NGƯỜI TỪ SỔ NHÂN SỰ SANG BẢN CHI PHÍ MÁY TỰ ĐỘNG (MTD).
 *
 * =================================================================================================
 * Anh Thắng 15/09/2026, ngay sau bản Văn phòng: *"làm tab máy tự động luôn nhé"*.
 *
 ...
 *
 * ⚠️ SỔ "ĐÃ ĐẨY" PHẢI RIÊNG (`O_DA_DAY`). Đẩy một người sang bản Máy tự động không có nghĩa họ có
 ...
 *
 * ⚠️ NHƯNG HAI BẢN ĐỒ PHÒNG BAN (`O_BAN_DO`, `O_BAN_DO_BP`) THÌ DÙNG CHUNG, cố ý. Bản MTD sinh ra
...
class VHCC_DayChiPhiMTD extends VHCC_DayChiPhi {

	/** Tên cột trên bảng "Ai vào được trang nào" — KHÁC cột của bản khu vui chơi. */
	const COT = 'chi_phi_mtd';

	/** Sổ mã NV đã đẩy sang bản VP: [ maNV => tên đã ghi sang ]. Riêng, xem chú thích đầu tệp. */
	const O_DA_DAY = 'vhcc_day_chi_phi_mtd';

	public static function ten_he() { return 'Chi phí Máy tự động'; }

	public static function co_he() {
		return class_exists( 'VHCPMTD_Cfg' )
			&& method_exists( 'VHCPMTD_Cfg', 'read' )
			&& method_exists( 'VHCPMTD_Cfg', 'write' )
			&& defined( 'VHCPMTD_Cfg::USER' );
	}

	public static function doc_user() {
		if ( ! class_exists( 'VHCPMTD_Cfg' ) || ! method_exists( 'VHCPMTD_Cfg', 'read' )
			|| ! defined( 'VHCPMTD_Cfg::USER' ) ) {
			return array();
		}
		return (array) VHCPMTD_Cfg::read( VHCPMTD_Cfg::USER );
	}

	public static function ghi_user( $rows ) {
		if ( ! class_exists( 'VHCPMTD_Cfg' ) || ! method_exists( 'VHCPMTD_Cfg', 'write' )
			|| ! defined( 'VHCPMTD_Cfg::USER' ) ) {
			return false;
		}
		VHCPMTD_Cfg::write( VHCPMTD_Cfg::USER, $rows );
		return true;
	}

	public static function bp_chuan( $x ) {
		if ( ! class_exists( 'VHCPMTD_Cfg' ) || ! method_exists( 'VHCPMTD_Cfg', 'bo_phan_chuan' ) ) {
			return '';
		}
		return (string) VHCPMTD_Cfg::bo_phan_chuan( $x );
	}

	public static function bp_ds() {
		if ( ! class_exists( 'VHCPMTD_Cfg' ) || ! defined( 'VHCPMTD_Cfg::BO_PHAN_DS' ) ) { return array(); }
		return array_values( (array) VHCPMTD_Cfg::BO_PHAN_DS );
	}
}
