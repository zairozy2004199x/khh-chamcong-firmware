...
class VHCC_Cty {
	...
	public static function css() {
		...
		/* 🔴 MỌI `var()` Ở ĐÂY PHẢI CÓ GIÁ TRỊ LUI — `var(--x,#hex)`.
		   Chân trang này còn được plugin NỘI BỘ gọi (`class-vhnb-trang.php:1724`), mà trang ấy
		   khai một bộ token KHÁC hẳn: nó có `--mo`, không có `--chu-mo`. Viết `var(--chu-mo)`
		   trần thì ở trang nội bộ nó trả rỗng, trình duyệt LẶNG LẼ bỏ cả dòng ấy, và chữ chân
		   trang đang xám hoá thành đen thui — không lỗi, không cảnh báo, chỉ là một trang xấu
		   đi mà không ai biết vì sao. Giá trị lui đúng bằng màu cũ, nên trang nội bộ giữ nguyên
		   dáng còn trang chấm công ăn theo bộ áo mới. */
		return '.cty{margin:26px 0 0;padding:var(--d5,20px) 0 30px;'
			. 'border-top:1px solid var(--vien,#e2e8f0);'
			. 'color:var(--chu-mo,#64748b);font-size:12.5px;line-height:1.7}'
			...
			. '.cty-in{display:flex;flex-wrap:wrap;gap:var(--d4,16px) 34px}'
			. '.cty-cot{flex:1 1 250px;min-width:0}'
			. '.cty-ten{color:var(--chu-dam,#0f172a);font-weight:700;margin-bottom:var(--d1,4px);font-size:13px}'
			. '.cty-qt{font-style:italic;color:var(--chu-mo,#94a3b8);margin-bottom:var(--d1,4px)}'
			. '.cty-d span{color:var(--chu-mo,#94a3b8)}'
			. '.cty a{color:var(--nhan,#2563eb);text-decoration:none}'
			. '.cty a:hover{text-decoration:underline}'
			. '.cty-bq{margin-top:14px;padding-top:11px;border-top:1px solid var(--vien,#f1f5f9);'
			. 'color:var(--chu-mo,#94a3b8);'
			. 'font-size:12px}'
			...
			. '@media(max-width:640px){.cty{font-size:12px;padding:var(--d4,16px) 0 22px}.cty-in{gap:13px}}'
