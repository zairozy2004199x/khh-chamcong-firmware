...
require_once VHCC_DIR . 'includes/class-vhcc-giu-luong.php';
...
require_once VHCC_DIR . 'includes/class-vhcc-nhap-mon.php';
require_once VHCC_DIR . 'includes/class-vhcc-thu.php';
...
require_once VHCC_DIR . 'includes/class-vhcc-dia-chi.php';
require_once VHCC_DIR . 'includes/class-vhcc-chat.php';
require_once VHCC_DIR . 'includes/class-vhcc-goi.php';
require_once VHCC_DIR . 'includes/class-vhcc-nap-doc.php';
require_once VHCC_DIR . 'includes/class-vhcc-don-dem.php';
require_once VHCC_DIR . 'includes/class-vhcc-tu-dong-ra.php';
...
VHCC_Push::init();
/* Cùng lý do với dòng trên: `init()` chỉ khai móc và xếp lịch cron, phải chạy sớm. Lớp này
   điền dần tên đường cho mấy cặp toạ độ đã ghi — không khởi động thì cột địa chỉ trống mãi,
   mà bảng vẫn có cặp số nên không ai thấy là thiếu. */
VHCC_DiaChi::init();
/* Cùng lý do — lượt quét tự động gán giờ ra mặc định khi quên bấm ra (26/09/2026), xem
   `VHCC_TuDongRa`. Không khởi động thì cron không bao giờ được xếp lịch, hỏng im lặng y hệt
   hai lớp trên. */
VHCC_TuDongRa::init();
/* Gửi thư qua SMTP (khi đã khai) + ghi lỗi gửi — `VHCC_Thu`. Khai sớm để `phpmailer_init` có
   người nghe trước lượt `wp_mail()` đầu tiên. */
VHCC_Thu::init();
