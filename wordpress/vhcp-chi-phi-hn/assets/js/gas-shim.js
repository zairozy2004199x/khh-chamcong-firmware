...
(function () {
	...
	var CFG        = window.VHCPHN_CFG || {};
	var TOKEN_KEY  = 'vhcphn_token';
	var USER_KEY   = 'vhcphn_user';
	var reloading  = false;
	 ...
	 */
	var DUONG_KEY = 'vhcphn_duong';
	var duong = 'rest';
	try {
		var d = localStorage.getItem( DUONG_KEY );
		if ( d === 'ajax' || d === 'trang' ) { duong = d; }
		else if ( localStorage.getItem( 'vhcphn_ajax' ) === '1' ) { duong = 'ajax'; }
	} catch ( e ) {}
	function nhoDuong( d ) {
		duong = d;
	...
	/**
	 * Đường CUỐI: gửi vào chính URL của app (…/chi-phi/?vhcphn_api=1).
	 *
	...
	function call( fn, args, onOk, onErr ) {
		...
		function thu( d ) {
			...
			return boGui( d )( fn, args, tok ).then( docKetQua ).then( function ( r ) {
				...
				try { localStorage.removeItem( DUONG_KEY ); localStorage.removeItem( 'vhcphn_ajax' ); } catch ( e ) {}
				return r;
			} );
		}
