...
require_once VHCPHN_DIR . 'includes/class-vhcp-truc.php';
...
require_once VHCPHN_DIR . 'includes/class-vhcp-doanh-thu.php';
/* Chi phí Vending HCMC kéo về thành đơn thật — dùng khoang_thang của lớp Doanh thu nên nạp sau nó. */
require_once VHCPHN_DIR . 'includes/class-vhcp-vending.php';
...
require_once VHCPHN_DIR . 'includes/class-vhcp-gop.php';
...
/* Lớp vỏ app điện thoại (PWA) — gắn vào chính đường của trang, không đẻ đường thứ hai.
   Nạp SAU `class-vhcp-app.php`: nó dựng địa chỉ từ `VHCPHN_App::cac_slug()`. */
require_once VHCPHN_DIR . 'includes/class-vhcp-pwa.php';
require_once VHCPHN_DIR . 'includes/class-vhcp-admin.php';
require_once VHCPHN_DIR . 'includes/class-vhcp-tu-cap-nhat.php';
...
/* Tự cập nhật từ GitHub Releases — hiện nút "Cập nhật" ngay ở màn Plugin.
   ...
   Xem khối dài ở `VHCPHN_TuCapNhat`. Chưa khai khoá GitHub thì nó im lặng không làm gì. */
...
function vhcphn_maybe_upgrade() {
	...
	/* 🔴 DỜI CỘT BẢNG PHÂN QUYỀN VỀ ĐÚNG VAI — cùng lý do đặt ở đây với bản vá ngay trên.
	   ...
	   đúng một vai. Xem khối dài ở `VHCPHN_Cfg::va_cot_quyen_them_vai()`.

	   ⚠️ ĐẶT NGOÀI CHỐT `vhcphn_db_version`: bản vá này không đổi sơ đồ bảng nào, nên nằm trong
	   ...
	   ⚠️ Hàm tự giữ mốc, chạy lại không đổi gì, nên lượt nạp thứ hai trở đi chỉ tốn một lượt
	      đọc meta. */
	if ( method_exists( 'VHCPHN_Cfg', 'va_cot_quyen_them_vai' ) ) {
		VHCPHN_Cfg::va_cot_quyen_them_vai();
	}
	/* 🔴 HÚT CƠ SỞ BÊN GHẾ SANG — LƯỢT ĐẦU.
	   ...
	   🔴 CHẠY ĐÚNG MỘT LẦN CHO CẢ ĐờI SITE, KHÔNG PHẢI MỖI PHIÊN BẢN — đổi 21/09/2026.
	      ...
	      Bản cũ so cờ với `VHCPHN_VERSION`, tức mỗi lần cài bản mới là hút LẠI cả danh mục bên Ghế.
	...
	if ( ! get_option( 'vhcphn_hut_coso_ghe' ) ) {
		update_option( 'vhcphn_hut_coso_ghe', '1' );
...
// …và cửa dự phòng admin-ajax.php cho chính điểm nhận ấy (hosting chặn /wp-json/), không cần đăng nhập — khoá là cửa
add_action( 'wp_ajax_nopriv_vhcphn_vending_nhan', array( 'VHCPHN_Vending', 'ajax_nhan' ) );
add_action( 'wp_ajax_vhcphn_vending_nhan', array( 'VHCPHN_Vending', 'ajax_nhan' ) );
...
/* ⚠️ CÙNG ƯU TIÊN 5, VÀ ĐẶT NGAY SAU. Hai bên cùng khai luật đường dẫn, mà lượt nạp lại bảng
   luật (`vhcphn_flush_rewrite`, ưu tiên 99) phải thấy ĐỦ cả hai — khai muộn hơn 99 là luật của
   manifest/sw không vào bảng, và app báo "manifest không đọc được" mà không nói vì sao. */
add_action( 'init', array( 'VHCPHN_Pwa', 'init' ), 5 );
