...
class VHCPHN_Upload {
	...
	public static function doi_ten_mien( $cu, $moi = '', $thu = false ) {
		...
		if ( $cu === '' ) { return VHCPHN_Util::err( 'Nhập tên miền cũ' ); }
		if ( $moi === '' ) { return VHCPHN_Util::err( 'Không đọc được tên miền hiện tại' ); }
		if ( strcasecmp( $cu, $moi ) === 0 ) { return VHCPHN_Util::err( 'Tên miền cũ và mới giống nhau' ); }
	...
	public static function upload_image( $data, $ma_don = '', $coso = null ) {
		...
		if ( $bin === null ) { return VHCPHN_Util::err( 'Ảnh trống' ); }
		if ( strlen( $bin ) > self::MAX_SIZE ) { return VHCPHN_Util::err( 'Ảnh quá lớn (tối đa 15MB)' ); }
		$mime = isset( $data['type'] ) ? $data['type'] : 'image/jpeg';
		$ext  = self::img_ext( $mime );
		if ( $ext === '' ) { return VHCPHN_Util::err( 'Chỉ nhận ảnh (JPG/PNG/GIF/WEBP/HEIC) hoặc PDF' ); }
		...
		if ( 'pdf' === $ext && '%PDF-' !== substr( $bin, 0, 5 ) ) {
			return VHCPHN_Util::err( 'Tệp không phải PDF thật (thiếu chữ ký %PDF-)' );
		}

		$meta      = VHCPHN_Don::don_folder_meta( $ma_don );
		...
		$file = $dir . '/' . $name;
		if ( false === file_put_contents( $file, $bin ) ) { return VHCPHN_Util::err( 'Không ghi được file lên hosting (kiểm tra quyền thư mục uploads)' ); }
		@chmod( $file, 0644 );

		return VHCPHN_Util::ok( array( 'url' => $url . '/' . rawurlencode( $name ), 'fileId' => $name ) );
	}
	...
	public static function upload_doc( $data, $ma_da = '', $thu_muc = 'HoSo_DuAn' ) {
		$data = (array) $data;
		$bin  = self::decode( isset( $data['base64'] ) ? $data['base64'] : '' );
		if ( $bin === null ) { return VHCPHN_Util::err( 'File trống' ); }
		if ( strlen( $bin ) > self::MAX_SIZE ) { return VHCPHN_Util::err( 'File quá lớn (tối đa 15MB)' ); }
		...
		if ( ! self::doc_ext_ok( $ext ) ) { return VHCPHN_Util::err( 'Loại file không được phép: .' . $ext ); }
		...
		return VHCPHN_Util::ok( array( 'url' => $url . '/' . rawurlencode( $name ), 'name' => $name ) );
	}
