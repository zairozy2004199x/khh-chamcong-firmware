...
class VHCPHN_DuAn {
	...
	public static function create_du_an( $loai, $ten, $nguoi, $tu = '', $den = '' ) {
		...
		if ( ! in_array( $loai, self::LOAI_DU_AN, true ) ) {
			return VHCPHN_Util::err( 'Loại dự án không hợp lệ: "' . $loai . '". Hợp lệ: '
		...
		$wpdb->insert( VHCPHN_DB::t( 'da_index' ), array(
			...
			'loai'       => $loai,
			'trang_thai' => 'Đang làm',
			/* Mảng đóng dấu ngay lúc lập — xem chốt ở `VHCPHN_Don::create_don()`. */
			'khoi'       => VHCPHN_DB::khoi(),
	...
	public static function ma_coso_chung() {
		global $wpdb;
		$m = trim( (string) VHCPHN_Meta::get( self::MK_COSO_CHUNG, '' ) );
		if ( '-' === $m ) { return ''; }
		...
		VHCPHN_Meta::set( self::MK_COSO_CHUNG, (string) $r['ma_da'] );
	...
	public static function tao_don_coso( $ten, $nguoi, $tu = '', $den = '' ) {
		global $wpdb;
		$ten = VHCPHN_Util::san( $ten );
		if ( '' === $ten ) { return VHCPHN_Util::err( 'Nhập tên đợt chi phí cơ sở' ); }
		$tu  = trim( (string) $tu );
		$den = trim( (string) $den );
		if ( '' !== $tu && '' !== $den && $den < $tu ) {
			return VHCPHN_Util::err( 'Ngày kết thúc không được trước ngày bắt đầu.' );
		}
		/* Ghim trạng thái "đơn chung" TRƯỚC khi thêm đơn mới — xem chốt ⚠️ ở trên. */
		if ( '' === self::ma_coso_chung() ) { VHCPHN_Meta::set( self::MK_COSO_CHUNG, '-' ); }
		$ma = VHCPHN_Util::uid( 'DA' );
		$wpdb->insert( VHCPHN_DB::t( 'da_index' ), array(
			'ma_da'      => $ma,
			'ten'        => $ten,
		...
		if ( '' !== $tu || '' !== $den ) { VHCPHN_Meta::set_json( 'daKy_' . $ma, array( 'tu' => $tu, 'den' => $den ) ); }
		VHCPHN_Log::log_action( array(
			'actor'  => (string) ( '' !== $nguoi ? $nguoi : VHCPHN_Auth::nguoi() ),
			'role'   => VHCPHN_Auth::vai_tro(),
			'action' => 'Lập đơn chi phí cơ sở',
			'target' => $ma,
			'detail' => $ten . ( ( '' !== $tu || '' !== $den ) ? ( ' · ' . $tu . ' → ' . $den ) : '' ),
		) );
		return VHCPHN_Util::ok( array( 'maDA' => $ma, 'ten' => $ten, 'loai' => 'Chi phí cơ sở', 'sheet' => '', 'trangThai' => 'Đang làm', 'url' => '', 'kyDA' => array( 'tu' => $tu, 'den' => $den ) ) );
	}
	...
	public static function ensure_co_so_chung( $nguoi ) {
		...
		VHCPHN_Meta::set( self::MK_COSO_CHUNG, $ma );
		...
		foreach ( self::all_with_lines() as $r ) {
			...
			foreach ( $r['lines'] as $x ) {
				...
				if ( '' !== $cap && '(Phát sinh)' !== $cap ) {
					$con[ $cap ] = ( isset( $con[ $cap ] ) ? $con[ $cap ] : 0 ) + VHCPHN_Util::num( $x['thuc_te'] );
				}
			}
			foreach ( $r['lines'] as $x ) {
				...
				$out[] = array(
					...
					'duToan'    => VHCPHN_Util::num( $x['du_toan'] ),
					'thucTe'    => ( isset( $con[ $nd ] ) && $con[ $nd ] > 0 ) ? $con[ $nd ] : VHCPHN_Util::num( $x['thuc_te'] ),
					'hm'        => $h,
					'dotHien'   => $bd_tu,
					'qtDotHien' => $bd_qt,
	...
	public static function list_du_an() {
		...
		if ( class_exists( 'VHCPHN_DoanhThu' ) ) { $out = VHCPHN_DoanhThu::doi_gian( $out ); }
	...
	public static function rename_du_an( $ma_da, $ten ) {
		...
		if ( self::la_coso_chung( $ma_da ) ) { return VHCPHN_Util::err( 'Sổ Chi phí cơ sở CHUNG không đổi tên — đơn cơ sở theo đợt thì đổi được' ); }
	...
	public static function get_du_an( $ma_da ) {
		...
		foreach ( self::lines_of( $ma_da ) as $r ) {
			...
			$lines[] = array(
				...
				'taoLuc'    => VHCPHN_Util::fmt( isset( $r['tao_luc'] ) ? $r['tao_luc'] : '' ),
		...
		foreach ( self::ds_dot( $ma_da ) as $d ) {
			...
			if ( in_array( $d['tt'], array( 'xin', 'duyet', 'ung' ), true ) ) {
				...
				foreach ( (array) $d['lich'] as $lx ) {
					$tong_lich += VHCPHN_Util::num( isset( $lx['soTien'] ) ? $lx['soTien'] : 0 );
				}
				if ( $tong_lich > 0 ) {
					$da_xin += $tong_lich;
		...
		return VHCPHN_Util::ok( array(
			...
			'ngayTao'         => VHCPHN_Util::fmt( $f['ngay_tao'] ),
			'kyDA'            => self::get_ky_da( $ma_da ),
			'lenh'            => $lenh,
			'lenhQT'          => $lenhQT,
	...
	public static function get_du_toan_da( $ma_da ) {
		return VHCPHN_Util::num( VHCPHN_Meta::get( 'daDuToan_' . $ma_da, 0 ) );
	}
	...
	public static function get_ky_da( $ma_da ) {
		$o = VHCPHN_Meta::get_json( 'daKy_' . $ma_da, array() );
		return array(
	...
	public static function set_ky_da( $ma_da, $tu, $den, $nguoi = '' ) {
		...
		VHCPHN_Meta::set_json( 'daKy_' . $ma_da, array( 'tu' => $tu, 'den' => $den ) );
		VHCPHN_Log::log_action( array(
			'actor'  => (string) ( '' !== $nguoi ? $nguoi : VHCPHN_Auth::nguoi() ),
			'role'   => VHCPHN_Auth::vai_tro(),
		...
		return VHCPHN_Util::ok( array( 'ky' => array( 'tu' => $tu, 'den' => $den ) ) );
	}

	public static function set_du_toan_da( $ma_da, $so, $nguoi = '' ) {
		if ( ! self::find( $ma_da ) ) { return VHCPHN_Util::err( 'Không tìm thấy dự án' ); }
		$n = VHCPHN_Util::num( $so );
		/* 🔴 SỐ ÂM LÀ VÔ NGHĨA, và nó lặng lẽ làm phần "còn dự phòng" phình ra. Chối thẳng. */
		if ( $n < 0 ) { return VHCPHN_Util::err( 'Tổng dự toán không được âm.' ); }
		VHCPHN_Meta::set( 'daDuToan_' . $ma_da, $n );
		VHCPHN_Log::log_action( array(
			'actor'  => (string) ( '' !== $nguoi ? $nguoi : VHCPHN_Auth::nguoi() ),
			'role'   => VHCPHN_Auth::vai_tro(),
		...
		return VHCPHN_Util::ok( array( 'duToanDA' => $n ) );
	}
	...
	public static function pay_tong( $p, $phase, $loai ) {
		$t = 0;
		foreach ( self::pay_ds( $p, $phase, $loai ) as $x ) { $t += VHCPHN_Util::num( isset( $x['amount'] ) ? $x['amount'] : 0 ); }
		return $t;
	}
	...
	public static function confirm_pay( $ma_da, $phase, $loai, $amount, $nguoi, $ghi_chu = '' ) {
		...
		$so = VHCPHN_Util::num( $amount );
		if ( $so <= 0 ) { return VHCPHN_Util::err( 'Số tiền phải lớn hơn 0.' ); }
	...
	public static function unconfirm_pay( $ma_da, $phase, $loai, $idx = null ) {
		...
		$p  = self::get_pay( $ma_da );
		$ds = self::pay_ds( $p, $phase, $loai );
		if ( ! $ds ) { return VHCPHN_Util::err( 'Chưa có lần chi nào để hoàn tác.' ); }
		$i = ( null === $idx || '' === $idx ) ? ( count( $ds ) - 1 ) : (int) $idx;
		if ( $i < 0 || $i >= count( $ds ) ) { return VHCPHN_Util::err( 'Không có lần chi thứ ' . ( (int) $idx + 1 ) . '.' ); }
		array_splice( $ds, $i, 1 );
		if ( ! isset( $p[ $phase ] ) || ! is_array( $p[ $phase ] ) ) { $p[ $phase ] = array(); }
	...
	public static function get_hm( $ma_da ) {
		$o = VHCPHN_Meta::get_json( 'daHM_' . $ma_da, array() );
	...
	private static function hm_ghi_( $ma_da, $row, $sua ) {
		...
		$moi['moc'] = $cu['moc'];
		$moi['moc'][ $moi['tt'] ] = VHCPHN_Util::now()->format( 'd/m/Y H:i' ) . ' · ' . VHCPHN_Auth::nguoi();
		$o[ $k ] = $moi;
		VHCPHN_Meta::set_json( 'daHM_' . $ma_da, $o );
		VHCPHN_Log::log_action( array(
	...
	public static function get_dot( $ma_da, $loai = 'tu' ) {
		$o = VHCPHN_Meta::get_json( self::dot_meta_( $loai ) . $ma_da, array() );
		return is_array( $o ) ? $o : array();
	}
	...
	public static function dot_cua( $ma_da, $dot, $loai = 'tu' ) {
		...
		return array(
			...
			'soTien' => isset( $x['soTien'] ) ? VHCPHN_Util::num( $x['soTien'] ) : 0,
			'unc'    => isset( $x['unc'] ) ? (string) $x['unc'] : '',
	...
	public static function da_cap_tong( $d ) {
		...
		foreach ( (array) ( isset( $d['daCap'] ) ? $d['daCap'] : array() ) as $x ) {
			$t += VHCPHN_Util::num( isset( $x['soTien'] ) ? $x['soTien'] : 0 );
		}
		return $t;
	}
	...
	public static function da_cap_theo_lan( $d ) {
		...
		foreach ( (array) ( isset( $d['daCap'] ) ? $d['daCap'] : array() ) as $x ) {
			...
			if ( $l <= 0 ) { continue; }
			if ( ! isset( $m[ $l ] ) ) { $m[ $l ] = 0; }
			$m[ $l ] += VHCPHN_Util::num( isset( $x['soTien'] ) ? $x['soTien'] : 0 );
		}
		return $m;
	}
	...
	public static function tien_lan_lich( $d, $lan ) {
		foreach ( (array) ( isset( $d['lich'] ) ? $d['lich'] : array() ) as $y ) {
			if ( (int) ( isset( $y['lan'] ) ? $y['lan'] : 0 ) === (int) $lan ) {
				return VHCPHN_Util::num( isset( $y['soTien'] ) ? $y['soTien'] : 0 );
			}
		}
		return 0;
	}
	...
	public static function con_phai_cap( $d ) {
		$con = VHCPHN_Util::num( isset( $d['soTien'] ) ? $d['soTien'] : 0 ) - self::da_cap_tong( $d );
		return $con > 0 ? $con : 0;
	}
	...
	private static function dot_ghi_( $ma_da, $dot, $sua, $viec, $loai = 'tu' ) {
		...
		$moi['moc'][ $moi['tt'] ] = VHCPHN_Util::now()->format( 'd/m/Y H:i' ) . ' · ' . VHCPHN_Auth::nguoi();
		$o[ $k ] = $moi;
		VHCPHN_Meta::set_json( self::dot_meta_( $loai ) . $ma_da, $o );
		VHCPHN_Log::log_action( array(
			'actor'  => VHCPHN_Auth::nguoi(),
			'role'   => VHCPHN_Auth::vai_tro(),
			'action' => (string) $viec,
	...
	public static function tien_hm( $ma_da, $row ) {
		...
		foreach ( $lines as $l ) {
			if ( (int) $l['row_no'] === (int) $row ) {
				...
				$tu_than = VHCPHN_Util::num( $l['thuc_te'] );
			}
		}
		if ( '' === $nd ) { return 0; }
		$con = 0; $co_con = false;
		foreach ( $lines as $l ) {
			if ( trim( (string) $l['cap_cha'] ) === $nd ) { $co_con = true; $con += VHCPHN_Util::num( $l['thuc_te'] ); }
		}
	...
	public static function tien_hm_du_kien( $ma_da, $row ) {
		...
		foreach ( $lines as $l ) {
			if ( (int) $l['row_no'] === (int) $row ) {
				$nd      = trim( (string) $l['noi_dung'] );
				$tu_than = VHCPHN_Util::num( $l['thuc_te'] );
				$dt      = VHCPHN_Util::num( $l['du_toan'] );
				$tht     = VHCPHN_Util::num( $l['thanh_tien'] );
	...
	public static function xin_tam_ung_dot( $ma_da, $rows, $lich = array(), $ghi_chu = '' ) {
		...
		if ( ! $rows ) { return VHCPHN_Util::err( 'Chưa tích hạng mục nào để xin tạm ứng.' ); }
		...
		foreach ( $rows as $r ) {
			if ( ! isset( $lon[ $r ] ) ) {
				return VHCPHN_Util::err( 'Dòng ' . $r . ' không phải hạng mục lớn của dự án này.' );
			}
			if ( self::hm_la_ncc( $ma_da, $r ) ) {
				return VHCPHN_Util::err( '"' . $lon[ $r ] . '" là khoản kế toán trả thẳng nhà cung cấp — không xin tạm ứng.' );
			}
			$h = self::hm_cua( $ma_da, $r );
			if ( ! in_array( $h['tt'], array( 'nhap', 'tra' ), true ) ) {
				return VHCPHN_Util::err( '"' . $lon[ $r ] . '" đã nằm trong một lệnh tạm ứng rồi.' );
			}
			$nhan[] = $r;
		...
		if ( $tong <= 0 ) {
			return VHCPHN_Util::err( 'Mấy hạng mục vừa tích đều chưa có số tiền nào — Dự toán, '
		...
		foreach ( (array) $lich as $x ) {
			...
			foreach ( (array) ( isset( $x['rows'] ) ? $x['rows'] : array() ) as $rr ) {
				...
				if ( ! isset( $trong_lenh[ $rr ] ) ) {
					return VHCPHN_Util::err( 'Dòng ' . $rr . ' được xếp vào một lần nhận tiền nhưng '
						. 'không nằm trong lệnh này.' );
				}
				if ( isset( $da_gan[ $rr ] ) ) {
					return VHCPHN_Util::err( '"' . $lon[ $rr ] . '" bị xếp vào hai lần nhận tiền — '
		...
		return VHCPHN_Util::ok( array( 'dot' => $moi, 'soTien' => $tong, 'so' => count( $nhan ) ) );
	}
	...
	public static function xoa_dot_tra( $ma_da, $dot, $loai = 'tu' ) {
		if ( 'Admin' !== VHCPHN_Auth::vai_tro() ) { return VHCPHN_Util::err( 'Chỉ Admin dọn được lệnh bị trả.' ); }
		if ( ! self::find( $ma_da ) ) { return VHCPHN_Util::err( 'Không tìm thấy dự án' ); }
		$d = self::dot_cua( $ma_da, $dot, $loai );
		if ( ! $d ) { return VHCPHN_Util::err( 'Không tìm thấy lệnh ' . (int) $dot ); }
		if ( 'tra' !== $d['tt'] ) {
			return VHCPHN_Util::err( 'Chỉ dọn được lệnh ĐÃ BỊ TRẢ LẠI. Lệnh này đang ở "' . $d['tt']
		...
		foreach ( self::lines_of( $ma_da ) as $l ) {
			...
			if ( $h && (int) $h['dot'] === (int) $dot ) {
				return VHCPHN_Util::err( 'Hạng mục "' . trim( (string) $l['noi_dung'] ) . '" vẫn trỏ vào lệnh này — chưa dọn được.' );
			}
		}
		VHCPHN_Log::log_action( array(
			'actor'  => VHCPHN_Auth::nguoi(), 'role' => VHCPHN_Auth::vai_tro(),
		...
		VHCPHN_Meta::set_json( self::dot_meta_( $loai ) . $ma_da, $o );
		return VHCPHN_Util::ok( array( 'daXoa' => (int) $dot, 'conLai' => count( $o ) ) );
	}

	public static function cap_tien_phan( $ma_da, $dot, $them = array() ) {
		if ( ! self::find( $ma_da ) ) { return VHCPHN_Util::err( 'Không tìm thấy dự án' ); }
		$d = self::dot_cua( $ma_da, $dot );
		if ( ! $d ) { return VHCPHN_Util::err( 'Không tìm thấy lệnh tạm ứng ' . (int) $dot ); }
		$vai = VHCPHN_Auth::vai_tro();
		if ( ! in_array( $vai, array( 'Admin', 'Kế toán cá nhân', 'Kế toán NCC' ), true ) ) {
			return VHCPHN_Util::err( 'Chỉ kế toán cấp tạm ứng được.' );
		}
		if ( 'ung' === $d['tt'] ) { return VHCPHN_Util::err( 'Lệnh này đã cấp đủ tiền rồi.' ); }
		if ( 'duyet' !== $d['tt'] ) {
			return VHCPHN_Util::err( 'Chỉ cấp tiền cho lệnh ĐÃ DUYỆT. Lệnh này đang ở bước "'
				. $d['tt'] . '".' );
		}

		$them   = (array) $them;
		$so     = VHCPHN_Util::num( isset( $them['soTien'] ) ? $them['soTien'] : 0 );
		$con    = self::con_phai_cap( $d );
		if ( $so <= 0 ) {
			return VHCPHN_Util::err( 'Nhập số tiền thật sự đưa lần này (lớn hơn 0).' );
		}
		/* ══════════════════════════════════════════════════════════════════════════════════
		...
		if ( $cho_lan > 0 ) {
			...
			if ( $tien_lan <= 0 ) {
				return VHCPHN_Util::err( 'Lệnh này không có lần nhận tiền số ' . $cho_lan . '.' );
			}
			...
			if ( $so > $con_lan ) {
				...
				foreach ( (array) ( isset( $d['lich'] ) ? $d['lich'] : array() ) as $y ) {
					...
					$c = VHCPHN_Util::num( isset( $y['soTien'] ) ? $y['soTien'] : 0 ) - ( isset( $da_lan[ $l ] ) ? $da_lan[ $l ] : 0 );
					if ( $c > 0 ) { $con_khac += $c; }
				}
				if ( $con_khac > 0 ) {
					return VHCPHN_Util::err( 'Lần ' . $cho_lan . ' chỉ còn '
		...
		$luc   = VHCPHN_Util::now()->format( 'd/m/Y H:i' );
	...
	public static function dat_tt_dot( $ma_da, $dot, $tt, $them = array() ) {
		if ( ! self::find( $ma_da ) ) { return VHCPHN_Util::err( 'Không tìm thấy dự án' ); }
		$tt = (string) $tt;
		if ( ! in_array( $tt, self::TT_DOT, true ) ) { return VHCPHN_Util::err( 'Trạng thái không hợp lệ' ); }
		$d = self::dot_cua( $ma_da, $dot );
		if ( ! $d ) { return VHCPHN_Util::err( 'Không tìm thấy lệnh tạm ứng ' . (int) $dot ); }
		$them = (array) $them;
		$vai  = VHCPHN_Auth::vai_tro();
		...
		if ( in_array( $tt, array( 'duyet', 'tra' ), true ) && ! $duyet_duoc ) {
			return VHCPHN_Util::err( 'Chỉ quản lý hoặc kế toán duyệt / trả lại được.' );
		}
		if ( 'ung' === $tt && ! $ke_toan ) { return VHCPHN_Util::err( 'Chỉ kế toán cấp tạm ứng được.' ); }
		/* ═════════════════════════════════════════════════════════════════════════════════════
		 ...
		 * ═════════════════════════════════════════════════════════════════════════════════════ */
		if ( 'ung' === $d['tt'] ) {
			if ( 'tra' !== $tt ) { return VHCPHN_Util::err( 'Lệnh ' . $d['dot'] . ' đã cấp tiền rồi.' ); }
			if ( 'Admin' !== $vai ) {
				return VHCPHN_Util::err( 'Lệnh ' . $d['dot'] . ' đã cấp tiền rồi — chỉ Admin thu hồi được.' );
			}
			if ( '' === trim( (string) ( isset( $them['lyDo'] ) ? $them['lyDo'] : '' ) ) ) {
				return VHCPHN_Util::err( 'Thu hồi một lệnh ĐÃ CẤP TIỀN thì phải ghi lý do — '
					. 'đây là chỗ duy nhất gỡ ngược một khoản tiền thật.' );
			}
			foreach ( $d['rows'] as $r ) {
				$h = self::hm_cua( $ma_da, $r );
				if ( 'xong' === $h['tt'] || (int) $h['qtDot'] > 0 ) {
					return VHCPHN_Util::err( 'Trong lệnh này có hạng mục đã chốt xong'
		...
		if ( 'duyet' === $tt && 'xin' !== $d['tt'] ) { return VHCPHN_Util::err( 'Chỉ duyệt được lệnh đang xin tạm ứng.' ); }
		if ( 'ung' === $tt && 'duyet' !== $d['tt'] ) { return VHCPHN_Util::err( 'Chỉ cấp tiền cho lệnh đã duyệt.' ); }
		if ( 'tra' === $tt && ! in_array( $d['tt'], array( 'xin', 'duyet', 'ung' ), true ) ) {
			return VHCPHN_Util::err( 'Lệnh này không ở bước trả lại được.' );
		}
		...
		if ( 'ung' === $tt ) {
			...
			if ( $con_lai > 0 ) {
				$luc_c   = VHCPHN_Util::now()->format( 'd/m/Y H:i' );
				$ghi_c   = $d['daCap'];
		...
		return VHCPHN_Util::ok( array( 'dot' => $moi, 'tuQuyetToan' => (int) $tu_qt['so'] ) );
	}
	...
	public static function xin_quyet_toan_dot( $ma_da, $rows, $ghi_chu = '' ) {
		if ( ! self::find( $ma_da ) ) { return VHCPHN_Util::err( 'Không tìm thấy dự án' ); }
		$rows = array_values( array_unique( array_map( 'intval', (array) $rows ) ) );
		if ( ! $rows ) { return VHCPHN_Util::err( 'Chưa tích hạng mục nào để gửi quyết toán.' ); }
		...
		foreach ( $rows as $r ) {
			...
			$h = self::hm_cua( $ma_da, $r );
			if ( 'xong' !== $h['tt'] ) {
				return VHCPHN_Util::err( '"' . $lon[ $r ] . '" chưa chốt hoàn thành — chốt hoá đơn xong mới gửi quyết toán được.' );
			}
			if ( $h['qtDot'] > 0 ) {
				return VHCPHN_Util::err( '"' . $lon[ $r ] . '" đã nằm trong một lệnh quyết toán rồi.' );
			}
			$nhan[] = $r;
	...
	public static function dat_tt_qt( $ma_da, $dot, $tt, $them = array() ) {
		if ( ! self::find( $ma_da ) ) { return VHCPHN_Util::err( 'Không tìm thấy dự án' ); }
		$tt = (string) $tt;
		if ( ! in_array( $tt, array( 'xong', 'tra' ), true ) ) { return VHCPHN_Util::err( 'Trạng thái không hợp lệ' ); }
		$d = self::dot_cua( $ma_da, $dot, 'qt' );
		if ( ! $d ) { return VHCPHN_Util::err( 'Không tìm thấy lệnh quyết toán ' . (int) $dot ); }
		$vai = VHCPHN_Auth::vai_tro();
		if ( ! in_array( $vai, array( 'Admin', 'Quản lý', 'Kế toán cá nhân', 'Kế toán NCC' ), true ) ) {
			return VHCPHN_Util::err( 'Chỉ quản lý hoặc kế toán chốt / trả lại quyết toán được.' );
		}
		if ( 'xong' === $d['tt'] ) { return VHCPHN_Util::err( 'Lệnh quyết toán ' . $d['dot'] . ' đã chốt sổ rồi.' ); }
		...
		return VHCPHN_Util::ok( array( 'dot' => $moi ) );
	}
	...
	public static function dat_hm( $ma_da, $row, $tt, $them = array() ) {
		if ( ! self::find( $ma_da ) ) { return VHCPHN_Util::err( 'Không tìm thấy dự án' ); }
		$tt = (string) $tt;
		if ( ! in_array( $tt, self::TT_HM, true ) ) { return VHCPHN_Util::err( 'Trạng thái không hợp lệ' ); }
		$them = (array) $them;
		$vai  = VHCPHN_Auth::vai_tro();
		$duyet_duoc = in_array( $vai, array( 'Admin', 'Quản lý', 'Kế toán cá nhân', 'Kế toán NCC' ), true );
		$ke_toan    = in_array( $vai, array( 'Admin', 'Kế toán cá nhân', 'Kế toán NCC' ), true );
		...
		if ( 'xong' === $cu['tt'] && 'nhap' !== $tt ) {
			return VHCPHN_Util::err( 'Hạng mục đã chốt là chi thực tế — không đổi được nữa.' );
		}
		if ( 'xong' === $cu['tt'] && 'nhap' === $tt && ! $ke_toan ) {
			return VHCPHN_Util::err( 'Chỉ kế toán mở lại được hạng mục đã chốt.' );
		}
		if ( in_array( $tt, array( 'duyet', 'tra' ), true ) && ! $duyet_duoc ) {
			return VHCPHN_Util::err( 'Chỉ quản lý hoặc kế toán duyệt / trả lại được.' );
		}
		...
		if ( $la_ncc && in_array( $tt, array( 'xong', 'ung' ), true ) && ! $ke_toan ) {
			return VHCPHN_Util::err( 'Hạng mục này kế toán trả thẳng nhà cung cấp — chỉ kế toán xác nhận đã chi và khoá đơn.' );
		}
		if ( $la_ncc && 'xin' === $tt ) {
			return VHCPHN_Util::err( 'Hạng mục này kế toán trả thẳng nhà cung cấp — không xin tạm ứng. Kế toán tích xác nhận đã chi.' );
		}
		...
		if ( ! $la_ncc && in_array( $tt, array( 'xin', 'duyet', 'ung' ), true ) ) {
			return VHCPHN_Util::err( 'Bước này nay đi theo LỆNH tạm ứng của cả dự án — '
		...
		if ( ! $bo_qua_chuoi ) {
			if ( 'xin' === $tt && ! in_array( $cu['tt'], array( 'nhap', 'tra' ), true ) ) {
				return VHCPHN_Util::err( 'Hạng mục này đã qua bước xin tạm ứng rồi.' );
			}
			if ( 'duyet' === $tt && 'xin' !== $cu['tt'] ) {
				return VHCPHN_Util::err( 'Chỉ duyệt được hạng mục đang xin tạm ứng.' );
			}
			if ( 'ung' === $tt && 'duyet' !== $cu['tt'] ) {
				return VHCPHN_Util::err( 'Chỉ cấp tạm ứng cho hạng mục đã duyệt.' );
			}
		}
		...
		if ( 'xong' === $tt ) {
			...
			if ( $la_ncc ) {
				if ( '' === $hd && '' === $unc_moi ) {
					return VHCPHN_Util::err( 'Phải đính ít nhất một chứng từ (uỷ nhiệm chi hoặc hoá đơn) trước khi khoá đơn.' );
				}
			} elseif ( '' === $hd ) {
				/* ═══════════════════════════════════════════════════════════════════════════════
				...
				if ( '' === $san ) {
					return VHCPHN_Util::err( 'Phải đính hoá đơn trước khi chốt hoàn thành — '
			...
			if ( self::tien_hm( $ma_da, $row ) <= 0 ) {
				return VHCPHN_Util::err( 'Hạng mục này chưa có số tiền thực tế nào (đang là 0đ) — '
		...
		if ( 'xin' === $tt && isset( $them['lich'] ) && is_array( $them['lich'] ) ) {
			...
			foreach ( $them['lich'] as $x ) {
				...
				$ds[] = array(
					'lan'    => $lan,
					'ngay'   => $ngay,
					'soTien' => VHCPHN_Util::num( isset( $x['soTien'] ) ? $x['soTien'] : 0 ),
				);
			}
			$sua['lich'] = $ds;
		}
		...
		return VHCPHN_Util::ok( array( 'hm' => $moi, 'tuQuyetToan' => (int) $tu_qt['so'] ) );
	}
	...
	private static function line_data( $rec ) {
		...
		return array(
			...
			'thuc_te'    => ( ( null === $g( 'thucTe' ) || '' === $g( 'thucTe' ) ) && $sl * $dg > 0 )
				? ( $sl * $dg ) : VHCPHN_Util::num( $g( 'thucTe' ) ),
			'so_luong'   => $sl,
			'don_gia'    => $dg,
			'thanh_tien' => $sl * $dg,
	...
	public static function them_dong_muc_con_cu( $ma_da, $rec ) {
		...
		$data['tao_luc'] = VHCPHN_Util::now_sql();   // xem 🔴 ở `add_line()`
		$ok_dl = $wpdb->insert( VHCPHN_DB::t( 'da_line' ), $data );
		/* 🔴 SOI KẾT QUẢ — xem `kiem-ghi-so-phai-soi-ket-qua.php`. */
		if ( ! $ok_dl ) { $ct = trim( (string) $wpdb->last_error );
			return VHCPHN_Util::err( 'Không ghi được dòng dự án' . ( '' !== $ct ? ' — ' . $ct : '.' ) ); }
	...
	public static function add_line( $ma_da, $rec ) {
		...
		if ( '' !== $_mc ) { return VHCPHN_Util::err( $_mc ); }
		$data['ma_da']  = (string) $ma_da;
		$data['row_no'] = self::next_row( $ma_da );
	...
	public static function dat_o_line( $ma_da, $row, $cot, $gt ) {
		...
		if ( ! $f ) { return VHCPHN_Util::err( 'Không tìm thấy dự án' ); }
		$st = (string) ( $f['trang_thai'] !== '' ? $f['trang_thai'] : 'Đang làm' );
		if ( 'Đã đóng' === $st ) { return VHCPHN_Util::err( 'Dự án đã đóng — bấm "Mở lại" rồi sửa' ); }
		$row = (int) $row;
		$cot = trim( (string) $cot );
		$map = array(
			'duToan'  => 'du_toan',
		...
		if ( ! isset( $map[ $cot ] ) ) {
			return VHCPHN_Util::err( 'Ô "' . $cot . '" không sửa thẳng trong bảng được — bấm ✏️ để mở form.' );
		}
		...
		$sua = array();
		if ( 'note' === $cot ) {
			$sua['note'] = VHCPHN_Util::st( $gt );
		} else {
			$so = VHCPHN_Util::num( $gt );
			/* Mục con không có dự toán riêng — tiền của nó nằm ở cột thực tế / thành tiền. */
			if ( 'duToan' === $cot && '' !== trim( (string) $cur['cap_cha'] ) ) {
				return VHCPHN_Util::err( 'Mục con không có ô dự toán riêng — dự toán nằm ở hạng mục lớn.' );
			}
			$sua[ $map[ $cot ] ] = $so;
			if ( 'soLuong' === $cot || 'donGia' === $cot ) {
				$sl = ( 'soLuong' === $cot ) ? $so : VHCPHN_Util::num( $cur['so_luong'] );
				$dg = ( 'donGia' === $cot )  ? $so : VHCPHN_Util::num( $cur['don_gia'] );
				$sua['thanh_tien'] = $sl * $dg;
			}
		}
		$_dt = self::loi_sua_du_toan_( $ma_da, $row, $cur, $sua );
		if ( '' !== $_dt ) { return VHCPHN_Util::err( $_dt ); }
	 ...
	 * ═════════════════════════════════════════════════════════════════════════════════════════
	 * Loại chi phí quyết định TK Nợ / TK Có / mã đối tượng (`VHCPHN_Cfg::resolve_tk`). Chọn sai là
	 * dòng ấy vào sai tài khoản, và cái sai chỉ lộ ra lúc xuất MISA — khi đã quá muộn để hỏi lại
	 * người nhập họ mua cái gì.
	 *
	 * ⚠️ CHỈ KẾ TOÁN — cùng luật với `VHCPHN_Don::set_line_nhom()` bên đơn tuần. Gắn mã hạch toán
	...
	public static function dat_loai_cp_line( $ma_da, $row, $loai_cp ) {
		...
		if ( '' === $loai_cp ) { return VHCPHN_Util::err( 'Chọn loại chi phí' ); }

		$tk = VHCPHN_Cfg::resolve_tk( $loai_cp, (string) $cur['hinh_thuc'],
		...
		return VHCPHN_Util::ok( array( 'loaiCp' => $loai_cp, 'tkNo' => $tk['tk_no'], 'tkCo' => $tk['tk_co'] ) );
	}
	...
	private static function sua_o_line_( $ma_da, $row, $cot, $url, $cong_them, $viec ) {
		...
		if ( ! $cu ) { return VHCPHN_Util::err( 'Không tìm thấy dòng ' . $row ); }
	...
	public static function nhat_ky_du_an( $ma_da, $limit = 50 ) {
		global $wpdb;
		$ma_da = trim( (string) $ma_da );
		if ( '' === $ma_da ) { return VHCPHN_Util::ok( array( 'items' => array() ) ); }
		$limit = (int) $limit;
	...
	private static function loi_sua_du_toan_( $ma_da, $row, $cur, $data ) {
		...
		foreach ( array( 'du_toan' => 'dự toán', 'so_luong' => 'số lượng',
			...
			$moi = (float) VHCPHN_Util::num( $data[ $cot ] );
			$cu  = (float) VHCPHN_Util::num( isset( $cur[ $cot ] ) ? $cur[ $cot ] : 0 );
	...
	public static function loi_hang_muc_da_chot_( $ma_da, $row, $viec = 'sửa' ) {
		global $wpdb;
		$t   = VHCPHN_DB::t( 'da_line' );
		$cur = VHCPHN_DB::row( $wpdb->prepare( "SELECT * FROM $t WHERE ma_da=%s AND row_no=%d", (string) $ma_da, (int) $row ) );
	...
	public static function submit( $ma_da ) {
		$f = self::find( $ma_da );
		if ( ! $f ) { return VHCPHN_Util::err( 'Không tìm thấy dự án' ); }
		if ( self::la_coso_chung( $ma_da ) ) { return VHCPHN_Util::err( 'Sổ Chi phí cơ sở CHUNG không cần duyệt' ); }
	...
	public static function close( $ma_da ) {
		$f = self::find( $ma_da );
		if ( ! $f ) { return VHCPHN_Util::err( 'Không tìm thấy dự án' ); }
		if ( self::la_coso_chung( $ma_da ) ) { return VHCPHN_Util::err( 'Sổ Chi phí cơ sở CHUNG không đóng — nó chạy xuyên suốt' ); }
	...
	public static function delete( $ma_da ) {
		...
		if ( $la_chung ) {
			...
			$n_sc = count( VHCPHN_SoChi::theo_du_an( (string) $ma_da ) );
			if ( ! $n_sc ) { $n_sc = count( VHCPHN_SoChi::theo_du_an( (string) $f['ten'] ) ); }
			if ( $n_dong || $n_sc ) {
				return VHCPHN_Util::err( 'Sổ Chi phí cơ sở CHUNG còn ' . ( $n_dong + $n_sc )
		...
		if ( $la_chung ) { VHCPHN_Meta::set( self::MK_COSO_CHUNG, '-' ); }
		return VHCPHN_Util::ok();
	}
