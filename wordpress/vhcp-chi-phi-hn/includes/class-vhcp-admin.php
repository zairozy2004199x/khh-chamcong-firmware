...
class VHCPHN_Admin {
	...
	public static function handle_post() {
		if ( ! isset( $_POST['vhcphn_action'] ) ) { return; }
		if ( ! current_user_can( self::CAP ) ) { wp_die( 'Không đủ quyền.' ); }
		$action = sanitize_text_field( wp_unslash( $_POST['vhcphn_action'] ) );
		...
		if ( $action === 'settings' ) {
			...
			update_option( 'vhcphn_slug', $slug );
			if ( $old !== $slug ) { update_option( 'vhcphn_flush_rewrite', 1 ); }

			$tz = isset( $_POST['vhcphn_timezone'] ) ? sanitize_text_field( wp_unslash( $_POST['vhcphn_timezone'] ) ) : 'Asia/Bangkok';
			if ( in_array( $tz, timezone_identifiers_list(), true ) ) { update_option( 'vhcphn_timezone', $tz ); }
		...
		if ( $action === 'import' ) {
			$type    = isset( $_POST['vhcphn_type'] ) ? sanitize_text_field( wp_unslash( $_POST['vhcphn_type'] ) ) : '';
			$text    = isset( $_POST['vhcphn_csv'] ) ? (string) wp_unslash( $_POST['vhcphn_csv'] ) : '';
			$opts    = array(
				'header'  => ! empty( $_POST['vhcphn_header'] ),
				'replace' => ! empty( $_POST['vhcphn_replace'] ),
				'ma'      => isset( $_POST['vhcphn_ma'] ) ? sanitize_text_field( wp_unslash( $_POST['vhcphn_ma'] ) ) : '',
			);

			if ( ! empty( $_FILES['vhcphn_file']['tmp_name'] ) && is_uploaded_file( $_FILES['vhcphn_file']['tmp_name'] ) ) {
				$raw = file_get_contents( $_FILES['vhcphn_file']['tmp_name'] );
		...
		if ( $action === 'sheet' ) {
			$url = isset( $_POST['vhcphn_url'] ) ? esc_url_raw( wp_unslash( $_POST['vhcphn_url'] ) ) : '';
			$tabs_txt = isset( $_POST['vhcphn_tabs'] ) ? (string) wp_unslash( $_POST['vhcphn_tabs'] ) : '';
			$tabs     = array();
			foreach ( preg_split( '/[\r\n]+/', $tabs_txt ) as $t ) {
				$t = sanitize_text_field( $t );
				if ( trim( $t ) !== '' ) { $tabs[] = trim( $t ); }
			}
			$res = VHCPHN_Sheet::nap_ca_file( $url, array(
				'thu'     => ! empty( $_POST['vhcphn_thu'] ),
				'replace' => ! empty( $_POST['vhcphn_replace'] ),
				'taoCha'  => ! empty( $_POST['vhcphn_taocha'] ),
				'tabs'    => $tabs,
			) );
		...
		if ( $action === 'doiten' ) {
			$cu  = isset( $_POST['vhcphn_cu'] ) ? sanitize_text_field( wp_unslash( $_POST['vhcphn_cu'] ) ) : '';
			$moi = isset( $_POST['vhcphn_moi'] ) ? sanitize_text_field( wp_unslash( $_POST['vhcphn_moi'] ) ) : '';
			$thu = ! empty( $_POST['vhcphn_thu'] );
		...
		if ( $action === 'xoadl' ) {
			$xn = isset( $_POST['vhcphn_xacnhan'] ) ? trim( (string) wp_unslash( $_POST['vhcphn_xacnhan'] ) ) : '';
			if ( mb_strtoupper( $xn ) !== 'XOA' ) {
				set_transient( 'vhcphn_xoadl_' . get_current_user_id(), array( 'loi' => 'Chưa xóa — phải gõ đúng chữ XOA để xác nhận.' ), 60 );
			} else {
	...
	public static function page_main() {
		...
		$dt = get_transient( 'vhcphn_doiten_res_' . get_current_user_id() );
		if ( $dt ) {
			delete_transient( 'vhcphn_doiten_res_' . get_current_user_id() );
			if ( empty( $dt['success'] ) ) {
		...
		echo '<tr><th scope="row">Tên miền mới</th><td><input name="vhcphn_moi" class="regular-text" placeholder="' . esc_attr( (string) $host ) . '"> <span class="description">để trống = tên miền hiện tại</span></td></tr>';
		echo '<tr><th scope="row">Chỉ thử</th><td><label><input type="checkbox" name="vhcphn_thu" value="1" checked> chỉ đếm, chưa ghi</label></td></tr></table>';
		submit_button( 'Đổi tên miền trong link ảnh', 'secondary', 'submit', false );
		echo '</form>';

		$xd = get_transient( 'vhcphn_xoadl_' . get_current_user_id() );
		if ( $xd ) {
			delete_transient( 'vhcphn_xoadl_' . get_current_user_id() );
			if ( ! empty( $xd['loi'] ) ) {
		...
		echo '<table class="form-table"><tr><th scope="row">Gõ chữ <code>XOA</code> để xác nhận</th><td>'
			. '<input name="vhcphn_xacnhan" class="regular-text" placeholder="XOA" autocomplete="off"></td></tr></table>';
		...
		echo '<p>Nhúng vào 1 trang WordPress bằng shortcode: <code>[vhcphn_app height="900"]</code>. ';
		...
		foreach ( VHCPHN_Import::counts() as $label => $c ) {
		...
		wp_nonce_field( 'vhcphn_flush' );
		echo '<input type="hidden" name="vhcphn_action" value="flush">';
	...
	public static function page_sheet() {
		...
		$res = get_transient( 'vhcphn_sheet_res_' . $uid );
		if ( $res ) { delete_transient( 'vhcphn_sheet_res_' . $uid ); }
		$url  = (string) get_transient( 'vhcphn_sheet_url_' . $uid );
		$tabs = (string) get_transient( 'vhcphn_sheet_tabs_' . $uid );
		...
		echo '<table class="form-table"><tr><th scope="row">Link bảng tính</th><td>'
			. '<input name="vhcphn_url" class="large-text" value="' . esc_attr( $url ) . '" placeholder="https://docs.google.com/spreadsheets/d/…"></td></tr>';
		echo '<tr><th scope="row">Tên các tab</th><td>'
			. '<textarea name="vhcphn_tabs" rows="6" class="large-text" placeholder="VH_Index&#10;VH_Line&#10;CT_ChiTiet&#10;DA SNOW NHÀ TUYẾT BÌNH DƯƠNG">' . esc_textarea( $tabs ) . '</textarea>'
		...
		echo '<tr><th scope="row">Chỉ thử</th><td><label><input type="checkbox" name="vhcphn_thu" value="1" checked>'
			. ' chỉ xem sẽ nạp gì, chưa ghi vào database</label></td></tr>';
		echo '<tr><th scope="row">Xóa dữ liệu cũ</th><td><label><input type="checkbox" name="vhcphn_replace" value="1">'
			. ' xóa sạch bảng tương ứng trước khi nạp (chỉ tích cho lượt nạp đầu)</label></td></tr>';
		echo '<tr><th scope="row">Tự tạo dòng cha</th><td><label><input type="checkbox" name="vhcphn_taocha" value="1" checked>'
	...
	public static function page_import() {
		$res = get_transient( 'vhcphn_import_res_' . get_current_user_id() );
		if ( $res ) { delete_transient( 'vhcphn_import_res_' . get_current_user_id() ); }
		...
		wp_nonce_field( 'vhcphn_import' );
		echo '<input type="hidden" name="vhcphn_action" value="import">';
		echo '<table class="form-table"><tbody>';

		echo '<tr><th scope="row"><label for="vhcphn_type">Tab đang nạp</label></th><td><select name="vhcphn_type" id="vhcphn_type" required>';
		foreach ( VHCPHN_Import::types() as $k => $v ) {
			echo '<option value="' . esc_attr( $k ) . '">' . esc_html( $v['label'] ) . '</option>';
		}
		echo '</select></td></tr>';

		echo '<tr><th scope="row"><label for="vhcphn_ma">Dự án / Đợt nhận dòng</label></th><td><select name="vhcphn_ma" id="vhcphn_ma"><option value="">— chỉ cần khi nạp tab của 1 dự án / 1 đợt —</option>';
		$da = VHCPHN_DuAn::list_du_an();
		foreach ( (array) $da['items'] as $x ) {
			echo '<option value="' . esc_attr( $x['maDA'] ) . '">🔧 ' . esc_html( $x['ten'] . ' · ' . $x['loai'] ) . '</option>';
		}
		$bp = VHCPHN_BP::list_bp( 'all' );
		foreach ( (array) $bp['items'] as $x ) {
			echo '<option value="' . esc_attr( $x['ma'] ) . '">' . ( $x['loai'] === 'Công tác' ? '✈️ ' : '🛠️ ' ) . esc_html( $x['ten'] ) . '</option>';
		}
		echo '</select></td></tr>';

		echo '<tr><th scope="row">File CSV</th><td><input type="file" name="vhcphn_file" accept=".csv,.tsv,.txt,text/csv">'
		...
		echo '<tr><th scope="row"><label for="vhcphn_csv">…hoặc dán nội dung</label></th><td><textarea name="vhcphn_csv" id="vhcphn_csv" rows="8" style="width:100%;font-family:monospace"></textarea></td></tr>';
		echo '<tr><th scope="row">Tùy chọn</th><td>';
		echo '<label><input type="checkbox" name="vhcphn_header" value="1" checked> Dòng đầu là tiêu đề (bỏ qua)</label><br>';
		echo '<label><input type="checkbox" name="vhcphn_replace" value="1"> Xóa dữ liệu cũ của bảng này trước khi nạp</label>';
		echo '</td></tr>';
		echo '</tbody></table>';
	...
	public static function page_settings() {
		$slug    = get_option( 'vhcphn_slug', 'chi-phi' );
		$tz     = get_option( 'vhcphn_timezone', 'Asia/Bangkok' );
		$secret = (string) VHCPHN_Meta::get( 'SSO_SECRET', '' );

		echo '<div class="wrap"><h1>Cài đặt Vận Hành Chi Phí</h1>';
		if ( isset( $_GET['vhcphn_msg'] ) && $_GET['vhcphn_msg'] === 'saved' ) {
