 ...
 *
 *   DonHang   -> vhcphn_don            TamUng  -> vhcphn_tamung        ChiPhi  -> vhcphn_chiphi
 *   DA_Index  -> vhcphn_da_index       <sheet dự án> -> vhcphn_da_line (khóa theo ma_da + row_no)
 *   MK_Don    -> vhcphn_mk_don         MK_Line -> vhcphn_mk_line
 *   BP_Index  -> vhcphn_bp_index       <sheet đợt>   -> vhcphn_bp_line (khóa theo ma + row_no)
 *   CH_*      -> vhcphn_cfg (bảng dùng chung, mỗi dòng là 1 hàng sheet dạng JSON)
 *   NhatKy    -> vhcphn_log
 *   Document/Script Properties -> vhcphn_meta
 *
...
class VHCPHN_DB {
	 ...
	 * Bản 1.266.0 thêm cột `chiphi.giai_doan` mà quên nâng số này. `vhcphn_maybe_upgrade()` chỉ
	 * gọi `install()` khi số này KHÁC `vhcphn_db_version` đang lưu — nên `dbDelta()` không chạy,
	...
	public static function t( $name ) {
		global $wpdb;
		return $wpdb->prefix . 'vhcphn_' . $name;
	}
	...
	public static function xoa_du_lieu() {
		...
		foreach ( array( 'daApp_', 'daPay_' ) as $p ) {
			foreach ( VHCPHN_Meta::get_prefix( $p ) as $k => $v ) { VHCPHN_Meta::del( $k ); }
		}
	...
	public static function install() {
		...
		/* Cột `don_vi` = ĐƠN VỊ (K&H · POSH) — xem `VHCPHN_DonVi`. Ghi MỘT LẦN lúc lập đơn, theo
		 ...
		 *    lượt đổi chữ thành một lượt sửa dữ liệu. Xem `VHCPHN_Don::LUONG_MA`.
		 * ══════════════════════════════════════════════════════════════════════════════════ */
		...
		update_option( 'vhcphn_flush_rewrite', 1 );   // để vhcphn_flush_rewrite() nạp lại /chi-phi/ và /hop-dong/
		VHCPHN_Cfg::seed();
		/* Bật "Xác nhận quyết toán" cho các vai kế toán trên bảng quyền ĐANG LƯU — xem chú
		   thích dài ở `VHCPHN_Cfg::va_quyen_quyet_toan()`. Đặt SAU `seed()` vì bảng quyền phải
		   có mặt trước đã. */
