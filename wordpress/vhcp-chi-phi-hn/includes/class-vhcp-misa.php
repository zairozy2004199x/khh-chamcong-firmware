...
class VHCPHN_Misa {
	...
	/** Khoảng ngày của kỳ dài ĐÚNG 7 ngày — cách tuần chuẩn được sinh ra (`VHCPHN_Don::nhan_ky`). */
	...
	public static function export_misa( $ky = 'all', $mode = 'chuaxuat', $pl_f = 'all', $mau = self::MAU_CHUAN, $tk_f = 'all' ) {
		...
		foreach ( $cp as $r ) {
			...
			if ( $tk_no === '' && ! empty( $m_no_mx[ $mx_k ] ) && ! VHCPHN_Cfg::la_tk_ben_tra( $m_no_mx[ $mx_k ] ) ) { $tk_no = $m_no_mx[ $mx_k ]; }
			if ( $tk_no === '' && ! empty( $m_no[ $nhom ] ) && ! VHCPHN_Cfg::la_tk_ben_tra( $m_no[ $nhom ] ) ) { $tk_no = $m_no[ $nhom ]; }
			/* TK Có = BÊN TRẢ TIỀN. Ba bậc, và bậc đầu là mới (anh Thắng 21/09/2026: *"MTĐ tùy
			   ...
			   Bậc 1 bỏ trống thì hai bậc sau y như cũ — xem chốt dài ở `VHCPHN_Cfg::tkco_xuat()`.
	...
	public static function mark_exported( $ma_dons, $pl_f = 'all' ) {
		...
		return VHCPHN_Util::ok( array( 'count' => $n ) );
	}
}
