...
class VHCPHN_Auth {
	...
	public static function xem_duoc_loai( $ten_loai ) {
		...
		if ( ! class_exists( 'VHCPHN_Cfg' ) ) { return true; }
		/* 🔴 VÙNG TẮT LỌC THEO VAI thì MỌI VAI thấy đủ loại — xem `VHCPHN_Cfg::loc_loai_theo_vai()`.
		...
		if ( ! VHCPHN_Cfg::loc_loai_theo_vai() ) { return true; }
		return VHCPHN_Cfg::loai_thuoc_vai( $ten_loai, $vai );
	}
	...
	public static function so_ma_tat() {
		...
		if ( ! class_exists( 'VHCPHN_Meta' ) || ! method_exists( 'VHCPHN_Meta', 'get' ) ) { return self::$so_tat; }
		$raw = (string) call_user_func( array( 'VHCPHN_Meta', 'get' ), self::O_MA_TAT, '' );
		foreach ( preg_split( '/\r\n|\r|\n/', $raw ) as $dong ) {
	...
	public static function dat_ma_tat( $raw ) {
		...
		/* ⚠️ Gác CÙNG HÀM với lời gọi — luật `tools/test/kiem-goi-cheo.php`. */
		if ( class_exists( 'VHCPHN_Meta' ) && method_exists( 'VHCPHN_Meta', 'set' ) ) {
			call_user_func( array( 'VHCPHN_Meta', 'set' ), self::O_MA_TAT, implode( "\n", $sach ) );
		}
	...
	public static function so_coso() {
		...
		/* ⚠️ ĐỌC `cfg_static()`, KHÔNG ĐỌC `get_config()`. Hàm này chạy trên MỌI lượt lọc đơn,
		   ...
		   ⚠️ Gác CÙNG HÀM với lời gọi — luật `tools/test/kiem-goi-cheo.php`. */
		if ( class_exists( 'VHCPHN_Cfg' ) && method_exists( 'VHCPHN_Cfg', 'cfg_static' ) ) {
			$cfg = (array) call_user_func( array( 'VHCPHN_Cfg', 'cfg_static' ) );
	 ...
	 *
	 * 🔴 CHỈ TRẢ LỜI "CÓ ĐƯỢC ĐỌC KHÔNG". Chốt đơn vị nằm chỗ khác (`VHCPHN_DonVi::duoc_xem`),
	...
	public static function coso_cua_nguoi( $ten ) {
		...
		if ( '' === $k ) { return array(); }
		/* ⚠️ Gác CÙNG HÀM với lời gọi — luật `tools/test/kiem-goi-cheo.php`. */
		if ( ! class_exists( 'VHCPHN_Cfg' ) || ! method_exists( 'VHCPHN_Cfg', 'get_users' ) ) { return array(); }
		foreach ( (array) call_user_func( array( 'VHCPHN_Cfg', 'get_users' ) ) as $u ) {
			$u = (array) $u;
	...
	public static function login( $pin ) {
		...
		$u = null;
		foreach ( VHCPHN_Cfg::get_users() as $x ) {   // get_users() tự seed nếu cấu hình còn trống
	...
	public static function dong_chi_phi_cua( $ma_nv, $ten ) {
		...
		$theo_ten = null;
		foreach ( VHCPHN_Cfg::get_users() as $x ) {
	...
	public static function resolve_sso_user( $ident ) {
		...
		return array(
			'name'    => (string) $x['ten'],
			'role'    => $role,
			'roleGoc' => VHCPHN_Cfg::vai_goc( $role ),
