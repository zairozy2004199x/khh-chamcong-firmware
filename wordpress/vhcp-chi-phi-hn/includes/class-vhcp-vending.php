 ...
 *
 * 🔴 KHÔNG NHẬP HAI LẦN. Bản đồ id bên Vending → mã đơn giữ ở `VHCPHN_Meta` (`vending_map`). Đẩy lại
 ...
 * 🔴 KHOÁ LÀ BÍ MẬT — cùng luật `VHCPHN_DoanhThu`: không trả xuống màn, ô trống = giữ, đi trong header.
 * ⚠️ Đơn vị 'VENDING' — kế toán bị bó đơn vị khác không thấy các đơn này; Admin / nhà mẹ thấy đủ.
 */
class VHCPHN_Vending {

	const O_URL   = 'vhcphn_vd_url';
	const DUONG_NHAN = '/vhcphn/v1/vending-nhan';
	const O_KHOA  = 'vhcphn_vd_khoa';
	...
	public static function cau_hinh() {
		...
		$map = VHCPHN_Meta::get_json( self::META_MAP, array() );
		...
		return array( 'san' => $co, 'url' => $admin ? $url : '', 'khoaCo' => $co,
			...
			'diaChiNhanAjax' => function_exists( 'admin_url' ) ? admin_url( 'admin-ajax.php?action=vhcphn_vending_nhan' ) : 'admin-ajax.php?action=vhcphn_vending_nhan',
			'soDaNhap' => count( (array) $map ), 'lanCuoi' => (string) VHCPHN_Meta::get( self::META_LAN, '' ),
	...
	public static function luu( $d ) {
		...
		if ( null !== $url ) {
			if ( '' !== $url && ! preg_match( '#^https?://[^\s/]+#i', $url ) ) { return VHCPHN_Util::err( 'Địa chỉ web Vending phải bắt đầu bằng http:// hoặc https://' ); }
		...
		VHCPHN_Log::log_action( array( 'actor' => VHCPHN_Auth::nguoi(), 'role' => VHCPHN_Auth::vai_tro(), 'action' => 'Sửa kết nối web Vending',
	...
	public static function kiem_tra() {
		...
		return VHCPHN_Util::ok( array( 'soKhoan' => count( $kq['chiPhi'] ), 'tu' => $tu, 'den' => $den, 'web' => $kq['web'], 'theoTrangThai' => $tt ) );
	}
	...
	public static function gieo_danh_muc() {
		...
		$rows = VHCPHN_Cfg::read( VHCPHN_Cfg::COSO ); $co = array();
		...
		if ( $them_cs ) { VHCPHN_Cfg::write( VHCPHN_Cfg::COSO, $rows, false ); }
		$rows = VHCPHN_Cfg::read( VHCPHN_Cfg::LOAI ); $co = array();
		...
		if ( $them_loai ) { VHCPHN_Cfg::write( VHCPHN_Cfg::LOAI, $rows, false ); }
		if ( $them_cs || $them_loai ) { VHCPHN_Cfg::clear_cache(); }
	...
	public static function routes() {
		register_rest_route( 'vhcphn/v1', '/vending-nhan', array(
			'methods'             => 'POST',
	...
	public static function duoc_nhan( $req ) {
		...
		if ( '' === $luu || '' === $gui || ! hash_equals( $luu, $gui ) ) {
			return new WP_Error( 'vhcphn_vd_khoa', 'Chưa có khoá chia sẻ hoặc khoá không khớp.', array( 'status' => 401 ) );
		}
		return true;
	}
	...
	public static function rest_nhan( $req ) {
		...
		if ( is_array( $body ) && ! empty( $body['ping'] ) ) {
			return VHCPHN_Util::ok( array( 'ping' => true, 'web' => function_exists( 'get_bloginfo' ) ? (string) get_bloginfo( 'name' ) : '', 'banChiPhi' => defined( 'VHCPHN_VERSION' ) ? VHCPHN_VERSION : '' ) );
		}
		$ds = ( is_array( $body ) && isset( $body['khoan'] ) ) ? (array) $body['khoan'] : array();
		if ( ! $ds ) { return new WP_Error( 'vhcphn_vd_rong', 'Không có khoản nào trong gói gửi.', array( 'status' => 400 ) ); }
		$web = ( is_array( $body ) && isset( $body['web'] ) ) ? (string) $body['web'] : 'Vending HCMC';
		VHCPHN_Auth::dat_vai_tro( 'Admin', 'Vending HCMC (đẩy tự động)' );
		$chuan = array();
		foreach ( $ds as $x ) {
	...
	public static function dong_bo( $a = array() ) {
		$a = (array) $a;
		list( $thang, $tu, $den ) = VHCPHN_DoanhThu::khoang_thang( isset( $a['thang'] ) ? $a['thang'] : '' );
		$kq = self::goi( $tu, $den );
		if ( ! $kq['ok'] ) { return VHCPHN_Util::err( $kq['error'] ); }
	...
	public static function nhan_khoan( $ds, $nguon = '' ) {
		...
		$map  = (array) VHCPHN_Meta::get_json( self::META_MAP, array() );
		global $wpdb;
		$t_don = VHCPHN_DB::t( 'don' ); $t_cp = VHCPHN_DB::t( 'chiphi' );
		...
		foreach ( (array) $ds as $x ) {
			...
			$tt_moi = self::TT_DON;
			$ngay   = VHCPHN_Util::parse_date( $x['date'] );
			...
			if ( isset( $map[ $x['id'] ] ) ) {
				...
				if ( ! $d ) { unset( $map[ $x['id'] ] ); }   // đơn đã bị xoá bên này → lập lại như mới
					...
					if ( VHCPHN_Don::da_chot( (string) $d['trang_thai'] ) ) { $da_chot++; continue; }
			...
			$r = VHCPHN_Don::create_don( VHCPHN_Don::nhan_ky( $ngay ), ( $x['requester'] ?: 'Vending HCMC' ), 'tt' );
			...
			$l = VHCPHN_Don::add_line( $m, array( 'coso' => $x['department'], 'ngay' => $ngay, 'phanLoaiTT' => 'Thanh toán cá nhân', 'nhom' => $x['type'],
			...
			$wpdb->update( $t_don, array( 'trang_thai' => $tt_moi, 'nguoi_duyet' => $x['approver'], 'ngay_duyet' => $ngay, 'ngay_gui_qt' => VHCPHN_Util::now_sql(),
				'so_tien_thuc_mua' => $x['amount'], 'hinh_thuc_tt' => 'Vending', 'hoa_don_qt' => $x['receiptLink'], 'ghi_chu' => $ghi ), array( 'ma_don' => $m ) );
			$map[ $x['id'] ] = $m; $moi++;
		}
		VHCPHN_Meta::set_json( self::META_MAP, $map );
		VHCPHN_Meta::set( self::META_LAN, VHCPHN_Util::now_sql() . ' · ' . $nguon . ' · mới ' . $moi . ' · cập nhật ' . $cap );
		VHCPHN_Log::log_action( array( 'actor' => VHCPHN_Auth::nguoi(), 'role' => VHCPHN_Auth::vai_tro(), 'action' => 'Nhận chi phí Vending',
		...
		return VHCPHN_Util::ok( array( 'nguon' => $nguon, 'tong' => count( (array) $ds ),
