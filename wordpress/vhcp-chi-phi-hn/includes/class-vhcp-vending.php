 ...
 *
 * 🔴 KHÔNG NHẬP HAI LẦN. Bản đồ id bên Vending → mã đơn giữ ở `VHCPHN_Meta` (`vending_map`). Kéo lại
...
class VHCPHN_Vending {
	...
	public static function dong_bo( $a = array() ) {
		...
		foreach ( $kq['chiPhi'] as $x ) {
			...
			$wpdb->update( $t_don, array( 'trang_thai' => $tt_moi, 'ngay_qt' => $ngay, 'nguoi_qt' => $x['approver'], 'ngay_gui_qt' => VHCPHN_Util::now_sql(),
		...
		VHCPHN_Meta::set( self::META_LAN, VHCPHN_Util::now_sql() . ' · ' . $thang . ' · mới ' . $moi . ' · cập nhật ' . $cap );
		VHCPHN_Log::log_action( array( 'actor' => VHCPHN_Auth::nguoi(), 'role' => VHCPHN_Auth::vai_tro(), 'action' => 'Kéo chi phí Vending về',
		...
		return VHCPHN_Util::ok( array( 'thang' => $thang, 'tu' => $tu, 'den' => $den, 'web' => $kq['web'], 'tong' => count( $kq['chiPhi'] ),
