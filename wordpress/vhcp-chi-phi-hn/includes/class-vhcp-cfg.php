...
class VHCPHN_Cfg {
	...
	public static function cfg_static() {
		...
		foreach ( self::rows_of( $all, self::LOAI ) as $r ) {
			...
			$out['loaiChiPhi'][] = array( 'ten' => $r[0], 'tkNo' => VHCPHN_Util::ma_so( $r[1] ), 'tkCo' => VHCPHN_Util::ma_so( $r[2] ), 'maDt' => VHCPHN_Util::ma_so( $r[3] ), 'boPhan' => $r[4], 'note' => $r[5], 'tenMisa' => isset( $r[6] ) ? $r[6] : '', 'loaiTt' => isset( $r[7] ) ? $r[7] : '', 'donVi' => isset( $r[8] ) ? $r[8] : '', 'khoi' => isset( $r[9] ) ? $r[9] : '', 'vaiTro' => isset( $r[10] ) ? $r[10] : '' );
		}
	...
	public static function save_config( $cfg ) {
		...
		if ( isset( $cfg['loaiChiPhi'] ) && is_array( $cfg['loaiChiPhi'] ) ) {
			...
			foreach ( $cfg['loaiChiPhi'] as $x ) {
				...
				$rows[] = array( $tn, VHCPHN_Util::ma_so( $g( $x, 'tkNo' ) ), VHCPHN_Util::ma_so( $g( $x, 'tkCo' ) ), VHCPHN_Util::ma_so( $g( $x, 'maDt' ) ), $g( $x, 'boPhan' ), $nt, $g( $x, 'tenMisa' ), $g( $x, 'loaiTt' ), $g( $x, 'donVi' ), $kh, $g( $x, 'vaiTro' ) );
			}
